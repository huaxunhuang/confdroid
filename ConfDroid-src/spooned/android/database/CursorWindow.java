...
public class CursorWindow extends android.database.sqlite.SQLiteClosable implements android.os.Parcelable {
    ...
    private static native void nativeCopyStringToBuffer(long windowPtr, int row, int column, android.database.CharArrayBuffer buffer);
    ...
    public CursorWindow(java.lang.String name) {
        ...
        if (android.database.CursorWindow.sCursorWindowSize < 0) {
            /**
             * The cursor window size. resource xml file specifies the value in kB.
             * convert it to bytes here by multiplying with 1024.
             */
            android.database.CursorWindow.sCursorWindowSize = android.content.res.Resources.getSystem().getInteger(com.android.internal.R.integer.config_cursorWindowSize) * 1024;
        }
        mWindowPtr = android.database.CursorWindow.nativeCreate(mName, android.database.CursorWindow.sCursorWindowSize);
        if (mWindowPtr == 0) {
            throw new android.database.CursorWindowAllocationException((("Cursor window allocation of " + (android.database.CursorWindow.sCursorWindowSize / 1024)) + " kb failed. ") + printStats());
        }
        mCloseGuard.open("close");
        recordNewWindow(android.os.Binder.getCallingPid(), mWindowPtr);
    }
    ...
    private CursorWindow(android.os.Parcel source) {
        ...
        mWindowPtr = android.database.CursorWindow.nativeCreateFromParcel(source);
        if (mWindowPtr == 0) {
            throw new android.database.CursorWindowAllocationException("Cursor window could not be " + "created from binder.");
        }
        mName = android.database.CursorWindow.nativeGetName(mWindowPtr);
        mCloseGuard.open("close");
    }
    ...
    private void dispose() {
        ...
        if (mWindowPtr != 0) {
            ...
            android.database.CursorWindow.nativeDispose(mWindowPtr);
            mWindowPtr = 0;
        }
    }
    ...
    public void clear() {
        ...
        try {
            mStartPos = 0;
            android.database.CursorWindow.nativeClear(mWindowPtr);
        } finally {
            releaseReference();
        }
    }
    ...
    public int getNumRows() {
        acquireReference();
        try {
            return android.database.CursorWindow.nativeGetNumRows(mWindowPtr);
        } finally {
            releaseReference();
        }
    }
    ...
    public boolean setNumColumns(int columnNum) {
        acquireReference();
        try {
            return android.database.CursorWindow.nativeSetNumColumns(mWindowPtr, columnNum);
        } finally {
            releaseReference();
        }
    }
    ...
    public boolean allocRow() {
        acquireReference();
        try {
            return android.database.CursorWindow.nativeAllocRow(mWindowPtr);
        } finally {
            releaseReference();
        }
    }
    ...
    public void freeLastRow() {
        acquireReference();
        try {
            android.database.CursorWindow.nativeFreeLastRow(mWindowPtr);
        } finally {
            releaseReference();
        }
    }
    ...
    public boolean isNull(int row, int column) {
        return getType(row, column) == android.database.Cursor.FIELD_TYPE_NULL;
    }
    ...
    public boolean isBlob(int row, int column) {
        ...
        return (type == android.database.Cursor.FIELD_TYPE_BLOB) || (type == android.database.Cursor.FIELD_TYPE_NULL);
    }
    ...
    public boolean isLong(int row, int column) {
        return getType(row, column) == android.database.Cursor.FIELD_TYPE_INTEGER;
    }
    ...
    public boolean isFloat(int row, int column) {
        return getType(row, column) == android.database.Cursor.FIELD_TYPE_FLOAT;
    }
    ...
    public boolean isString(int row, int column) {
        int type = getType(row, column);
        return (type == android.database.Cursor.FIELD_TYPE_STRING) || (type == android.database.Cursor.FIELD_TYPE_NULL);
    }
    ...
    public int getType(int row, int column) {
        acquireReference();
        try {
            return android.database.CursorWindow.nativeGetType(mWindowPtr, row - mStartPos, column);
        } finally {
            releaseReference();
        }
    }
    ...
    public byte[] getBlob(int row, int column) {
        acquireReference();
        try {
            return android.database.CursorWindow.nativeGetBlob(mWindowPtr, row - mStartPos, column);
        } finally {
            releaseReference();
        }
    }
    ...
    public java.lang.String getString(int row, int column) {
        acquireReference();
        try {
            return android.database.CursorWindow.nativeGetString(mWindowPtr, row - mStartPos, column);
        } finally {
            releaseReference();
        }
    }
     ...
     * 		The zero-based column index.
     * @param buffer
     * 		The {@link CharArrayBuffer} to hold the string.  It is automatically
     * 		resized if the requested string is larger than the buffer's current capacity.
     */
    public void copyStringToBuffer(int row, int column, android.database.CharArrayBuffer buffer) {
        if (buffer == null) {
            throw new java.lang.IllegalArgumentException("CharArrayBuffer should not be null");
        }
        acquireReference();
        try {
            android.database.CursorWindow.nativeCopyStringToBuffer(mWindowPtr, row - mStartPos, column, buffer);
        } finally {
            releaseReference();
        }
    }
    ...
    public long getLong(int row, int column) {
        acquireReference();
        try {
            return android.database.CursorWindow.nativeGetLong(mWindowPtr, row - mStartPos, column);
        } finally {
            releaseReference();
        }
    }
    ...
    public double getDouble(int row, int column) {
        acquireReference();
        try {
            return android.database.CursorWindow.nativeGetDouble(mWindowPtr, row - mStartPos, column);
        } finally {
            releaseReference();
        }
    }
    ...
    public boolean putBlob(byte[] value, int row, int column) {
        acquireReference();
        try {
            return android.database.CursorWindow.nativePutBlob(mWindowPtr, value, row - mStartPos, column);
        } finally {
            releaseReference();
        }
    }
     ...
     * @return True if successful.
     */
    public boolean putString(java.lang.String value, int row, int column) {
        acquireReference();
        try {
            return android.database.CursorWindow.nativePutString(mWindowPtr, value, row - mStartPos, column);
        } finally {
            releaseReference();
        }
    }
    ...
    public boolean putLong(long value, int row, int column) {
        acquireReference();
        try {
            return android.database.CursorWindow.nativePutLong(mWindowPtr, value, row - mStartPos, column);
        } finally {
            releaseReference();
        }
    }
    ...
    public boolean putDouble(double value, int row, int column) {
        acquireReference();
        try {
            return android.database.CursorWindow.nativePutDouble(mWindowPtr, value, row - mStartPos, column);
        } finally {
            releaseReference();
        }
    }
    ...
    public boolean putNull(int row, int column) {
        acquireReference();
        try {
            return android.database.CursorWindow.nativePutNull(mWindowPtr, row - mStartPos, column);
        } finally {
            releaseReference();
        }
    }

    public static final android.os.Parcelable.Creator<android.database.CursorWindow> CREATOR = new android.os.Parcelable.Creator<android.database.CursorWindow>() {
        ...
        public android.database.CursorWindow[] newArray(int size) {
            return new android.database.CursorWindow[size];
        }
    };
    ...
    public void writeToParcel(android.os.Parcel dest, int flags) {
        ...
        try {
            ...
            android.database.CursorWindow.nativeWriteToParcel(mWindowPtr, dest);
        } finally {
            releaseReference();
        }
    ...
    private static final android.util.LongSparseArray<java.lang.Integer> sWindowToPidMap = new android.util.LongSparseArray<java.lang.Integer>();

    private void recordNewWindow(int pid, long window) {
        synchronized(android.database.CursorWindow.sWindowToPidMap) {
            android.database.CursorWindow.sWindowToPidMap.put(window, pid);
            if (android.util.Log.isLoggable(android.database.CursorWindow.STATS_TAG, android.util.Log.VERBOSE)) {
                android.util.Log.i(android.database.CursorWindow.STATS_TAG, "Created a new Cursor. " + printStats());
            }
        }
    }
    ...
    private void recordClosingOfWindow(long window) {
        synchronized(android.database.CursorWindow.sWindowToPidMap) {
            if (android.database.CursorWindow.sWindowToPidMap.size() == 0) {
                // this means we are not in the ContentProvider.
                return;
            }
            android.database.CursorWindow.sWindowToPidMap.delete(window);
        }
    }
    ...
    private java.lang.String printStats() {
        ...
        synchronized(android.database.CursorWindow.sWindowToPidMap) {
            int size = android.database.CursorWindow.sWindowToPidMap.size();
            if (size == 0) {
                // this means we are not in the ContentProvider.
                return "";
            }
            for (int indx = 0; indx < size; indx++) {
                int pid = android.database.CursorWindow.sWindowToPidMap.valueAt(indx);
        ...
        java.lang.String s = (buff.length() > 980) ? buff.substring(0, 980) : buff.toString();
        return ("# Open Cursors=" + total) + s;
    }

    @java.lang.Override
    public java.lang.String toString() {
        return ((getName() + " {") + java.lang.Long.toHexString(mWindowPtr)) + "}";
    }
}
