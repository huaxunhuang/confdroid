...
public final class CursorToBulkCursorAdaptor extends android.database.BulkCursorNative implements android.os.IBinder.DeathRecipient {
    ...
    private static final class ContentObserverProxy extends android.database.ContentObserver {
        ...
        public ContentObserverProxy(android.database.IContentObserver remoteObserver, android.os.IBinder.DeathRecipient recipient) {
            super(null);
        ...
        public boolean unlinkToDeath(android.os.IBinder.DeathRecipient recipient) {
            return unlinkToDeath(recipient, 0);
        }
    ...
    public CursorToBulkCursorAdaptor(android.database.Cursor cursor, android.database.IContentObserver observer, java.lang.String providerName) {
        if (cursor instanceof android.database.CrossProcessCursor) {
            mCursor = ((android.database.CrossProcessCursor) (cursor));
        } else {
            mCursor = new android.database.CrossProcessCursorWrapper(cursor);
        }
    ...
    public android.database.CursorWindow getWindow(int position) {
        synchronized(mLock) {
            ...
            android.database.CursorWindow window = mCursor.getWindow();
            if (window != null) {
                closeFilledWindowLocked();
            } else {
                window = mFilledWindow;
                if (window == null) {
                    mFilledWindow = new android.database.CursorWindow(mProviderName);
                    window = mFilledWindow;
                } else
                    if ((position < window.getStartPosition()) || (position >= (window.getStartPosition() + window.getNumRows()))) {
                        window.clear();
                    }
    ...
    public int requery(android.database.IContentObserver observer) {
        synchronized(mLock) {
            ...
            try {
                ...
                java.lang.IllegalStateException leakProgram = new java.lang.IllegalStateException((mProviderName + " Requery misuse db, mCursor isClosed:") + mCursor.isClosed(), e);
                throw leakProgram;
            }
            unregisterObserverProxyLocked();
            createAndRegisterObserverProxyLocked(observer);
     ...
     */
    private void createAndRegisterObserverProxyLocked(android.database.IContentObserver observer) {
        if (mObserver != null) {
            throw new java.lang.IllegalStateException("an observer is already registered");
        }
        mObserver = new android.database.CursorToBulkCursorAdaptor.ContentObserverProxy(observer, this);
