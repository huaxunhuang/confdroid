...
public class CrossProcessCursorWrapper extends android.database.CursorWrapper implements android.database.CrossProcessCursor {
    ...
    public void fillWindow(int position, android.database.CursorWindow window) {
        if (mCursor instanceof android.database.CrossProcessCursor) {
            final android.database.CrossProcessCursor crossProcessCursor = ((android.database.CrossProcessCursor) (mCursor));
            crossProcessCursor.fillWindow(position, window);
            return;
        }
        android.database.DatabaseUtils.cursorFillWindow(mCursor, position, window);
    }
