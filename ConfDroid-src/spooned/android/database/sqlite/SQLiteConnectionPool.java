...
public final class SQLiteConnectionPool implements java.io.Closeable {
    ...
    private final java.util.ArrayList<android.database.sqlite.SQLiteConnection> mAvailableNonPrimaryConnections = new java.util.ArrayList<android.database.sqlite.SQLiteConnection>();
    ...
    private final java.util.WeakHashMap<android.database.sqlite.SQLiteConnection, android.database.sqlite.SQLiteConnectionPool.AcquiredConnectionStatus> mAcquiredConnections = new java.util.WeakHashMap<android.database.sqlite.SQLiteConnection, android.database.sqlite.SQLiteConnectionPool.AcquiredConnectionStatus>();
    ...
    private SQLiteConnectionPool(android.database.sqlite.SQLiteDatabaseConfiguration configuration) {
    ...
    public static android.database.sqlite.SQLiteConnectionPool open(android.database.sqlite.SQLiteDatabaseConfiguration configuration) {
        ...
        // Create the pool.
        android.database.sqlite.SQLiteConnectionPool pool = new android.database.sqlite.SQLiteConnectionPool(configuration);
    ...
    private void open() {
        ...
        mAvailablePrimaryConnection = /* primaryConnection */
        openConnectionLocked(mConfiguration, true);// might throw
    ...
    private void dispose(boolean finalized) {
        ...
        if (!finalized) {
            ...
            synchronized(mLock) {
                ...
                if (pendingCount != 0) {
                    android.util.Log.i(android.database.sqlite.SQLiteConnectionPool.TAG, (((("The connection pool for " + mConfiguration.label) + " has been closed but there are still ") + pendingCount) + " connections in use.  They will be closed ") + "as they are released back to the pool.");
                }
    ...
    public void reconfigure(android.database.sqlite.SQLiteDatabaseConfiguration configuration) {
        ...
        synchronized(mLock) {
            ...
            if (mConfiguration.openFlags != configuration.openFlags) {
                ...
                android.database.sqlite.SQLiteConnection newPrimaryConnection = /* primaryConnection */
                openConnectionLocked(configuration, true);// might throw
     ...
     */
    public android.database.sqlite.SQLiteConnection acquireConnection(java.lang.String sql, int connectionFlags, android.os.CancellationSignal cancellationSignal) {
    ...
    public void releaseConnection(android.database.sqlite.SQLiteConnection connection) {
        synchronized(mLock) {
            android.database.sqlite.SQLiteConnectionPool.AcquiredConnectionStatus status = mAcquiredConnections.remove(connection);
            if (status == null) {
    ...
    // Can't throw.
    private boolean recycleConnectionLocked(android.database.sqlite.SQLiteConnection connection, android.database.sqlite.SQLiteConnectionPool.AcquiredConnectionStatus status) {
        if (status == android.database.sqlite.SQLiteConnectionPool.AcquiredConnectionStatus.RECONFIGURE) {
            try {
                connection.reconfigure(mConfiguration);// might throw

            } catch (java.lang.RuntimeException ex) {
                android.util.Log.e(android.database.sqlite.SQLiteConnectionPool.TAG, "Failed to reconfigure released connection, closing it: " + connection, ex);
                status = android.database.sqlite.SQLiteConnectionPool.AcquiredConnectionStatus.DISCARD;
            }
        }
        if (status == android.database.sqlite.SQLiteConnectionPool.AcquiredConnectionStatus.DISCARD) {
            closeConnectionAndLogExceptionsLocked(connection);
            return false;
        }
        return true;
     ...
     *
     * @param connection
     * 		The connection owned by the session.
     * @param connectionFlags
     * 		The connection request flags.
     ...
     * 		from this pool or if it has already been released.
     */
    public boolean shouldYieldConnection(android.database.sqlite.SQLiteConnection connection, int connectionFlags) {
        synchronized(mLock) {
    ...
    public void collectDbStats(java.util.ArrayList<android.database.sqlite.SQLiteDebug.DbStats> dbStatsList) {
        synchronized(mLock) {
            ...
            for (android.database.sqlite.SQLiteConnection connection : mAvailableNonPrimaryConnections) {
                connection.collectDbStats(dbStatsList);
            }
            for (android.database.sqlite.SQLiteConnection connection : mAcquiredConnections.keySet()) {
    ...
    // Might throw.
    private android.database.sqlite.SQLiteConnection openConnectionLocked(android.database.sqlite.SQLiteDatabaseConfiguration configuration, boolean primaryConnection) {
        final int connectionId = mNextConnectionId++;
        return android.database.sqlite.SQLiteConnection.open(this, configuration, connectionId, primaryConnection);// might throw
    ...
    void onConnectionLeaked() {
        ...
        android.util.Log.w(android.database.sqlite.SQLiteConnectionPool.TAG, ((("A SQLiteConnection object for database '" + mConfiguration.label) + "' was leaked!  Please fix your application ") + "to end transactions in progress properly and to close the database ") + "when it is no longer needed.");
    ...
    private void closeExcessConnectionsAndLogExceptionsLocked() {
        ...
        while ((availableCount--) > (mMaxConnectionPoolSize - 1)) {
            android.database.sqlite.SQLiteConnection connection = mAvailableNonPrimaryConnections.remove(availableCount);
            closeConnectionAndLogExceptionsLocked(connection);
        } 
    }

    // Can't throw.
    private void closeConnectionAndLogExceptionsLocked(android.database.sqlite.SQLiteConnection connection) {
        try {
            connection.close();// might throw

        } catch (java.lang.RuntimeException ex) {
            android.util.Log.e(android.database.sqlite.SQLiteConnectionPool.TAG, ("Failed to close connection, its fate is now in the hands " + "of the merciful GC: ") + connection, ex);
        }
    }
    ...
    private void reconfigureAllConnectionsLocked() {
        if (mAvailablePrimaryConnection != null) {
            try {
                ...
                android.util.Log.e(android.database.sqlite.SQLiteConnectionPool.TAG, "Failed to reconfigure available primary connection, closing it: " + mAvailablePrimaryConnection, ex);
                closeConnectionAndLogExceptionsLocked(mAvailablePrimaryConnection);
                mAvailablePrimaryConnection = null;
            }
        }
        ...
        for (int i = 0; i < count; i++) {
            final android.database.sqlite.SQLiteConnection connection = mAvailableNonPrimaryConnections.get(i);
            try {
                connection.reconfigure(mConfiguration);// might throw

            } catch (java.lang.RuntimeException ex) {
                android.util.Log.e(android.database.sqlite.SQLiteConnectionPool.TAG, "Failed to reconfigure available non-primary connection, closing it: " + connection, ex);
                closeConnectionAndLogExceptionsLocked(connection);
    ...
    // Can't throw.
    private void markAcquiredConnectionsLocked(android.database.sqlite.SQLiteConnectionPool.AcquiredConnectionStatus status) {
        if (!mAcquiredConnections.isEmpty()) {
            java.util.ArrayList<android.database.sqlite.SQLiteConnection> keysToUpdate = new java.util.ArrayList<android.database.sqlite.SQLiteConnection>(mAcquiredConnections.size());
            for (java.util.Map.Entry<android.database.sqlite.SQLiteConnection, android.database.sqlite.SQLiteConnectionPool.AcquiredConnectionStatus> entry : mAcquiredConnections.entrySet()) {
                android.database.sqlite.SQLiteConnectionPool.AcquiredConnectionStatus oldStatus = entry.getValue();
                if ((status != oldStatus) && (oldStatus != android.database.sqlite.SQLiteConnectionPool.AcquiredConnectionStatus.DISCARD)) {
    ...
    // Might throw.
    private android.database.sqlite.SQLiteConnection waitForConnection(java.lang.String sql, int connectionFlags, android.os.CancellationSignal cancellationSignal) {
        final boolean wantPrimaryConnection = (connectionFlags & android.database.sqlite.SQLiteConnectionPool.CONNECTION_FLAG_PRIMARY_CONNECTION_AFFINITY) != 0;
        final android.database.sqlite.SQLiteConnectionPool.ConnectionWaiter waiter;
        final int nonce;
        synchronized(mLock) {
            throwIfClosedLocked();
            // Abort if canceled.
            ...
            android.database.sqlite.SQLiteConnection connection = null;
            ...
            final int priority = android.database.sqlite.SQLiteConnectionPool.getPriority(connectionFlags);
            final long startTime = android.os.SystemClock.uptimeMillis();
            waiter = obtainConnectionWaiterLocked(java.lang.Thread.currentThread(), startTime, priority, wantPrimaryConnection, sql, connectionFlags);
            android.database.sqlite.SQLiteConnectionPool.ConnectionWaiter predecessor = null;
            android.database.sqlite.SQLiteConnectionPool.ConnectionWaiter successor = mConnectionWaiterQueue;
        ...
        try {
            ...
            long busyTimeoutMillis = android.database.sqlite.SQLiteConnectionPool.CONNECTION_POOL_BUSY_MILLIS;
            ...
            for (; ;) {
                ...
                java.util.concurrent.locks.LockSupport.parkNanos(this, busyTimeoutMillis * 1000000L);
                // Clear the interrupted flag, just in case.
                java.lang.Thread.interrupted();
                // Check whether we are done waiting yet.
                synchronized(mLock) {
                    throwIfClosedLocked();
                    final android.database.sqlite.SQLiteConnection connection = waiter.mAssignedConnection;
                    final java.lang.RuntimeException ex = waiter.mException;
                    ...
                    if (now < nextBusyTimeoutTime) {
                        ...
                        busyTimeoutMillis = android.database.sqlite.SQLiteConnectionPool.CONNECTION_POOL_BUSY_MILLIS;
    ...
    // Can't throw.
    private void cancelConnectionWaiterLocked(android.database.sqlite.SQLiteConnectionPool.ConnectionWaiter waiter) {
        ...
        android.database.sqlite.SQLiteConnectionPool.ConnectionWaiter predecessor = null;
        android.database.sqlite.SQLiteConnectionPool.ConnectionWaiter current = mConnectionWaiterQueue;
        ...
        waiter.mException = new android.os.OperationCanceledException();
        java.util.concurrent.locks.LockSupport.unpark(waiter.mThread);
    ...
    private void logConnectionPoolBusyLocked(long waitMillis, int connectionFlags) {
        final java.lang.Thread thread = java.lang.Thread.currentThread();
        ...
        msg.append("with flags 0x").append(java.lang.Integer.toHexString(connectionFlags));
        msg.append(" for ").append(waitMillis * 0.001F).append(" seconds.\n");
        java.util.ArrayList<java.lang.String> requests = new java.util.ArrayList<java.lang.String>();
        int activeConnections = 0;
        int idleConnections = 0;
        if (!mAcquiredConnections.isEmpty()) {
            for (android.database.sqlite.SQLiteConnection connection : mAcquiredConnections.keySet()) {
                java.lang.String description = connection.describeCurrentOperationUnsafe();
                if (description != null) {
        ...
        if (!requests.isEmpty()) {
            ...
            for (java.lang.String request : requests) {
                msg.append("  ").append(request).append("\n");
            }
        }
        android.util.Log.w(android.database.sqlite.SQLiteConnectionPool.TAG, msg.toString());
    }
    ...
    private void wakeConnectionWaitersLocked() {
        ...
        android.database.sqlite.SQLiteConnectionPool.ConnectionWaiter predecessor = null;
        android.database.sqlite.SQLiteConnectionPool.ConnectionWaiter waiter = mConnectionWaiterQueue;
        ...
        while (waiter != null) {
            ...
            if (!mIsOpen) {
                ...
                try {
                    ...
                    if (connection != null) {
                        ...
                        if (nonPrimaryConnectionNotAvailable && primaryConnectionNotAvailable) {
            ...
            final android.database.sqlite.SQLiteConnectionPool.ConnectionWaiter successor = waiter.mNext;
            if (unpark) {
                if (predecessor != null) {
    ...
    // Might throw.
    private android.database.sqlite.SQLiteConnection tryAcquirePrimaryConnectionLocked(int connectionFlags) {
        // If the primary connection is available, acquire it now.
        android.database.sqlite.SQLiteConnection connection = mAvailablePrimaryConnection;
        if (connection != null) {
            mAvailablePrimaryConnection = null;
        ...
        for (android.database.sqlite.SQLiteConnection acquiredConnection : mAcquiredConnections.keySet()) {
    ...
    // Might throw.
    private android.database.sqlite.SQLiteConnection tryAcquireNonPrimaryConnectionLocked(java.lang.String sql, int connectionFlags) {
        ...
        connection = /* primaryConnection */
        openConnectionLocked(mConfiguration, false);// might throw
    ...
    // Might throw.
    private void finishAcquireConnectionLocked(android.database.sqlite.SQLiteConnection connection, int connectionFlags) {
        try {
            final boolean readOnly = (connectionFlags & android.database.sqlite.SQLiteConnectionPool.CONNECTION_FLAG_READ_ONLY) != 0;
            connection.setOnlyAllowReadOnlyOperations(readOnly);
            mAcquiredConnections.put(connection, android.database.sqlite.SQLiteConnectionPool.AcquiredConnectionStatus.NORMAL);
        } catch (java.lang.RuntimeException ex) {
            android.util.Log.e(android.database.sqlite.SQLiteConnectionPool.TAG, (("Failed to prepare acquired connection for session, closing it: " + connection) + ", connectionFlags=") + connectionFlags);
            closeConnectionAndLogExceptionsLocked(connection);
            throw ex;// rethrow!
    ...
    private static int getPriority(int connectionFlags) {
        return (connectionFlags & android.database.sqlite.SQLiteConnectionPool.CONNECTION_FLAG_INTERACTIVE) != 0 ? 1 : 0;
    }

    private void setMaxConnectionPoolSizeLocked() {
        if ((mConfiguration.openFlags & android.database.sqlite.SQLiteDatabase.ENABLE_WRITE_AHEAD_LOGGING) != 0) {
            mMaxConnectionPoolSize = android.database.sqlite.SQLiteGlobal.getWALConnectionPoolSize();
        } else {
    ...
    private android.database.sqlite.SQLiteConnectionPool.ConnectionWaiter obtainConnectionWaiterLocked(java.lang.Thread thread, long startTime, int priority, boolean wantPrimaryConnection, java.lang.String sql, int connectionFlags) {
        android.database.sqlite.SQLiteConnectionPool.ConnectionWaiter waiter = mConnectionWaiterPool;
        if (waiter != null) {
            mConnectionWaiterPool = waiter.mNext;
            waiter.mNext = null;
        } else {
            waiter = new android.database.sqlite.SQLiteConnectionPool.ConnectionWaiter();
        }
    ...
    private void recycleConnectionWaiterLocked(android.database.sqlite.SQLiteConnectionPool.ConnectionWaiter waiter) {
    ...
    public void dump(android.util.Printer printer, boolean verbose) {
        android.util.Printer indentedPrinter = android.util.PrefixPrinter.create(printer, "    ");
        synchronized(mLock) {
            ...
            if (!mAcquiredConnections.isEmpty()) {
                for (java.util.Map.Entry<android.database.sqlite.SQLiteConnection, android.database.sqlite.SQLiteConnectionPool.AcquiredConnectionStatus> entry : mAcquiredConnections.entrySet()) {
                    final android.database.sqlite.SQLiteConnection connection = entry.getKey();
            ...
            if (mConnectionWaiterQueue != null) {
                int i = 0;
                final long now = android.os.SystemClock.uptimeMillis();
                for (android.database.sqlite.SQLiteConnectionPool.ConnectionWaiter waiter = mConnectionWaiterQueue; waiter != null; waiter = waiter.mNext , i++) {
