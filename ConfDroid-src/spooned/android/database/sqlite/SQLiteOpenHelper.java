...
public abstract class SQLiteOpenHelper {
    ...
    public SQLiteOpenHelper(android.content.Context context, java.lang.String name, android.database.sqlite.SQLiteDatabase.CursorFactory factory, int version, android.database.DatabaseErrorHandler errorHandler) {
        if (version < 1)
            throw new java.lang.IllegalArgumentException("Version must be >= 1, was " + version);
        ...
        try {
            ...
            if (db != null) {
                ...
                if (mName == null) {
                    ...
                    try {
                        if (android.database.sqlite.SQLiteOpenHelper.DEBUG_STRICT_READONLY && (!writable)) {
                            final java.lang.String path = mContext.getDatabasePath(mName).getPath();
                            db = android.database.sqlite.SQLiteDatabase.openDatabase(path, mFactory, android.database.sqlite.SQLiteDatabase.OPEN_READONLY, mErrorHandler);
                        } else {
                            db = mContext.openOrCreateDatabase(mName, mEnableWriteAheadLogging ? android.content.Context.MODE_ENABLE_WRITE_AHEAD_LOGGING : 0, mFactory, mErrorHandler);
                        }
                    } catch (android.database.sqlite.SQLiteException ex) {
                        if (writable) {
                            throw ex;
                        }
                        android.util.Log.e(android.database.sqlite.SQLiteOpenHelper.TAG, ("Couldn't open " + mName) + " for writing (will try read-only):", ex);
                        final java.lang.String path = mContext.getDatabasePath(mName).getPath();
                        db = android.database.sqlite.SQLiteDatabase.openDatabase(path, mFactory, android.database.sqlite.SQLiteDatabase.OPEN_READONLY, mErrorHandler);
                    }
                }
            ...
            if (version != mNewVersion) {
                if (db.isReadOnly()) {
                    throw new android.database.sqlite.SQLiteException((((("Can't upgrade read-only database from version " + db.getVersion()) + " to ") + mNewVersion) + ": ") + mName);
                }
                db.beginTransaction();
                try {
                    if (version == 0) {
            ...
            onOpen(db);
            if (db.isReadOnly()) {
                android.util.Log.w(android.database.sqlite.SQLiteOpenHelper.TAG, ("Opened " + mName) + " in read-only mode");
            }
            mDatabase = db;
            return db;
        } finally {
