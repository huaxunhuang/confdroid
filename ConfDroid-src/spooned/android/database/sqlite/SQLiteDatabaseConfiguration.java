...
public final class SQLiteDatabaseConfiguration {
     ...
     */
    public final java.util.ArrayList<android.database.sqlite.SQLiteCustomFunction> customFunctions = new java.util.ArrayList<android.database.sqlite.SQLiteCustomFunction>();
     ...
     * 		The database path.
     * @param openFlags
     * 		Open flags for the database, such as {@link SQLiteDatabase#OPEN_READWRITE}.
     */
    public SQLiteDatabaseConfiguration(java.lang.String path, int openFlags) {
        if (path == null) {
            throw new java.lang.IllegalArgumentException("path must not be null.");
        }
        this.path = path;
        label = android.database.sqlite.SQLiteDatabaseConfiguration.stripPathForLogs(path);
    ...
    public boolean isInMemoryDb() {
        return path.equalsIgnoreCase(android.database.sqlite.SQLiteDatabaseConfiguration.MEMORY_DB_PATH);
    }

    private static java.lang.String stripPathForLogs(java.lang.String path) {
        if (path.indexOf('@') == (-1)) {
            return path;
        }
        return android.database.sqlite.SQLiteDatabaseConfiguration.EMAIL_IN_DB_PATTERN.matcher(path).replaceAll("XX@YY");
    }
}
