...
public final class SQLiteSession {
     ...
     * 		if {@link #setTransactionSuccessful} has already been
     ...
     * @see #endTransaction
     */
    public void beginTransaction(int transactionMode, android.database.sqlite.SQLiteTransactionListener transactionListener, int connectionFlags, android.os.CancellationSignal cancellationSignal) {
        throwIfTransactionMarkedSuccessful();
        beginTransactionUnchecked(transactionMode, transactionListener, connectionFlags, cancellationSignal);
    }

    private void beginTransactionUnchecked(int transactionMode, android.database.sqlite.SQLiteTransactionListener transactionListener, int connectionFlags, android.os.CancellationSignal cancellationSignal) {
        ...
        try {
            ...
            if (mTransactionStack == null) {
                ...
                switch (transactionMode) {
                    case android.database.sqlite.SQLiteSession.TRANSACTION_MODE_IMMEDIATE :
                        mConnection.execute("BEGIN IMMEDIATE;", null, cancellationSignal);// might throw

                        break;
                    case android.database.sqlite.SQLiteSession.TRANSACTION_MODE_EXCLUSIVE :
            ...
            android.database.sqlite.SQLiteSession.Transaction transaction = obtainTransaction(transactionMode, transactionListener);
    ...
    private void endTransactionUnchecked(android.os.CancellationSignal cancellationSignal, boolean yielding) {
        if (cancellationSignal != null) {
            cancellationSignal.throwIfCanceled();
        }
        final android.database.sqlite.SQLiteSession.Transaction top = mTransactionStack;
        boolean successful = (top.mMarkedSuccessful || yielding) && (!top.mChildFailed);
        java.lang.RuntimeException listenerException = null;
        final android.database.sqlite.SQLiteTransactionListener listener = top.mListener;
        if (listener != null) {
            try {
                if (successful) {
     ...
     * </p>
     *
     * @param sleepAfterYieldDelayMillis
     ...
     * @throws IllegalStateException
     * 		if <code>throwIfNested</code> is true and
     ...
     * @see #endTransaction
     */
    public boolean yieldTransaction(long sleepAfterYieldDelayMillis, boolean throwIfUnsafe, android.os.CancellationSignal cancellationSignal) {
        if (throwIfUnsafe) {
            throwIfNoTransaction();
            throwIfTransactionMarkedSuccessful();
    ...
    private boolean yieldTransactionUnchecked(long sleepAfterYieldDelayMillis, android.os.CancellationSignal cancellationSignal) {
        ...
        final android.database.sqlite.SQLiteTransactionListener listener = mTransactionStack.mListener;
        final int connectionFlags = mConnectionFlags;
        endTransactionUnchecked(cancellationSignal, true);// might throw

        if (sleepAfterYieldDelayMillis > 0) {
            try {
                java.lang.Thread.sleep(sleepAfterYieldDelayMillis);
     ...
     * 		if the operation was canceled.
     */
    public void prepare(java.lang.String sql, int connectionFlags, android.os.CancellationSignal cancellationSignal, android.database.sqlite.SQLiteStatementInfo outStatementInfo) {
        if (sql == null) {
     ...
     * 		if the operation was canceled.
     */
    public void execute(java.lang.String sql, java.lang.Object[] bindArgs, int connectionFlags, android.os.CancellationSignal cancellationSignal) {
        if (sql == null) {
            throw new java.lang.IllegalArgumentException("sql must not be null.");
        }
     ...
     * 		if the operation was canceled.
     */
    public long executeForLong(java.lang.String sql, java.lang.Object[] bindArgs, int connectionFlags, android.os.CancellationSignal cancellationSignal) {
        if (sql == null) {
            throw new java.lang.IllegalArgumentException("sql must not be null.");
        }
        if (executeSpecial(sql, bindArgs, connectionFlags, cancellationSignal)) {
     ...
     * 		if the operation was canceled.
     */
    public java.lang.String executeForString(java.lang.String sql, java.lang.Object[] bindArgs, int connectionFlags, android.os.CancellationSignal cancellationSignal) {
        if (sql == null) {
            throw new java.lang.IllegalArgumentException("sql must not be null.");
        }
        if (executeSpecial(sql, bindArgs, connectionFlags, cancellationSignal)) {
     ...
     * 		if the operation was canceled.
     */
    public android.os.ParcelFileDescriptor executeForBlobFileDescriptor(java.lang.String sql, java.lang.Object[] bindArgs, int connectionFlags, android.os.CancellationSignal cancellationSignal) {
        if (sql == null) {
            throw new java.lang.IllegalArgumentException("sql must not be null.");
        }
        if (executeSpecial(sql, bindArgs, connectionFlags, cancellationSignal)) {
        ...
        try {
            return mConnection.executeForBlobFileDescriptor(sql, bindArgs, cancellationSignal);// might throw
     ...
     * 		if the operation was canceled.
     */
    public int executeForChangedRowCount(java.lang.String sql, java.lang.Object[] bindArgs, int connectionFlags, android.os.CancellationSignal cancellationSignal) {
        if (sql == null) {
            throw new java.lang.IllegalArgumentException("sql must not be null.");
        }
        if (executeSpecial(sql, bindArgs, connectionFlags, cancellationSignal)) {
     ...
     * 		if the operation was canceled.
     */
    public long executeForLastInsertedRowId(java.lang.String sql, java.lang.Object[] bindArgs, int connectionFlags, android.os.CancellationSignal cancellationSignal) {
        if (sql == null) {
            throw new java.lang.IllegalArgumentException("sql must not be null.");
        }
        if (executeSpecial(sql, bindArgs, connectionFlags, cancellationSignal)) {
     ...
     * 		if the operation was canceled.
     */
    public int executeForCursorWindow(java.lang.String sql, java.lang.Object[] bindArgs, android.database.CursorWindow window, int startPos, int requiredPos, boolean countAllRows, int connectionFlags, android.os.CancellationSignal cancellationSignal) {
        if (sql == null) {
            throw new java.lang.IllegalArgumentException("sql must not be null.");
        }
        if (window == null) {
        ...
        try {
            return mConnection.executeForCursorWindow(sql, bindArgs, window, startPos, requiredPos, countAllRows, cancellationSignal);// might throw
     ...
     * 		if the operation was canceled.
     */
    private boolean executeSpecial(java.lang.String sql, java.lang.Object[] bindArgs, int connectionFlags, android.os.CancellationSignal cancellationSignal) {
        if (cancellationSignal != null) {
            cancellationSignal.throwIfCanceled();
        }
        final int type = android.database.DatabaseUtils.getSqlStatementType(sql);
        switch (type) {
            case android.database.DatabaseUtils.STATEMENT_BEGIN :
                beginTransaction(android.database.sqlite.SQLiteSession.TRANSACTION_MODE_EXCLUSIVE, null, connectionFlags, cancellationSignal);
                return true;
            case android.database.DatabaseUtils.STATEMENT_COMMIT :
                setTransactionSuccessful();
                endTransaction(cancellationSignal);
                return true;
            case android.database.DatabaseUtils.STATEMENT_ABORT :
                endTransaction(cancellationSignal);
                return true;
        }
        return false;
    ...
    private void acquireConnection(java.lang.String sql, int connectionFlags, android.os.CancellationSignal cancellationSignal) {
        if (mConnection == null) {
            assert mConnectionUseCount == 0;
            mConnection = mConnectionPool.acquireConnection(sql, connectionFlags, cancellationSignal);// might throw
    ...
    private android.database.sqlite.SQLiteSession.Transaction obtainTransaction(int mode, android.database.sqlite.SQLiteTransactionListener listener) {
        android.database.sqlite.SQLiteSession.Transaction transaction = mTransactionPool;
        if (transaction != null) {
