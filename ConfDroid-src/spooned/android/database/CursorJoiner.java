...
public final class CursorJoiner implements java.lang.Iterable<android.database.CursorJoiner.Result> , java.util.Iterator<android.database.CursorJoiner.Result> {
     ...
     */
    public CursorJoiner(android.database.Cursor cursorLeft, java.lang.String[] columnNamesLeft, android.database.Cursor cursorRight, java.lang.String[] columnNamesRight) {
        if (columnNamesLeft.length != columnNamesRight.length) {
            throw new java.lang.IllegalArgumentException((("you must have the same number of columns on the left and right, " + columnNamesLeft.length) + " != ") + columnNamesRight.length);
        }
        ...
        mValues = new java.lang.String[mColumnsLeft.length * 2];
    }
     ...
     */
    private int[] buildColumnIndiciesArray(android.database.Cursor cursor, java.lang.String[] columnNames) {
    ...
    public boolean hasNext() {
        if (mCompareResultIsValid) {
            switch (mCompareResult) {
                ...
                default :
                    throw new java.lang.IllegalStateException("bad value for mCompareResult, " + mCompareResult);
            }
        } else {
    ...
    public android.database.CursorJoiner.Result next() {
        ...
        if (hasLeft && hasRight) {
            /* start filling at index 0 */
            android.database.CursorJoiner.populateValues(mValues, mCursorLeft, mColumnsLeft, 0);
            /* start filling at index 1 */
            android.database.CursorJoiner.populateValues(mValues, mCursorRight, mColumnsRight, 1);
            switch (android.database.CursorJoiner.compareStrings(mValues)) {
                case -1 :
                    mCompareResult = android.database.CursorJoiner.Result.LEFT;
                    break;
                case 0 :
                    mCompareResult = android.database.CursorJoiner.Result.BOTH;
                    break;
                case 1 :
                    mCompareResult = android.database.CursorJoiner.Result.RIGHT;
                    break;
            }
        } else
            if (hasLeft) {
     ...
     */
    private static void populateValues(java.lang.String[] values, android.database.Cursor cursor, int[] columnIndicies, int startingIndex) {
     ...
     */
    private static int compareStrings(java.lang.String... values) {
