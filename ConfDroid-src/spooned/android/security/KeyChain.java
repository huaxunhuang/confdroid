 ...
 * certificates via the {@code Intent} provided by {@link #createInstallIntent}. Private keys installed via this {@code Intent} will be accessible via {@link #choosePrivateKeyAlias} while
...
public final class KeyChain {
     ...
     *
     *
     * @unknown Also used by KeyChainService implementation
     */
     ...
     *
     * @unknown Also used by KeyChainActivity implementation
     */
     ...
     * the {@code Intent} returned by {@link #createInstallIntent}.
     * The extra value should be a PEM or ASN.1 DER encoded {@code byte[]}. An {@link X509Certificate} can be converted to DER
    ...
    public static android.content.Intent createInstallIntent() {
        android.content.Intent intent = new android.content.Intent(android.security.KeyChain.ACTION_INSTALL);
        intent.setClassName(android.security.KeyChain.CERT_INSTALLER_PACKAGE, "com.android.certinstaller.CertInstallerMain");
        return intent;
    }
     ...
     *
     * @param activity
     * 		The {@link Activity} context to use for
     ...
     * 		unavailable.
     */
    public static void choosePrivateKeyAlias(@android.annotation.NonNull
    android.app.Activity activity, @android.annotation.NonNull
    android.security.KeyChainAliasCallback response, @android.security.keystore.KeyProperties.KeyAlgorithmEnum
    java.lang.String[] keyTypes, java.security.Principal[] issuers, @android.annotation.Nullable
    java.lang.String host, int port, @android.annotation.Nullable
    java.lang.String alias) {
        android.net.Uri uri = null;
        if (host != null) {
            uri = new android.net.Uri.Builder().authority(host + (port != (-1) ? ":" + port : "")).build();
        }
        android.security.KeyChain.choosePrivateKeyAlias(activity, response, keyTypes, issuers, uri, alias);
    }
    ...
    java.lang.String alias) {
        ...
        android.content.Intent intent = new android.content.Intent(android.security.KeyChain.ACTION_CHOOSER);
        intent.setPackage(android.security.KeyChain.KEYCHAIN_PACKAGE);
        intent.putExtra(android.security.KeyChain.EXTRA_RESPONSE, new android.security.KeyChain.AliasResponse(response));
        intent.putExtra(android.security.KeyChain.EXTRA_URI, uri);
        intent.putExtra(android.security.KeyChain.EXTRA_ALIAS, alias);
        // the PendingIntent is used to get calling package name
        intent.putExtra(android.security.KeyChain.EXTRA_SENDER, android.app.PendingIntent.getActivity(activity, 0, new android.content.Intent(), 0));
    ...
    private static class AliasResponse extends android.security.IKeyChainAliasCallback.Stub {
        ...
        private AliasResponse(android.security.KeyChainAliasCallback keyChainAliasResponse) {
    ...
    public static java.security.PrivateKey getPrivateKey(@android.annotation.NonNull
    android.content.Context context, @android.annotation.NonNull
    java.lang.String alias) throws android.security.KeyChainException, java.lang.InterruptedException {
        if (alias == null) {
            throw new java.lang.NullPointerException("alias == null");
        }
        android.security.KeyChain.KeyChainConnection keyChainConnection = android.security.KeyChain.bind(context.getApplicationContext());
        try {
            final android.security.IKeyChainService keyChainService = keyChainConnection.getService();
            final java.lang.String keyId = keyChainService.requestPrivateKey(alias);
            if (keyId == null) {
                return null;
            }
            return android.security.keystore.AndroidKeyStoreProvider.loadAndroidKeyStorePrivateKeyFromKeystore(android.security.KeyStore.getInstance(), keyId, android.security.KeyStore.UID_SELF);
    ...
    @android.annotation.Nullable
    @android.annotation.WorkerThread
    public static java.security.cert.X509Certificate[] getCertificateChain(@android.annotation.NonNull
    android.content.Context context, @android.annotation.NonNull
    java.lang.String alias) throws android.security.KeyChainException, java.lang.InterruptedException {
        if (alias == null) {
            throw new java.lang.NullPointerException("alias == null");
        ...
        android.security.KeyChain.KeyChainConnection keyChainConnection = android.security.KeyChain.bind(context.getApplicationContext());
        try {
            android.security.IKeyChainService keyChainService = keyChainConnection.getService();
            final byte[] certificateBytes = keyChainService.getCertificate(alias);
            if (certificateBytes == null) {
                return null;
            }
            java.security.cert.X509Certificate leafCert = android.security.KeyChain.toCertificate(certificateBytes);
            ...
            if ((certChainBytes != null) && (certChainBytes.length != 0)) {
                java.util.Collection<java.security.cert.X509Certificate> chain = android.security.KeyChain.toCertificates(certChainBytes);
                java.util.ArrayList<java.security.cert.X509Certificate> fullChain = new java.util.ArrayList<>(chain.size() + 1);
                fullChain.add(leafCert);
                fullChain.addAll(chain);
                return fullChain.toArray(new java.security.cert.X509Certificate[fullChain.size()]);
            } else {
                ...
                com.android.org.conscrypt.TrustedCertificateStore store = new com.android.org.conscrypt.TrustedCertificateStore();
                java.util.List<java.security.cert.X509Certificate> chain = store.getCertificateChain(leafCert);
     ...
     * "RSA").
     */
    public static boolean isKeyAlgorithmSupported(@android.annotation.NonNull
    @android.security.keystore.KeyProperties.KeyAlgorithmEnum
    java.lang.String algorithm) {
        final java.lang.String algUpper = algorithm.toUpperCase(java.util.Locale.US);
        return android.security.keystore.KeyProperties.KEY_ALGORITHM_EC.equals(algUpper) || android.security.keystore.KeyProperties.KEY_ALGORITHM_RSA.equals(algUpper);
    }
    ...
    <pre>{@code PrivateKey key = ...; // private key from KeyChain

    KeyFactory keyFactory =
    KeyFactory.getInstance(key.getAlgorithm(), "AndroidKeyStore");
    KeyInfo keyInfo = keyFactory.getKeySpec(key, KeyInfo.class);
    if (keyInfo.isInsideSecureHardware()) {
    // The key is bound to the secure hardware of this Android}}</pre>
     */
    @java.lang.Deprecated
    public static boolean isBoundKeyAlgorithm(@android.annotation.NonNull
    @android.security.keystore.KeyProperties.KeyAlgorithmEnum
    java.lang.String algorithm) {
        if (!android.security.KeyChain.isKeyAlgorithmSupported(algorithm)) {
            return false;
        }
        return android.security.KeyStore.getInstance().isHardwareBacked(algorithm);
    }
     ...
     */
    @android.annotation.NonNull
    public static java.security.cert.X509Certificate toCertificate(@android.annotation.NonNull
    byte[] bytes) {
        if (bytes == null) {
     ...
     */
    @android.annotation.NonNull
    public static java.util.Collection<java.security.cert.X509Certificate> toCertificates(@android.annotation.NonNull
    byte[] bytes) {
        if (bytes == null) {
            throw new java.lang.IllegalArgumentException("bytes == null");
        }
     ...
     *
     *
     * @unknown for reuse by CertInstaller and Settings.
     * @see KeyChain#bind
     */
    public static final class KeyChainConnection implements java.io.Closeable {
        ...
        private KeyChainConnection(android.content.Context context, android.content.ServiceConnection serviceConnection, android.security.IKeyChainService service) {
     ...
     */
    @android.annotation.WorkerThread
    public static android.security.KeyChain.KeyChainConnection bind(@android.annotation.NonNull
    android.content.Context context) throws java.lang.InterruptedException {
        return android.security.KeyChain.bindAsUser(context, android.os.Process.myUserHandle());
    }
     ...
     */
    @android.annotation.WorkerThread
    public static android.security.KeyChain.KeyChainConnection bindAsUser(@android.annotation.NonNull
    android.content.Context context, android.os.UserHandle user) throws java.lang.InterruptedException {
        if (context == null) {
            throw new java.lang.NullPointerException("context == null");
        }
        android.security.KeyChain.ensureNotOnMainThread(context);
        final java.util.concurrent.BlockingQueue<android.security.IKeyChainService> q = new java.util.concurrent.LinkedBlockingQueue<android.security.IKeyChainService>(1);
        android.content.ServiceConnection keyChainServiceConnection = new android.content.ServiceConnection() {
        ...
        android.content.ComponentName comp = intent.resolveSystemService(context.getPackageManager(), 0);
        intent.setComponent(comp);
        if ((comp == null) || (!context.bindServiceAsUser(intent, keyChainServiceConnection, android.content.Context.BIND_AUTO_CREATE, user))) {
            throw new java.lang.AssertionError("could not bind to KeyChainService");
        }
        return new android.security.KeyChain.KeyChainConnection(context, keyChainServiceConnection, q.take());
    }

    private static void ensureNotOnMainThread(@android.annotation.NonNull
