...
class GLErrorWrapper extends android.opengl.GLWrapperBase {
    ...
    public GLErrorWrapper(javax.microedition.khronos.opengles.GL gl, int configFlags) {
        super(gl);
        mCheckError = (configFlags & android.opengl.GLDebugHelper.CONFIG_CHECK_GL_ERROR) != 0;
        mCheckThread = (configFlags & android.opengl.GLDebugHelper.CONFIG_CHECK_THREAD) != 0;
    }
        ...
        if (mCheckThread) {
            ...
            if (mOurThread == null) {
                ...
                if (!mOurThread.equals(currentThread)) {
                    throw new android.opengl.GLException(android.opengl.GLDebugHelper.ERROR_WRONG_THREAD, "OpenGL method called from wrong thread.");
                }
            }
        }
    }
