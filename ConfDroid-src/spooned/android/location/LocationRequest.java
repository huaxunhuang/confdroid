...
public final class LocationRequest implements android.os.Parcelable {
    ...
    private int mQuality = android.location.LocationRequest.POWER_LOW;

    private long mInterval = (60 * 60) * 1000;// 60 minutes


    private long mFastestInterval = ((long) (mInterval / android.location.LocationRequest.FASTEST_INTERVAL_FACTOR));// 10 minutes


    private boolean mExplicitFastestInterval = false;

    private long mExpireAt = java.lang.Long.MAX_VALUE;// no expiry


    private int mNumUpdates = java.lang.Integer.MAX_VALUE;// no expiry


    private float mSmallestDisplacement = 0.0F;// meters


    private android.os.WorkSource mWorkSource = null;

    private boolean mHideFromAppOps = false;// True if this request shouldn't be counted by AppOps


    private java.lang.String mProvider = android.location.LocationManager.FUSED_PROVIDER;// for deprecated APIs that explicitly request a provider
    ...
    public static android.location.LocationRequest create() {
        android.location.LocationRequest request = new android.location.LocationRequest();
        return request;
    }
    ...
    public static android.location.LocationRequest createFromDeprecatedProvider(java.lang.String provider, long minTime, float minDistance, boolean singleShot) {
        ...
        int quality;
        if (android.location.LocationManager.PASSIVE_PROVIDER.equals(provider)) {
            quality = android.location.LocationRequest.POWER_NONE;
        } else
            if (android.location.LocationManager.GPS_PROVIDER.equals(provider)) {
                quality = android.location.LocationRequest.ACCURACY_FINE;
            } else {
                quality = android.location.LocationRequest.POWER_LOW;
            }

        android.location.LocationRequest request = new android.location.LocationRequest().setProvider(provider).setQuality(quality).setInterval(minTime).setFastestInterval(minTime).setSmallestDisplacement(minDistance);
        if (singleShot)
    ...
    public static android.location.LocationRequest createFromDeprecatedCriteria(android.location.Criteria criteria, long minTime, float minDistance, boolean singleShot) {
        ...
        switch (criteria.getAccuracy()) {
            case android.location.Criteria.ACCURACY_COARSE :
                quality = android.location.LocationRequest.ACCURACY_BLOCK;
                break;
            case android.location.Criteria.ACCURACY_FINE :
                quality = android.location.LocationRequest.ACCURACY_FINE;
                break;
            default :
                {
                    switch (criteria.getPowerRequirement()) {
                        case android.location.Criteria.POWER_HIGH :
                            quality = android.location.LocationRequest.POWER_HIGH;
                            break;
                        default :
                            quality = android.location.LocationRequest.POWER_LOW;
                    }
        ...
        android.location.LocationRequest request = new android.location.LocationRequest().setQuality(quality).setInterval(minTime).setFastestInterval(minTime).setSmallestDisplacement(minDistance);
        if (singleShot)
            request.setNumUpdates(1);
     ...
     * @unknown 
     */
    public LocationRequest(android.location.LocationRequest src) {
     ...
     * 		if the quality constant is not valid
     * @return the same object, so that setters can be chained
     */
    public android.location.LocationRequest setQuality(int quality) {
        android.location.LocationRequest.checkQuality(quality);
        mQuality = quality;
        return this;
    }
     ...
     * @throws InvalidArgumentException
     * 		if the interval is less than zero
     * @return the same object, so that setters can be chained
     */
    public android.location.LocationRequest setInterval(long millis) {
        android.location.LocationRequest.checkInterval(millis);
        mInterval = millis;
        if (!mExplicitFastestInterval) {
            mFastestInterval = ((long) (mInterval / android.location.LocationRequest.FASTEST_INTERVAL_FACTOR));
        }
        return this;
    }
    ...
    public android.location.LocationRequest setExpireIn(long millis) {
        ...
        if (millis > (java.lang.Long.MAX_VALUE - elapsedRealtime)) {
            mExpireAt = java.lang.Long.MAX_VALUE;
        } else {
    ...
    public android.location.LocationRequest setNumUpdates(int numUpdates) {
        if (numUpdates <= 0)
            throw new java.lang.IllegalArgumentException("invalid numUpdates: " + numUpdates);
    ...
    public void decrementNumUpdates() {
        if (mNumUpdates != java.lang.Integer.MAX_VALUE) {
            mNumUpdates--;
        }
        if (mNumUpdates < 0) {
            mNumUpdates = 0;
    ...
    public android.location.LocationRequest setProvider(java.lang.String provider) {
        android.location.LocationRequest.checkProvider(provider);
        mProvider = provider;
        return this;
    }
    ...
    public android.location.LocationRequest setSmallestDisplacement(float meters) {
        android.location.LocationRequest.checkDisplacement(meters);
     ...
     *
     * @param workSource
     * 		WorkSource defining power blame for this location request.
     * @unknown 
     */
    @android.annotation.SystemApi
     ...
     *
     * @param hideFromAppOps
     * 		If true AppOps won't keep track of this location request.
    ...
    private static void checkInterval(long millis) {
        if (millis < 0) {
            throw new java.lang.IllegalArgumentException("invalid interval: " + millis);
        }
    }

    private static void checkQuality(int quality) {
        switch (quality) {
            case android.location.LocationRequest.ACCURACY_FINE :
            case android.location.LocationRequest.ACCURACY_BLOCK :
            case android.location.LocationRequest.ACCURACY_CITY :
            case android.location.LocationRequest.POWER_NONE :
            case android.location.LocationRequest.POWER_LOW :
            case android.location.LocationRequest.POWER_HIGH :
                break;
            default :
                throw new java.lang.IllegalArgumentException("invalid quality: " + quality);
        }
    }

    private static void checkDisplacement(float meters) {
        if (meters < 0.0F) {
            throw new java.lang.IllegalArgumentException("invalid displacement: " + meters);
        }
    }

    private static void checkProvider(java.lang.String name) {
        if (name == null) {
            throw new java.lang.IllegalArgumentException("invalid provider: " + name);
        }
    }

    public static final android.os.Parcelable.Creator<android.location.LocationRequest> CREATOR = new android.os.Parcelable.Creator<android.location.LocationRequest>() {
        @java.lang.Override
        public android.location.LocationRequest createFromParcel(android.os.Parcel in) {
            android.location.LocationRequest request = new android.location.LocationRequest();
            ...
            java.lang.String provider = in.readString();
            if (provider != null)
                request.setProvider(provider);

            android.os.WorkSource workSource = in.readParcelable(null);
            if (workSource != null)
        ...
        @java.lang.Override
        public android.location.LocationRequest[] newArray(int size) {
            return new android.location.LocationRequest[size];
        }
    };
    ...
    public java.lang.String toString() {
        ...
        s.append("Request[").append(android.location.LocationRequest.qualityToString(mQuality));
        if (mProvider != null)
            s.append(' ').append(mProvider);

        if (mQuality != android.location.LocationRequest.POWER_NONE) {
            s.append(" requested=");
            android.util.TimeUtils.formatDuration(mInterval, s);
        }
        s.append(" fastest=");
        android.util.TimeUtils.formatDuration(mFastestInterval, s);
        if (mExpireAt != java.lang.Long.MAX_VALUE) {
            long expireIn = mExpireAt - android.os.SystemClock.elapsedRealtime();
            s.append(" expireIn=");
            android.util.TimeUtils.formatDuration(expireIn, s);
        }
        if (mNumUpdates != java.lang.Integer.MAX_VALUE) {
