...
public final class InputEventConsistencyVerifier {
     ...
     * 		The object to which the verifier is attached.
     * @param flags
     * 		Flags to the verifier, or 0 if none.
     */
    @android.annotation.UnsupportedAppUsage
    public InputEventConsistencyVerifier(java.lang.Object caller, int flags) {
     ...
     * 		Flags to the verifier, or 0 if none.
     * @param logTag
     * 		Tag for logging. If null defaults to the short class name.
     */
    public InputEventConsistencyVerifier(java.lang.Object caller, int flags, java.lang.String logTag) {
        this.mCaller = caller;
        this.mFlags = flags;
    ...
    public static boolean isInstrumentationEnabled() {
        return android.view.InputEventConsistencyVerifier.IS_ENG_BUILD;
    }
    ...
    public void reset() {
        ...
        while (mKeyStateList != null) {
            final android.view.InputEventConsistencyVerifier.KeyState state = mKeyStateList;
    ...
    public void onInputEvent(android.view.InputEvent event, int nestingLevel) {
        if (event instanceof android.view.KeyEvent) {
            ...
            if (motionEvent.isTouchEvent()) {
                ...
                if ((motionEvent.getSource() & android.view.InputDevice.SOURCE_CLASS_TRACKBALL) != 0) {
    ...
    public void onKeyEvent(android.view.KeyEvent event, int nestingLevel) {
        if (!startEvent(event, nestingLevel, android.view.InputEventConsistencyVerifier.EVENT_TYPE_KEY)) {
            return;
        }
        try {
            ...
            switch (action) {
                case android.view.KeyEvent.ACTION_DOWN :
                    {
                        android.view.InputEventConsistencyVerifier.KeyState state = /* remove */
                        findKeyState(deviceId, source, keyCode, false);
                        if (state != null) {
                            ...
                            if (state.unhandled) {
                                ...
                                if (((mFlags & android.view.InputEventConsistencyVerifier.FLAG_RAW_DEVICE_INPUT) == 0) && (event.getRepeatCount() == 0)) {
                ...
                case android.view.KeyEvent.ACTION_UP :
                    {
                        android.view.InputEventConsistencyVerifier.KeyState state = /* remove */
                        findKeyState(deviceId, source, keyCode, true);
                        if (state == null) {
    ...
    public void onTrackballEvent(android.view.MotionEvent event, int nestingLevel) {
        if (!startEvent(event, nestingLevel, android.view.InputEventConsistencyVerifier.EVENT_TYPE_TRACKBALL)) {
            return;
        }
        try {
            ensureMetaStateIsNormalized(event.getMetaState());
    ...
    public void onTouchEvent(android.view.MotionEvent event, int nestingLevel) {
        if (!startEvent(event, nestingLevel, android.view.InputEventConsistencyVerifier.EVENT_TYPE_TOUCH)) {
            return;
        }
        final int action = event.getAction();
        final boolean newStream = ((action == android.view.MotionEvent.ACTION_DOWN) || (action == android.view.MotionEvent.ACTION_CANCEL)) || (action == android.view.MotionEvent.ACTION_OUTSIDE);
        ...
        try {
            ...
            if (((!newStream) && (mTouchEventStreamDeviceId != (-1))) && ((mTouchEventStreamDeviceId != deviceId) || (mTouchEventStreamSource != source))) {
                problem(((((((("Touch event stream contains events from multiple sources: " + "previous device id ") + mTouchEventStreamDeviceId) + ", previous source ") + java.lang.Integer.toHexString(mTouchEventStreamSource)) + ", new device id ") + deviceId) + ", new source ") + java.lang.Integer.toHexString(source));
            }
            ...
            if ((source & android.view.InputDevice.SOURCE_CLASS_POINTER) != 0) {
                switch (action) {
                    ...
                    case android.view.MotionEvent.ACTION_MOVE :
                        {
                            final int expectedPointerCount = java.lang.Integer.bitCount(mTouchEventStreamPointers);
    ...
    public void onGenericMotionEvent(android.view.MotionEvent event, int nestingLevel) {
        if (!startEvent(event, nestingLevel, android.view.InputEventConsistencyVerifier.EVENT_TYPE_GENERIC_MOTION)) {
            return;
        }
        try {
            ensureMetaStateIsNormalized(event.getMetaState());
            ...
            if ((source & android.view.InputDevice.SOURCE_CLASS_POINTER) != 0) {
                switch (action) {
                    ...
                    case android.view.MotionEvent.ACTION_BUTTON_PRESS :
                        ...
                        if ((actionButton == android.view.MotionEvent.BUTTON_STYLUS_PRIMARY) && ((buttonState & android.view.MotionEvent.BUTTON_SECONDARY) != 0)) {
                            mButtonsPressed |= android.view.MotionEvent.BUTTON_SECONDARY;
                        } else
                            if ((actionButton == android.view.MotionEvent.BUTTON_STYLUS_SECONDARY) && ((buttonState & android.view.MotionEvent.BUTTON_TERTIARY) != 0)) {
                                mButtonsPressed |= android.view.MotionEvent.BUTTON_TERTIARY;
                            }

                        if (mButtonsPressed != buttonState) {
                            problem(java.lang.String.format("Reported button state differs from " + ("expected button state based on press and release events. " + "Is 0x%08x but expected 0x%08x."), buttonState, mButtonsPressed));
                        }
                        break;
    ...
    public void onUnhandledEvent(android.view.InputEvent event, int nestingLevel) {
        ...
        if (event instanceof android.view.KeyEvent) {
            ...
            final android.view.InputEventConsistencyVerifier.KeyState state = /* remove */
            findKeyState(deviceId, source, keyCode, false);
            if (state != null) {
    ...
    private void ensureMetaStateIsNormalized(int metaState) {
        final int normalizedMetaState = android.view.KeyEvent.normalizeMetaState(metaState);
        if (normalizedMetaState != metaState) {
            problem(java.lang.String.format("Metastate not normalized.  Was 0x%08x but expected 0x%08x.", metaState, normalizedMetaState));
        }
    }
    ...
    private void ensurePointerCountIsOneForThisAction(android.view.MotionEvent event) {
        final int pointerCount = event.getPointerCount();
        if (pointerCount != 1) {
            problem((("Pointer count is " + pointerCount) + " but it should always be 1 for ") + android.view.MotionEvent.actionToString(event.getAction()));
        }
    }

    private void ensureActionButtonIsNonZeroForThisAction(android.view.MotionEvent event) {
        final int actionButton = event.getActionButton();
        if (actionButton == 0) {
            problem("No action button set. Action button should always be non-zero for " + android.view.MotionEvent.actionToString(event.getAction()));
        }
    }
    ...
    private void ensureHistorySizeIsZeroForThisAction(android.view.MotionEvent event) {
        ...
        if (historySize != 0) {
            problem((("History size is " + historySize) + " but it should always be 0 for ") + android.view.MotionEvent.actionToString(event.getAction()));
        }
    }

    private boolean startEvent(android.view.InputEvent event, int nestingLevel, java.lang.String eventType) {
    ...
    private void finishEvent() {
        if ((mViolationMessage != null) && (mViolationMessage.length() != 0)) {
            if (!mCurrentEvent.isTainted()) {
                ...
                mViolationMessage.append("\n  ");
                android.view.InputEventConsistencyVerifier.appendEvent(mViolationMessage, 0, mCurrentEvent, false);
                if ((android.view.InputEventConsistencyVerifier.RECENT_EVENTS_TO_LOG != 0) && (mRecentEvents != null)) {
                    mViolationMessage.append("\n  -- recent events --");
                    for (int i = 0; i < android.view.InputEventConsistencyVerifier.RECENT_EVENTS_TO_LOG; i++) {
                        final int index = ((mMostRecentEventIndex + android.view.InputEventConsistencyVerifier.RECENT_EVENTS_TO_LOG) - i) % android.view.InputEventConsistencyVerifier.RECENT_EVENTS_TO_LOG;
                        final android.view.InputEvent event = mRecentEvents[index];
                        if (event == null) {
                            break;
                        }
                        mViolationMessage.append("\n  ");
                        android.view.InputEventConsistencyVerifier.appendEvent(mViolationMessage, i + 1, event, mRecentEventsUnhandled[index]);
                    }
                }
        ...
        if (android.view.InputEventConsistencyVerifier.RECENT_EVENTS_TO_LOG != 0) {
            if (mRecentEvents == null) {
                mRecentEvents = new android.view.InputEvent[android.view.InputEventConsistencyVerifier.RECENT_EVENTS_TO_LOG];
                mRecentEventsUnhandled = new boolean[android.view.InputEventConsistencyVerifier.RECENT_EVENTS_TO_LOG];
            }
            final int index = (mMostRecentEventIndex + 1) % android.view.InputEventConsistencyVerifier.RECENT_EVENTS_TO_LOG;
    ...
    private static void appendEvent(java.lang.StringBuilder message, int index, android.view.InputEvent event, boolean unhandled) {
    ...
    private void addKeyState(int deviceId, int source, int keyCode) {
        android.view.InputEventConsistencyVerifier.KeyState state = android.view.InputEventConsistencyVerifier.KeyState.obtain(deviceId, source, keyCode);
    ...
    private static final class KeyState {
        ...
        public static android.view.InputEventConsistencyVerifier.KeyState obtain(int deviceId, int source, int keyCode) {
            android.view.InputEventConsistencyVerifier.KeyState state;
            synchronized(android.view.InputEventConsistencyVerifier.KeyState.mRecycledListLock) {
                state = android.view.InputEventConsistencyVerifier.KeyState.mRecycledList;
                if (state != null) {
                    android.view.InputEventConsistencyVerifier.KeyState.mRecycledList = state.next;
                } else {
        ...
        public void recycle() {
            synchronized(android.view.InputEventConsistencyVerifier.KeyState.mRecycledListLock) {
                next = android.view.InputEventConsistencyVerifier.KeyState.mRecycledList;
                android.view.InputEventConsistencyVerifier.KeyState.mRecycledList = next;
            }
        }
    }
}
