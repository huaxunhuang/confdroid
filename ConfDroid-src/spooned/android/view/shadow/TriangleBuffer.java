...
class TriangleBuffer {
    ...
    public void setSize(int width, int height, int border) {
        ...
        mBitmap = android.graphics.Bitmap.createBitmap(width, height, android.graphics.Bitmap.Config.ARGB_8888);
    ...
    public void drawTriangles(int[] index, float[] vert, float[] color, float scale) {
        ...
        for (int i = 0; i < indexSize; i++) {
            ...
            float fy1 = vy;
            float fz1 = c;
            android.view.shadow.TriangleBuffer.triangleZBuffMin(mData, mImgWidth, mImgHeight, fx3, fy3, fz3, fx2, fy2, fz2, fx1, fy1, fz1);
            android.view.shadow.TriangleBuffer.triangleZBuffMin(mData, mImgWidth, mImgHeight, fx1, fy1, fz1, fx2, fy2, fz2, fx3, fy3, fz3);
        }
    ...
    private static void triangleZBuffMin(int[] buff, int w, int h, float fx3, float fy3, float fz3, float fx2, float fy2, float fz2, float fx1, float fy1, float fz1) {
        ...
        float zoff = ((float) ((((fx1 * ((fy3 * fz2) - (fy2 * fz3))) + (fy1 * ((fx2 * fz3) - (fx3 * fz2)))) + (((fx3 * fy2) - (fx2 * fy3)) * fz1)) / d));
        ...
        int fdy23 = dy23 << 4;
        int fdy31 = dy31 << 4;
        int minx = (android.view.math.Math3DHelper.min(x1, x2, x3) + 0xf) >> 4;
        int maxx = (android.view.math.Math3DHelper.max(x1, x2, x3) + 0xf) >> 4;
        int miny = (android.view.math.Math3DHelper.min(y1, y2, y3) + 0xf) >> 4;
        int maxy = (android.view.math.Math3DHelper.max(y1, y2, y3) + 0xf) >> 4;
        if (miny < 0) {
            miny = 0;
        }
        if (minx < 0) {
