...
class SpotShadowVertexCalculator {
     ...
     * @param points
     * 		- how many light source points to generate
     * @param x
     * 		- center X of the light source
     * @param y
     * 		- center Y of the light source
     * @param height
     * 		- how high (z depth) the light should be
    ...
    public static float[] calculateLight(float radius, int points, float x, float y, float height) {
        ...
        for (int i = 0; i < points; i++) {
            double angle = ((2 * i) * java.lang.Math.PI) / points;
            ret[i * 3] = (((float) (java.lang.Math.sin(angle))) * radius) + x;
            ret[(i * 3) + 1] = (((float) (java.lang.Math.cos(angle))) * radius) + y;
            ret[(i * 3) + 2] = height;
        }
        return ret;
    }
     ...
     *
     * @param rays
     * 		- Number of rays to use for tracing
     * @param layers
     * 		- Number of layers for shadow rendering.
     ...
     *
     * @param lightPoly
     * 		- Vertices of a light source.
     * @param lightPolyLength
     * 		- Size of the vertices (usually lightPoly.length/3 unless w is
     * 		included)
     * @param poly
     * 		- Vertices of opaque object casting shadow
     * @param polyLength
     * 		- Size of the vertices
     * @param rays
     * 		- Number of rays to use for tracing. It determines accuracy of the outline
     * 		(bounds) of the shadow
     * @param layers
     * 		- Number of layers for shadow. It determines intensity of pen-umbra
     * @param strength
     * 		- Strength of the shadow overall [0-1]
     * @param retstrips
     * 		- Array mData to be filled in format : {x1, y1, z1, x2, y2, z2}
    ...
    public static int calculateShadow(float[] lightPoly, int lightPolyLength, float[] poly, int polyLength, int rays, int layers, float strength, float[] retstrips) {
        ...
        for (int j = 0; j < lightPolyLength; j++) {
            ...
            if (umbraLength == 0) {
                ...
                umbraLength = android.view.math.Math3DHelper.intersection(outline, polyLength, umbra, umbraLength);
                if (umbraLength == 0) {
                    break;
                }
            }
        ...
        int penumbraLength = android.view.math.Math3DHelper.hull(shadowRegion, shadowRegionLength, penumbra);
        if (umbraLength < 3) {
            // no real umbra make a fake one
            float[] p = new float[3];
            android.view.math.Math3DHelper.centroid3d(lightPoly, lightPolyLength, p);
            ...
            float[] c = new float[2];
            android.view.math.Math3DHelper.centroid2d(centShadow, polyLength, c);
            for (int i = 0; i < polyLength; i++) {
        ...
        android.view.math.Math3DHelper.donutPie2(penumbra, penumbraLength, umbra, umbraLength, rays, layers, strength, retstrips);
        return 1;
    }
}
