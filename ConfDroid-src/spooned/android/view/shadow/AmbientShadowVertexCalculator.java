...
class AmbientShadowVertexCalculator {
    ...
    private final android.view.shadow.AmbientShadowConfig mConfig;

    public AmbientShadowVertexCalculator(android.view.shadow.AmbientShadowConfig config) {
        mConfig = config;
     ...
     *
     * @param polygon
     * 		3d polygon info in format : {x1, y1, z1, x2, y2, z2 ...}
    ...
    public boolean generateVertex(float[] polygon) {
        ...
        android.view.math.Math3DHelper.centroid3d(polygon, polygonLength, centroidxy);
        float cx = centroidxy[0];
        float cy = centroidxy[1];
        android.view.shadow.AmbientShadowVertexCalculator.Rays rays = new android.view.shadow.AmbientShadowVertexCalculator.Rays(mConfig.getRays());
        ...
        for (int i = 0; i < raysLength; i++) {
            float dx = rays.dx[i];
            float dy = rays.dy[i];
            float[] intersection = android.view.math.Math3DHelper.rayIntersectPoly(polygon, polygonLength, cx, cy, dx, dy, 3);
        ...
        for (int i = 0; i < raysLength; i++) {
            ...
            float opacity = 0.8F * (0.5F / (mConfig.getEdgeScale() / 10.0F));
    ...
    private static class Rays {
        ...
        public Rays(int rays) {
            dx = new float[rays];
            dy = new float[rays];
            deltaAngle = (2 * java.lang.Math.PI) / rays;
            for (int i = 0; i < rays; i++) {
                dx[i] = ((float) (java.lang.Math.sin(deltaAngle * i)));
                dy[i] = ((float) (java.lang.Math.cos(deltaAngle * i)));
            }
        }
    }
}
