...
public abstract class DisplayEventReceiver {
    ...
    private static native long nativeInit(java.lang.ref.WeakReference<android.view.DisplayEventReceiver> receiver, android.os.MessageQueue messageQueue, int vsyncSource);
    ...
    public DisplayEventReceiver(android.os.Looper looper) {
        this(looper, android.view.DisplayEventReceiver.VSYNC_SOURCE_APP);
    }
     ...
     * 		The looper to use when invoking callbacks.
     * @param vsyncSource
     * 		The source of the vsync tick. Must be on of the VSYNC_SOURCE_* values.
     */
    public DisplayEventReceiver(android.os.Looper looper, int vsyncSource) {
        if (looper == null) {
            throw new java.lang.IllegalArgumentException("looper must not be null");
        }
        mMessageQueue = looper.getQueue();
        mReceiverPtr = android.view.DisplayEventReceiver.nativeInit(new java.lang.ref.WeakReference<android.view.DisplayEventReceiver>(this), mMessageQueue, vsyncSource);
    ...
    private void dispose(boolean finalized) {
        ...
        if (mReceiverPtr != 0) {
            android.view.DisplayEventReceiver.nativeDispose(mReceiverPtr);
            mReceiverPtr = 0;
        }
        mMessageQueue = null;
    }
     ...
     * 		The timestamp of the pulse, in the {@link System#nanoTime()}
     * 		timebase.
     * @param physicalDisplayId
     * 		Stable display ID that uniquely describes a (display, port) pair.
     * @param frame
     * 		The frame number.  Increases by one for each vertical sync interval.
     */
    @android.annotation.UnsupportedAppUsage
     ...
     *
     * @param timestampNanos
     * 		The timestamp of the event, in the {@link System#nanoTime()}
     * 		timebase.
     * @param physicalDisplayId
     * 		Stable display ID that uniquely describes a (display, port) pair.
     * @param connected
     * 		True if the display is connected, false if it disconnected.
     */
    @android.annotation.UnsupportedAppUsage
    ...
    public void scheduleVsync() {
        if (mReceiverPtr == 0) {
            android.util.Log.w(android.view.DisplayEventReceiver.TAG, "Attempted to schedule a vertical sync pulse but the display event " + "receiver has already been disposed.");
        } else {
            android.view.DisplayEventReceiver.nativeScheduleVsync(mReceiverPtr);
        }
    }
