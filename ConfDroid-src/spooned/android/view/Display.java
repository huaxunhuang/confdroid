...
public final class Display {
    ...
    private android.view.DisplayInfo mDisplayInfo;// never null


    private boolean mIsValid;

    // Temporary display metrics structure used for compatibility mode.
    private final android.util.DisplayMetrics mTempMetrics = new android.util.DisplayMetrics();
     ...
     * @unknown 
     */
    public Display(android.hardware.display.DisplayManagerGlobal global, int displayId, /* @NotNull */
    android.view.DisplayInfo displayInfo, android.view.DisplayAdjustments daj) {
        /* res */
     ...
     */
    public Display(android.hardware.display.DisplayManagerGlobal global, int displayId, /* @NotNull */
    android.view.DisplayInfo displayInfo, android.content.res.Resources res) {
        /* daj */
        this(global, displayId, displayInfo, null, res);
    }

    private Display(android.hardware.display.DisplayManagerGlobal global, int displayId, /* @NotNull */
    android.view.DisplayInfo displayInfo, android.view.DisplayAdjustments daj, android.content.res.Resources res) {
        mGlobal = global;
        mDisplayId = displayId;
        mDisplayInfo = displayInfo;
        mResources = res;
        mDisplayAdjustments = (mResources != null) ? new android.view.DisplayAdjustments(mResources.getConfiguration()) : daj != null ? new android.view.DisplayAdjustments(daj) : null;
        mIsValid = true;
    ...
    public android.view.DisplayAdjustments getDisplayAdjustments() {
        if (mResources != null) {
            final android.view.DisplayAdjustments currentAdjustements = mResources.getDisplayAdjustments();
            if (!mDisplayAdjustments.equals(currentAdjustements)) {
                mDisplayAdjustments = new android.view.DisplayAdjustments(currentAdjustements);
            }
        }
     ...
     *
     * @param outSize
     * 		A {@link Point} object to receive the size information.
     */
     ...
     *
     * @param outSize
     * 		A {@link Rect} object to receive the size information.
     * @see #getSize(Point)
     */
    ...
    public int getMaximumSizeDimension() {
        synchronized(this) {
            updateDisplayInfoLocked();
            return java.lang.Math.max(mDisplayInfo.logicalWidth, mDisplayInfo.logicalHeight);
        }
    }
     ...
     *
     *
     * @unknown Return a rectangle defining the insets of the overscan region of the display.
    Each field of the rectangle is the number of pixels the overscan area extends
    into the display on that side.
     */
    public void getOverscanInsets(android.graphics.Rect outRect) {
        synchronized(this) {
            updateDisplayInfoLocked();
            outRect.set(mDisplayInfo.overscanLeft, mDisplayInfo.overscanTop, mDisplayInfo.overscanRight, mDisplayInfo.overscanBottom);
        }
    }
    ...
    public int getPixelFormat() {
        return android.graphics.PixelFormat.RGBA_8888;
    }
    ...
    public android.view.Display.Mode[] getSupportedModes() {
        synchronized(this) {
            updateDisplayInfoLocked();
            final android.view.Display.Mode[] modes = mDisplayInfo.supportedModes;
            return java.util.Arrays.copyOf(modes, modes.length);
        }
    }
     ...
     * @unknown 
     */
    @android.annotation.RequiresPermission(android.Manifest.permission.CONFIGURE_DISPLAY_COLOR_MODE)
    ...
    public int[] getSupportedColorModes() {
        synchronized(this) {
            ...
            return java.util.Arrays.copyOf(colorModes, colorModes.length);
        }
    }
     ...
     *
     * @param outMetrics
     * 		A {@link DisplayMetrics} object to receive the metrics.
     */
    ...
    public void getRealMetrics(android.util.DisplayMetrics outMetrics) {
        synchronized(this) {
            updateDisplayInfoLocked();
            mDisplayInfo.getLogicalMetrics(outMetrics, android.content.res.CompatibilityInfo.DEFAULT_COMPATIBILITY_INFO, null);
        }
    }
    ...
    public int getState() {
        synchronized(this) {
            updateDisplayInfoLocked();
            return mIsValid ? mDisplayInfo.state : android.view.Display.STATE_UNKNOWN;
        }
    }
    ...
    public boolean hasAccess(int uid) {
        return android.view.Display.hasAccess(uid, mFlags, mOwnerUid, mDisplayId);
    }
    ...
    public static boolean hasAccess(int uid, int flags, int ownerUid, int displayId) {
        return (((((flags & android.view.Display.FLAG_PRIVATE) == 0) || (uid == ownerUid)) || (uid == SYSTEM_UID)) || (uid == 0)) || // Check if the UID is present on given display.
        android.hardware.display.DisplayManagerGlobal.getInstance().isUidPresentOnDisplay(uid, displayId);
    }
    ...
    public boolean isPublicPresentation() {
        return (mFlags & (android.view.Display.FLAG_PRIVATE | android.view.Display.FLAG_PRESENTATION)) == android.view.Display.FLAG_PRESENTATION;
    }

    private void updateDisplayInfoLocked() {
        // Note: The display manager caches display info objects on our behalf.
        android.view.DisplayInfo newInfo = mGlobal.getDisplayInfo(mDisplayId);
        if (newInfo == null) {
            // Preserve the old mDisplayInfo after the display is removed.
            if (mIsValid) {
                mIsValid = false;
                if (android.view.Display.DEBUG) {
                    android.util.Log.d(android.view.Display.TAG, ("Logical display " + mDisplayId) + " was removed.");
                }
            }
        } else {
            ...
            if (!mIsValid) {
                mIsValid = true;
                if (android.view.Display.DEBUG) {
                    android.util.Log.d(android.view.Display.TAG, ("Logical display " + mDisplayId) + " was recreated.");
                }
            }
        }
    }
    ...
    private void updateCachedAppSizeIfNeededLocked() {
        ...
        if (now > (mLastCachedAppSizeUpdate + android.view.Display.CACHED_APP_SIZE_DURATION_MILLIS)) {
            updateDisplayInfoLocked();
            mDisplayInfo.getAppMetrics(mTempMetrics, getDisplayAdjustments());
    ...
    public static java.lang.String typeToString(int type) {
        switch (type) {
            case android.view.Display.TYPE_UNKNOWN :
                return "UNKNOWN";
            case android.view.Display.TYPE_BUILT_IN :
                return "BUILT_IN";
            case android.view.Display.TYPE_HDMI :
                return "HDMI";
            case android.view.Display.TYPE_WIFI :
                return "WIFI";
            case android.view.Display.TYPE_OVERLAY :
                return "OVERLAY";
            case android.view.Display.TYPE_VIRTUAL :
                return "VIRTUAL";
            default :
    ...
    public static java.lang.String stateToString(int state) {
        switch (state) {
            case android.view.Display.STATE_UNKNOWN :
                return "UNKNOWN";
            ...
            case android.view.Display.STATE_DOZE_SUSPEND :
                return "DOZE_SUSPEND";
            case android.view.Display.STATE_VR :
                return "VR";
            case android.view.Display.STATE_ON_SUSPEND :
                return "ON_SUSPEND";
            default :
    ...
    public static boolean isSuspendedState(int state) {
        return ((state == android.view.Display.STATE_OFF) || (state == android.view.Display.STATE_DOZE_SUSPEND)) || (state == android.view.Display.STATE_ON_SUSPEND);
    }
    ...
    public static boolean isDozeState(int state) {
        return (state == android.view.Display.STATE_DOZE) || (state == android.view.Display.STATE_DOZE_SUSPEND);
    }
    ...
    public static final class Mode implements android.os.Parcelable {
        ...
        public boolean matches(int width, int height, float refreshRate) {
            return ((mWidth == width) && (mHeight == height)) && (java.lang.Float.floatToIntBits(mRefreshRate) == java.lang.Float.floatToIntBits(refreshRate));
        }
        ...
        public boolean equals(java.lang.Object other) {
            ...
            if (!(other instanceof android.view.Display.Mode)) {
                return false;
            }
            android.view.Display.Mode that = ((android.view.Display.Mode) (other));
        ...
        public int hashCode() {
            ...
            hash = (hash * 17) + java.lang.Float.floatToIntBits(mRefreshRate);
            return hash;
        }

        @java.lang.Override
        public java.lang.String toString() {
            return new java.lang.StringBuilder("{").append("id=").append(mModeId).append(", width=").append(mWidth).append(", height=").append(mHeight).append(", fps=").append(mRefreshRate).append("}").toString();
        }
        ...
        private Mode(android.os.Parcel in) {
        ...
        public static final android.os.Parcelable.Creator<android.view.Display.Mode> CREATOR = new android.os.Parcelable.Creator<android.view.Display.Mode>() {
            @java.lang.Override
            public android.view.Mode createFromParcel(android.os.Parcel in) {
                return new android.view.Mode(in);
            }

            @java.lang.Override
            public android.view.Mode[] newArray(int size) {
                return new android.view.Mode[size];
            }
        };
    }
    ...
    public static final class HdrCapabilities implements android.os.Parcelable {
         ...
         * @unknown 
         */
        @android.annotation.IntDef(prefix = { "HDR_TYPE_" }, value = { android.view.Display.HdrCapabilities.HDR_TYPE_DOLBY_VISION, android.view.Display.HdrCapabilities.HDR_TYPE_HDR10, android.view.Display.HdrCapabilities.HDR_TYPE_HLG, android.view.Display.HdrCapabilities.HDR_TYPE_HDR10_PLUS })
        ...
        private float mMaxLuminance = android.view.Display.HdrCapabilities.INVALID_LUMINANCE;

        private float mMaxAverageLuminance = android.view.Display.HdrCapabilities.INVALID_LUMINANCE;

        private float mMinLuminance = android.view.Display.HdrCapabilities.INVALID_LUMINANCE;
        ...
        public boolean equals(java.lang.Object other) {
            ...
            if (!(other instanceof android.view.Display.HdrCapabilities)) {
                return false;
            }
            android.view.Display.HdrCapabilities that = ((android.view.Display.HdrCapabilities) (other));
            return ((java.util.Arrays.equals(mSupportedHdrTypes, that.mSupportedHdrTypes) && (mMaxLuminance == that.mMaxLuminance)) && (mMaxAverageLuminance == that.mMaxAverageLuminance)) && (mMinLuminance == that.mMinLuminance);
        }

        @java.lang.Override
        public int hashCode() {
            int hash = 23;
            hash = (hash * 17) + java.util.Arrays.hashCode(mSupportedHdrTypes);
            hash = (hash * 17) + java.lang.Float.floatToIntBits(mMaxLuminance);
            hash = (hash * 17) + java.lang.Float.floatToIntBits(mMaxAverageLuminance);
            hash = (hash * 17) + java.lang.Float.floatToIntBits(mMinLuminance);
            return hash;
        }

        @android.annotation.NonNull
        public static final android.view.Creator<android.view.Display.HdrCapabilities> CREATOR = new android.view.Creator<android.view.Display.HdrCapabilities>() {
            @java.lang.Override
            public android.view.Display.HdrCapabilities createFromParcel(android.os.Parcel source) {
                return new android.view.Display.HdrCapabilities(source);
            }

            @java.lang.Override
            public android.view.Display.HdrCapabilities[] newArray(int size) {
                return new android.view.Display.HdrCapabilities[size];
            }
        };

        private HdrCapabilities(android.os.Parcel source) {
