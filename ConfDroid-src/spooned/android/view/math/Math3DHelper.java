...
public class Math3DHelper {
     ...
     *
     * @param d
     * 		- dimension in which the poly is represented (supports 2 or 3D)
    ...
    public static float[] rayIntersectPoly(float[] poly, int polyLength, float px, float py, float dx, float dy, int d) {
        ...
        return new float[]{ java.lang.Float.NaN };
    }
    ...
    public static void centroid3d(float[] poly, int len, float[] ret) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            double areaOfTriangle = java.lang.Math.sqrt(((c0 * c0) + (c1 * c1)) + (c2 * c2));
            area += areaOfTriangle;
    ...
    private static void xsort(float[] points, int pointsLength) {
        android.view.math.Math3DHelper.quicksortX(points, 0, pointsLength - 1);
    }

    public static int hull(float[] points, int pointsLength, float[] retPoly) {
        android.view.math.Math3DHelper.xsort(points, pointsLength);
        int n = pointsLength;
        ...
        for (int i = 2; i < n; i++) {
            ...
            lUpperSize++;
            while ((lUpperSize > 2) && (!android.view.math.Math3DHelper.rightTurn(lUpper[(lUpperSize - 3) * 2], lUpper[((lUpperSize - 3) * 2) + 1], lUpper[(lUpperSize - 2) * 2], lUpper[((lUpperSize - 2) * 2) + 1], lUpper[(lUpperSize - 1) * 2], lUpper[((lUpperSize - 1) * 2) + 1]))) {
        ...
        for (int i = n - 3; i >= 0; i--) {
            ...
            lLowerSize++;
            while ((lLowerSize > 2) && (!android.view.math.Math3DHelper.rightTurn(lLower[(lLowerSize - 3) * 2], lLower[((lLowerSize - 3) * 2) + 1], lLower[(lLowerSize - 2) * 2], lLower[((lLowerSize - 2) * 2) + 1], lLower[(lLowerSize - 1) * 2], lLower[((lLowerSize - 1) * 2) + 1]))) {
                // Remove the middle point of the three last
    ...
    private static boolean rightTurn(float ax, float ay, float bx, float by, float cx, float cy) {
        return (((bx - ax) * (cy - ay)) - ((by - ay) * (cx - ax))) > 1.0E-5;
    }
    ...
    public static int intersection(float[] poly1, int poly1length, float[] poly2, int poly2length) {
        android.view.math.Math3DHelper.makeClockwise(poly1, poly1length);
        android.view.math.Math3DHelper.makeClockwise(poly2, poly2length);
        float[] poly = new float[((poly1length * poly2length) + 2) * 2];
        int count = 0;
        int pcount = 0;
        for (int i = 0; i < poly1length; i++) {
            if (android.view.math.Math3DHelper.pointInsidePolygon(poly1[i * 2], poly1[(i * 2) + 1], poly2, poly2length)) {
        ...
        for (int i = 0; i < poly2length; i++) {
            if (android.view.math.Math3DHelper.pointInsidePolygon(poly2[i * 2], poly2[(i * 2) + 1], poly1, poly1length)) {
        ...
        for (int i = 0; i < poly2length; i++) {
            for (int j = 0; j < poly1length; j++) {
                ...
                boolean found = android.view.math.Math3DHelper.lineIntersection(poly2[i1_by_2], poly2[i1_by_2 + 1], poly2[i2_by_2], poly2[i2_by_2 + 1], poly1[j1_by_2], poly1[j1_by_2 + 1], poly1[j2_by_2], poly1[j2_by_2 + 1], intersection);
                if (found) {
        ...
        android.view.math.Math3DHelper.sort(poly, count, ctr);
        int size = count;
        poly2[0] = poly[0];
        poly2[1] = poly[1];
        count = 1;
    ...
    public static void sort(float[] poly, int polyLength, float[] ctr) {
        android.view.math.Math3DHelper.quicksortCirc(poly, 0, polyLength - 1, ctr);
    }

    public static float angle(float x1, float y1, float[] ctr) {
        return -((float) (java.lang.Math.atan2(x1 - ctr[0], y1 - ctr[1])));
    }
    ...
    private static void quicksortCirc(float[] points, int low, int high, float[] ctr) {
        ...
        float pivot = android.view.math.Math3DHelper.angle(points[p * 2], points[(p * 2) + 1], ctr);
        while (i <= j) {
            while (android.view.math.Math3DHelper.angle(points[i * 2], points[(i * 2) + 1], ctr) < pivot) {
                i++;
            } 
            while (android.view.math.Math3DHelper.angle(points[j * 2], points[(j * 2) + 1], ctr) > pivot) {
                j--;
            } 
            if (i <= j) {
                android.view.math.Math3DHelper.swap(points, i, j);
                i++;
                j--;
            }
        } 
        if (low < j) {
            android.view.math.Math3DHelper.quicksortCirc(points, low, j, ctr);
        }
        if (i < high) {
            android.view.math.Math3DHelper.quicksortCirc(points, i, high, ctr);
        }
    }
    ...
    private static void quicksortX(float[] points, int low, int high) {
        ...
        if (low < j) {
            android.view.math.Math3DHelper.quicksortX(points, low, j);
        }
        if (i < high) {
            android.view.math.Math3DHelper.quicksortX(points, i, high);
        }
    }
    ...
    private static void makeClockwise(float[] polygon, int len) {
        ...
        if (!android.view.math.Math3DHelper.isClockwise(polygon, len)) {
            android.view.math.Math3DHelper.reverse(polygon, len);
        }
    }
    ...
    private static final boolean lineIntersection(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, float[] ret) {
        ...
        if ((((((x - x1) * (x - x2)) > android.view.math.Math3DHelper.EPSILON) || (((x - x3) * (x - x4)) > android.view.math.Math3DHelper.EPSILON)) || (((y - y1) * (y - y2)) > android.view.math.Math3DHelper.EPSILON)) || (((y - y3) * (y - y4)) > android.view.math.Math3DHelper.EPSILON)) {
            return false;
        }
        ret[0] = x;
        ret[1] = y;
    ...
    public static void donutPie2(float[] penumbra, int penumbraLength, float[] umbra, int umbraLength, int rays, int layers, float strength, float[] retstrips) {
        int rings = layers + 1;
        double step = (java.lang.Math.PI * 2) / rays;
        float[] retxy = new float[2];
        android.view.math.Math3DHelper.centroid2d(umbra, umbraLength, retxy);
        float cx = retxy[0];
        float cy = retxy[1];
        float[] t1 = new float[rays];
        float[] t2 = new float[rays];
        for (int i = 0; i < rays; i++) {
            float dx = ((float) (java.lang.Math.sin((java.lang.Math.PI / 4) + (step * i))));
            float dy = ((float) (java.lang.Math.cos((java.lang.Math.PI / 4) + (step * i))));
            t2[i] = android.view.math.Math3DHelper.rayIntersectPoly(umbra, umbraLength, cx, cy, dx, dy, 2)[0];
            t1[i] = android.view.math.Math3DHelper.rayIntersectPoly(penumbra, penumbraLength, cx, cy, dx, dy, 2)[0];
        }
        int p = 0;
        // Calc the vertex
        ...
        for (int k = 0; k < rays; k++) {
            int i = k / 2;
            if ((k & 1) == 1) {
                // traverse the inside in a zig zag pattern
                // for strips
                i = (rays - i) - 1;
            }
            float dx = ((float) (java.lang.Math.sin((java.lang.Math.PI / 4) + (step * i))));
     ...
     *
     * @param dimension
     * 		in what dimension is polygon represented (supports 2 or 3D).
     */
