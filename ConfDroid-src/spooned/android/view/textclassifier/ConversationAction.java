/**
 * Copyright (C) 2018 The Android Open Source Project
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package android.view.textclassifier;


/**
 * Represents the action suggested by a {@link TextClassifier} on a given conversation.
 */
public final class ConversationAction implements android.os.Parcelable {
    /**
     *
     *
     * @unknown 
     */
    @java.lang.annotation.Retention(java.lang.annotation.RetentionPolicy.SOURCE)
    @android.annotation.StringDef(value = { android.view.textclassifier.ConversationAction.TYPE_VIEW_CALENDAR, android.view.textclassifier.ConversationAction.TYPE_VIEW_MAP, android.view.textclassifier.ConversationAction.TYPE_TRACK_FLIGHT, android.view.textclassifier.ConversationAction.TYPE_OPEN_URL, android.view.textclassifier.ConversationAction.TYPE_SEND_SMS, android.view.textclassifier.ConversationAction.TYPE_CALL_PHONE, android.view.textclassifier.ConversationAction.TYPE_SEND_EMAIL, android.view.textclassifier.ConversationAction.TYPE_TEXT_REPLY, android.view.textclassifier.ConversationAction.TYPE_CREATE_REMINDER, android.view.textclassifier.ConversationAction.TYPE_SHARE_LOCATION }, prefix = "TYPE_")
    public @interface ActionType {}

    /**
     * Indicates an action to view a calendar at a specified time.
     */
    public static final java.lang.String TYPE_VIEW_CALENDAR = "view_calendar";

    /**
     * Indicates an action to view the map at a specified location.
     */
    public static final java.lang.String TYPE_VIEW_MAP = "view_map";

    /**
     * Indicates an action to track a flight.
     */
    public static final java.lang.String TYPE_TRACK_FLIGHT = "track_flight";

    /**
     * Indicates an action to open an URL.
     */
    public static final java.lang.String TYPE_OPEN_URL = "open_url";

    /**
     * Indicates an action to send a SMS.
     */
    public static final java.lang.String TYPE_SEND_SMS = "send_sms";

    /**
     * Indicates an action to call a phone number.
     */
    public static final java.lang.String TYPE_CALL_PHONE = "call_phone";

    /**
     * Indicates an action to send an email.
     */
    public static final java.lang.String TYPE_SEND_EMAIL = "send_email";

    /**
     * Indicates an action to reply with a text message.
     */
    public static final java.lang.String TYPE_TEXT_REPLY = "text_reply";

    /**
     * Indicates an action to create a reminder.
     */
    public static final java.lang.String TYPE_CREATE_REMINDER = "create_reminder";

    /**
     * Indicates an action to reply with a location.
     */
    public static final java.lang.String TYPE_SHARE_LOCATION = "share_location";

    // TODO: Make this public API
    /**
     *
     *
     * @unknown *
     */
    public static final java.lang.String TYPE_ADD_CONTACT = "add_contact";

    // TODO: Make this public API
    /**
     *
     *
     * @unknown *
     */
    public static final java.lang.String TYPE_COPY = "copy";

    @android.annotation.NonNull
    public static final android.view.textclassifier.Creator<android.view.textclassifier.ConversationAction> CREATOR = new android.view.textclassifier.Creator<android.view.textclassifier.ConversationAction>() {
        @java.lang.Override
        public android.view.textclassifier.ConversationAction createFromParcel(android.os.Parcel in) {
            return new android.view.textclassifier.ConversationAction(in);
        }

        @java.lang.Override
        public android.view.textclassifier.ConversationAction[] newArray(int size) {
            return new android.view.textclassifier.ConversationAction[size];
        }
    };

    @android.annotation.NonNull
    @android.view.textclassifier.ConversationAction.ActionType
    private final java.lang.String mType;

    @android.annotation.NonNull
    private final java.lang.CharSequence mTextReply;

    @android.annotation.Nullable
    private final android.app.RemoteAction mAction;

    @android.annotation.FloatRange(from = 0, to = 1)
    private final float mScore;

    @android.annotation.NonNull
    private final android.os.Bundle mExtras;

    private ConversationAction(@android.annotation.NonNull
    java.lang.String type, @android.annotation.Nullable
    android.app.RemoteAction action, @android.annotation.Nullable
    java.lang.CharSequence textReply, float score, @android.annotation.NonNull
    android.os.Bundle extras) {
        mType = com.android.internal.util.Preconditions.checkNotNull(type);
        mAction = action;
        mTextReply = textReply;
        mScore = score;
        mExtras = com.android.internal.util.Preconditions.checkNotNull(extras);
    }

    private ConversationAction(android.os.Parcel in) {
        mType = in.readString();
        mAction = in.readParcelable(null);
        mTextReply = in.readCharSequence();
        mScore = in.readFloat();
        mExtras = in.readBundle();
    }

    @java.lang.Override
    public void writeToParcel(android.os.Parcel parcel, int flags) {
        parcel.writeString(mType);
        parcel.writeParcelable(mAction, flags);
        parcel.writeCharSequence(mTextReply);
        parcel.writeFloat(mScore);
        parcel.writeBundle(mExtras);
    }

    @java.lang.Override
    public int describeContents() {
        return 0;
    }

    /**
     * Returns the type of this action, for example, {@link #TYPE_VIEW_CALENDAR}.
     */
    @android.annotation.NonNull
    @android.view.textclassifier.ConversationAction.ActionType
    public java.lang.String getType() {
        return mType;
    }

    /**
     * Returns a RemoteAction object, which contains the icon, label and a PendingIntent, for
     * the specified action type.
     */
    @android.annotation.Nullable
    public android.app.RemoteAction getAction() {
        return mAction;
    }

    /**
     * Returns the confidence score for the specified action. The value ranges from 0 (low
     * confidence) to 1 (high confidence).
     */
    @android.annotation.FloatRange(from = 0, to = 1)
    public float getConfidenceScore() {
        return mScore;
    }

    /**
     * Returns the text reply that could be sent as a reply to the given conversation.
     * <p>
     * This is only available when the type of the action is {@link #TYPE_TEXT_REPLY}.
     */
    @android.annotation.Nullable
    public java.lang.CharSequence getTextReply() {
        return mTextReply;
    }

    /**
     * Returns the extended data related to this conversation action.
     *
     * <p><b>NOTE: </b>Do not modify this bundle.
     */
    @android.annotation.NonNull
    public android.os.Bundle getExtras() {
        return mExtras;
    }

    /**
     * Builder class to construct {@link ConversationAction}.
     */
    public static final class Builder {
        @android.annotation.Nullable
        @android.view.textclassifier.ConversationAction.ActionType
        private java.lang.String mType;

        @android.annotation.Nullable
        private android.app.RemoteAction mAction;

        @android.annotation.Nullable
        private java.lang.CharSequence mTextReply;

        private float mScore;

        @android.annotation.Nullable
        private android.os.Bundle mExtras;

        public Builder(@android.annotation.NonNull
        @android.view.textclassifier.ConversationAction.ActionType
        java.lang.String actionType) {
            mType = com.android.internal.util.Preconditions.checkNotNull(actionType);
        }

        /**
         * Sets an action that may be performed on the given conversation.
         */
        @android.annotation.NonNull
        public android.view.textclassifier.ConversationAction.Builder setAction(@android.annotation.Nullable
        android.app.RemoteAction action) {
            mAction = action;
            return this;
        }

        /**
         * Sets a text reply that may be performed on the given conversation.
         */
        @android.annotation.NonNull
        public android.view.textclassifier.ConversationAction.Builder setTextReply(@android.annotation.Nullable
        java.lang.CharSequence textReply) {
            mTextReply = textReply;
            return this;
        }

        /**
         * Sets the confident score.
         */
        @android.annotation.NonNull
        public android.view.textclassifier.ConversationAction.Builder setConfidenceScore(@android.annotation.FloatRange(from = 0, to = 1)
        float score) {
            mScore = score;
            return this;
        }

        /**
         * Sets the extended data for the conversation action object.
         */
        @android.annotation.NonNull
        public android.view.textclassifier.ConversationAction.Builder setExtras(@android.annotation.Nullable
        android.os.Bundle extras) {
            mExtras = extras;
            return this;
        }

        /**
         * Builds the {@link ConversationAction} object.
         */
        @android.annotation.NonNull
        public android.view.textclassifier.ConversationAction build() {
            return new android.view.textclassifier.ConversationAction(mType, mAction, mTextReply, mScore, mExtras == null ? android.os.Bundle.EMPTY : mExtras);
        }
    }
}

