...
public final class ThreadedRenderer extends android.graphics.HardwareRenderer {
    ...
    public static void disable(boolean system) {
        android.view.ThreadedRenderer.sRendererDisabled = true;
        if (system) {
            android.view.ThreadedRenderer.sSystemRendererDisabled = true;
        }
    }
    ...
    public static void enableForegroundTrimming() {
        android.view.ThreadedRenderer.sTrimForeground = true;
    }
    ...
    public static boolean isAvailable() {
        if (android.view.ThreadedRenderer.sSupportsOpenGL != null) {
            return android.view.ThreadedRenderer.sSupportsOpenGL.booleanValue();
        }
        if (android.os.SystemProperties.getInt("ro.kernel.qemu", 0) == 0) {
            // Device is not an emulator.
            android.view.ThreadedRenderer.sSupportsOpenGL = true;
            return true;
        }
        int qemu_gles = android.os.SystemProperties.getInt("qemu.gles", -1);
        if (qemu_gles == (-1)) {
        ...
        android.view.ThreadedRenderer.sSupportsOpenGL = qemu_gles > 0;
        return android.view.ThreadedRenderer.sSupportsOpenGL.booleanValue();
    }
     ...
     *
     * @param translucent
     * 		True if the surface is translucent, false otherwise
     * @return A threaded renderer backed by OpenGL.
     */
    public static android.view.ThreadedRenderer create(android.content.Context context, boolean translucent, java.lang.String name) {
        android.view.ThreadedRenderer renderer = null;
        if (android.view.ThreadedRenderer.isAvailable()) {
            renderer = new android.view.ThreadedRenderer(context, translucent, name);
        }
        return renderer;
    }

    private static final java.lang.String[] VISUALIZERS = new java.lang.String[]{ android.view.ThreadedRenderer.PROFILE_PROPERTY_VISUALIZE_BARS };
    ...
    private android.graphics.HardwareRenderer.FrameDrawingCallback mNextRtFrameCallback;

    ThreadedRenderer(android.content.Context context, boolean translucent, java.lang.String name) {
        super();
        setName(name);
        setOpaque(!translucent);
        final android.content.res.TypedArray a = context.obtainStyledAttributes(null, R.styleable.Lighting, 0, 0);
     ...
     *
     * @param enabled
     * 		True if the threaded renderer is in use, false otherwise.
     */
     ...
     * 		The surface to render
     * @param surfaceInsets
     * 		The drawing surface insets to apply
     * @return true if the surface was initialized, false otherwise. Returning
    false might mean that the surface was already initialized.
     */
    boolean initializeIfNeeded(int width, int height, android.view.View.AttachInfo attachInfo, android.view.Surface surface, android.graphics.Rect surfaceInsets) throws android.view.Surface.OutOfResourcesException {
        if (isRequested()) {
     ...
     */
    void registerRtFrameCallback(android.graphics.HardwareRenderer.FrameDrawingCallback callback) {
    ...
    void destroyHardwareResources(android.view.View view) {
        android.view.ThreadedRenderer.destroyResources(view);
        clearContent();
    }
     ...
     * 		The drawing surface insets to apply
     */
    void setup(int width, int height, android.view.View.AttachInfo attachInfo, android.graphics.Rect surfaceInsets) {
        mWidth = width;
        mHeight = height;
    ...
    void setLightCenter(android.view.View.AttachInfo attachInfo) {
        ...
        final android.graphics.Point displaySize = attachInfo.mPoint;
        attachInfo.mDisplay.getRealSize(displaySize);
        final float lightX = (displaySize.x / 2.0F) - attachInfo.mWindowLeft;
     ...
     */
    void dumpGfxInfo(java.io.PrintWriter pw, java.io.FileDescriptor fd, java.lang.String[] args) {
        pw.flush();
        ...
        int flags = ((args == null) || (args.length == 0)) ? android.graphics.HardwareRenderer.FLAG_DUMP_ALL : 0;
        for (int i = 0; i < args.length; i++) {
            switch (args[i]) {
                case "framestats" :
                    flags |= android.graphics.HardwareRenderer.FLAG_DUMP_FRAMESTATS;
                    break;
                case "reset" :
                    flags |= android.graphics.HardwareRenderer.FLAG_DUMP_RESET;
                    break;
                case "-a" :
                    // magic option passed when dumping a bugreport.
                    flags = android.graphics.HardwareRenderer.FLAG_DUMP_ALL;
                    break;
            }
        }
    ...
    private void updateViewTreeDisplayList(android.view.View view) {
        view.mPrivateFlags |= android.view.View.PFLAG_DRAWN;
        view.mRecreateDisplayList = (view.mPrivateFlags & android.view.View.PFLAG_INVALIDATED) == android.view.View.PFLAG_INVALIDATED;
        view.mPrivateFlags &= ~android.view.View.PFLAG_INVALIDATED;
        view.updateDisplayListIfDirty();
        view.mRecreateDisplayList = false;
    }

    private void updateRootDisplayList(android.view.View view, android.view.ThreadedRenderer.DrawCallbacks callbacks) {
        android.os.Trace.traceBegin(Trace.TRACE_TAG_VIEW, "Record View#draw()");
        ...
        final android.graphics.HardwareRenderer.FrameDrawingCallback callback = mNextRtFrameCallback;
        mNextRtFrameCallback = null;
        if (callback != null) {
            setFrameCallback(callback);
        }
        if (mRootNodeNeedsUpdate || (!mRootNode.hasDisplayList())) {
            android.graphics.RecordingCanvas canvas = mRootNode.beginRecording(mSurfaceWidth, mSurfaceHeight);
            try {
    ...
    interface DrawCallbacks {
         ...
         *
         * @param canvas
         * 		The Canvas used to render the view.
         */
        void onPreDraw(android.graphics.RecordingCanvas canvas);
         ...
         * 		The Canvas used to render the view.
         */
        void onPostDraw(android.graphics.RecordingCanvas canvas);
    }
     ...
     * 		The view to draw.
     * @param attachInfo
     * 		AttachInfo tied to the specified view.
     */
    void draw(android.view.View view, android.view.View.AttachInfo attachInfo, android.view.ThreadedRenderer.DrawCallbacks callbacks) {
        final android.view.Choreographer choreographer = attachInfo.mViewRootImpl.mChoreographer;
        ...
        if ((syncResult & android.graphics.HardwareRenderer.SYNC_LOST_SURFACE_REWARD_IF_FOUND) != 0) {
            setEnabled(false);
        ...
        if ((syncResult & android.graphics.HardwareRenderer.SYNC_REDRAW_REQUESTED) != 0) {
            attachInfo.mViewRootImpl.invalidate();
        }
    }
    ...
    public static class SimpleRenderer extends android.graphics.HardwareRenderer {
        ...
        public SimpleRenderer(final android.content.Context context, final java.lang.String name, final android.view.Surface surface) {
            super();
            setName(name);
            setOpaque(false);
            setSurface(surface);
         ...
         */
        public void setLightCenter(final android.view.Display display, final int windowLeft, final int windowTop) {
            // Adjust light position for window offsets.
            final android.graphics.Point displaySize = new android.graphics.Point();
            display.getRealSize(displaySize);
            final float lightX = (displaySize.x / 2.0F) - windowLeft;
         ...
         * Draw the surface.
         */
        public void draw(final android.graphics.HardwareRenderer.FrameDrawingCallback callback) {
            final long vsync = android.view.animation.AnimationUtils.currentAnimationTimeMillis() * 1000000L;
            if (callback != null) {
                setFrameCallback(callback);
            }
