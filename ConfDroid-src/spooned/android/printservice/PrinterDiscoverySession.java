 ...
 * performing printer discovery multiple times. You will receive a call to {@link PrinterDiscoverySession#onStartPrinterDiscovery(List)} to start printer
 ...
 * </p>
 * <p>
 * Discovered printers are added by invoking {@link PrinterDiscoverySession#addPrinters(List)}. Added printers that disappeared are
...
public abstract class PrinterDiscoverySession {
    ...
    private final android.util.ArrayMap<android.print.PrinterId, android.print.PrinterInfo> mPrinters = new android.util.ArrayMap<android.print.PrinterId, android.print.PrinterInfo>();

    private final java.util.List<android.print.PrinterId> mTrackedPrinters = new java.util.ArrayList<android.print.PrinterId>();
    ...
    public PrinterDiscoverySession() {
        mId = android.printservice.PrinterDiscoverySession.sIdCounter++;
    }

    void setObserver(android.printservice.IPrintServiceClient observer) {
        mObserver = observer;
        // If some printers were added in the method that
        // created the session, send them over.
        if (!mPrinters.isEmpty()) {
            try {
                mObserver.onPrintersAdded(new android.content.pm.ParceledListSlice<android.print.PrinterInfo>(getPrinters()));
            } catch (android.os.RemoteException re) {
                android.util.Log.e(android.printservice.PrinterDiscoverySession.LOG_TAG, "Error sending added printers", re);
            }
        }
    }
     ...
     * @see #isDestroyed()
     */
    public final void addPrinters(@android.annotation.NonNull
    java.util.List<android.print.PrinterInfo> printers) {
        android.printservice.PrintService.throwIfNotCalledOnMainThread();
        // If the session is destroyed - nothing do to.
        if (mIsDestroyed) {
            android.util.Log.w(android.printservice.PrinterDiscoverySession.LOG_TAG, "Not adding printers - session destroyed.");
            return;
        }
        if (mIsDiscoveryStarted) {
            ...
            for (int i = 0; i < addedPrinterCount; i++) {
                android.print.PrinterInfo addedPrinter = printers.get(i);
                android.print.PrinterInfo oldPrinter = mPrinters.put(addedPrinter.getId(), addedPrinter);
                if ((oldPrinter == null) || (!oldPrinter.equals(addedPrinter))) {
                    if (addedPrinters == null) {
                        addedPrinters = new java.util.ArrayList<android.print.PrinterInfo>();
                    }
            ...
            if (addedPrinters != null) {
                try {
                    mObserver.onPrintersAdded(new android.content.pm.ParceledListSlice<android.print.PrinterInfo>(addedPrinters));
                } catch (android.os.RemoteException re) {
                    android.util.Log.e(android.printservice.PrinterDiscoverySession.LOG_TAG, "Error sending added printers", re);
                }
            }
            ...
            if (mLastSentPrinters == null) {
                mLastSentPrinters = new android.util.ArrayMap<android.print.PrinterId, android.print.PrinterInfo>(mPrinters);
            }
            // Update the printers.
            final int addedPrinterCount = printers.size();
            for (int i = 0; i < addedPrinterCount; i++) {
     ...
     * @see #isDestroyed()
     */
    public final void removePrinters(@android.annotation.NonNull
    java.util.List<android.print.PrinterId> printerIds) {
        android.printservice.PrintService.throwIfNotCalledOnMainThread();
        // If the session is destroyed - nothing do to.
        if (mIsDestroyed) {
            android.util.Log.w(android.printservice.PrinterDiscoverySession.LOG_TAG, "Not removing printers - session destroyed.");
            return;
        }
        if (mIsDiscoveryStarted) {
            // If during discovery, remove existing printers and send them.
            java.util.List<android.print.PrinterId> removedPrinterIds = new java.util.ArrayList<android.print.PrinterId>();
            ...
            if (!removedPrinterIds.isEmpty()) {
                try {
                    mObserver.onPrintersRemoved(new android.content.pm.ParceledListSlice<android.print.PrinterId>(removedPrinterIds));
                } catch (android.os.RemoteException re) {
                    android.util.Log.e(android.printservice.PrinterDiscoverySession.LOG_TAG, "Error sending removed printers", re);
                }
            }
        } else {
            // Remember the last sent printers if needed.
    ...
    private void sendOutOfDiscoveryPeriodPrinterChanges() {
        ...
        java.util.List<android.print.PrinterInfo> addedPrinters = null;
        for (android.print.PrinterInfo printer : mPrinters.values()) {
            android.print.PrinterInfo sentPrinter = mLastSentPrinters.get(printer.getId());
        ...
        for (android.print.PrinterInfo sentPrinter : mLastSentPrinters.values()) {
            if (!mPrinters.containsKey(sentPrinter.getId())) {
                if (removedPrinterIds == null) {
                    removedPrinterIds = new java.util.ArrayList<android.print.PrinterId>();
                }
     ...
     */
    public abstract void onStartPrinterDiscovery(@android.annotation.NonNull
     ...
     */
    public abstract void onValidatePrinters(@android.annotation.NonNull
    java.util.List<android.print.PrinterId> printerIds);
    ...
    PrinterInfo.Builder.setCapabilities(PrinterCapabilitiesInfo)
     */
    public abstract void onStartPrinterStateTracking(@android.annotation.NonNull
     ...
     */
    public void onRequestCustomPrinterIcon(@android.annotation.NonNull
    android.print.PrinterId printerId, @android.annotation.NonNull
    android.os.CancellationSignal cancellationSignal, @android.annotation.NonNull
     ...
     * in printer id is the one for which you received a call to {@link #onStartPrinterStateTracking(PrinterId)}.
     *
     * @param printerId
     * 		The printer to stop tracking.
     * @see #onStartPrinterStateTracking(PrinterId)
     */
    public abstract void onStopPrinterStateTracking(@android.annotation.NonNull
    android.print.PrinterId printerId);

    /**
     * Gets the printers that should be tracked. These are printers that are
     * important to the user and for which you received a call to {@link #onStartPrinterStateTracking(PrinterId)} asking you to observer their
    ...
    public final java.util.List<android.print.PrinterId> getTrackedPrinters() {
        ...
        return new java.util.ArrayList<android.print.PrinterId>(mTrackedPrinters);
    }
    ...
    void startPrinterDiscovery(@android.annotation.NonNull
    java.util.List<android.print.PrinterId> priorityList) {
        if (!mIsDestroyed) {
            mIsDiscoveryStarted = true;
            sendOutOfDiscoveryPeriodPrinterChanges();
            if (priorityList == null) {
                priorityList = java.util.Collections.emptyList();
            }
    ...
    void validatePrinters(@android.annotation.NonNull
    java.util.List<android.print.PrinterId> printerIds) {
    ...
    void startPrinterStateTracking(@android.annotation.NonNull
    android.print.PrinterId printerId) {
        if (((!mIsDestroyed) && (mObserver != null)) && (!mTrackedPrinters.contains(printerId))) {
     ...
     */
    void requestCustomPrinterIcon(@android.annotation.NonNull
    android.print.PrinterId printerId) {
        if ((!mIsDestroyed) && (mObserver != null)) {
            android.printservice.CustomPrinterIconCallback callback = new android.printservice.CustomPrinterIconCallback(printerId, mObserver);
            onRequestCustomPrinterIcon(printerId, new android.os.CancellationSignal(), callback);
        }
    }

    void stopPrinterStateTracking(@android.annotation.NonNull
    android.print.PrinterId printerId) {
