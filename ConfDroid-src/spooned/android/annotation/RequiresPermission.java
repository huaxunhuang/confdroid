 ...
 * <pre>{@code {@literal @}RequiresPermission(Manifest.permission.SET_WALLPAPER)
 ...
 * <pre>{@code {@literal @}RequiresPermission(anyOf = {ACCESS_COARSE_LOCATION, ACCESS_FINE_LOCATION})
 *   public abstract Location getLastKnownLocation(String provider);
 * }</pre>
 * Example of requiring multiple permissions:
 * <pre>{@code {@literal @}RequiresPermission(allOf = {ACCESS_COARSE_LOCATION, ACCESS_FINE_LOCATION})
 *   public abstract Location getLastKnownLocation(String provider);
 * }</pre>
 * Example of requiring separate read and write permissions for a content provider:
 * <pre>{@code {@literal @}RequiresPermission.Read(@RequiresPermission(READ_HISTORY_BOOKMARKS))
 ...
 * <pre>{@code public void startActivity(@RequiresPermission Intent intent) { ...}
 * }</pre>
 ...
 * <pre>{@code Intent intent = new Intent(Intent.ACTION_CALL);
 *   startActivity(intent);}</pre>
 * and the actual permission requirement for this particular intent is described on
 * the Intent name itself:
 * <pre>{@code {@literal @}RequiresPermission(Manifest.permission.CALL_PHONE)
