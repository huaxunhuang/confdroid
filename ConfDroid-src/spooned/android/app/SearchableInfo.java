...
public final class SearchableInfo implements android.os.Parcelable {
    ...
    private static final int SEARCH_MODE_BADGE_LABEL = 0x4;

    private static final int SEARCH_MODE_BADGE_ICON = 0x8;
     ...
     * @unknown This feature is deprecated, no need to add it to the API.
     */
    public boolean useBadgeLabel() {
        return 0 != (mSearchMode & android.app.SearchableInfo.SEARCH_MODE_BADGE_LABEL);
    }
    ...
    public boolean useBadgeIcon() {
        return (0 != (mSearchMode & android.app.SearchableInfo.SEARCH_MODE_BADGE_ICON)) && (mIconId != 0);
    }
    ...
    public boolean shouldRewriteQueryFromData() {
        return 0 != (mSearchMode & android.app.SearchableInfo.SEARCH_MODE_QUERY_REWRITE_FROM_DATA);
    }
    ...
    public boolean shouldRewriteQueryFromText() {
        return 0 != (mSearchMode & android.app.SearchableInfo.SEARCH_MODE_QUERY_REWRITE_FROM_TEXT);
    }
     ...
     *
     * @param context
     * 		You need to supply a context to start with
    ...
    public android.content.Context getActivityContext(android.content.Context context) {
        return android.app.SearchableInfo.createActivityContext(context, mSearchActivity);
    }

    /**
     * Creates a context for another activity.
     */
    private static android.content.Context createActivityContext(android.content.Context context, android.content.ComponentName activity) {
        android.content.Context theirContext = null;
        try {
            theirContext = context.createPackageContext(activity.getPackageName(), 0);
        } catch (android.content.pm.PackageManager.NameNotFoundException e) {
            android.util.Log.e(android.app.SearchableInfo.LOG_TAG, "Package not found " + activity.getPackageName());
        } catch (java.lang.SecurityException e) {
            android.util.Log.e(android.app.SearchableInfo.LOG_TAG, "Can't make context for " + activity.getPackageName(), e);
        }
        return theirContext;
    }
     ...
     * @unknown 
     */
    public android.content.Context getProviderContext(android.content.Context context, android.content.Context activityContext) {
        android.content.Context theirContext = null;
     ...
     * @param attr
     * 		The attribute set we found in the XML file, contains the values that are used to
     * 		construct the object.
     * @param cName
     * 		The component name of the searchable activity
     * @throws IllegalArgumentException
     * 		if the searchability info is invalid or insufficient
     */
    private SearchableInfo(android.content.Context activityContext, android.util.AttributeSet attr, final android.content.ComponentName cName) {
        mSearchActivity = cName;
        android.content.res.TypedArray a = activityContext.obtainStyledAttributes(attr, com.android.internal.R.styleable.Searchable);
        ...
        mSearchInputType = a.getInt(com.android.internal.R.styleable.Searchable_inputType, android.text.InputType.TYPE_CLASS_TEXT | android.text.InputType.TYPE_TEXT_VARIATION_NORMAL);
        mSearchImeOptions = a.getInt(com.android.internal.R.styleable.Searchable_imeOptions, android.view.inputmethod.EditorInfo.IME_ACTION_GO);
        ...
        java.lang.String suggestProviderPackage = null;
        if (mSuggestAuthority != null) {
            android.content.pm.PackageManager pm = activityContext.getPackageManager();
            android.content.pm.ProviderInfo pi = pm.resolveContentProvider(mSuggestAuthority, android.content.pm.PackageManager.MATCH_DEBUG_TRIAGED_MISSING);
            if (pi != null) {
     ...
     * @unknown This feature is used very little, and on many devices there are no reasonable
    ...
    public static class ActionKeyInfo implements android.os.Parcelable {
         ...
         * 		construct the object.
         * @throws IllegalArgumentException
         * 		if the action key configuration is invalid
         */
        ActionKeyInfo(android.content.Context activityContext, android.util.AttributeSet attr) {
            android.content.res.TypedArray a = activityContext.obtainStyledAttributes(attr, com.android.internal.R.styleable.SearchableActionKey);
            ...
            mSuggestActionMsgColumn = a.getString(com.android.internal.R.styleable.SearchableActionKey_suggestActionMsgColumn);
            a.recycle();
            // sanity check.
            if (mKeyCode == 0) {
     ...
     * @unknown ActionKeyInfo is hidden
     */
    ...
    private void addActionKey(android.app.SearchableInfo.ActionKeyInfo keyInfo) {
        if (mActionKeys == null) {
            mActionKeys = new java.util.HashMap<java.lang.Integer, android.app.SearchableInfo.ActionKeyInfo>();
        }
     ...
     *
     * @param context
     * 		Context to use for reading activity resources.
     * @param activityInfo
     * 		Activity to get search information from.
     * @return Search information about the given activity, or {@code null} if
    the activity has no or invalid searchability meta-data.
     * @unknown For use by SearchManagerService.
     */
    public static android.app.SearchableInfo getActivityMetaData(android.content.Context context, android.content.pm.ActivityInfo activityInfo, int userId) {
        android.content.Context userContext = null;
        try {
            userContext = context.createPackageContextAsUser("system", 0, new android.os.UserHandle(userId));
        } catch (android.content.pm.PackageManager.NameNotFoundException nnfe) {
            android.util.Log.e(android.app.SearchableInfo.LOG_TAG, "Couldn't create package context for user " + userId);
            return null;
        }
        // for each component, try to find metadata
        android.content.res.XmlResourceParser xml = activityInfo.loadXmlMetaData(userContext.getPackageManager(), android.app.SearchableInfo.MD_LABEL_SEARCHABLE);
        if (xml == null) {
            return null;
        }
        android.content.ComponentName cName = new android.content.ComponentName(activityInfo.packageName, activityInfo.name);
        android.app.SearchableInfo searchable = android.app.SearchableInfo.getActivityMetaData(userContext, xml, cName);
        xml.close();
        if (android.app.SearchableInfo.DBG) {
            if (searchable != null) {
                android.util.Log.d(android.app.SearchableInfo.LOG_TAG, (((((((((((((("Checked " + activityInfo.name) + ",label=") + searchable.getLabelId()) + ",icon=") + searchable.getIconId()) + ",suggestAuthority=") + searchable.getSuggestAuthority()) + ",target=") + searchable.getSearchActivity().getClassName()) + ",global=") + searchable.shouldIncludeInGlobalSearch()) + ",settingsDescription=") + searchable.getSettingsDescriptionId()) + ",threshold=") + searchable.getSuggestThreshold());
            } else {
                android.util.Log.d(android.app.SearchableInfo.LOG_TAG, ("Checked " + activityInfo.name) + ", no searchable meta-data");
            }
        }
        return searchable;
    }
     ...
     * 		runtime context
     * @param xml
     * 		XML parser for reading attributes
     * @param cName
     * 		The component name of the searchable activity
     * @unknown A completely constructed SearchableInfo, or null if insufficient XML data for it
     */
    private static android.app.SearchableInfo getActivityMetaData(android.content.Context context, org.xmlpull.v1.XmlPullParser xml, final android.content.ComponentName cName) {
        android.app.SearchableInfo result = null;
        android.content.Context activityContext = android.app.SearchableInfo.createActivityContext(context, cName);
        ...
        try {
            ...
            while (tagType != org.xmlpull.v1.XmlPullParser.END_DOCUMENT) {
                if (tagType == org.xmlpull.v1.XmlPullParser.START_TAG) {
                    if (xml.getName().equals(android.app.SearchableInfo.MD_XML_ELEMENT_SEARCHABLE)) {
                        android.util.AttributeSet attr = android.util.Xml.asAttributeSet(xml);
                        if (attr != null) {
                            try {
                                result = new android.app.SearchableInfo(activityContext, attr, cName);
                            } catch (java.lang.IllegalArgumentException ex) {
                                android.util.Log.w(android.app.SearchableInfo.LOG_TAG, (("Invalid searchable metadata for " + cName.flattenToShortString()) + ": ") + ex.getMessage());
                                return null;
                            }
                        }
                    } else
                        if (xml.getName().equals(android.app.SearchableInfo.MD_XML_ELEMENT_SEARCHABLE_ACTION_KEY)) {
                            if (result == null) {
                            ...
                            if (attr != null) {
                                try {
                                    result.addActionKey(new android.app.SearchableInfo.ActionKeyInfo(activityContext, attr));
                                } catch (java.lang.IllegalArgumentException ex) {
                                    android.util.Log.w(android.app.SearchableInfo.LOG_TAG, (("Invalid action key for " + cName.flattenToShortString()) + ": ") + ex.getMessage());
                                    return null;
                                }
                            }
                        }
            ...
            android.util.Log.w(android.app.SearchableInfo.LOG_TAG, "Reading searchable metadata for " + cName.flattenToShortString(), e);
            return null;
     ...
     * @unknown deprecated functionality
     */
    ...
    public boolean getVoiceSearchEnabled() {
        return 0 != (mVoiceSearchMode & android.app.SearchableInfo.VOICE_SEARCH_SHOW_BUTTON);
    }
    ...
    public boolean getVoiceSearchLaunchWebSearch() {
        return 0 != (mVoiceSearchMode & android.app.SearchableInfo.VOICE_SEARCH_LAUNCH_WEB_SEARCH);
    }
    ...
    public boolean getVoiceSearchLaunchRecognizer() {
        return 0 != (mVoiceSearchMode & android.app.SearchableInfo.VOICE_SEARCH_LAUNCH_RECOGNIZER);
    }
     ...
     */
    public static final android.os.Parcelable.Creator<android.app.SearchableInfo> CREATOR = new android.os.Parcelable.Creator<android.app.SearchableInfo>() {
        public android.app.SearchableInfo createFromParcel(android.os.Parcel in) {
            return new android.app.SearchableInfo(in);
        }

        public android.app.SearchableInfo[] newArray(int size) {
            return new android.app.SearchableInfo[size];
        }
    };
     ...
     *
     * @param in
     * 		The Parcel containing the previously written SearchableInfo,
     * 		positioned at the location in the buffer where it was written.
     */
    SearchableInfo(android.os.Parcel in) {
        mLabelId = in.readInt();
        mSearchActivity = android.content.ComponentName.readFromParcel(in);
        mHintId = in.readInt();
        ...
        for (int count = in.readInt(); count > 0; count--) {
            addActionKey(new android.app.SearchableInfo.ActionKeyInfo(in));
        }
    ...
    public void writeToParcel(android.os.Parcel dest, int flags) {
        ...
        if (mActionKeys == null) {
            ...
            for (android.app.SearchableInfo.ActionKeyInfo actionKey : mActionKeys.values()) {
