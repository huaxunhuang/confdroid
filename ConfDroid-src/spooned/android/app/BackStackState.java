...
final class BackStackState implements android.os.Parcelable {
    ...
    public BackStackState(android.app.FragmentManagerImpl fm, android.app.BackStackRecord bse) {
        int numRemoved = 0;
        android.app.BackStackRecord.Op op = bse.mHead;
        while (op != null) {
            if (op.removed != null) {
    ...
    public android.app.BackStackRecord instantiate(android.app.FragmentManagerImpl fm) {
        android.app.BackStackRecord bse = new android.app.BackStackRecord(fm);
        int pos = 0;
        int num = 0;
        while (pos < mOps.length) {
            ...
            if (android.app.FragmentManagerImpl.DEBUG) {
                android.util.Log.v(android.app.FragmentManagerImpl.TAG, (((("Instantiate " + bse) + " op #") + num) + " base fragment #") + mOps[pos]);
            }
            int findex = mOps[pos++];
            if (findex >= 0) {
                android.app.Fragment f = fm.mActive.get(findex);
                op.fragment = f;
            } else {
                op.fragment = null;
            }
            ...
            if (N > 0) {
                op.removed = new java.util.ArrayList<android.app.Fragment>(N);
                for (int i = 0; i < N; i++) {
                    if (android.app.FragmentManagerImpl.DEBUG) {
                        android.util.Log.v(android.app.FragmentManagerImpl.TAG, (("Instantiate " + bse) + " set remove fragment #") + mOps[pos]);
                    }
                    android.app.Fragment r = fm.mActive.get(mOps[pos++]);
                    op.removed.add(r);
                }
            }
    ...
    public static final android.os.Parcelable.Creator<android.app.BackStackState> CREATOR = new android.os.Parcelable.Creator<android.app.BackStackState>() {
        public android.app.BackStackState createFromParcel(android.os.Parcel in) {
            return new android.app.BackStackState(in);
        }

        public android.app.BackStackState[] newArray(int size) {
            return new android.app.BackStackState[size];
        }
    };
}
