...
public final class NetworkStats implements java.lang.AutoCloseable {
     ...
     */
    private android.net.NetworkStatsHistory mHistory = null;
    ...
    private android.net.NetworkStatsHistory.Entry mRecycledHistoryEntry = null;
     ...
     * @unknown 
     */
    NetworkStats(android.content.Context context, android.net.NetworkTemplate template, long startTimestamp, long endTimestamp) throws android.os.RemoteException, java.lang.SecurityException {
        final android.net.INetworkStatsService statsService = INetworkStatsService.Stub.asInterface(android.os.ServiceManager.getService(android.content.Context.NETWORK_STATS_SERVICE));
    ...
    public static class Bucket {
         ...
         * @unknown 
         */
        @android.annotation.IntDef({ android.app.usage.NetworkStats.Bucket.STATE_ALL, android.app.usage.NetworkStats.Bucket.STATE_DEFAULT, android.app.usage.NetworkStats.Bucket.STATE_FOREGROUND })
         ...
         */
        public static final int UID_REMOVED = android.net.TrafficStats.UID_REMOVED;

        /**
         * Special UID value for data usage by tethering.
         */
        public static final int UID_TETHERING = android.net.TrafficStats.UID_TETHERING;
         ...
         * @unknown 
         */
        @android.annotation.IntDef({ android.app.usage.NetworkStats.Bucket.ROAMING_ALL, android.app.usage.NetworkStats.Bucket.ROAMING_NO, android.app.usage.NetworkStats.Bucket.ROAMING_YES })
        @java.lang.annotation.Retention(java.lang.annotation.RetentionPolicy.SOURCE)
        ...
        private static int convertState(int networkStatsSet) {
            switch (networkStatsSet) {
                case android.net.NetworkStats.SET_ALL :
                    return android.app.usage.NetworkStats.Bucket.STATE_ALL;
                case android.net.NetworkStats.SET_DEFAULT :
                    return android.app.usage.NetworkStats.Bucket.STATE_DEFAULT;
                case android.net.NetworkStats.SET_FOREGROUND :
                    return android.app.usage.NetworkStats.Bucket.STATE_FOREGROUND;
            }
            return 0;
        }
        ...
        private static int convertUid(int uid) {
            switch (uid) {
                case android.net.TrafficStats.UID_REMOVED :
                    return android.app.usage.NetworkStats.Bucket.UID_REMOVED;
                case android.net.TrafficStats.UID_TETHERING :
                    return android.app.usage.NetworkStats.Bucket.UID_TETHERING;
            }
            return uid;
        }
        ...
        private static int convertTag(int tag) {
            switch (tag) {
                case android.net.NetworkStats.TAG_NONE :
                    return android.app.usage.NetworkStats.Bucket.TAG_NONE;
            }
            return tag;
        }
        ...
        private static int convertRoaming(int roaming) {
            switch (roaming) {
                case android.net.NetworkStats.ROAMING_ALL :
                    return android.app.usage.NetworkStats.Bucket.ROAMING_ALL;
                case android.net.NetworkStats.ROAMING_NO :
                    return android.app.usage.NetworkStats.Bucket.ROAMING_NO;
                case android.net.NetworkStats.ROAMING_YES :
                    return android.app.usage.NetworkStats.Bucket.ROAMING_YES;
            }
            return 0;
        }
     ...
     */
    public boolean getNextBucket(android.app.usage.NetworkStats.Bucket bucketOut) {
        if (mSummary != null) {
    ...
    public boolean hasNextBucket() {
        if (mSummary != null) {
            ...
            if (mHistory != null) {
                return (mEnumerationIndex < mHistory.size()) || hasNextUid();
            }
    ...
    public void close() {
        if (mSession != null) {
            try {
                ...
                android.util.Log.w(android.app.usage.NetworkStats.TAG, e);
                // Otherwise, meh
            }
        }
        mSession = null;
    ...
    void startSummaryEnumeration() throws android.os.RemoteException {
        mSummary = /* includeTags */
        mSession.getSummaryForAllUid(mTemplate, mStartTimeStamp, mEndTimeStamp, false);
        mEnumerationIndex = 0;
    }
    ...
    void startHistoryEnumeration(int uid, int tag) {
        mHistory = null;
        try {
            mHistory = mSession.getHistoryIntervalForUid(mTemplate, uid, android.net.NetworkStats.SET_ALL, tag, android.net.NetworkStatsHistory.FIELD_ALL, mStartTimeStamp, mEndTimeStamp);
    ...
    void startUserUidEnumeration() throws android.os.RemoteException {
        ...
        android.util.IntArray filteredUids = new android.util.IntArray(uids.length);
        for (int uid : uids) {
            try {
                android.net.NetworkStatsHistory history = mSession.getHistoryIntervalForUid(mTemplate, uid, android.net.NetworkStats.SET_ALL, android.net.NetworkStats.TAG_NONE, android.net.NetworkStatsHistory.FIELD_ALL, mStartTimeStamp, mEndTimeStamp);
                if ((history != null) && (history.size() > 0)) {
                    filteredUids.add(uid);
                }
            } catch (android.os.RemoteException e) {
                android.util.Log.w(android.app.usage.NetworkStats.TAG, "Error while getting history of uid " + uid, e);
            }
        }
    ...
    private void stepHistory() {
        if (hasNextUid()) {
            ...
            mHistory = null;
            try {
                mHistory = mSession.getHistoryIntervalForUid(mTemplate, getUid(), android.net.NetworkStats.SET_ALL, android.net.NetworkStats.TAG_NONE, android.net.NetworkStatsHistory.FIELD_ALL, mStartTimeStamp, mEndTimeStamp);
            } catch (android.os.RemoteException e) {
                android.util.Log.w(android.app.usage.NetworkStats.TAG, e);
                // Leaving mHistory null
            }
    ...
    private void fillBucketFromSummaryEntry(android.app.usage.NetworkStats.Bucket bucketOut) {
        bucketOut.mUid = android.app.usage.NetworkStats.Bucket.convertUid(mRecycledSummaryEntry.uid);
        bucketOut.mTag = android.app.usage.NetworkStats.Bucket.convertTag(mRecycledSummaryEntry.tag);
        bucketOut.mState = android.app.usage.NetworkStats.Bucket.convertState(mRecycledSummaryEntry.set);
        bucketOut.mRoaming = android.app.usage.NetworkStats.Bucket.convertRoaming(mRecycledSummaryEntry.roaming);
     ...
     */
    private boolean getNextSummaryBucket(android.app.usage.NetworkStats.Bucket bucketOut) {
     ...
     * @return true if a next item could be set.
     */
    private boolean getNextHistoryBucket(android.app.usage.NetworkStats.Bucket bucketOut) {
        if ((bucketOut != null) && (mHistory != null)) {
            if (mEnumerationIndex < mHistory.size()) {
                mRecycledHistoryEntry = mHistory.getValues(mEnumerationIndex++, mRecycledHistoryEntry);
                bucketOut.mUid = android.app.usage.NetworkStats.Bucket.convertUid(getUid());
                bucketOut.mTag = android.app.usage.NetworkStats.Bucket.convertTag(mTag);
                bucketOut.mState = android.app.usage.NetworkStats.Bucket.STATE_ALL;
                bucketOut.mRoaming = android.app.usage.NetworkStats.Bucket.ROAMING_ALL;
    ...
    private int getUid() {
        ...
        if (isUidEnumeration()) {
            if ((mUidOrUidIndex < 0) || (mUidOrUidIndex >= mUids.length)) {
                throw new java.lang.IndexOutOfBoundsException((("Index=" + mUidOrUidIndex) + " mUids.length=") + mUids.length);
            }
