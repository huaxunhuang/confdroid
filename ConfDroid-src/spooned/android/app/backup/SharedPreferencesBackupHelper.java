...
public class SharedPreferencesBackupHelper extends android.app.backup.FileBackupHelperBase implements android.app.backup.BackupHelper {
     ...
     * @param prefGroups
     * 		The names of each {@link android.content.SharedPreferences} file to
     * 		back up
     */
    public SharedPreferencesBackupHelper(android.content.Context context, java.lang.String... prefGroups) {
        super(context);
        mContext = context;
    ...
    public void performBackup(android.os.ParcelFileDescriptor oldState, android.app.backup.BackupDataOutput data, android.os.ParcelFileDescriptor newState) {
        ...
        java.lang.String[] prefGroups = mPrefGroups;
        final int N = prefGroups.length;
        java.lang.String[] files = new java.lang.String[N];
        for (int i = 0; i < N; i++) {
            files[i] = context.getSharedPrefsFile(prefGroups[i]).getAbsolutePath();
        }
        // go
        android.app.backup.FileBackupHelperBase.performBackup_checked(oldState, data, newState, files, prefGroups);
    }
    ...
    public void restoreEntity(android.app.backup.BackupDataInputStream data) {
        ...
        if (android.app.backup.SharedPreferencesBackupHelper.DEBUG)
            android.util.Log.d(android.app.backup.SharedPreferencesBackupHelper.TAG, (("got entity '" + key) + "' size=") + data.size());

        if (isKeyInList(key, mPrefGroups)) {
            java.io.File f = context.getSharedPrefsFile(key).getAbsoluteFile();
            writeFile(f, data);
        }
    }
}
