...
public abstract class ChooserTargetService extends android.app.Service {
     ...
     * 		the ComponentName of the matched activity that referred the system
    ...
    public android.os.IBinder onBind(android.content.Intent intent) {
        if (android.service.chooser.ChooserTargetService.DEBUG)
            android.util.Log.d(TAG, "onBind " + intent);

        if (!android.service.chooser.ChooserTargetService.SERVICE_INTERFACE.equals(intent.getAction())) {
            if (android.service.chooser.ChooserTargetService.DEBUG)
                android.util.Log.d(TAG, ("bad intent action " + intent.getAction()) + "; returning null");

            return null;
        }
        if (mWrapper == null) {
            mWrapper = new android.service.chooser.ChooserTargetService.IChooserTargetServiceWrapper();
        }
        return mWrapper;
    }
    ...
    private class IChooserTargetServiceWrapper extends android.service.chooser.IChooserTargetService.Stub {
        ...
        public void getChooserTargets(android.content.ComponentName targetComponentName, android.content.IntentFilter matchedFilter, android.service.chooser.IChooserTargetResult result) throws android.os.RemoteException {
            java.util.List<android.service.chooser.ChooserTarget> targets = null;
            final long id = clearCallingIdentity();
            try {
                if (android.service.chooser.ChooserTargetService.DEBUG) {
                    android.util.Log.d(TAG, (("getChooserTargets calling onGetChooserTargets; " + targetComponentName) + " filter: ") + matchedFilter);
                }
                ...
                if (android.service.chooser.ChooserTargetService.DEBUG)
                    android.util.Log.d(TAG, "Sent results");
