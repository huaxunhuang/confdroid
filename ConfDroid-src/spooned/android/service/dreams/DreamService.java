...
public class DreamService extends android.app.Service implements android.view.Window.Callback {
    private final java.lang.String TAG = ((android.service.dreams.DreamService.class.getSimpleName() + "[") + getClass().getSimpleName()) + "]";
    ...
    private int mDozeScreenState = android.view.Display.STATE_UNKNOWN;

    private int mDozeScreenBrightness = android.os.PowerManager.BRIGHTNESS_DEFAULT;

    private boolean mDebug = false;

    public DreamService() {
        mSandman = IDreamManager.Stub.asInterface(android.os.ServiceManager.getService(android.service.dreams.DreamService.DREAM_SERVICE));
    }
    ...
    public boolean dispatchKeyEvent(android.view.KeyEvent event) {
        ...
        if (!mInteractive) {
            if (mDebug)
                android.util.Slog.v(TAG, "Waking up on keyEvent");
            ...
            if (event.getKeyCode() == android.view.KeyEvent.KEYCODE_BACK) {
                if (mDebug)
                    android.util.Slog.v(TAG, "Waking up on back key");
    ...
    public boolean dispatchKeyShortcutEvent(android.view.KeyEvent event) {
        if (!mInteractive) {
            if (mDebug)
                android.util.Slog.v(TAG, "Waking up on keyShortcutEvent");
    ...
    public boolean dispatchTouchEvent(android.view.MotionEvent event) {
        ...
        if (!mInteractive) {
            if (mDebug)
                android.util.Slog.v(TAG, "Waking up on touchEvent");
    ...
    public boolean dispatchTrackballEvent(android.view.MotionEvent event) {
        if (!mInteractive) {
            if (mDebug)
                android.util.Slog.v(TAG, "Waking up on trackballEvent");
    ...
    public boolean dispatchGenericMotionEvent(android.view.MotionEvent event) {
        if (!mInteractive) {
            if (mDebug)
                android.util.Slog.v(TAG, "Waking up on genericMotionEvent");
     ...
     */
    @java.lang.Override
    public boolean onSearchRequested(android.view.SearchEvent event) {
     ...
     * @unknown There is no reason to have this -- dreams can set this flag
    ...
    public void setLowProfile(boolean lowProfile) {
        if (mLowProfile != lowProfile) {
            ...
            int flag = android.view.View.SYSTEM_UI_FLAG_LOW_PROFILE;
    ...
    public boolean isLowProfile() {
        return getSystemUiVisibilityFlagValue(android.view.View.SYSTEM_UI_FLAG_LOW_PROFILE, mLowProfile);
    }
    ...
    public void setFullscreen(boolean fullscreen) {
        if (mFullscreen != fullscreen) {
            ...
            int flag = android.view.WindowManager.LayoutParams.FLAG_FULLSCREEN;
    ...
    public boolean isScreenBright() {
        return getWindowFlagValue(android.view.WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON, mScreenBright);
    }
     ...
     * @see #startDozing
     * @unknown For use by system UI components only.
     */
     ...
     *
     * @param state
     * 		The screen state to use while dozing, such as {@link Display#STATE_ON},
     * 		{@link Display#STATE_DOZE}, {@link Display#STATE_DOZE_SUSPEND},
     * 		or {@link Display#STATE_OFF}, or {@link Display#STATE_UNKNOWN} for the default
     * 		behavior.
     * @unknown For use by system UI components only.
    ...
    public void setDozeScreenBrightness(int brightness) {
        if (brightness != android.os.PowerManager.BRIGHTNESS_DEFAULT) {
            brightness = android.service.dreams.DreamService.clampAbsoluteBrightness(brightness);
        }
    ...
    public void onCreate() {
        if (mDebug)
            android.util.Slog.v(TAG, "onCreate()");
    ...
    public void onDreamingStarted() {
        if (mDebug)
            android.util.Slog.v(TAG, "onDreamingStarted()");
    ...
    public void onDreamingStopped() {
        if (mDebug)
            android.util.Slog.v(TAG, "onDreamingStopped()");
    ...
    public final android.os.IBinder onBind(android.content.Intent intent) {
        ...
        return new android.service.dreams.DreamService.DreamServiceWrapper();
    }
    ...
    public final void finish() {
        if (mDebug)
            android.util.Slog.v(TAG, "finish(): mFinished=" + mFinished);

        if (!mFinished) {
            mFinished = true;
            if (mWindowToken == null) {
                android.util.Slog.w(TAG, "Finish was called before the dream was attached.");
            } else {
                try {
                    /* immediate */
    ...
    private void wakeUp(boolean fromSystem) {
        if (mDebug)
            android.util.Slog.v(TAG, (((("wakeUp(): fromSystem=" + fromSystem) + ", mWaking=") + mWaking) + ", mFinished=") + mFinished);
        ...
        if ((!mWaking) && (!mFinished)) {
            ...
            if ((!fromSystem) && (!mFinished)) {
                if (mWindowToken == null) {
                    android.util.Slog.w(TAG, "WakeUp was called before the dream was attached.");
                } else {
                    try {
                        /* immediate */
    ...
    public void onDestroy() {
        if (mDebug)
            android.util.Slog.v(TAG, "onDestroy()");
    ...
    private final void detach() {
        if (mStarted) {
            if (mDebug)
                android.util.Slog.v(TAG, "detach(): Calling onDreamingStopped()");
        ...
        if (mWindow != null) {
            ...
            if (mDebug)
                android.util.Slog.v(TAG, "detach(): Removing window from window manager");
        ...
        if (mWindowToken != null) {
            ...
            android.view.WindowManagerGlobal.getInstance().closeAll(mWindowToken, this.getClass().getName(), "Dream");
            mWindowToken = null;
            mCanDoze = false;
        }
    }
     ...
     *
     * @param windowToken
     * 		A window token that will allow a window to be created in the correct layer.
     * @param started
     * 		A callback that will be invoked once onDreamingStarted has completed.
     */
    private final void attach(android.os.IBinder windowToken, boolean canDoze, android.os.IRemoteCallback started) {
        if (mWindowToken != null) {
            android.util.Slog.e(TAG, "attach() called when already attached with token=" + mWindowToken);
            return;
        }
        if (mFinished || mWaking) {
            android.util.Slog.w(TAG, "attach() called after dream already finished");
            try {
                /* immediate */
        ...
        if (mWindowless && (!mCanDoze)) {
            throw new java.lang.IllegalStateException("Only doze dreams can be windowless");
        }
        if (!mWindowless) {
            mWindow = new com.android.internal.policy.PhoneWindow(this);
            mWindow.setCallback(this);
            mWindow.requestFeature(android.view.Window.FEATURE_NO_TITLE);
            mWindow.setBackgroundDrawable(new android.graphics.drawable.ColorDrawable(0xff000000));
            mWindow.setFormat(android.graphics.PixelFormat.OPAQUE);
            if (mDebug)
                android.util.Slog.v(TAG, java.lang.String.format("Attaching window token: %s to window of type %s", windowToken, android.view.WindowManager.LayoutParams.TYPE_DREAM));

            android.view.WindowManager.LayoutParams lp = mWindow.getAttributes();
            lp.type = android.view.WindowManager.LayoutParams.TYPE_DREAM;
            lp.token = windowToken;
            lp.windowAnimations = com.android.internal.R.style.Animation_Dream;
            lp.flags |= (((((android.view.WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN | android.view.WindowManager.LayoutParams.FLAG_LAYOUT_INSET_DECOR) | android.view.WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED) | android.view.WindowManager.LayoutParams.FLAG_DISMISS_KEYGUARD) | android.view.WindowManager.LayoutParams.FLAG_ALLOW_LOCK_WHILE_SCREEN_ON) | (mFullscreen ? android.view.WindowManager.LayoutParams.FLAG_FULLSCREEN : 0)) | (mScreenBright ? android.view.WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON : 0);
            mWindow.setAttributes(lp);
            // Workaround: Currently low-profile and in-window system bar backgrounds don't go
            // along well. Dreams usually don't need such bars anyways, so disable them by default.
            mWindow.clearFlags(android.view.WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            mWindow.setWindowManager(null, windowToken, "dream", true);
            applySystemUiVisibilityFlags(mLowProfile ? android.view.View.SYSTEM_UI_FLAG_LOW_PROFILE : 0, android.view.View.SYSTEM_UI_FLAG_LOW_PROFILE);
            try {
                getWindowManager().addView(mWindow.getDecorView(), mWindow.getAttributes());
            } catch (android.view.WindowManager.BadTokenException ex) {
                // This can happen because the dream manager service will remove the token
                // immediately without necessarily waiting for the dream to start.
                // We should receive a finish message soon.
                android.util.Slog.i(TAG, "attach() called after window token already removed, dream will " + "finish soon");
                mWindow = null;
                return;
            }
        }
        ...
        mHandler.post(new java.lang.Runnable() {
            ...
            public void run() {
                if ((mWindow != null) || mWindowless) {
                    if (mDebug)
                        android.util.Slog.v(TAG, "Calling onDreamingStarted()");
    ...
    private boolean getSystemUiVisibilityFlagValue(int flag, boolean defaultValue) {
        android.view.View v = (mWindow == null) ? null : mWindow.getDecorView();
    ...
    @java.lang.Override
    protected void dump(final java.io.FileDescriptor fd, java.io.PrintWriter pw, final java.lang.String[] args) {
        com.android.internal.util.DumpUtils.dumpAsync(mHandler, new com.android.internal.util.DumpUtils.Dump() {
            @java.lang.Override
    ...
    protected void dumpOnHandler(java.io.FileDescriptor fd, java.io.PrintWriter pw, java.lang.String[] args) {
        ...
        pw.println();
        if (canDoze()) {
            pw.println("  doze screen state: " + android.view.Display.stateToString(mDozeScreenState));
            pw.println("  doze screen brightness: " + mDozeScreenBrightness);
        }
    }

    private static int clampAbsoluteBrightness(int value) {
        return android.util.MathUtils.constrain(value, android.os.PowerManager.BRIGHTNESS_OFF, android.os.PowerManager.BRIGHTNESS_ON);
    }

    private final class DreamServiceWrapper extends android.service.dreams.IDreamService.Stub {
        @java.lang.Override
        public void attach(final android.os.IBinder windowToken, final boolean canDoze, android.os.IRemoteCallback started) {
            mHandler.post(new java.lang.Runnable() {
                @java.lang.Override
                public void run() {
                    android.service.dreams.DreamService.this.attach(windowToken, canDoze, started);
                }
            });
        }
