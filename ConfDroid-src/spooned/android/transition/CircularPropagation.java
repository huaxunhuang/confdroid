...
public class CircularPropagation extends android.transition.VisibilityPropagation {
    ...
    public long getStartDelay(android.view.ViewGroup sceneRoot, android.transition.Transition transition, android.transition.TransitionValues startValues, android.transition.TransitionValues endValues) {
        ...
        if (epicenter != null) {
            ...
            epicenterX = java.lang.Math.round((loc[0] + (sceneRoot.getWidth() / 2)) + sceneRoot.getTranslationX());
            epicenterY = java.lang.Math.round((loc[1] + (sceneRoot.getHeight() / 2)) + sceneRoot.getTranslationY());
        }
        double distance = android.transition.CircularPropagation.distance(viewCenterX, viewCenterY, epicenterX, epicenterY);
        double maxDistance = android.transition.CircularPropagation.distance(0, 0, sceneRoot.getWidth(), sceneRoot.getHeight());
