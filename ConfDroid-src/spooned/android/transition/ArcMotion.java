...
public class ArcMotion extends android.transition.PathMotion {
    ...
    private static final float DEFAULT_MAX_TANGENT = ((float) (java.lang.Math.tan(java.lang.Math.toRadians(android.transition.ArcMotion.DEFAULT_MAX_ANGLE_DEGREES / 2))));

    private float mMinimumHorizontalAngle = 0;

    private float mMinimumVerticalAngle = 0;

    private float mMaximumAngle = android.transition.ArcMotion.DEFAULT_MAX_ANGLE_DEGREES;

    private float mMinimumHorizontalTangent = 0;

    private float mMinimumVerticalTangent = 0;

    private float mMaximumTangent = android.transition.ArcMotion.DEFAULT_MAX_TANGENT;
    ...
    public ArcMotion(android.content.Context context, android.util.AttributeSet attrs) {
        super(context, attrs);
        android.content.res.TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ArcMotion);
        float minimumVerticalAngle = a.getFloat(R.styleable.ArcMotion_minimumVerticalAngle, android.transition.ArcMotion.DEFAULT_MIN_ANGLE_DEGREES);
        setMinimumVerticalAngle(minimumVerticalAngle);
        float minimumHorizontalAngle = a.getFloat(R.styleable.ArcMotion_minimumHorizontalAngle, android.transition.ArcMotion.DEFAULT_MIN_ANGLE_DEGREES);
        setMinimumHorizontalAngle(minimumHorizontalAngle);
        float maximumAngle = a.getFloat(R.styleable.ArcMotion_maximumAngle, android.transition.ArcMotion.DEFAULT_MAX_ANGLE_DEGREES);
     ...
     * @unknown ref android.R.styleable#ArcMotion_minimumHorizontalAngle
     */
    public void setMinimumHorizontalAngle(float angleInDegrees) {
        mMinimumHorizontalAngle = angleInDegrees;
        mMinimumHorizontalTangent = android.transition.ArcMotion.toTangent(angleInDegrees);
    }
     ...
     * @unknown ref android.R.styleable#ArcMotion_minimumVerticalAngle
     */
    public void setMinimumVerticalAngle(float angleInDegrees) {
        mMinimumVerticalAngle = angleInDegrees;
        mMinimumVerticalTangent = android.transition.ArcMotion.toTangent(angleInDegrees);
    }
     ...
     * @unknown ref android.R.styleable#ArcMotion_maximumAngle
     */
    public void setMaximumAngle(float angleInDegrees) {
        mMaximumAngle = angleInDegrees;
        mMaximumTangent = android.transition.ArcMotion.toTangent(angleInDegrees);
    }
    ...
    private static float toTangent(float arcInDegrees) {
        ...
        return ((float) (java.lang.Math.tan(java.lang.Math.toRadians(arcInDegrees / 2))));
    }
    ...
    public android.graphics.Path getPath(float startX, float startY, float endX, float endY) {
        ...
        if (deltaY == 0) {
            ex = dx;
            ey = dy + ((java.lang.Math.abs(deltaX) * 0.5F) * mMinimumHorizontalTangent);
        } else
            if (deltaX == 0) {
                ex = dx + ((java.lang.Math.abs(deltaY) * 0.5F) * mMinimumVerticalTangent);
                ey = dy;
            } else
                if (java.lang.Math.abs(deltaX) < java.lang.Math.abs(deltaY)) {
                    ...
                    // bd = hypotenuse/2
                    // fb = deltaY
                    float eDistY = java.lang.Math.abs(h2 / (2 * deltaY));
                    if (isMovingUpwards) {
                        ey = endY + eDistY;
                        ex = endX;
                    } else {
        ...
        if (newArcDistance2 != 0) {
            ...
            float ratio = ((float) (java.lang.Math.sqrt(ratio2)));
            ex = dx + (ratio * (ex - dx));
            ey = dy + (ratio * (ey - dy));
        }
