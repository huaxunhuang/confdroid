...
public class PathDashPathEffect extends android.graphics.PathEffect {
     ...
     * @param advance
     * 		spacing between each stamp of shape
     * @param phase
    ...
    public PathDashPathEffect(android.graphics.Path shape, float advance, float phase, android.graphics.PathDashPathEffect.Style style) {
        native_instance = android.graphics.PathDashPathEffect.nativeCreate(shape.readOnlyNI(), advance, phase, style.native_style);
    }

    private static native long nativeCreate(long native_path, float advance, float phase, int native_style);
}
