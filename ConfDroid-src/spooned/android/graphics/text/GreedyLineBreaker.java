...
public class GreedyLineBreaker extends android.graphics.text.BaseLineBreaker {
    public GreedyLineBreaker(@android.annotation.NonNull
    ...
    public android.graphics.text.BaseLineBreaker.Result computeBreaks() {
        ...
        for (int i = 0; i < numPrimitives; i++) {
            ...
            if (printedWidth > maxWidth) {
                ...
                if (breakFound || goodBreakFound) {
                    if (goodBreakFound) {
                        ...
                        result.mLineFlags.add(firstTabIndex < goodBreakIndex ? android.graphics.text.BaseLineBreaker.TAB_MASK : 0);
                        firstTabIndex = java.lang.Integer.MAX_VALUE;
                    } else {
                        ...
                        result.mLineAscents.add(0.0F);
                        result.mLineDescents.add(0.0F);
                        result.mLineFlags.add(firstTabIndex < breakIndex ? android.graphics.text.BaseLineBreaker.TAB_MASK : 0);
                        firstTabIndex = java.lang.Integer.MAX_VALUE;
                    }
            ...
            if ((p.type == android.graphics.text.Primitive.PrimitiveType.PENALTY) && (p.penalty < android.graphics.text.Primitive.PrimitiveType.PENALTY_INFINITY)) {
                ...
                if (p.penalty == (-android.graphics.text.Primitive.PrimitiveType.PENALTY_INFINITY)) {
                    ...
                    result.mLineAscents.add(0.0F);
                    result.mLineDescents.add(0.0F);
                    result.mLineFlags.add(firstTabIndex < i ? android.graphics.text.BaseLineBreaker.TAB_MASK : 0);
                    firstTabIndex = java.lang.Integer.MAX_VALUE;
                    printedWidth = width = 0;
                    goodBreakFound = breakFound = false;
                    goodBreakWidth = breakWidth = 0;
                ...
                if (p.type == android.graphics.text.Primitive.PrimitiveType.WORD_BREAK) {
