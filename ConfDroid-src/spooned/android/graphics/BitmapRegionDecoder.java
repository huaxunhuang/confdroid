...
public final class BitmapRegionDecoder {
     ...
     */
    public static android.graphics.BitmapRegionDecoder newInstance(byte[] data, int offset, int length, boolean isShareable) throws java.io.IOException {
        if (((offset | length) < 0) || (data.length < (offset + length))) {
            throw new java.lang.ArrayIndexOutOfBoundsException();
        }
        return android.graphics.BitmapRegionDecoder.nativeNewInstance(data, offset, length, isShareable);
    }
     ...
     * 		if the image format is not supported or can not be decoded.
     */
    public static android.graphics.BitmapRegionDecoder newInstance(java.io.FileDescriptor fd, boolean isShareable) throws java.io.IOException {
        return android.graphics.BitmapRegionDecoder.nativeNewInstance(fd, isShareable);
    }
     ...
     */
    public static android.graphics.BitmapRegionDecoder newInstance(java.io.InputStream is, boolean isShareable) throws java.io.IOException {
        if (is instanceof android.content.res.AssetManager.AssetInputStream) {
            return android.graphics.BitmapRegionDecoder.nativeNewInstance(((android.content.res.AssetManager.AssetInputStream) (is)).getNativeAsset(), isShareable);
        } else {
            // pass some temp storage down to the native code. 1024 is made up,
            // but should be large enough to avoid too many small calls back
            // into is.read(...).
            byte[] tempStorage = new byte[16 * 1024];
            return android.graphics.BitmapRegionDecoder.nativeNewInstance(is, tempStorage, isShareable);
        }
    }
     ...
     * 		if the image format is not supported or can not be decoded.
     */
    public static android.graphics.BitmapRegionDecoder newInstance(java.lang.String pathName, boolean isShareable) throws java.io.IOException {
        android.graphics.BitmapRegionDecoder decoder = null;
        java.io.InputStream stream = null;
        try {
            stream = new java.io.FileInputStream(pathName);
            decoder = android.graphics.BitmapRegionDecoder.newInstance(stream, isShareable);
        } finally {
            if (stream != null) {
                try {
                    stream.close();
    ...
    public android.graphics.Bitmap decodeRegion(android.graphics.Rect rect, android.graphics.BitmapFactory.Options options) {
        ...
        synchronized(mNativeLock) {
            ...
            return android.graphics.BitmapRegionDecoder.nativeDecodeRegion(mNativeBitmapRegionDecoder, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, options, android.graphics.BitmapFactory.Options.nativeInBitmap(options), android.graphics.BitmapFactory.Options.nativeColorSpace(options));
        }
    }
    ...
    public int getWidth() {
        synchronized(mNativeLock) {
            ...
            return android.graphics.BitmapRegionDecoder.nativeGetWidth(mNativeBitmapRegionDecoder);
        }
    }
    ...
    public int getHeight() {
        synchronized(mNativeLock) {
            ...
            return android.graphics.BitmapRegionDecoder.nativeGetHeight(mNativeBitmapRegionDecoder);
        }
    }
    ...
    public void recycle() {
        synchronized(mNativeLock) {
            if (!mRecycled) {
                android.graphics.BitmapRegionDecoder.nativeClean(mNativeBitmapRegionDecoder);
                mRecycled = true;
            }
        }
    }
    ...
    private static native android.graphics.Bitmap nativeDecodeRegion(long lbm, int start_x, int start_y, int width, int height, android.graphics.BitmapFactory.Options options, long inBitmapHandle, long colorSpaceHandle);
