...
public class YuvImage {
     ...
     * 		concatenated into a single byte array.
     * @param format
     * 		The YUV data format as defined in {@link ImageFormat}.
     * @param width
    ...
    public YuvImage(byte[] yuv, int format, int width, int height, int[] strides) {
        if ((format != android.graphics.ImageFormat.NV21) && (format != android.graphics.ImageFormat.YUY2)) {
            throw new java.lang.IllegalArgumentException("only support ImageFormat.NV21 " + "and ImageFormat.YUY2 for now");
        }
        if ((width <= 0) || (height <= 0)) {
    ...
    public boolean compressToJpeg(android.graphics.Rect rectangle, int quality, java.io.OutputStream stream) {
        ...
        return android.graphics.YuvImage.nativeCompressToJpeg(mData, mFormat, rectangle.width(), rectangle.height(), offsets, mStrides, quality, stream, new byte[android.graphics.YuvImage.WORKING_COMPRESS_STORAGE]);
    }
    ...
    int[] calculateOffsets(int left, int top) {
        ...
        if (mFormat == android.graphics.ImageFormat.NV21) {
            offsets = new int[]{ (top * mStrides[0]) + left, ((mHeight * mStrides[0]) + ((top / 2) * mStrides[1])) + ((left / 2) * 2) };
            return offsets;
        }
