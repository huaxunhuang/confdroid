...
public final class PdfRenderer implements java.lang.AutoCloseable {
     ...
     * @unknown 
     */
    @android.annotation.IntDef({ android.graphics.pdf.PdfRenderer.Page.RENDER_MODE_FOR_DISPLAY, android.graphics.pdf.PdfRenderer.Page.RENDER_MODE_FOR_PRINT })
     ...
     */
    public PdfRenderer(@android.annotation.NonNull
    android.os.ParcelFileDescriptor input) throws java.io.IOException {
        ...
        synchronized(android.graphics.pdf.PdfRenderer.sPdfiumLock) {
            mNativeDocument = android.graphics.pdf.PdfRenderer.nativeCreate(mInput.getFd(), size);
            try {
                mPageCount = android.graphics.pdf.PdfRenderer.nativeGetPageCount(mNativeDocument);
            } catch (java.lang.Throwable t) {
                android.graphics.pdf.PdfRenderer.nativeClose(mNativeDocument);
                mNativeDocument = 0;
                throw t;
            }
        }
    ...
    public boolean shouldScaleForPrinting() {
        throwIfClosed();
        synchronized(android.graphics.pdf.PdfRenderer.sPdfiumLock) {
            return android.graphics.pdf.PdfRenderer.nativeScaleForPrinting(mNativeDocument);
        }
    }
    ...
    public android.graphics.pdf.PdfRenderer.Page openPage(int index) {
        ...
        mCurrentPage = new android.graphics.pdf.PdfRenderer.Page(index);
        return mCurrentPage;
    }
    ...
    public final class Page implements java.lang.AutoCloseable {
        ...
        private Page(int index) {
            android.graphics.Point size = mTempPoint;
            synchronized(android.graphics.pdf.PdfRenderer.sPdfiumLock) {
                mNativePage = android.graphics.pdf.PdfRenderer.nativeOpenPageAndGetSize(mNativeDocument, index, size);
            }
            mIndex = index;
            mWidth = size.x;
            mHeight = size.y;
         ...
         */
        public void render(@android.annotation.NonNull
        android.graphics.Bitmap destination, @android.annotation.Nullable
        android.graphics.Rect destClip, @android.annotation.Nullable
        android.graphics.Matrix transform, @android.graphics.pdf.PdfRenderer.RenderMode
        int renderMode) {
            if (mNativePage == 0) {
                throw new java.lang.NullPointerException();
            }
            destination = com.android.internal.util.Preconditions.checkNotNull(destination, "bitmap null");
            if (destination.getConfig() != android.graphics.Bitmap.Config.ARGB_8888) {
            ...
            if ((renderMode != android.graphics.pdf.PdfRenderer.Page.RENDER_MODE_FOR_PRINT) && (renderMode != android.graphics.pdf.PdfRenderer.Page.RENDER_MODE_FOR_DISPLAY)) {
            ...
            synchronized(android.graphics.pdf.PdfRenderer.sPdfiumLock) {
                android.graphics.pdf.PdfRenderer.nativeRenderPage(mNativeDocument, mNativePage, destination.getNativeInstance(), contentLeft, contentTop, contentRight, contentBottom, transformPtr, renderMode);
            }
        }
        ...
        private void doClose() {
            if (mNativePage != 0) {
                synchronized(android.graphics.pdf.PdfRenderer.sPdfiumLock) {
                    android.graphics.pdf.PdfRenderer.nativeClosePage(mNativePage);
                }
                mNativePage = 0;
            }
            mCloseGuard.close();
    ...
    private static native void nativeRenderPage(long documentPtr, long pagePtr, long bitmapHandle, int clipLeft, int clipTop, int clipRight, int clipBottom, long transformPtr, int renderMode);
