 ...
 * a {@link BluetoothSocket} to manage the connection. Once the {@link BluetoothSocket} is acquired, it's a good idea to call {@link #close()} on
...
public final class BluetoothServerSocket implements java.io.Closeable {
     ...
     */
    /* package */
    BluetoothServerSocket(int type, boolean auth, boolean encrypt, int port) throws java.io.IOException {
        mChannel = port;
        mSocket = new android.bluetooth.BluetoothSocket(type, -1, auth, encrypt, null, port, null);
     ...
     */
    /* package */
    BluetoothServerSocket(int type, boolean auth, boolean encrypt, int port, boolean mitm, boolean min16DigitPin) throws java.io.IOException {
        mChannel = port;
        mSocket = new android.bluetooth.BluetoothSocket(type, -1, auth, encrypt, null, port, null, mitm, min16DigitPin);
        if (port == android.bluetooth.BluetoothAdapter.SOCKET_CHANNEL_AUTO_STATIC_NO_SDP) {
            mSocket.setExcludeSdp(true);
        }
    }
     ...
     */
    /* package */
    BluetoothServerSocket(int type, boolean auth, boolean encrypt, android.os.ParcelUuid uuid) throws java.io.IOException {
        mSocket = new android.bluetooth.BluetoothSocket(type, -1, auth, encrypt, null, -1, uuid);
     ...
     * 		timeout
     */
    public android.bluetooth.BluetoothSocket accept(int timeout) throws java.io.IOException {
    ...
    void setChannel(int newChannel) {
        ...
        if (mSocket != null) {
            if (mSocket.getPort() != newChannel) {
                android.util.Log.w(android.bluetooth.BluetoothServerSocket.TAG, (("The port set is different that the underlying port. mSocket.getPort(): " + mSocket.getPort()) + " requested newChannel: ") + newChannel);
            }
        }
        mChannel = newChannel;
    }
    ...
    public java.lang.String toString() {
        ...
        switch (mSocket.getConnectionType()) {
            case android.bluetooth.BluetoothSocket.TYPE_RFCOMM :
                {
                    sb.append("TYPE_RFCOMM");
                    break;
                }
            case android.bluetooth.BluetoothSocket.TYPE_L2CAP :
                {
                    sb.append("TYPE_L2CAP");
                    break;
                }
            case android.bluetooth.BluetoothSocket.TYPE_SCO :
                {
                    sb.append("TYPE_SCO");
                    break;
                }
