/**
 * Copyright (C) 2006 The Android Open Source Project
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package android.os;


/**
 *
 *
 * @unknown 
 */
public class Broadcaster {
    public Broadcaster() {
    }

    /**
     * Sign up for notifications about something.
     *
     *  When this broadcaster pushes a message with senderWhat in the what field,
     *  target will be sent a copy of that message with targetWhat in the what field.
     */
    public void request(int senderWhat, android.os.Handler target, int targetWhat) {
        synchronized(this) {
            android.os.Broadcaster.Registration r = null;
            if (mReg == null) {
                r = new android.os.Broadcaster.Registration();
                r.senderWhat = senderWhat;
                r.targets = new android.os.Handler[1];
                r.targetWhats = new int[1];
                r.targets[0] = target;
                r.targetWhats[0] = targetWhat;
                mReg = r;
                r.next = r;
                r.prev = r;
            } else {
                // find its place in the map
                android.os.Broadcaster.Registration start = mReg;
                r = start;
                do {
                    if (r.senderWhat >= senderWhat) {
                        break;
                    }
                    r = r.next;
                } while (r != start );
                int n;
                if (r.senderWhat != senderWhat) {
                    // we didn't find a senderWhat match, but r is right
                    // after where it goes
                    android.os.Broadcaster.Registration reg = new android.os.Broadcaster.Registration();
                    reg.senderWhat = senderWhat;
                    reg.targets = new android.os.Handler[1];
                    reg.targetWhats = new int[1];
                    reg.next = r;
                    reg.prev = r.prev;
                    r.prev.next = reg;
                    r.prev = reg;
                    if ((r == mReg) && (r.senderWhat > reg.senderWhat)) {
                        mReg = reg;
                    }
                    r = reg;
                    n = 0;
                } else {
                    n = r.targets.length;
                    android.os.Handler[] oldTargets = r.targets;
                    int[] oldWhats = r.targetWhats;
                    // check for duplicates, and don't do it if we are dup.
                    for (int i = 0; i < n; i++) {
                        if ((oldTargets[i] == target) && (oldWhats[i] == targetWhat)) {
                            return;
                        }
                    }
                    r.targets = new android.os.Handler[n + 1];
                    java.lang.System.arraycopy(oldTargets, 0, r.targets, 0, n);
                    r.targetWhats = new int[n + 1];
                    java.lang.System.arraycopy(oldWhats, 0, r.targetWhats, 0, n);
                }
                r.targets[n] = target;
                r.targetWhats[n] = targetWhat;
            }
        }
    }

    /**
     * Unregister for notifications for this senderWhat/target/targetWhat tuple.
     */
    public void cancelRequest(int senderWhat, android.os.Handler target, int targetWhat) {
        synchronized(this) {
            android.os.Broadcaster.Registration start = mReg;
            android.os.Broadcaster.Registration r = start;
            if (r == null) {
                return;
            }
            do {
                if (r.senderWhat >= senderWhat) {
                    break;
                }
                r = r.next;
            } while (r != start );
            if (r.senderWhat == senderWhat) {
                android.os.Handler[] targets = r.targets;
                int[] whats = r.targetWhats;
                int oldLen = targets.length;
                for (int i = 0; i < oldLen; i++) {
                    if ((targets[i] == target) && (whats[i] == targetWhat)) {
                        r.targets = new android.os.Handler[oldLen - 1];
                        r.targetWhats = new int[oldLen - 1];
                        if (i > 0) {
                            java.lang.System.arraycopy(targets, 0, r.targets, 0, i);
                            java.lang.System.arraycopy(whats, 0, r.targetWhats, 0, i);
                        }
                        int remainingLen = (oldLen - i) - 1;
                        if (remainingLen != 0) {
                            java.lang.System.arraycopy(targets, i + 1, r.targets, i, remainingLen);
                            java.lang.System.arraycopy(whats, i + 1, r.targetWhats, i, remainingLen);
                        }
                        break;
                    }
                }
            }
        }
    }

    /**
     * For debugging purposes, print the registrations to System.out
     */
    public void dumpRegistrations() {
        synchronized(this) {
            android.os.Broadcaster.Registration start = mReg;
            java.lang.System.out.println(("Broadcaster " + this) + " {");
            if (start != null) {
                android.os.Broadcaster.Registration r = start;
                do {
                    java.lang.System.out.println("    senderWhat=" + r.senderWhat);
                    int n = r.targets.length;
                    for (int i = 0; i < n; i++) {
                        java.lang.System.out.println((("        [" + r.targetWhats[i]) + "] ") + r.targets[i]);
                    }
                    r = r.next;
                } while (r != start );
            }
            java.lang.System.out.println("}");
        }
    }

    /**
     * Send out msg.  Anyone who has registered via the request() method will be
     * sent the message.
     */
    public void broadcast(android.os.Message msg) {
        synchronized(this) {
            if (mReg == null) {
                return;
            }
            int senderWhat = msg.what;
            android.os.Broadcaster.Registration start = mReg;
            android.os.Broadcaster.Registration r = start;
            do {
                if (r.senderWhat >= senderWhat) {
                    break;
                }
                r = r.next;
            } while (r != start );
            if (r.senderWhat == senderWhat) {
                android.os.Handler[] targets = r.targets;
                int[] whats = r.targetWhats;
                int n = targets.length;
                for (int i = 0; i < n; i++) {
                    android.os.Handler target = targets[i];
                    android.os.Message m = android.os.Message.obtain();
                    m.copyFrom(msg);
                    m.what = whats[i];
                    target.sendMessage(m);
                }
            }
        }
    }

    private class Registration {
        android.os.Broadcaster.Registration next;

        android.os.Broadcaster.Registration prev;

        int senderWhat;

        android.os.Handler[] targets;

        int[] targetWhats;
    }

    private android.os.Broadcaster.Registration mReg;
}

