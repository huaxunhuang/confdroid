...
public class HealthKeys {
    ...
    public static class Constants {
        ...
        private final int[][] mKeys = new int[android.os.health.HealthKeys.TYPE_COUNT][];
        ...
        public Constants(java.lang.Class clazz) {
            ...
            final java.lang.Class<android.os.health.HealthKeys.Constant> annotationClass = android.os.health.HealthKeys.Constant.class;
            final int N = fields.length;
            final android.os.health.HealthKeys.SortedIntArray[] keys = new android.os.health.HealthKeys.SortedIntArray[mKeys.length];
            for (int i = 0; i < keys.length; i++) {
                keys[i] = new android.os.health.HealthKeys.SortedIntArray(N);
            }
            for (int i = 0; i < N; i++) {
                final java.lang.reflect.Field field = fields[i];
                final android.os.health.HealthKeys.Constant constant = field.getAnnotation(annotationClass);
                if (constant != null) {
                    ...
                    try {
                        ...
                        throw new java.lang.RuntimeException((("Can't read constant value type=" + type) + " field=") + field, ex);
                    }
                }
            }
            for (int i = 0; i < keys.length; i++) {
        ...
        public int getIndex(int type, int key) {
            final int index = java.util.Arrays.binarySearch(mKeys[type], key);
            if (index >= 0) {
                return index;
            } else {
                throw new java.lang.RuntimeException(((("Unknown Constant " + key) + " (of type ") + type) + " )");
            }
        }
    ...
    private static class SortedIntArray {
        ...
        int[] getArray() {
            if (mCount == mArray.length) {
                java.util.Arrays.sort(mArray);
                return mArray;
            } else {
                final int[] result = new int[mCount];
                java.lang.System.arraycopy(mArray, 0, result, 0, mCount);
