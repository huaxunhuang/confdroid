...
public class MulticlassPA {
    public MulticlassPA(int numClasses, int numDimensions, float aggressiveness) {
        nativeClassifier = initNativeClassifier(numClasses, numDimensions, aggressiveness);
    }

    /**
     * Train on one example
     */
    public boolean sparseTrainOneExample(int[] index_array, float[] float_array, int target) {
        return nativeSparseTrainOneExample(index_array, float_array, target, nativeClassifier);
    }
