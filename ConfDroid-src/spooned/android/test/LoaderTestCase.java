...
public class LoaderTestCase extends android.test.AndroidTestCase {
     ...
     */
    public <T> T getLoaderResultSynchronously(final android.content.Loader<T> loader) {
        // The test thread blocks on this queue until the loader puts it's result in
        final java.util.concurrent.ArrayBlockingQueue<T> queue = new java.util.concurrent.ArrayBlockingQueue<T>(1);
        // This callback runs on the "main" thread and unblocks the test thread
        // when it puts the result into the blocking queue
        final android.content.Loader.OnLoadCompleteListener<T> listener = new android.content.Loader.OnLoadCompleteListener<T>() {
            @java.lang.Override
            public void onLoadComplete(android.content.Loader<T> completedLoader, T data) {
        ...
        // AsyncTask
        final android.os.Handler mainThreadHandler = new android.os.Handler(android.os.Looper.getMainLooper()) {
            @java.lang.Override
        ...
        while (true) {
            try {
                ...
                throw new java.lang.RuntimeException("waiting thread interrupted", e);
            }
        } 
        return result;
    }
