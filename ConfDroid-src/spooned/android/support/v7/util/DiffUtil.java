...
public class DiffUtil {
    ...
    private static final java.util.Comparator<android.support.v7.util.DiffUtil.Snake> SNAKE_COMPARATOR = new java.util.Comparator<android.support.v7.util.DiffUtil.Snake>() {
        @java.lang.Override
        public int compare(android.support.v7.util.DiffUtil.Snake o1, android.support.v7.util.DiffUtil.Snake o2) {
            int cmpX = o1.x - o2.x;
     ...
     */
    public static android.support.v7.util.DiffUtil.DiffResult calculateDiff(android.support.v7.util.DiffUtil.Callback cb) {
        return android.support.v7.util.DiffUtil.calculateDiff(cb, true);
    }
    ...
    old list into the new list.
     */
    public static android.support.v7.util.DiffUtil.DiffResult calculateDiff(android.support.v7.util.DiffUtil.Callback cb, boolean detectMoves) {
        final int oldSize = cb.getOldListSize();
        final int newSize = cb.getNewListSize();
        final java.util.List<android.support.v7.util.DiffUtil.Snake> snakes = new java.util.ArrayList<>();
        // instead of a recursive implementation, we keep our own stack to avoid potential stack
        // overflow exceptions
        final java.util.List<android.support.v7.util.DiffUtil.Range> stack = new java.util.ArrayList<>();
        stack.add(new android.support.v7.util.DiffUtil.Range(0, oldSize, 0, newSize));
        final int max = (oldSize + newSize) + java.lang.Math.abs(oldSize - newSize);
        ...
        final java.util.List<android.support.v7.util.DiffUtil.Range> rangePool = new java.util.ArrayList<>();
        while (!stack.isEmpty()) {
            final android.support.v7.util.DiffUtil.Range range = stack.remove(stack.size() - 1);
            final android.support.v7.util.DiffUtil.Snake snake = android.support.v7.util.DiffUtil.diffPartial(cb, range.oldListStart, range.oldListEnd, range.newListStart, range.newListEnd, forward, backward, max);
            if (snake != null) {
                if (snake.size > 0) {
                    snakes.add(snake);
                }
                ...
                final android.support.v7.util.DiffUtil.Range left = (rangePool.isEmpty()) ? new android.support.v7.util.DiffUtil.Range() : rangePool.remove(rangePool.size() - 1);
                ...
                final android.support.v7.util.DiffUtil.Range right = range;
                if (snake.reverse) {
                    if (snake.removal) {
        ...
        // sort snakes
        java.util.Collections.sort(snakes, android.support.v7.util.DiffUtil.SNAKE_COMPARATOR);
        return new android.support.v7.util.DiffUtil.DiffResult(cb, snakes, forward, backward, detectMoves);
    }

    private static android.support.v7.util.DiffUtil.Snake diffPartial(android.support.v7.util.DiffUtil.Callback cb, int startOld, int endOld, int startNew, int endNew, int[] forward, int[] backward, int kOffset) {
        ...
        java.util.Arrays.fill(forward, (kOffset - dLimit) - 1, (kOffset + dLimit) + 1, 0);
        java.util.Arrays.fill(backward, ((kOffset - dLimit) - 1) + delta, ((kOffset + dLimit) + 1) + delta, oldSize);
            ...
            for (int k = -d; k <= d; k += 2) {
                ...
                if ((checkInFwd && (k >= ((delta - d) + 1))) && (k <= ((delta + d) - 1))) {
                    if (forward[kOffset + k] >= backward[kOffset + k]) {
                        android.support.v7.util.DiffUtil.Snake outSnake = new android.support.v7.util.DiffUtil.Snake();
    ...
    public static class DiffResult {
        ...
        private static final int FLAG_CHANGED = android.support.v7.util.DiffUtil.DiffResult.FLAG_NOT_CHANGED << 1;

        // Item has moved and also changed.
        private static final int FLAG_MOVED_CHANGED = android.support.v7.util.DiffUtil.DiffResult.FLAG_CHANGED << 1;

        // Item has moved but did not change.
        private static final int FLAG_MOVED_NOT_CHANGED = android.support.v7.util.DiffUtil.DiffResult.FLAG_MOVED_CHANGED << 1;
        ...
        // addition.
        private static final int FLAG_IGNORE = android.support.v7.util.DiffUtil.DiffResult.FLAG_MOVED_NOT_CHANGED << 1;

        // since we are re-using the int arrays that were created in the Myers' step, we mask
        // change flags
        private static final int FLAG_OFFSET = 5;

        private static final int FLAG_MASK = (1 << android.support.v7.util.DiffUtil.DiffResult.FLAG_OFFSET) - 1;
         ...
         */
        DiffResult(android.support.v7.util.DiffUtil.Callback callback, java.util.List<android.support.v7.util.DiffUtil.Snake> snakes, int[] oldItemStatuses, int[] newItemStatuses, boolean detectMoves) {
            mSnakes = snakes;
            mOldItemStatuses = oldItemStatuses;
            mNewItemStatuses = newItemStatuses;
            java.util.Arrays.fill(mOldItemStatuses, 0);
            java.util.Arrays.fill(mNewItemStatuses, 0);
            mCallback = callback;
        ...
        private void addRootSnake() {
            android.support.v7.util.DiffUtil.Snake firstSnake = (mSnakes.isEmpty()) ? null : mSnakes.get(0);
            if (((firstSnake == null) || (firstSnake.x != 0)) || (firstSnake.y != 0)) {
                android.support.v7.util.DiffUtil.Snake root = new android.support.v7.util.DiffUtil.Snake();
                root.x = 0;
                root.y = 0;
                root.removal = false;
                root.size = 0;
        ...
        private void findMatchingItems() {
            ...
            for (int i = mSnakes.size() - 1; i >= 0; i--) {
                final android.support.v7.util.DiffUtil.Snake snake = mSnakes.get(i);
                ...
                for (int j = 0; j < snake.size; j++) {
                    ...
                    final int changeFlag = (theSame) ? android.support.v7.util.DiffUtil.DiffResult.FLAG_NOT_CHANGED : android.support.v7.util.DiffUtil.DiffResult.FLAG_CHANGED;
                    mOldItemStatuses[oldItemPos] = (newItemPos << android.support.v7.util.DiffUtil.DiffResult.FLAG_OFFSET) | changeFlag;
                    mNewItemStatuses[newItemPos] = (oldItemPos << android.support.v7.util.DiffUtil.DiffResult.FLAG_OFFSET) | changeFlag;
                }
                posOld = snake.x;
                posNew = snake.y;
            }
                ...
                if (removal) {
                    ...
                    for (int pos = curX - 1; pos >= endX; pos--) {
                        if (mCallback.areItemsTheSame(pos, myItemPos)) {
                            ...
                            final int changeFlag = (theSame) ? android.support.v7.util.DiffUtil.DiffResult.FLAG_MOVED_NOT_CHANGED : android.support.v7.util.DiffUtil.DiffResult.FLAG_MOVED_CHANGED;
                            mNewItemStatuses[myItemPos] = (pos << android.support.v7.util.DiffUtil.DiffResult.FLAG_OFFSET) | android.support.v7.util.DiffUtil.DiffResult.FLAG_IGNORE;
                            mOldItemStatuses[pos] = (myItemPos << android.support.v7.util.DiffUtil.DiffResult.FLAG_OFFSET) | changeFlag;
                            return true;
                        }
                    }
                } else {
                    ...
                    for (int pos = curY - 1; pos >= endY; pos--) {
                        if (mCallback.areItemsTheSame(myItemPos, pos)) {
                            ...
                            final int changeFlag = (theSame) ? android.support.v7.util.DiffUtil.DiffResult.FLAG_MOVED_NOT_CHANGED : android.support.v7.util.DiffUtil.DiffResult.FLAG_MOVED_CHANGED;
                            mOldItemStatuses[x - 1] = (pos << android.support.v7.util.DiffUtil.DiffResult.FLAG_OFFSET) | android.support.v7.util.DiffUtil.DiffResult.FLAG_IGNORE;
                            mNewItemStatuses[pos] = ((x - 1) << android.support.v7.util.DiffUtil.DiffResult.FLAG_OFFSET) | changeFlag;
                            return true;
                        }
                    }
                }
         ...
         *
         * @param adapter
         * 		A RecyclerView adapter which was displaying the old list and will start
         * 		displaying the new list.
         */
        public void dispatchUpdatesTo(final android.support.v7.widget.RecyclerView.Adapter adapter) {
            dispatchUpdatesTo(new android.support.v7.util.ListUpdateCallback() {
                @java.lang.Override
         ...
         */
        public void dispatchUpdatesTo(android.support.v7.util.ListUpdateCallback updateCallback) {
            final android.support.v7.util.BatchingListUpdateCallback batchingCallback;
            if (updateCallback instanceof android.support.v7.util.BatchingListUpdateCallback) {
                batchingCallback = ((android.support.v7.util.BatchingListUpdateCallback) (updateCallback));
            } else {
                batchingCallback = new android.support.v7.util.BatchingListUpdateCallback(updateCallback);
            ...
            final java.util.List<android.support.v7.util.DiffUtil.PostponedUpdate> postponedUpdates = new java.util.ArrayList<>();
            int posOld = mOldListSize;
            int posNew = mNewListSize;
            for (int snakeIndex = mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                final android.support.v7.util.DiffUtil.Snake snake = mSnakes.get(snakeIndex);
                ...
                for (int i = snakeSize - 1; i >= 0; i--) {
                    if ((mOldItemStatuses[snake.x + i] & android.support.v7.util.DiffUtil.DiffResult.FLAG_MASK) == android.support.v7.util.DiffUtil.DiffResult.FLAG_CHANGED) {
        ...
        private static android.support.v7.util.DiffUtil.PostponedUpdate removePostponedUpdate(java.util.List<android.support.v7.util.DiffUtil.PostponedUpdate> updates, int pos, boolean removal) {
            for (int i = updates.size() - 1; i >= 0; i--) {
                final android.support.v7.util.DiffUtil.PostponedUpdate update = updates.get(i);
        ...
        private void dispatchAdditions(java.util.List<android.support.v7.util.DiffUtil.PostponedUpdate> postponedUpdates, android.support.v7.util.ListUpdateCallback updateCallback, int start, int count, int globalIndex) {
            if (!mDetectMoves) {
                updateCallback.onInserted(start, count);
                return;
            }
            for (int i = count - 1; i >= 0; i--) {
                int status = mNewItemStatuses[globalIndex + i] & android.support.v7.util.DiffUtil.DiffResult.FLAG_MASK;
                switch (status) {
                    case 0 :
                        // real addition
                        updateCallback.onInserted(start, 1);
                        for (android.support.v7.util.DiffUtil.PostponedUpdate update : postponedUpdates) {
                            update.currentPos += 1;
                        }
                        break;
                    case android.support.v7.util.DiffUtil.DiffResult.FLAG_MOVED_CHANGED :
                    case android.support.v7.util.DiffUtil.DiffResult.FLAG_MOVED_NOT_CHANGED :
                        final int pos = mNewItemStatuses[globalIndex + i] >> android.support.v7.util.DiffUtil.DiffResult.FLAG_OFFSET;
                        final android.support.v7.util.DiffUtil.PostponedUpdate update = android.support.v7.util.DiffUtil.DiffResult.removePostponedUpdate(postponedUpdates, pos, true);
                        // the item was moved from that position
                        // noinspection ConstantConditions
                        updateCallback.onMoved(update.currentPos, start);
                        if (status == android.support.v7.util.DiffUtil.DiffResult.FLAG_MOVED_CHANGED) {
                            // also dispatch a change
                            updateCallback.onChanged(start, 1, mCallback.getChangePayload(pos, globalIndex + i));
                        }
                        break;
                    case android.support.v7.util.DiffUtil.DiffResult.FLAG_IGNORE :
                        // ignoring this
                        postponedUpdates.add(new android.support.v7.util.DiffUtil.PostponedUpdate(globalIndex + i, start, false));
                        break;
                    default :
                        throw new java.lang.IllegalStateException((("unknown flag for pos " + (globalIndex + i)) + " ") + java.lang.Long.toBinaryString(status));
                }
            }
        }

        private void dispatchRemovals(java.util.List<android.support.v7.util.DiffUtil.PostponedUpdate> postponedUpdates, android.support.v7.util.ListUpdateCallback updateCallback, int start, int count, int globalIndex) {
            if (!mDetectMoves) {
                updateCallback.onRemoved(start, count);
                return;
            }
            for (int i = count - 1; i >= 0; i--) {
                final int status = mOldItemStatuses[globalIndex + i] & android.support.v7.util.DiffUtil.DiffResult.FLAG_MASK;
                switch (status) {
                    case 0 :
                        // real removal
                    ...
                    case android.support.v7.util.DiffUtil.DiffResult.FLAG_MOVED_CHANGED :
                    case android.support.v7.util.DiffUtil.DiffResult.FLAG_MOVED_NOT_CHANGED :
                        final int pos = mOldItemStatuses[globalIndex + i] >> android.support.v7.util.DiffUtil.DiffResult.FLAG_OFFSET;
                        final android.support.v7.util.DiffUtil.PostponedUpdate update = android.support.v7.util.DiffUtil.DiffResult.removePostponedUpdate(postponedUpdates, pos, false);
                    ...
                    case android.support.v7.util.DiffUtil.DiffResult.FLAG_IGNORE :
                        // ignoring this
                        postponedUpdates.add(new android.support.v7.util.DiffUtil.PostponedUpdate(globalIndex + i, start + i, true));
                        break;
                    default :
                        throw new java.lang.IllegalStateException((("unknown flag for pos " + (globalIndex + i)) + " ") + java.lang.Long.toBinaryString(status));
                }
