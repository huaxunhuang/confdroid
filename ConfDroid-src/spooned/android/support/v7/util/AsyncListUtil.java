...
public class AsyncListUtil<T> {
    ...
    void log(java.lang.String s, java.lang.Object... args) {
        android.util.Log.d(android.support.v7.util.AsyncListUtil.TAG, "[MAIN] " + java.lang.String.format(s, args));
    }
     ...
     */
    public AsyncListUtil(java.lang.Class<T> klass, int tileSize, android.support.v7.util.AsyncListUtil.DataCallback<T> dataCallback, android.support.v7.util.AsyncListUtil.ViewCallback viewCallback) {
        mTClass = klass;
        mTileSize = tileSize;
        mDataCallback = dataCallback;
        mViewCallback = viewCallback;
        mTileList = new android.support.v7.util.TileList<T>(mTileSize);
        android.support.v7.util.ThreadUtil<T> threadUtil = new android.support.v7.util.MessageThreadUtil<T>();
    ...
    public T getItem(int position) {
        if ((position < 0) || (position >= mItemCount)) {
            throw new java.lang.IndexOutOfBoundsException((position + " is not within 0 and ") + mItemCount);
        }
    ...
    void updateRange() {
        ...
        mTmpRangeExtended[0] = java.lang.Math.min(mTmpRange[0], java.lang.Math.max(mTmpRangeExtended[0], 0));
        mTmpRangeExtended[1] = java.lang.Math.max(mTmpRange[1], java.lang.Math.min(mTmpRangeExtended[1], mItemCount - 1));
        mBackgroundProxy.updateRange(mTmpRange[0], mTmpRange[1], mTmpRangeExtended[0], mTmpRangeExtended[1], mScrollHint);
    }

    private final android.support.v7.util.ThreadUtil.MainThreadCallback<T> mMainThreadCallback = new android.support.v7.util.ThreadUtil.MainThreadCallback<T>() {
        @java.lang.Override
        public void updateItemCount(int generation, int itemCount) {
            if (android.support.v7.util.AsyncListUtil.DEBUG) {
        ...
        public void addTile(int generation, android.support.v7.util.TileList.Tile<T> tile) {
            ...
            android.support.v7.util.TileList.Tile<T> duplicate = mTileList.addOrReplace(tile);
            if (duplicate != null) {
                android.util.Log.e(android.support.v7.util.AsyncListUtil.TAG, "duplicate tile @" + duplicate.mStartPosition);
        ...
        public void removeTile(int generation, int position) {
            ...
            android.support.v7.util.TileList.Tile<T> tile = mTileList.removeAtPos(position);
            if (tile == null) {
                android.util.Log.e(android.support.v7.util.AsyncListUtil.TAG, "tile not found @" + position);
                return;
            }
            if (android.support.v7.util.AsyncListUtil.DEBUG) {
    ...
    private final android.support.v7.util.ThreadUtil.BackgroundCallback<T> mBackgroundCallback = new android.support.v7.util.ThreadUtil.BackgroundCallback<T>() {
        private android.support.v7.util.TileList.Tile<T> mRecycledRoot;
        ...
        public void loadTile(int position, int scrollHint) {
            ...
            android.support.v7.util.TileList.Tile<T> tile = acquireTile();
            tile.mStartPosition = position;
            tile.mItemCount = java.lang.Math.min(mTileSize, mItemCount - tile.mStartPosition);
        ...
        private android.support.v7.util.TileList.Tile<T> acquireTile() {
            if (mRecycledRoot != null) {
                android.support.v7.util.TileList.Tile<T> result = mRecycledRoot;
                mRecycledRoot = mRecycledRoot.mNext;
                return result;
            }
            return new android.support.v7.util.TileList.Tile<T>(mTClass, mTileSize);
        }
        ...
        private void addTile(android.support.v7.util.TileList.Tile<T> tile) {
        ...
        private void flushTileCache(int scrollHint) {
            ...
            while (mLoadedTiles.size() >= cacheSizeLimit) {
                ...
                if ((startMargin > 0) && ((startMargin >= endMargin) || (scrollHint == android.support.v7.util.AsyncListUtil.ViewCallback.HINT_SCROLL_ASC))) {
                    removeTile(firstLoadedTileStart);
                } else
                    if ((endMargin > 0) && ((startMargin < endMargin) || (scrollHint == android.support.v7.util.AsyncListUtil.ViewCallback.HINT_SCROLL_DESC))) {
        ...
        private void log(java.lang.String s, java.lang.Object... args) {
            android.util.Log.d(android.support.v7.util.AsyncListUtil.TAG, "[BKGR] " + java.lang.String.format(s, args));
        }
    };
    ...
    public static abstract class DataCallback<T> {
         ...
         *
         * @param data
         * 		Array of data items. Should not be accessed beyond <code>itemCount</code>.
         * @param itemCount
         * 		The data item count.
         */
        @android.support.annotation.WorkerThread
    ...
    public static abstract class ViewCallback {
        ...
        public void extendRangeInto(int[] range, int[] outRange, int scrollHint) {
            ...
            outRange[0] = range[0] - (scrollHint == android.support.v7.util.AsyncListUtil.ViewCallback.HINT_SCROLL_DESC ? fullRange : halfRange);
            outRange[1] = range[1] + (scrollHint == android.support.v7.util.AsyncListUtil.ViewCallback.HINT_SCROLL_ASC ? fullRange : halfRange);
        }
