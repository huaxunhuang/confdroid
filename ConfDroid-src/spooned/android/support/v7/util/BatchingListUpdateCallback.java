...
public class BatchingListUpdateCallback implements android.support.v7.util.ListUpdateCallback {
    ...
    int mLastEventType = android.support.v7.util.BatchingListUpdateCallback.TYPE_NONE;
    ...
    public BatchingListUpdateCallback(android.support.v7.util.ListUpdateCallback callback) {
        mWrapped = callback;
    }
    ...
    public void dispatchLastEvent() {
        if (mLastEventType == android.support.v7.util.BatchingListUpdateCallback.TYPE_NONE) {
            return;
        }
        switch (mLastEventType) {
            case android.support.v7.util.BatchingListUpdateCallback.TYPE_ADD :
                mWrapped.onInserted(mLastEventPosition, mLastEventCount);
                break;
            case android.support.v7.util.BatchingListUpdateCallback.TYPE_REMOVE :
                mWrapped.onRemoved(mLastEventPosition, mLastEventCount);
                break;
            case android.support.v7.util.BatchingListUpdateCallback.TYPE_CHANGE :
                mWrapped.onChanged(mLastEventPosition, mLastEventCount, mLastEventPayload);
                break;
        }
        mLastEventPayload = null;
        mLastEventType = android.support.v7.util.BatchingListUpdateCallback.TYPE_NONE;
    }

    @java.lang.Override
    public void onInserted(int position, int count) {
        if (((mLastEventType == android.support.v7.util.BatchingListUpdateCallback.TYPE_ADD) && (position >= mLastEventPosition)) && (position <= (mLastEventPosition + mLastEventCount))) {
            mLastEventCount += count;
            mLastEventPosition = java.lang.Math.min(position, mLastEventPosition);
            return;
        }
        dispatchLastEvent();
        mLastEventPosition = position;
        mLastEventCount = count;
        mLastEventType = android.support.v7.util.BatchingListUpdateCallback.TYPE_ADD;
    }

    @java.lang.Override
    public void onRemoved(int position, int count) {
        if (((mLastEventType == android.support.v7.util.BatchingListUpdateCallback.TYPE_REMOVE) && (mLastEventPosition >= position)) && (mLastEventPosition <= (position + count))) {
            mLastEventCount += count;
            mLastEventPosition = position;
            return;
        }
        ...
        mLastEventPosition = position;
        mLastEventCount = count;
        mLastEventType = android.support.v7.util.BatchingListUpdateCallback.TYPE_REMOVE;
    }
    ...
    public void onChanged(int position, int count, java.lang.Object payload) {
        if ((mLastEventType == android.support.v7.util.BatchingListUpdateCallback.TYPE_CHANGE) && (!(((position > (mLastEventPosition + mLastEventCount)) || ((position + count) < mLastEventPosition)) || (mLastEventPayload != payload)))) {
            // take potential overlap into account
            int previousEnd = mLastEventPosition + mLastEventCount;
            mLastEventPosition = java.lang.Math.min(position, mLastEventPosition);
            mLastEventCount = java.lang.Math.max(previousEnd, position + count) - mLastEventPosition;
            return;
        }
        dispatchLastEvent();
        mLastEventPosition = position;
        ...
        mLastEventType = android.support.v7.util.BatchingListUpdateCallback.TYPE_CHANGE;
    }
}
