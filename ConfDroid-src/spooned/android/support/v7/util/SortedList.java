...
public class SortedList<T> {
    ...
    private static final int CAPACITY_GROWTH = android.support.v7.util.SortedList.MIN_CAPACITY;
     ...
     */
    public SortedList(java.lang.Class<T> klass, android.support.v7.util.SortedList.Callback<T> callback) {
        this(klass, callback, android.support.v7.util.SortedList.MIN_CAPACITY);
    }
     ...
     */
    public SortedList(java.lang.Class<T> klass, android.support.v7.util.SortedList.Callback<T> callback, int initialCapacity) {
        mTClass = klass;
        mData = ((T[]) (java.lang.reflect.Array.newInstance(klass, initialCapacity)));
        mCallback = callback;
        mSize = 0;
    }
    ...
    public void addAll(T[] items, boolean mayModifyInput) {
        ...
        if (mayModifyInput) {
            ...
            T[] copy = ((T[]) (java.lang.reflect.Array.newInstance(mTClass, items.length)));
            java.lang.System.arraycopy(items, 0, copy, 0, items.length);
            addAllInternal(copy);
        }
    }
    ...
    public void addAll(java.util.Collection<T> items) {
        T[] copy = ((T[]) (java.lang.reflect.Array.newInstance(mTClass, items.size())));
        addAll(items.toArray(copy), true);
    }

    private void addAllInternal(T[] newItems) {
        final boolean forceBatchedUpdates = !(mCallback instanceof android.support.v7.util.SortedList.BatchedCallback);
        ...
        mOldDataStart = 0;
        mOldDataSize = mSize;
        java.util.Arrays.sort(newItems, mCallback);// Arrays.sort is stable.
    ...
    private int deduplicate(T[] items) {
        if (items.length == 0) {
            throw new java.lang.IllegalArgumentException("Input array must be non-empty");
        }
        ...
        for (int i = 1; i < items.length; ++i) {
            ...
            if (compare > 0) {
                throw new java.lang.IllegalArgumentException("Input must be sorted in ascending order.");
            }
            if (compare == 0) {
                // The range of equal items continues, update it.
                final int sameItemPos = findSameItem(currentItem, items, rangeStart, rangeEnd);
                if (sameItemPos != android.support.v7.util.SortedList.INVALID_POSITION) {
    ...
    private int findSameItem(T item, T[] items, int from, int to) {
        ...
        return android.support.v7.util.SortedList.INVALID_POSITION;
    }
    ...
    private void merge(T[] newData, int newDataSize) {
        final int mergedCapacity = (mSize + newDataSize) + android.support.v7.util.SortedList.CAPACITY_GROWTH;
        mData = ((T[]) (java.lang.reflect.Array.newInstance(mTClass, mergedCapacity)));
        mMergedSize = 0;
        int newDataStart = 0;
        while ((mOldDataStart < mOldDataSize) || (newDataStart < newDataSize)) {
            if (mOldDataStart == mOldDataSize) {
                ...
                java.lang.System.arraycopy(newData, newDataStart, mData, mMergedSize, itemCount);
            ...
            if (newDataStart == newDataSize) {
                ...
                java.lang.System.arraycopy(mOldData, mOldDataStart, mData, mMergedSize, itemCount);
                mMergedSize += itemCount;
                break;
            }
            ...
            if (compare > 0) {
                ...
                if ((compare == 0) && mCallback.areItemsTheSame(oldItem, newItem)) {
    ...
    private void throwIfMerging() {
        if (mOldData != null) {
            throw new java.lang.IllegalStateException("Cannot call this method from within addAll");
        }
    }
    ...
    public void beginBatchedUpdates() {
        throwIfMerging();
        if (mCallback instanceof android.support.v7.util.SortedList.BatchedCallback) {
            return;
        }
        if (mBatchedCallback == null) {
            mBatchedCallback = new android.support.v7.util.SortedList.BatchedCallback(mCallback);
        }
    ...
    public void endBatchedUpdates() {
        throwIfMerging();
        if (mCallback instanceof android.support.v7.util.SortedList.BatchedCallback) {
            ((android.support.v7.util.SortedList.BatchedCallback) (mCallback)).dispatchLastEvent();
        }
    ...
    private int add(T item, boolean notify) {
        int index = findIndexOf(item, mData, 0, mSize, android.support.v7.util.SortedList.INSERTION);
        if (index == android.support.v7.util.SortedList.INVALID_POSITION) {
            index = 0;
        } else
            if (index < mSize) {
    ...
    private boolean remove(T item, boolean notify) {
        int index = findIndexOf(item, mData, 0, mSize, android.support.v7.util.SortedList.DELETION);
        if (index == android.support.v7.util.SortedList.INVALID_POSITION) {
            return false;
        }
    ...
    private void removeItemAtIndex(int index, boolean notify) {
        java.lang.System.arraycopy(mData, index + 1, mData, index, (mSize - index) - 1);
        mSize--;
        mData[mSize] = null;
        if (notify) {
    ...
    public T get(int index) throws java.lang.IndexOutOfBoundsException {
        if ((index >= mSize) || (index < 0)) {
            throw new java.lang.IndexOutOfBoundsException((("Asked to get item at " + index) + " but size is ") + mSize);
        }
        if (mOldData != null) {
    ...
    public int indexOf(T item) {
        if (mOldData != null) {
            int index = findIndexOf(item, mData, 0, mMergedSize, android.support.v7.util.SortedList.LOOKUP);
            if (index != android.support.v7.util.SortedList.INVALID_POSITION) {
                return index;
            }
            index = findIndexOf(item, mOldData, mOldDataStart, mOldDataSize, android.support.v7.util.SortedList.LOOKUP);
            if (index != android.support.v7.util.SortedList.INVALID_POSITION) {
                return (index - mOldDataStart) + mMergedSize;
            }
            return android.support.v7.util.SortedList.INVALID_POSITION;
        }
        return findIndexOf(item, mData, 0, mSize, android.support.v7.util.SortedList.LOOKUP);
    }
    ...
    private int findIndexOf(T item, T[] mData, int left, int right, int reason) {
            ...
            if (cmp < 0) {
                ...
                if (cmp == 0) {
                    if (mCallback.areItemsTheSame(myItem, item)) {
                        ...
                        if (reason == android.support.v7.util.SortedList.INSERTION) {
                            return exact == android.support.v7.util.SortedList.INVALID_POSITION ? middle : exact;
                        } else {
                            return exact;
                        }
                    }
        ...
        return reason == android.support.v7.util.SortedList.INSERTION ? left : android.support.v7.util.SortedList.INVALID_POSITION;
    }
    ...
    private void addToData(int index, T item) {
        if (index > mSize) {
            throw new java.lang.IndexOutOfBoundsException((("cannot add item to " + index) + " because size is ") + mSize);
        }
        if (mSize == mData.length) {
            // we are at the limit enlarge
            T[] newData = ((T[]) (java.lang.reflect.Array.newInstance(mTClass, mData.length + android.support.v7.util.SortedList.CAPACITY_GROWTH)));
            java.lang.System.arraycopy(mData, 0, newData, 0, index);
            newData[index] = item;
            java.lang.System.arraycopy(mData, index, newData, index + 1, mSize - index);
            mData = newData;
        } else {
            // just shift, we fit
            java.lang.System.arraycopy(mData, index, mData, index + 1, mSize - index);
            mData[index] = item;
        }
        mSize++;
    }
    ...
    public void clear() {
        ...
        java.util.Arrays.fill(mData, 0, prevSize, null);
        mSize = 0;
     ...
     * data.
     */
    public static abstract class Callback<T2> implements android.support.v7.util.ListUpdateCallback , java.util.Comparator<T2> {
        /**
    ...
    public static class BatchedCallback<T2> extends android.support.v7.util.SortedList.Callback<T2> {
         ...
         */
        public BatchedCallback(android.support.v7.util.SortedList.Callback<T2> wrappedCallback) {
            mWrappedCallback = wrappedCallback;
            mBatchingListUpdateCallback = new android.support.v7.util.BatchingListUpdateCallback(mWrappedCallback);
        }
