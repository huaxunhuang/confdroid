...
public class LinearSmoothScroller extends android.support.v7.widget.RecyclerView.SmoothScroller {
     ...
     */
    @java.lang.Override
    protected void onSeekTargetStep(int dx, int dy, android.support.v7.widget.RecyclerView.State state, android.support.v7.widget.RecyclerView.SmoothScroller.Action action) {
        if (getChildCount() == 0) {
            stop();
            return;
        }
        // noinspection PointlessBooleanExpression
        if ((android.support.v7.widget.LinearSmoothScroller.DEBUG && (mTargetVector != null)) && (((mTargetVector.x * dx) < 0) || ((mTargetVector.y * dy) < 0))) {
    ...
    protected float calculateSpeedPerPixel(android.util.DisplayMetrics displayMetrics) {
        return android.support.v7.widget.LinearSmoothScroller.MILLISECONDS_PER_INCH / displayMetrics.densityDpi;
    }
    ...
    protected int calculateTimeForScrolling(int dx) {
        ...
        // time.
        return ((int) (java.lang.Math.ceil(java.lang.Math.abs(dx) * MILLISECONDS_PER_PX)));
    }
    ...
    protected int getHorizontalSnapPreference() {
        return (mTargetVector == null) || (mTargetVector.x == 0) ? android.support.v7.widget.LinearSmoothScroller.SNAP_TO_ANY : mTargetVector.x > 0 ? android.support.v7.widget.LinearSmoothScroller.SNAP_TO_END : android.support.v7.widget.LinearSmoothScroller.SNAP_TO_START;
    }
    ...
    protected int getVerticalSnapPreference() {
        return (mTargetVector == null) || (mTargetVector.y == 0) ? android.support.v7.widget.LinearSmoothScroller.SNAP_TO_ANY : mTargetVector.y > 0 ? android.support.v7.widget.LinearSmoothScroller.SNAP_TO_END : android.support.v7.widget.LinearSmoothScroller.SNAP_TO_START;
    }
    ...
    protected void updateActionForInterimTarget(android.support.v7.widget.RecyclerView.SmoothScroller.Action action) {
        ...
        android.graphics.PointF scrollVector = computeScrollVectorForPosition(getTargetPosition());
        ...
        mInterimTargetDx = ((int) (android.support.v7.widget.LinearSmoothScroller.TARGET_SEEK_SCROLL_DISTANCE_PX * scrollVector.x));
        mInterimTargetDy = ((int) (android.support.v7.widget.LinearSmoothScroller.TARGET_SEEK_SCROLL_DISTANCE_PX * scrollVector.y));
        final int time = calculateTimeForScrolling(android.support.v7.widget.LinearSmoothScroller.TARGET_SEEK_SCROLL_DISTANCE_PX);
        // To avoid UI hiccups, trigger a smooth scroll to a distance little further than the
        // interim target. Since we track the distance travelled in onSeekTargetStep callback, it
        // won't actually scroll more than what we need.
        action.update(((int) (mInterimTargetDx * android.support.v7.widget.LinearSmoothScroller.TARGET_SEEK_EXTRA_SCROLL_RATIO)), ((int) (mInterimTargetDy * android.support.v7.widget.LinearSmoothScroller.TARGET_SEEK_EXTRA_SCROLL_RATIO)), ((int) (time * android.support.v7.widget.LinearSmoothScroller.TARGET_SEEK_EXTRA_SCROLL_RATIO)), mLinearInterpolator);
    }
    ...
    public int calculateDtToFit(int viewStart, int viewEnd, int boxStart, int boxEnd, int snapPreference) {
        switch (snapPreference) {
            case android.support.v7.widget.LinearSmoothScroller.SNAP_TO_START :
                return boxStart - viewStart;
            case android.support.v7.widget.LinearSmoothScroller.SNAP_TO_END :
                return boxEnd - viewEnd;
            case android.support.v7.widget.LinearSmoothScroller.SNAP_TO_ANY :
    ...
    public int calculateDyToMakeVisible(android.view.View view, int snapPreference) {
        final android.support.v7.widget.RecyclerView.LayoutManager layoutManager = getLayoutManager();
    ...
    public android.graphics.PointF computeScrollVectorForPosition(int targetPosition) {
        ...
        if (layoutManager instanceof android.support.v7.widget.RecyclerView.SmoothScroller.ScrollVectorProvider) {
            return ((android.support.v7.widget.RecyclerView.SmoothScroller.ScrollVectorProvider) (layoutManager)).computeScrollVectorForPosition(targetPosition);
        }
        android.util.Log.w(android.support.v7.widget.LinearSmoothScroller.TAG, ("You should override computeScrollVectorForPosition when the LayoutManager" + " does not implement ") + android.support.v7.widget.RecyclerView.SmoothScroller.ScrollVectorProvider.class.getCanonicalName());
        return null;
    }
}
