...
public class AppLaunchChecker {
    ...
    public static boolean hasStartedFromLauncher(android.content.Context context) {
        return context.getSharedPreferences(android.support.v4.app.AppLaunchChecker.SHARED_PREFS_NAME, 0).getBoolean(android.support.v4.app.AppLaunchChecker.KEY_STARTED_FROM_LAUNCHER, false);
    }
    ...
    public static void onActivityCreate(android.app.Activity activity) {
        final android.content.SharedPreferences sp = activity.getSharedPreferences(android.support.v4.app.AppLaunchChecker.SHARED_PREFS_NAME, 0);
        if (sp.getBoolean(android.support.v4.app.AppLaunchChecker.KEY_STARTED_FROM_LAUNCHER, false)) {
            return;
        }
        final android.content.Intent launchIntent = activity.getIntent();
        if (launchIntent == null) {
            return;
        }
        if (android.content.Intent.ACTION_MAIN.equals(launchIntent.getAction()) && (launchIntent.hasCategory(android.content.Intent.CATEGORY_LAUNCHER) || launchIntent.hasCategory(android.support.v4.content.IntentCompat.CATEGORY_LEANBACK_LAUNCHER))) {
            android.support.v4.content.SharedPreferencesCompat.EditorCompat.getInstance().apply(sp.edit().putBoolean(android.support.v4.app.AppLaunchChecker.KEY_STARTED_FROM_LAUNCHER, true));
        }
    }
}
