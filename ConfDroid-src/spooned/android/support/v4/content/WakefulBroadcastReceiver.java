...
public abstract class WakefulBroadcastReceiver extends android.content.BroadcastReceiver {
    ...
    private static final android.util.SparseArray<android.os.PowerManager.WakeLock> mActiveWakeLocks = new android.util.SparseArray<android.os.PowerManager.WakeLock>();
    ...
    public static android.content.ComponentName startWakefulService(android.content.Context context, android.content.Intent intent) {
        synchronized(android.support.v4.content.WakefulBroadcastReceiver.mActiveWakeLocks) {
            int id = android.support.v4.content.WakefulBroadcastReceiver.mNextId;
            android.support.v4.content.WakefulBroadcastReceiver.mNextId++;
            ...
            intent.putExtra(android.support.v4.content.WakefulBroadcastReceiver.EXTRA_WAKE_LOCK_ID, id);
            android.content.ComponentName comp = context.startService(intent);
            if (comp == null) {
                return null;
            }
            android.os.PowerManager pm = ((android.os.PowerManager) (context.getSystemService(android.content.Context.POWER_SERVICE)));
            android.os.PowerManager.WakeLock wl = pm.newWakeLock(android.os.PowerManager.PARTIAL_WAKE_LOCK, "wake:" + comp.flattenToShortString());
            wl.setReferenceCounted(false);
            wl.acquire(60 * 1000);
            android.support.v4.content.WakefulBroadcastReceiver.mActiveWakeLocks.put(id, wl);
            return comp;
        }
    }
     ...
     *
     * @param intent
     * 		The Intent as originally generated by {@link #startWakefulService}.
    ...
    public static boolean completeWakefulIntent(android.content.Intent intent) {
        final int id = intent.getIntExtra(android.support.v4.content.WakefulBroadcastReceiver.EXTRA_WAKE_LOCK_ID, 0);
        if (id == 0) {
            return false;
        }
        synchronized(android.support.v4.content.WakefulBroadcastReceiver.mActiveWakeLocks) {
            android.os.PowerManager.WakeLock wl = android.support.v4.content.WakefulBroadcastReceiver.mActiveWakeLocks.get(id);
            if (wl != null) {
                wl.release();
                android.support.v4.content.WakefulBroadcastReceiver.mActiveWakeLocks.remove(id);
                return true;
            }
