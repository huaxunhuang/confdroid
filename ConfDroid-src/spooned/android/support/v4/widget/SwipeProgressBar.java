...
final class SwipeProgressBar {
    ...
    private static final android.view.animation.Interpolator INTERPOLATOR = new android.support.v4.view.animation.FastOutSlowInInterpolator();

    private final android.graphics.Paint mPaint = new android.graphics.Paint();

    private final android.graphics.RectF mClipRect = new android.graphics.RectF();
    ...
    SwipeProgressBar(android.view.View parent) {
        mParent = parent;
        mColor1 = android.support.v4.widget.SwipeProgressBar.COLOR1;
        mColor2 = android.support.v4.widget.SwipeProgressBar.COLOR2;
        mColor3 = android.support.v4.widget.SwipeProgressBar.COLOR3;
        mColor4 = android.support.v4.widget.SwipeProgressBar.COLOR4;
    }
    ...
    void setTriggerPercentage(float triggerPercentage) {
        ...
        mStartTime = 0;
        android.support.v4.view.ViewCompat.postInvalidateOnAnimation(mParent, mBounds.left, mBounds.top, mBounds.right, mBounds.bottom);
    }
    ...
    void stop() {
        if (mRunning) {
            mTriggerPercentage = 0;
            mFinishTime = android.view.animation.AnimationUtils.currentAnimationTimeMillis();
            mRunning = false;
            mParent.postInvalidate();
        }
    }
    ...
    void draw(android.graphics.Canvas canvas) {
        ...
        if (mRunning || (mFinishTime > 0)) {
            ...
            long elapsed = (now - mStartTime) % android.support.v4.widget.SwipeProgressBar.ANIMATION_DURATION_MS;
            long iterations = (now - mStartTime) / android.support.v4.widget.SwipeProgressBar.ANIMATION_DURATION_MS;
            float rawProgress = elapsed / (android.support.v4.widget.SwipeProgressBar.ANIMATION_DURATION_MS / 100.0F);
            // If we're not running anymore, that means we're running through
            // the finish animation.
            if (!mRunning) {
                // If the finish animation is done, don't draw anything, and
                // don't repost.
                if ((now - mFinishTime) >= android.support.v4.widget.SwipeProgressBar.FINISH_ANIMATION_DURATION_MS) {
                    mFinishTime = 0;
                    return;
                }
                ...
                long finishElapsed = (now - mFinishTime) % android.support.v4.widget.SwipeProgressBar.FINISH_ANIMATION_DURATION_MS;
                float finishProgress = finishElapsed / (android.support.v4.widget.SwipeProgressBar.FINISH_ANIMATION_DURATION_MS / 100.0F);
                float pct = finishProgress / 100.0F;
                // Radius of the circle is half of the screen.
                float clearRadius = (width / 2) * android.support.v4.widget.SwipeProgressBar.INTERPOLATOR.getInterpolation(pct);
            ...
            if (iterations == 0) {
                ...
                if ((rawProgress >= 0) && (rawProgress < 25)) {
                    ...
                    if ((rawProgress >= 25) && (rawProgress < 50)) {
                        canvas.drawColor(mColor1);
                    } else
                        if ((rawProgress >= 50) && (rawProgress < 75)) {
            ...
            if ((rawProgress >= 0) && (rawProgress <= 25)) {
                float pct = ((rawProgress + 25) * 2) / 100.0F;
                drawCircle(canvas, cx, cy, mColor1, pct);
            }
            if ((rawProgress >= 0) && (rawProgress <= 50)) {
                float pct = (rawProgress * 2) / 100.0F;
            ...
            if ((rawProgress >= 50) && (rawProgress <= 100)) {
                float pct = ((rawProgress - 50) * 2) / 100.0F;
                drawCircle(canvas, cx, cy, mColor4, pct);
            }
            if ((rawProgress >= 75) && (rawProgress <= 100)) {
                float pct = ((rawProgress - 75) * 2) / 100.0F;
                drawCircle(canvas, cx, cy, mColor1, pct);
            }
     ...
     */
    private void drawCircle(android.graphics.Canvas canvas, float cx, float cy, int color, float pct) {
        mPaint.setColor(color);
        canvas.save();
        canvas.translate(cx, cy);
        float radiusScale = android.support.v4.widget.SwipeProgressBar.INTERPOLATOR.getInterpolation(pct);
