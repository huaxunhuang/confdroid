...
public class ScriptIntrinsic3DLUT extends android.support.v8.renderscript.ScriptIntrinsic {
    ...
    protected ScriptIntrinsic3DLUT(long id, android.support.v8.renderscript.RenderScript rs, android.support.v8.renderscript.Element e) {
        super(id, rs);
        mElement = e;
    }
     ...
     */
    public static android.support.v8.renderscript.ScriptIntrinsic3DLUT create(android.support.v8.renderscript.RenderScript rs, android.support.v8.renderscript.Element e) {
        if (!e.isCompatible(android.support.v8.renderscript.Element.U8_4(rs))) {
            throw new android.support.v8.renderscript.RSIllegalArgumentException("Element must be compatible with uchar4.");
        }
        long id;
        boolean mUseIncSupp = rs.isUseNative() && (android.os.Build.VERSION.SDK_INT < android.support.v8.renderscript.ScriptIntrinsic3DLUT.INTRINSIC_API_LEVEL);
        id = rs.nScriptIntrinsicCreate(8, e.getID(rs), mUseIncSupp);
        android.support.v8.renderscript.ScriptIntrinsic3DLUT si = new android.support.v8.renderscript.ScriptIntrinsic3DLUT(id, rs, e);
    ...
    public void setLUT(android.support.v8.renderscript.Allocation lut) {
        final android.support.v8.renderscript.Type t = lut.getType();
        if (t.getZ() == 0) {
            throw new android.support.v8.renderscript.RSIllegalArgumentException("LUT must be 3d.");
        }
        if (!t.getElement().isCompatible(mElement)) {
            throw new android.support.v8.renderscript.RSIllegalArgumentException("LUT element type must match.");
        }
        mLUT = lut;
        setVar(0, mLUT);
    }
