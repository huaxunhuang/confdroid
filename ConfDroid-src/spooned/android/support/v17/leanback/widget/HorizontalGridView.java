...
public class HorizontalGridView extends android.support.v17.leanback.widget.BaseGridView {
    ...
    private android.graphics.Paint mTempPaint = new android.graphics.Paint();

    private android.graphics.Bitmap mTempBitmapLow;

    private android.graphics.LinearGradient mLowFadeShader;

    private int mLowFadeShaderLength;

    private int mLowFadeShaderOffset;

    private android.graphics.Bitmap mTempBitmapHigh;

    private android.graphics.LinearGradient mHighFadeShader;
    ...
    public HorizontalGridView(android.content.Context context, android.util.AttributeSet attrs, int defStyle) {
        ...
        mLayoutManager.setOrientation(android.support.v7.widget.RecyclerView.HORIZONTAL);
        initAttributes(context, attrs);
    }

    protected void initAttributes(android.content.Context context, android.util.AttributeSet attrs) {
        initBaseGridViewAttributes(context, attrs);
        android.content.res.TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.lbHorizontalGridView);
        setRowHeight(a);
        setNumRows(a.getInt(R.styleable.lbHorizontalGridView_numberOfRows, 1));
        a.recycle();
        updateLayerType();
        mTempPaint = new android.graphics.Paint();
        mTempPaint.setXfermode(new android.graphics.PorterDuffXfermode(android.graphics.PorterDuff.Mode.DST_IN));
    }

    void setRowHeight(android.content.res.TypedArray array) {
        android.util.TypedValue typedValue = array.peekValue(R.styleable.lbHorizontalGridView_rowHeight);
        if (typedValue != null) {
     ...
     *
     * @param height
     * 		May be {@link android.view.ViewGroup.LayoutParams#WRAP_CONTENT WRAP_CONTENT},
    ...
    public final void setFadingLeftEdgeLength(int fadeLength) {
        if (mLowFadeShaderLength != fadeLength) {
            ...
            if (mLowFadeShaderLength != 0) {
                mLowFadeShader = new android.graphics.LinearGradient(0, 0, mLowFadeShaderLength, 0, android.graphics.Color.TRANSPARENT, android.graphics.Color.BLACK, android.graphics.Shader.TileMode.CLAMP);
            } else {
                mLowFadeShader = null;
            }
            invalidate();
    ...
    public final void setFadingRightEdgeLength(int fadeLength) {
        if (mHighFadeShaderLength != fadeLength) {
            ...
            if (mHighFadeShaderLength != 0) {
                mHighFadeShader = new android.graphics.LinearGradient(0, 0, mHighFadeShaderLength, 0, android.graphics.Color.BLACK, android.graphics.Color.TRANSPARENT, android.graphics.Shader.TileMode.CLAMP);
            } else {
    ...
    private boolean needsFadingHighEdge() {
        ...
        for (int i = c - 1; i >= 0; i--) {
            android.view.View view = getChildAt(i);
            if (mLayoutManager.getOpticalRight(view) > ((getWidth() - getPaddingRight()) + mHighFadeShaderOffset)) {
                return true;
            }
        }
        return false;
    ...
    private android.graphics.Bitmap getTempBitmapLow() {
        if (((mTempBitmapLow == null) || (mTempBitmapLow.getWidth() != mLowFadeShaderLength)) || (mTempBitmapLow.getHeight() != getHeight())) {
            mTempBitmapLow = android.graphics.Bitmap.createBitmap(mLowFadeShaderLength, getHeight(), android.graphics.Bitmap.Config.ARGB_8888);
        }
        return mTempBitmapLow;
    }

    private android.graphics.Bitmap getTempBitmapHigh() {
        if (((mTempBitmapHigh == null) || (mTempBitmapHigh.getWidth() != mHighFadeShaderLength)) || (mTempBitmapHigh.getHeight() != getHeight())) {
            // TODO: fix logic for sharing mTempBitmapLow
            if (((false && (mTempBitmapLow != null)) && (mTempBitmapLow.getWidth() == mHighFadeShaderLength)) && (mTempBitmapLow.getHeight() == getHeight())) {
                // share same bitmap for low edge fading and high edge fading.
                mTempBitmapHigh = mTempBitmapLow;
            } else {
                mTempBitmapHigh = android.graphics.Bitmap.createBitmap(mHighFadeShaderLength, getHeight(), android.graphics.Bitmap.Config.ARGB_8888);
            }
        }
    ...
    public void draw(android.graphics.Canvas canvas) {
        ...
        int highEdge = (mFadingHighEdge) ? ((getWidth() - getPaddingRight()) + mHighFadeShaderOffset) + mHighFadeShaderLength : getWidth();
        // draw not-fade content
        int save = canvas.save();
        canvas.clipRect(lowEdge + (mFadingLowEdge ? mLowFadeShaderLength : 0), 0, highEdge - (mFadingHighEdge ? mHighFadeShaderLength : 0), getHeight());
        super.draw(canvas);
        canvas.restoreToCount(save);
        android.graphics.Canvas tmpCanvas = new android.graphics.Canvas();
        mTempRect.top = 0;
        mTempRect.bottom = getHeight();
        if (needsFadingLow && (mLowFadeShaderLength > 0)) {
            android.graphics.Bitmap tempBitmap = getTempBitmapLow();
        ...
        if (needsFadingHigh && (mHighFadeShaderLength > 0)) {
            android.graphics.Bitmap tempBitmap = getTempBitmapHigh();
            tempBitmap.eraseColor(android.graphics.Color.TRANSPARENT);
            tmpCanvas.setBitmap(tempBitmap);
            // draw original content
            int tmpSave = tmpCanvas.save();
