...
abstract class Grid {
    ...
    public static interface Provider {
         ...
         * @param append
         * 		True if new item is after last visible item, false if new item is
         * 		before first visible item.
         * @param item
         * 		item[0] returns created item that will be passed in addItem() call.
         * @return length of the item.
         */
        public abstract int createItem(int index, boolean append, java.lang.Object[] item);
         ...
         */
        public abstract void addItem(java.lang.Object item, int index, int length, int rowIndex, int edge);
    ...
    protected android.support.v17.leanback.widget.Grid.Provider mProvider;
    ...
    protected android.support.v4.util.CircularIntArray[] mTmpItemPositionsInRows;

    // the first index that grid will layout
    protected int mStartIndex = android.support.v17.leanback.widget.Grid.START_DEFAULT;

    /**
     * Creates a single or multiple rows (can be staggered or not staggered) grid
     */
    public static android.support.v17.leanback.widget.Grid createGrid(int rows) {
        android.support.v17.leanback.widget.Grid grid;
        if (rows == 1) {
            grid = new android.support.v17.leanback.widget.SingleRow();
        } else {
            // TODO support non staggered multiple rows grid
            grid = new android.support.v17.leanback.widget.StaggeredGridDefault();
     ...
     */
    public void setProvider(android.support.v17.leanback.widget.Grid.Provider provider) {
        mProvider = provider;
    }
    ...
    void setNumRows(int numRows) {
        ...
        mNumRows = numRows;
        mTmpItemPositionsInRows = new android.support.v4.util.CircularIntArray[mNumRows];
        for (int i = 0; i < mNumRows; i++) {
            mTmpItemPositionsInRows[i] = new android.support.v4.util.CircularIntArray();
        }
    }
     ...
     */
    public abstract android.support.v17.leanback.widget.Grid.Location getLocation(int index);
    ...
    protected final boolean checkAppendOverLimit(int toLimit) {
        ...
        return mReversedFlow ? findRowMin(true, null) <= (toLimit + mMargin) : findRowMax(false, null) >= (toLimit - mMargin);
    }
    ...
    protected final boolean checkPrependOverLimit(int toLimit) {
        ...
        return mReversedFlow ? findRowMax(false, null) >= (toLimit - mMargin) : findRowMin(true, null) <= (toLimit + mMargin);
    }
     ...
     */
    public abstract android.support.v4.util.CircularIntArray[] getItemPositionsInRows(int startPos, int endPos);
     ...
     */
    public final android.support.v4.util.CircularIntArray[] getItemPositionsInRows() {
    ...
    public final boolean prependOneColumnVisibleItems() {
        return prependVisibleItems(mReversedFlow ? java.lang.Integer.MIN_VALUE : java.lang.Integer.MAX_VALUE, true);
    }
     ...
     *
     * @param oneColumnMode
     * 		true when fills one column and stops,  false
    ...
    public boolean appendOneColumnVisibleItems() {
        return appendVisibleItems(mReversedFlow ? java.lang.Integer.MAX_VALUE : java.lang.Integer.MIN_VALUE, true);
    }
    ...
    public void removeInvisibleItemsAtEnd(int aboveIndex, int toLimit) {
        while ((mLastVisibleIndex >= mFirstVisibleIndex) && (mLastVisibleIndex > aboveIndex)) {
            boolean offEnd = (!mReversedFlow) ? mProvider.getEdge(mLastVisibleIndex) >= toLimit : mProvider.getEdge(mLastVisibleIndex) <= toLimit;
            if (offEnd) {
                mProvider.removeItem(mLastVisibleIndex);
                mLastVisibleIndex--;
            } else {
    ...
    public void removeInvisibleItemsAtFront(int belowIndex, int toLimit) {
        while ((mLastVisibleIndex >= mFirstVisibleIndex) && (mFirstVisibleIndex < belowIndex)) {
            boolean offFront = (!mReversedFlow) ? (mProvider.getEdge(mFirstVisibleIndex) + mProvider.getSize(mFirstVisibleIndex)) <= toLimit : (mProvider.getEdge(mFirstVisibleIndex) - mProvider.getSize(mFirstVisibleIndex)) >= toLimit;
            if (offFront) {
    ...
    public abstract void debugPrint(java.io.PrintWriter pw);
}
