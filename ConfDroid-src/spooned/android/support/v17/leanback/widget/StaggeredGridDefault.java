 ...
 */
final class StaggeredGridDefault extends android.support.v17.leanback.widget.StaggeredGrid {
    /**
    ...
    int getRowMax(int rowIndex) {
        ...
        if (mReversedFlow) {
            ...
            for (int i = mFirstVisibleIndex + 1; i <= getLastIndex(); i++) {
                android.support.v17.leanback.widget.StaggeredGrid.Location loc = getLocation(i);
                edge += loc.offset;
                if (loc.row == rowIndex) {
                    return edge;
                }
            ...
            android.support.v17.leanback.widget.StaggeredGrid.Location loc = getLocation(mLastVisibleIndex);
            if (loc.row == rowIndex) {
    ...
    public int findRowMax(boolean findLarge, int indexLimit, int[] indices) {
        ...
        android.support.v17.leanback.widget.StaggeredGrid.Location loc = getLocation(indexLimit);
        int row = loc.row;
    ...
    protected boolean appendVisibleItemsWithoutCache(int toLimit, boolean oneColumnMode) {
        ...
        if (mLastVisibleIndex >= 0) {
            ...
            if (edgeLimitSearchIndex < 0) {
                ...
                edgeLimit = java.lang.Integer.MIN_VALUE;
                for (int i = 0; i < mNumRows; i++) {
                    edgeLimit = (mReversedFlow) ? getRowMin(i) : getRowMax(i);
                    if (edgeLimit != java.lang.Integer.MIN_VALUE) {
                        break;
                    }
                }
            } else {
        ...
        while (true) {
            ...
            for (; rowIndex < mNumRows; rowIndex++) {
                ...
                if ((location == java.lang.Integer.MAX_VALUE) || (location == java.lang.Integer.MIN_VALUE)) {
                    // nothing on the row
                    if (rowIndex == 0) {
    ...
    protected boolean prependVisibleItemsWithoutCache(int toLimit, boolean oneColumnMode) {
        ...
        if (mFirstVisibleIndex >= 0) {
            ...
            if (edgeLimitSearchIndex < 0) {
                ...
                rowIndex = rowIndex - 1;
                edgeLimit = java.lang.Integer.MAX_VALUE;
                for (int i = mNumRows - 1; i >= 0; i--) {
                    edgeLimit = (mReversedFlow) ? getRowMax(i) : getRowMin(i);
                    if (edgeLimit != java.lang.Integer.MAX_VALUE) {
                        break;
                    }
                }
            } else {
                edgeLimit = (mReversedFlow) ? findRowMax(true, edgeLimitSearchIndex, null) : findRowMin(false, edgeLimitSearchIndex, null);
            }
            ...
            rowIndex = (mLocations.size() >= 0 ? (getLocation(getFirstIndex()).row + mNumRows) - 1 : itemIndex) % mNumRows;
            edgeLimit = 0;
            edgeLimitIsValid = false;
        }
        boolean filledOne = false;
