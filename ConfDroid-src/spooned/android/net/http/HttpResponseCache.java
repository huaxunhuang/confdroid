 ...
 * cache directory} of the filesystem}: <pre>   {@code protected void onCreate(Bundle savedInstanceState) {
 *       ...
 *
 *       try {
 ...
 * a full refresh, add the {@code no-cache} directive: <pre>   {@code connection.addRequestProperty("Cache-Control", "no-cache");}</pre>
 * If it is only necessary to force a cached response to be validated by the
 * server, use the more efficient {@code max-age=0} instead: <pre>   {@code connection.addRequestProperty("Cache-Control", "max-age=0");}</pre>
 *
 * <h3>Force a Cache Response</h3>
 * Sometimes you'll want to show resources if they are available immediately,
 * but not otherwise. This can be used so your application can show
 * <i>something</i> while waiting for the latest data to be downloaded. To
 * restrict a request to locally-cached resources, add the {@code only-if-cached} directive: <pre>   {@code try {
 *         connection.addRequestProperty("Cache-Control", "only-if-cached");
 *         InputStream cached = connection.getInputStream();
 *         // the resource was cached! show it} catch (FileNotFoundException e) {
 *         // the resource was not cached
 *     }
 * }</pre>
 * This technique works even better in situations where a stale response is
 * better than no response. To permit stale cached responses, use the {@code max-stale} directive with the maximum staleness in seconds: <pre>   {@code int maxStale = 60 * 60 * 24 * 28; // tolerate 4-weeks stale
 *         connection.addRequestProperty("Cache-Control", "max-stale=" + maxStale);}</pre>
 *
 * <h3>Working With Earlier Releases</h3>
 * This class was added in Android 4.0 (Ice Cream Sandwich). Use reflection to
 * enable the response cache without impacting earlier releases: <pre>   {@code try {
 *           File httpCacheDir = new File(context.getCacheDir(), "http");
 *           long httpCacheSize = 10 * 1024 * 1024; // 10 MiB
 *           Class.forName("android.net.http.HttpResponseCache")
 *                   .getMethod("install", File.class, long.class)
 *                   .invoke(null, httpCacheDir, httpCacheSize);} catch (Exception httpResponseCacheNotAvailable) {
 *       }}</pre>
 */
public final class HttpResponseCache extends java.net.ResponseCache implements com.android.okhttp.OkCacheContainer , java.io.Closeable {
    private final com.android.okhttp.AndroidShimResponseCache delegate;

    private HttpResponseCache(com.android.okhttp.AndroidShimResponseCache delegate) {
    ...
    public static android.net.http.HttpResponseCache getInstalled() {
        java.net.ResponseCache installed = java.net.ResponseCache.getDefault();
        if (installed instanceof android.net.http.HttpResponseCache) {
            return ((android.net.http.HttpResponseCache) (installed));
        }
        return null;
    }
     ...
     * @throws IOException
     * 		if {@code directory} cannot be used for this cache.
     * 		Most applications should respond to this exception by logging a
     * 		warning.
     */
    public static synchronized android.net.http.HttpResponseCache install(java.io.File directory, long maxSize) throws java.io.IOException {
        java.net.ResponseCache installed = java.net.ResponseCache.getDefault();
        if (installed instanceof android.net.http.HttpResponseCache) {
            android.net.http.HttpResponseCache installedResponseCache = ((android.net.http.HttpResponseCache) (installed));
            // don't close and reopen if an equivalent cache is already installed
            com.android.okhttp.AndroidShimResponseCache trueResponseCache = installedResponseCache.delegate;
        ...
        com.android.okhttp.AndroidShimResponseCache trueResponseCache = com.android.okhttp.AndroidShimResponseCache.create(directory, maxSize);
        android.net.http.HttpResponseCache newResponseCache = new android.net.http.HttpResponseCache(trueResponseCache);
        java.net.ResponseCache.setDefault(newResponseCache);
        return newResponseCache;
    }

    @java.lang.Override
    public java.net.CacheResponse get(java.net.URI uri, java.lang.String requestMethod, java.util.Map<java.lang.String, java.util.List<java.lang.String>> requestHeaders) throws java.io.IOException {
     ...
     *
     *
     * @unknown Needed for OkHttp integration.
     */
    @java.lang.Override
