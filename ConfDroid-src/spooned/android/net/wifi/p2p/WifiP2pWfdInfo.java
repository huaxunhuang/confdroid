...
public class WifiP2pWfdInfo implements android.os.Parcelable {
    ...
    public int getDeviceType() {
        return mDeviceInfo & android.net.wifi.p2p.WifiP2pWfdInfo.DEVICE_TYPE;
    }

    public boolean setDeviceType(int deviceType) {
        if ((deviceType >= android.net.wifi.p2p.WifiP2pWfdInfo.WFD_SOURCE) && (deviceType <= android.net.wifi.p2p.WifiP2pWfdInfo.SOURCE_OR_PRIMARY_SINK)) {
            mDeviceInfo &= ~android.net.wifi.p2p.WifiP2pWfdInfo.DEVICE_TYPE;
    ...
    public boolean isCoupledSinkSupportedAtSource() {
        return (mDeviceInfo & android.net.wifi.p2p.WifiP2pWfdInfo.COUPLED_SINK_SUPPORT_AT_SINK) != 0;
    }

    public void setCoupledSinkSupportAtSource(boolean enabled) {
        if (enabled) {
            mDeviceInfo |= android.net.wifi.p2p.WifiP2pWfdInfo.COUPLED_SINK_SUPPORT_AT_SINK;
        } else {
            mDeviceInfo &= ~android.net.wifi.p2p.WifiP2pWfdInfo.COUPLED_SINK_SUPPORT_AT_SINK;
        }
    }
    ...
    public boolean isSessionAvailable() {
        return (mDeviceInfo & android.net.wifi.p2p.WifiP2pWfdInfo.SESSION_AVAILABLE) != 0;
    }

    public void setSessionAvailable(boolean enabled) {
        if (enabled) {
            mDeviceInfo |= android.net.wifi.p2p.WifiP2pWfdInfo.SESSION_AVAILABLE_BIT1;
            mDeviceInfo &= ~android.net.wifi.p2p.WifiP2pWfdInfo.SESSION_AVAILABLE_BIT2;
        } else {
            mDeviceInfo &= ~android.net.wifi.p2p.WifiP2pWfdInfo.SESSION_AVAILABLE;
        }
    }
    ...
    public java.lang.String getDeviceInfoHex() {
        return java.lang.String.format(java.util.Locale.US, "%04x%04x%04x%04x", 6, mDeviceInfo, mCtrlPort, mMaxThroughput);
    }
     ...
     * copy constructor
     */
    public WifiP2pWfdInfo(android.net.wifi.p2p.WifiP2pWfdInfo source) {
        if (source != null) {
     ...
     * Implement the Parcelable interface
     */
    public static final android.os.Parcelable.Creator<android.net.wifi.p2p.WifiP2pWfdInfo> CREATOR = new android.os.Parcelable.Creator<android.net.wifi.p2p.WifiP2pWfdInfo>() {
        public android.net.wifi.p2p.WifiP2pWfdInfo createFromParcel(android.os.Parcel in) {
            android.net.wifi.p2p.WifiP2pWfdInfo device = new android.net.wifi.p2p.WifiP2pWfdInfo();
            device.readFromParcel(in);
            return device;
        }

        public android.net.wifi.p2p.WifiP2pWfdInfo[] newArray(int size) {
            return new android.net.wifi.p2p.WifiP2pWfdInfo[size];
        }
    };
}
