...
public class StructNdaCacheInfo {
    ...
    private static boolean hasAvailableSpace(java.nio.ByteBuffer byteBuffer) {
        return (byteBuffer != null) && (byteBuffer.remaining() >= android.net.netlink.StructNdaCacheInfo.STRUCT_SIZE);
    }

    public static android.net.netlink.StructNdaCacheInfo parse(java.nio.ByteBuffer byteBuffer) {
        if (!android.net.netlink.StructNdaCacheInfo.hasAvailableSpace(byteBuffer)) {
            return null;
        }
        // The ByteOrder must have already been set by the caller.  In most
        // cases ByteOrder.nativeOrder() is correct, with the possible
        // exception of usage within unittests.
        final android.net.netlink.StructNdaCacheInfo struct = new android.net.netlink.StructNdaCacheInfo();
    ...
    private static final long CLOCK_TICKS_PER_SECOND = android.system.Os.sysconf(android.system.OsConstants._SC_CLK_TCK);

    private static long ticksToMilliSeconds(int intClockTicks) {
        final long longClockTicks = ((long) (intClockTicks)) & 0xffffffff;
        return (longClockTicks * 1000) / android.net.netlink.StructNdaCacheInfo.CLOCK_TICKS_PER_SECOND;
    }
    ...
    public long lastUsed() {
        return android.net.netlink.StructNdaCacheInfo.ticksToMilliSeconds(ndm_used);
    }

    public long lastConfirmed() {
        return android.net.netlink.StructNdaCacheInfo.ticksToMilliSeconds(ndm_confirmed);
    }

    public long lastUpdated() {
        return android.net.netlink.StructNdaCacheInfo.ticksToMilliSeconds(ndm_updated);
    }

    @java.lang.Override
    public java.lang.String toString() {
        return (((((((((((("NdaCacheInfo{ " + "ndm_used{") + lastUsed()) + "}, ") + "ndm_confirmed{") + lastConfirmed()) + "}, ") + "ndm_updated{") + lastUpdated()) + "}, ") + "ndm_refcnt{") + ndm_refcnt) + "} ") + "}";
    }
}
