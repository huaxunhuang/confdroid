...
public class AudioGroup {
    ...
    private final java.util.Map<android.net.rtp.AudioStream, java.lang.Long> mStreams;

    private int mMode = android.net.rtp.AudioGroup.MODE_ON_HOLD;

    private long mNative;

    static {
        java.lang.System.loadLibrary("rtp_jni");
    }
     ...
     */
    public AudioGroup() {
        mStreams = new java.util.HashMap<android.net.rtp.AudioStream, java.lang.Long>();
    }
    ...
    public android.net.rtp.AudioStream[] getStreams() {
        synchronized(this) {
            return mStreams.keySet().toArray(new android.net.rtp.AudioStream[mStreams.size()]);
        }
    }
    ...
    public void setMode(int mode) {
        if ((mode < 0) || (mode > android.net.rtp.AudioGroup.MODE_LAST)) {
    ...
    synchronized void add(android.net.rtp.AudioStream stream) {
        if (!mStreams.containsKey(stream)) {
            try {
                android.net.rtp.AudioCodec codec = stream.getCodec();
                java.lang.String codecSpec = java.lang.String.format(java.util.Locale.US, "%d %s %s", codec.type, codec.rtpmap, codec.fmtp);
                long id = nativeAdd(stream.getMode(), stream.getSocket(), stream.getRemoteAddress().getHostAddress(), stream.getRemotePort(), codecSpec, stream.getDtmfType(), android.app.ActivityThread.currentOpPackageName());
    ...
    private native long nativeAdd(int mode, int socket, java.lang.String remoteAddress, int remotePort, java.lang.String codecSpec, int dtmfType, java.lang.String opPackageName);

    // Package-private method used by AudioStream.join().
    synchronized void remove(android.net.rtp.AudioStream stream) {
        java.lang.Long id = mStreams.remove(stream);
        if (id != null) {
            nativeRemove(id);
        }
    }
    ...
    public void sendDtmf(int event) {
        if ((event < 0) || (event > 15)) {
            throw new java.lang.IllegalArgumentException("Invalid event");
        }
        synchronized(this) {
     ...
     */
    public void clear() {
        for (android.net.rtp.AudioStream stream : getStreams()) {
            stream.join(null);
        }
    }
