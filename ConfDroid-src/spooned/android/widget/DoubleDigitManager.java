...
class DoubleDigitManager {
     ...
     * 		How long after the first digit is pressed does
     * 		the user have to press the second digit?
     * @param callBack
     * 		The callback to indicate what's going on with the user.
     */
    public DoubleDigitManager(long timeoutInMillis, android.widget.DoubleDigitManager.CallBack callBack) {
    ...
    public void reportDigit(int digit) {
        if (intermediateDigit == null) {
            ...
            if (mCallBack.twoDigitsFinal(intermediateDigit, digit)) {
                intermediateDigit = null;
            }
