...
class FastScroller {
     ...
     */
    private final java.lang.Runnable mDeferHide = new java.lang.Runnable() {
        @java.lang.Override
        public void run() {
            setState(android.widget.FastScroller.STATE_NONE);
        }
    };
     ...
     */
    private final android.animation.Animator.AnimatorListener mSwitchPrimaryListener = new android.animation.AnimatorListenerAdapter() {
        @java.lang.Override
    ...
    @android.annotation.UnsupportedAppUsage
    public FastScroller(android.widget.AbsListView listView, int styleResId) {
        mList = listView;
        mOldItemCount = listView.getCount();
        mOldChildCount = listView.getChildCount();
        final android.content.Context context = listView.getContext();
        ...
        mState = android.widget.FastScroller.STATE_VISIBLE;
        mMatchDragPosition = context.getApplicationInfo().targetSdkVersion >= Build.VERSION_CODES.HONEYCOMB;
        mTrackImage = new android.widget.ImageView(context);
        mTrackImage.setScaleType(android.widget.ImageView.ScaleType.FIT_XY);
        mThumbImage = new android.widget.ImageView(context);
        mThumbImage.setScaleType(android.widget.ImageView.ScaleType.FIT_XY);
        mPreviewImage = new android.view.View(context);
        ...
        setStyle(styleResId);
        final android.view.ViewGroupOverlay overlay = listView.getOverlay();
        mOverlay = overlay;
    ...
    private void updateAppearance() {
        ...
        if (mTrackDrawable != null) {
            width = java.lang.Math.max(width, mTrackDrawable.getIntrinsicWidth());
        }
        ...
        if (mThumbDrawable != null) {
            width = java.lang.Math.max(width, mThumbDrawable.getIntrinsicWidth());
        }
        // Account for minimum thumb width.
        mWidth = java.lang.Math.max(width, mThumbMinWidth);
    ...
    public void setStyle(@android.annotation.StyleRes
    int resId) {
        final android.content.Context context = mList.getContext();
        final android.content.res.TypedArray ta = context.obtainStyledAttributes(null, R.styleable.FastScroll, R.attr.fastScrollStyle, resId);
        final int N = ta.getIndexCount();
        for (int i = 0; i < N; i++) {
            final int index = ta.getIndex(i);
            switch (index) {
                case R.styleable.FastScroll_position :
                    mOverlayPosition = ta.getInt(index, android.widget.FastScroller.OVERLAY_FLOATING);
                    break;
                case R.styleable.FastScroll_backgroundLeft :
                    mPreviewResId[android.widget.FastScroller.PREVIEW_LEFT] = ta.getResourceId(index, 0);
                    break;
                case R.styleable.FastScroll_backgroundRight :
                    mPreviewResId[android.widget.FastScroller.PREVIEW_RIGHT] = ta.getResourceId(index, 0);
                    break;
                ...
                case R.styleable.FastScroll_thumbPosition :
                    mThumbPosition = ta.getInt(index, android.widget.FastScroller.THUMB_POSITION_MIDPOINT);
                    break;
            }
        }
        ta.recycle();
     ...
     *
     * @param enabled
     * 		Whether the fast scroll thumb is enabled.
     */
     ...
     *
     * @param alwaysShow
     * 		Whether the fast scroll thumb should always be shown
     */
     ...
     *
     * @param peekIfEnabled
     * 		whether the thumb should peek, if enabled
     */
    private void onStateDependencyChanged(boolean peekIfEnabled) {
        if (isEnabled()) {
            if (isAlwaysShowEnabled()) {
                setState(android.widget.FastScroller.STATE_VISIBLE);
            } else
                if (mState == android.widget.FastScroller.STATE_VISIBLE) {
                    postAutoHide();
                } else
                    if (peekIfEnabled) {
                        setState(android.widget.FastScroller.STATE_VISIBLE);
    ...
    public void setScrollbarPosition(int position) {
        if (position == android.view.View.SCROLLBAR_POSITION_DEFAULT) {
            position = (mList.isLayoutRtl()) ? android.view.View.SCROLLBAR_POSITION_LEFT : android.view.View.SCROLLBAR_POSITION_RIGHT;
        }
        if (mScrollbarPosition != position) {
            mScrollbarPosition = position;
            mLayoutFromRight = position != android.view.View.SCROLLBAR_POSITION_LEFT;
            final int previewResId = mPreviewResId[mLayoutFromRight ? android.widget.FastScroller.PREVIEW_RIGHT : android.widget.FastScroller.PREVIEW_LEFT];
            mPreviewImage.setBackgroundResource(previewResId);
            // Propagate padding to text min width/height.
            final int textMinWidth = java.lang.Math.max(0, (mPreviewMinWidth - mPreviewImage.getPaddingLeft()) - mPreviewImage.getPaddingRight());
            mPrimaryText.setMinimumWidth(textMinWidth);
            mSecondaryText.setMinimumWidth(textMinWidth);
            final int textMinHeight = java.lang.Math.max(0, (mPreviewMinHeight - mPreviewImage.getPaddingTop()) - mPreviewImage.getPaddingBottom());
    ...
    public void onItemCountChanged(int childCount, int itemCount) {
        if ((mOldItemCount != itemCount) || (mOldChildCount != childCount)) {
            ...
            if (hasMoreItems && (mState != android.widget.FastScroller.STATE_DRAGGING)) {
    ...
    private void updateLongList(int childCount, int itemCount) {
        final boolean longList = (childCount > 0) && ((itemCount / childCount) >= android.widget.FastScroller.MIN_PAGES);
    ...
    private android.widget.TextView createPreviewTextView(android.content.Context context) {
        final android.view.ViewGroup.LayoutParams params = new android.view.ViewGroup.LayoutParams(android.view.ViewGroup.LayoutParams.WRAP_CONTENT, android.view.ViewGroup.LayoutParams.WRAP_CONTENT);
    ...
    public void updateLayout() {
        ...
        final android.graphics.Rect bounds = mTempBounds;
     ...
     * 		The view to layout.
     * @param bounds
     * 		Bounds at which to layout the view.
     */
    private void applyLayout(android.view.View view, android.graphics.Rect bounds) {
     ...
     *
     * @param v
     * 		The preview text view to measure.
     * @param out
     * 		Rectangle into which measured bounds are placed.
     */
    private void measurePreview(android.view.View v, android.graphics.Rect out) {
        // Apply the preview image's padding as layout margins.
        final android.graphics.Rect margins = mTempMargins;
        ...
        if (mOverlayPosition == android.widget.FastScroller.OVERLAY_FLOATING) {
     ...
     *
     * @param view
     * 		The view to measure for layout.
     * @param adjacent
     * 		(Optional) The adjacent view, may be null to align to the
     * 		list edge.
     * @param margins
     * 		Layout margins to apply to the view.
     * @param out
     * 		Rectangle into which measured bounds are placed.
     */
    private void measureViewToSide(android.view.View view, android.view.View adjacent, android.graphics.Rect margins, android.graphics.Rect out) {
        ...
        final android.graphics.Rect container = mContainerRect;
        ...
        final int adjMaxHeight = java.lang.Math.max(0, container.height());
        final int adjMaxWidth = java.lang.Math.max(0, (maxWidth - marginLeft) - marginRight);
        final int widthMeasureSpec = android.view.View.MeasureSpec.makeMeasureSpec(adjMaxWidth, android.view.View.MeasureSpec.AT_MOST);
        final int heightMeasureSpec = android.view.View.MeasureSpec.makeSafeMeasureSpec(adjMaxHeight, android.view.View.MeasureSpec.UNSPECIFIED);
        view.measure(widthMeasureSpec, heightMeasureSpec);
        // Align to the left or right.
        final int width = java.lang.Math.min(adjMaxWidth, view.getMeasuredWidth());
        final int left;
        final int right;
        if (mLayoutFromRight) {
    ...
    private void measureFloating(android.view.View preview, android.graphics.Rect margins, android.graphics.Rect out) {
        ...
        final int containerWidth = container.width();
        final int adjMaxHeight = java.lang.Math.max(0, container.height());
        final int adjMaxWidth = java.lang.Math.max(0, (containerWidth - marginLeft) - marginRight);
        final int widthMeasureSpec = android.view.View.MeasureSpec.makeMeasureSpec(adjMaxWidth, android.view.View.MeasureSpec.AT_MOST);
        final int heightMeasureSpec = android.view.View.MeasureSpec.makeSafeMeasureSpec(adjMaxHeight, android.view.View.MeasureSpec.UNSPECIFIED);
    ...
    private void updateContainerRect() {
        final android.widget.AbsListView list = mList;
        list.resolvePadding();
        final android.graphics.Rect container = mContainerRect;
        container.left = 0;
        container.top = 0;
        ...
        if ((scrollbarStyle == android.view.View.SCROLLBARS_INSIDE_INSET) || (scrollbarStyle == android.view.View.SCROLLBARS_INSIDE_OVERLAY)) {
            ...
            if (scrollbarStyle == android.view.View.SCROLLBARS_INSIDE_INSET) {
                final int width = getWidth();
                if (mScrollbarPosition == android.view.View.SCROLLBAR_POSITION_RIGHT) {
    ...
    private void layoutTrack() {
        final android.view.View track = mTrackImage;
        final android.view.View thumb = mThumbImage;
        final android.graphics.Rect container = mContainerRect;
        final int maxWidth = java.lang.Math.max(0, container.width());
        final int maxHeight = java.lang.Math.max(0, container.height());
        final int widthMeasureSpec = android.view.View.MeasureSpec.makeMeasureSpec(maxWidth, android.view.View.MeasureSpec.AT_MOST);
        final int heightMeasureSpec = android.view.View.MeasureSpec.makeSafeMeasureSpec(maxHeight, android.view.View.MeasureSpec.UNSPECIFIED);
        track.measure(widthMeasureSpec, heightMeasureSpec);
        final int top;
        final int bottom;
        if (mThumbPosition == android.widget.FastScroller.THUMB_POSITION_INSIDE) {
            top = container.top;
    ...
    private void updateOffsetAndRange() {
        final android.view.View trackImage = mTrackImage;
        final android.view.View thumbImage = mThumbImage;
        final float min;
        final float max;
        if (mThumbPosition == android.widget.FastScroller.THUMB_POSITION_INSIDE) {
            final float halfThumbHeight = thumbImage.getHeight() / 2.0F;
    ...
    private void setState(int state) {
        ...
        if (mAlwaysShow && (state == android.widget.FastScroller.STATE_NONE)) {
            state = android.widget.FastScroller.STATE_VISIBLE;
        }
        if (state == mState) {
            return;
        }
        switch (state) {
            case android.widget.FastScroller.STATE_NONE :
                transitionToHidden();
                break;
            case android.widget.FastScroller.STATE_VISIBLE :
                transitionToVisible();
                break;
            case android.widget.FastScroller.STATE_DRAGGING :
    ...
    private void refreshDrawablePressedState() {
        final boolean isPressed = mState == android.widget.FastScroller.STATE_DRAGGING;
    ...
    private void transitionToHidden() {
        ...
        final android.animation.Animator fadeOut = android.widget.FastScroller.groupAnimatorOfFloat(android.view.View.ALPHA, 0.0F, mThumbImage, mTrackImage, mPreviewImage, mPrimaryText, mSecondaryText).setDuration(android.widget.FastScroller.DURATION_FADE_OUT);
        ...
        final android.animation.Animator slideOut = android.widget.FastScroller.groupAnimatorOfFloat(android.view.View.TRANSLATION_X, offset, mThumbImage, mTrackImage).setDuration(android.widget.FastScroller.DURATION_FADE_OUT);
        mDecorAnimation = new android.animation.AnimatorSet();
    ...
    private void transitionToVisible() {
        ...
        final android.animation.Animator fadeIn = android.widget.FastScroller.groupAnimatorOfFloat(android.view.View.ALPHA, 1.0F, mThumbImage, mTrackImage).setDuration(android.widget.FastScroller.DURATION_FADE_IN);
        final android.animation.Animator fadeOut = android.widget.FastScroller.groupAnimatorOfFloat(android.view.View.ALPHA, 0.0F, mPreviewImage, mPrimaryText, mSecondaryText).setDuration(android.widget.FastScroller.DURATION_FADE_OUT);
        final android.animation.Animator slideIn = android.widget.FastScroller.groupAnimatorOfFloat(android.view.View.TRANSLATION_X, 0.0F, mThumbImage, mTrackImage).setDuration(android.widget.FastScroller.DURATION_FADE_IN);
        mDecorAnimation = new android.animation.AnimatorSet();
    ...
    private void transitionToDragging() {
        ...
        final android.animation.Animator fadeIn = android.widget.FastScroller.groupAnimatorOfFloat(android.view.View.ALPHA, 1.0F, mThumbImage, mTrackImage, mPreviewImage).setDuration(android.widget.FastScroller.DURATION_FADE_IN);
        final android.animation.Animator slideIn = android.widget.FastScroller.groupAnimatorOfFloat(android.view.View.TRANSLATION_X, 0.0F, mThumbImage, mTrackImage).setDuration(android.widget.FastScroller.DURATION_FADE_IN);
        mDecorAnimation = new android.animation.AnimatorSet();
    ...
    private void postAutoHide() {
        mList.removeCallbacks(mDeferHide);
        mList.postDelayed(mDeferHide, android.widget.FastScroller.FADE_TIMEOUT);
    }
    ...
    public void onScroll(int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        ...
        if (mFirstVisibleItem != firstVisibleItem) {
            ...
            if (mState != android.widget.FastScroller.STATE_DRAGGING) {
                setState(android.widget.FastScroller.STATE_VISIBLE);
                postAutoHide();
            }
        }
    ...
    private void getSectionsFromIndexer() {
        ...
        if (adapter instanceof android.widget.HeaderViewListAdapter) {
            mHeaderCount = ((android.widget.HeaderViewListAdapter) (adapter)).getHeadersCount();
        ...
        if (adapter instanceof android.widget.ExpandableListConnector) {
            final android.widget.ExpandableListAdapter expAdapter = ((android.widget.ExpandableListConnector) (adapter)).getAdapter();
            if (expAdapter instanceof android.widget.SectionIndexer) {
    ...
    private void scrollTo(float position) {
        ...
        final java.lang.Object[] sections = mSections;
        final int sectionCount = (sections == null) ? 0 : sections.length;
        int sectionIndex;
        if ((sections != null) && (sectionCount > 1)) {
            final int exactSection = android.util.MathUtils.constrain(((int) (position * sectionCount)), 0, sectionCount - 1);
            ...
            final float snapThreshold = (count == 0) ? java.lang.Float.MAX_VALUE : 0.125F / count;
            ...
            targetIndex = android.util.MathUtils.constrain(targetIndex, 0, count - 1);
            if (mList instanceof android.widget.ExpandableListView) {
                final android.widget.ExpandableListView expList = ((android.widget.ExpandableListView) (mList));
                expList.setSelectionFromTop(expList.getFlatListPosition(android.widget.ExpandableListView.getPackedPositionForGroup(targetIndex + mHeaderCount)), 0);
            } else
                if (mList instanceof android.widget.ListView) {
                    ((android.widget.ListView) (mList)).setSelectionFromTop(targetIndex + mHeaderCount, 0);
                } else {
                    mList.setSelection(targetIndex + mHeaderCount);
                }

        } else {
            final int index = android.util.MathUtils.constrain(((int) (position * count)), 0, count - 1);
            if (mList instanceof android.widget.ExpandableListView) {
                android.widget.ExpandableListView expList = ((android.widget.ExpandableListView) (mList));
                expList.setSelectionFromTop(expList.getFlatListPosition(android.widget.ExpandableListView.getPackedPositionForGroup(index + mHeaderCount)), 0);
            } else
                if (mList instanceof android.widget.ListView) {
                    ((android.widget.ListView) (mList)).setSelectionFromTop(index + mHeaderCount, 0);
                } else {
    ...
    private boolean transitionPreviewLayout(int sectionIndex) {
        ...
        if (((sections != null) && (sectionIndex >= 0)) && (sectionIndex < sections.length)) {
            final java.lang.Object section = sections[sectionIndex];
            if (section != null) {
                text = section.toString();
            }
        }
        final android.graphics.Rect bounds = mTempBounds;
        final android.view.View preview = mPreviewImage;
        final android.widget.TextView showing;
        final android.widget.TextView target;
        if (mShowingPrimary) {
        ...
        final android.animation.Animator showTarget = android.widget.FastScroller.animateAlpha(target, 1.0F).setDuration(android.widget.FastScroller.DURATION_CROSS_FADE);
        final android.animation.Animator hideShowing = android.widget.FastScroller.animateAlpha(showing, 0.0F).setDuration(android.widget.FastScroller.DURATION_CROSS_FADE);
        ...
        final android.animation.Animator resizePreview = android.widget.FastScroller.animateBounds(preview, bounds);
        resizePreview.setDuration(android.widget.FastScroller.DURATION_RESIZE);
        mPreviewAnimation = new android.animation.AnimatorSet();
        final android.animation.AnimatorSet.Builder builder = mPreviewAnimation.play(hideShowing).with(showTarget);
        ...
        if (targetWidth > previewWidth) {
            ...
            final android.animation.Animator scaleAnim = android.widget.FastScroller.animateScaleX(target, 1.0F).setDuration(android.widget.FastScroller.DURATION_RESIZE);
        ...
        if (showingWidth > targetWidth) {
            ...
            final android.animation.Animator scaleAnim = android.widget.FastScroller.animateScaleX(showing, scale).setDuration(android.widget.FastScroller.DURATION_RESIZE);
            builder.with(scaleAnim);
        }
     ...
     *
     * @param position
     * 		The position, between 0 and 1, along the track at which
     * 		to place the thumb.
     */
    private void setThumbPos(float position) {
        final float thumbMiddle = (position * mThumbRange) + mThumbOffset;
        mThumbImage.setTranslationY(thumbMiddle - (mThumbImage.getHeight() / 2.0F));
        final android.view.View previewImage = mPreviewImage;
        final float previewHalfHeight = previewImage.getHeight() / 2.0F;
        final float previewPos;
        switch (mOverlayPosition) {
            case android.widget.FastScroller.OVERLAY_AT_THUMB :
                previewPos = thumbMiddle;
                break;
            case android.widget.FastScroller.OVERLAY_ABOVE_THUMB :
                previewPos = thumbMiddle - previewHalfHeight;
                break;
            case android.widget.FastScroller.OVERLAY_FLOATING :
            default :
                previewPos = 0;
                break;
        }
        ...
        final float previewMiddle = android.util.MathUtils.constrain(previewPos, minP, maxP);
    ...
    private float getPosFromMotionEvent(float y) {
        ...
        return android.util.MathUtils.constrain((y - mThumbOffset) / mThumbRange, 0.0F, 1.0F);
    }
    ...
    private float getPosFromItemCount(int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        ...
        final boolean hasSections = ((sectionIndexer != null) && (mSections != null)) && (mSections.length > 0);
        ...
        final android.view.View child = mList.getChildAt(0);
        ...
        if ((firstVisibleItem > 0) && ((firstVisibleItem + visibleItemCount) == totalItemCount)) {
            final android.view.View lastChild = mList.getChildAt(visibleItemCount - 1);
    ...
    private void cancelFling() {
        final android.view.MotionEvent cancelFling = android.view.MotionEvent.obtain(0, 0, android.view.MotionEvent.ACTION_CANCEL, 0, 0, 0);
    ...
    private void startPendingDrag() {
        mPendingDrag = android.os.SystemClock.uptimeMillis() + android.widget.FastScroller.TAP_TIMEOUT;
    }

    private void beginDrag() {
        mPendingDrag = -1;
        setState(android.widget.FastScroller.STATE_DRAGGING);
        if ((mListAdapter == null) && (mList != null)) {
            getSectionsFromIndexer();
        }
        if (mList != null) {
            mList.requestDisallowInterceptTouchEvent(true);
            mList.reportScrollStateChange(android.widget.AbsListView.OnScrollListener.SCROLL_STATE_TOUCH_SCROLL);
        }
        cancelFling();
    }
        ...
        switch (ev.getActionMasked()) {
            ...
            case android.view.MotionEvent.ACTION_MOVE :
                if (!isPointInside(ev.getX(), ev.getY())) {
                    ...
                    if ((mPendingDrag >= 0) && (mPendingDrag <= android.os.SystemClock.uptimeMillis())) {
                        beginDrag();
    ...
    public boolean onInterceptHoverEvent(android.view.MotionEvent ev) {
        ...
        if ((((actionMasked == android.view.MotionEvent.ACTION_HOVER_ENTER) || (actionMasked == android.view.MotionEvent.ACTION_HOVER_MOVE)) && (mState == android.widget.FastScroller.STATE_NONE)) && isPointInside(ev.getX(), ev.getY())) {
            setState(android.widget.FastScroller.STATE_VISIBLE);
            postAutoHide();
        }
        return false;
    ...
    public android.view.PointerIcon onResolvePointerIcon(android.view.MotionEvent event, int pointerIndex) {
        if ((mState == android.widget.FastScroller.STATE_DRAGGING) || isPointInside(event.getX(), event.getY())) {
            return android.view.PointerIcon.getSystemIcon(mList.getContext(), android.view.PointerIcon.TYPE_ARROW);
        }
        return null;
    }
    ...
    public boolean onTouchEvent(android.view.MotionEvent me) {
        ...
        switch (me.getActionMasked()) {
            ...
            case android.view.MotionEvent.ACTION_UP :
                {
                    ...
                    if (mState == android.widget.FastScroller.STATE_DRAGGING) {
                        if (mList != null) {
                            ...
                            mList.reportScrollStateChange(android.widget.AbsListView.OnScrollListener.SCROLL_STATE_IDLE);
                        }
                        setState(android.widget.FastScroller.STATE_VISIBLE);
                        postAutoHide();
                        return true;
            ...
            case android.view.MotionEvent.ACTION_MOVE :
                {
                    if ((mPendingDrag >= 0) && (java.lang.Math.abs(me.getY() - mInitialTouchY) > mScaledTouchSlop)) {
                        beginDrag();
                        // Will hit the STATE_DRAGGING check below
                    }
                    if (mState == android.widget.FastScroller.STATE_DRAGGING) {
     ...
     * @param value
     * 		The value to which that property should animate.
     * @param views
     * 		The target views to animate.
     * @return An animator for all the specified views.
     */
    private static android.animation.Animator groupAnimatorOfFloat(android.util.Property<android.view.View, java.lang.Float> property, float value, android.view.View... views) {
        ...
        for (int i = views.length - 1; i >= 0; i--) {
            final android.animation.Animator anim = android.animation.ObjectAnimator.ofFloat(views[i], property, value);
            if (builder == null) {
     ...
     */
    private static android.animation.Animator animateScaleX(android.view.View v, float target) {
        return android.animation.ObjectAnimator.ofFloat(v, android.view.View.SCALE_X, target);
    }

    /**
     * Returns an animator for the view's alpha value.
     */
    private static android.animation.Animator animateAlpha(android.view.View v, float alpha) {
        return android.animation.ObjectAnimator.ofFloat(v, android.view.View.ALPHA, alpha);
    }
     ...
     */
    private static android.util.Property<android.view.View, java.lang.Integer> LEFT = new android.util.IntProperty<android.view.View>("left") {
        @java.lang.Override
        public void setValue(android.view.View object, int value) {
            object.setLeft(value);
        }

        @java.lang.Override
        public java.lang.Integer get(android.view.View object) {
     ...
     */
    private static android.util.Property<android.view.View, java.lang.Integer> TOP = new android.util.IntProperty<android.view.View>("top") {
        @java.lang.Override
        public void setValue(android.view.View object, int value) {
            object.setTop(value);
        }
     ...
     */
    private static android.util.Property<android.view.View, java.lang.Integer> RIGHT = new android.util.IntProperty<android.view.View>("right") {
        @java.lang.Override
        public void setValue(android.view.View object, int value) {
     ...
     */
    private static android.util.Property<android.view.View, java.lang.Integer> BOTTOM = new android.util.IntProperty<android.view.View>("bottom") {
        @java.lang.Override
        public void setValue(android.view.View object, int value) {
     ...
     */
    private static android.animation.Animator animateBounds(android.view.View v, android.graphics.Rect bounds) {
        final android.animation.PropertyValuesHolder left = android.animation.PropertyValuesHolder.ofInt(android.widget.FastScroller.LEFT, bounds.left);
        final android.animation.PropertyValuesHolder top = android.animation.PropertyValuesHolder.ofInt(android.widget.FastScroller.TOP, bounds.top);
        final android.animation.PropertyValuesHolder right = android.animation.PropertyValuesHolder.ofInt(android.widget.FastScroller.RIGHT, bounds.right);
        final android.animation.PropertyValuesHolder bottom = android.animation.PropertyValuesHolder.ofInt(android.widget.FastScroller.BOTTOM, bounds.bottom);
        return android.animation.ObjectAnimator.ofPropertyValuesHolder(v, left, top, right, bottom);
    }
}
