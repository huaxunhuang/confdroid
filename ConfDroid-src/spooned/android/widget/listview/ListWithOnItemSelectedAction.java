...
public class ListWithOnItemSelectedAction extends android.util.ListScenario {
    protected void init(android.widget.listview.Params params) {
        params.setNumItems(8).setItemScreenSizeFactor(0.2).includeHeaderAboveList(true);
    }

    @java.lang.Override
    protected void positionSelected(int positon) {
        if (positon != getListView().getSelectedItemPosition()) {
            throw new java.lang.IllegalStateException("something is fishy... the selected postion does not " + "match what the list reports.");
        }
        setHeaderValue(((android.widget.TextView) (getListView().getSelectedView())).getText().toString());
    }
}
