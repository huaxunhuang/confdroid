...
public class ListOfItemsShorterThanScreenTest extends android.test.ActivityInstrumentationTestCase<android.widget.listview.ListOfItemsShorterThanScreen> {
    private android.widget.ListView mListView;

    private android.widget.listview.ListOfItemsShorterThanScreen mActivity;

    public ListOfItemsShorterThanScreenTest() {
        super("com.android.frameworks.coretests", android.widget.listview.ListOfItemsShorterThanScreen.class);
    }
    ...
    public void testMoveDownToOnScreenNextItem() {
        final android.view.View next = mListView.getChildAt(1);
        assertFalse(next.isSelected());
        final int secondPosition = mListView.getSelectedView().getBottom();
        assertEquals("second item should be positioned item height pixels from top.", secondPosition, next.getTop());
        sendKeys(android.view.KeyEvent.KEYCODE_DPAD_DOWN);
        assertEquals(1, mListView.getSelectedItemPosition());
        assertTrue(next.isSelected());
        assertEquals("next selected item shouldn't have moved", secondPosition, next.getTop());
    }
    ...
    public void testMoveDownToItemRequiringScrolling() {
        ...
        final android.view.View lastItem = mListView.getChildAt(lastOnScreenItemIndex);
        ...
        final android.widget.TextView view = ((android.widget.TextView) (mListView.getChildAt(mListView.getChildCount() - 1)));
    ...
    public void testMoveUpToItemRequiringScrolling() {
        ...
        assertEquals(1, mListView.getSelectedItemPosition());
        assertEquals("top should be just below vertical fading edge", mListView.getVerticalFadingEdgeLength() + mListView.getListPaddingTop(), mListView.getSelectedView().getTop());
    }
    ...
    public void testNoVerticalFadingEdgeWhenMovingToBottom() {
        ...
        assertEquals(lastItemPosition, mListView.getSelectedItemPosition());
        assertEquals("bottom of last item should be just above padding; no fading edge.", mListView.getHeight() - mListView.getListPaddingBottom(), mListView.getSelectedView().getBottom());
    }
