 ...
 *
 * @unknown ref android.R.styleable#HorizontalScrollView_fillViewport
 */
public class HorizontalScrollView extends android.widget.FrameLayout {
    private static final int ANIMATED_SCROLL_GAP = android.widget.ScrollView.ANIMATED_SCROLL_GAP;

    private static final float MAX_SCROLL_FACTOR = android.widget.ScrollView.MAX_SCROLL_FACTOR;
    ...
    @android.annotation.UnsupportedAppUsage(maxTargetSdk = Build.VERSION_CODES.P, trackingBug = 124053130)
    private android.widget.EdgeEffect mEdgeGlowLeft = new android.widget.EdgeEffect(getContext());
     ...
     * setting it via reflection and they need to keep working until they target Q.
     */
    @android.annotation.UnsupportedAppUsage(maxTargetSdk = Build.VERSION_CODES.P, trackingBug = 124052619)
    private android.widget.EdgeEffect mEdgeGlowRight = new android.widget.EdgeEffect(getContext());
     ...
     */
    private int mActivePointerId = android.widget.HorizontalScrollView.INVALID_POINTER;
    ...
    public HorizontalScrollView(android.content.Context context, android.util.AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        initScrollView();
        final android.content.res.TypedArray a = context.obtainStyledAttributes(attrs, android.R.styleable.HorizontalScrollView, defStyleAttr, defStyleRes);
        saveAttributeDataForStyleable(context, android.R.styleable.HorizontalScrollView, attrs, a, defStyleAttr, defStyleRes);
     ...
     * @see #getRightEdgeEffectColor()
     */
    public void setRightEdgeEffectColor(@android.annotation.ColorInt
    int color) {
     ...
     *
     * @param color
     * 		The color for the left edge effect.
     * @see #setRightEdgeEffectColor(int)
     * @see #setEdgeEffectColor(int)
     * @see #getLeftEdgeEffectColor()
     * @see #getRightEdgeEffectColor()
     */
    public void setLeftEdgeEffectColor(@android.annotation.ColorInt
    int color) {
    ...
    public int getMaxScrollAmount() {
        return ((int) (android.widget.HorizontalScrollView.MAX_SCROLL_FACTOR * (mRight - mLeft)));
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (getChildCount() > 0) {
            ...
            if (child.getMeasuredWidth() < desiredWidth) {
                final int childWidthMeasureSpec = android.view.View.MeasureSpec.makeMeasureSpec(desiredWidth, android.view.View.MeasureSpec.EXACTLY);
                final int childHeightMeasureSpec = android.view.ViewGroup.getChildMeasureSpec(heightMeasureSpec, heightPadding, lp.height);
    ...
    public boolean executeKeyEvent(android.view.KeyEvent event) {
        ...
        if (!canScroll()) {
            if (isFocused()) {
                ...
                android.view.View nextFocused = android.view.FocusFinder.getInstance().findNextFocus(this, currentFocused, android.view.View.FOCUS_RIGHT);
                return ((nextFocused != null) && (nextFocused != this)) && nextFocused.requestFocus(android.view.View.FOCUS_RIGHT);
            }
            return false;
        }
        ...
        if (event.getAction() == android.view.KeyEvent.ACTION_DOWN) {
            switch (event.getKeyCode()) {
                ...
                case android.view.KeyEvent.KEYCODE_SPACE :
                    pageScroll(event.isShiftPressed() ? android.view.View.FOCUS_LEFT : android.view.View.FOCUS_RIGHT);
                    break;
            }
        }
        return handled;
    ...
    public boolean onInterceptTouchEvent(android.view.MotionEvent ev) {
        ...
        switch (action & android.view.MotionEvent.ACTION_MASK) {
            case android.view.MotionEvent.ACTION_MOVE :
                {
                    ...
                    if (activePointerId == android.widget.HorizontalScrollView.INVALID_POINTER) {
                        // If we don't have a valid id, the touch down wasn't on content.
                        break;
                    }
                    final int pointerIndex = ev.findPointerIndex(activePointerId);
                    if (pointerIndex == (-1)) {
                        android.util.Log.e(android.widget.HorizontalScrollView.TAG, ("Invalid pointerId=" + activePointerId) + " in onInterceptTouchEvent");
                        break;
                    }
                    final int x = ((int) (ev.getX(pointerIndex)));
                    final int xDiff = ((int) (java.lang.Math.abs(x - mLastMotionX)));
                    if (xDiff > mTouchSlop) {
            ...
            case android.view.MotionEvent.ACTION_UP :
                /* Release the drag */
                mIsBeingDragged = false;
                mActivePointerId = android.widget.HorizontalScrollView.INVALID_POINTER;
        ...
        switch (action & android.view.MotionEvent.ACTION_MASK) {
            ...
            case android.view.MotionEvent.ACTION_MOVE :
                ...
                if (activePointerIndex == (-1)) {
                    android.util.Log.e(android.widget.HorizontalScrollView.TAG, ("Invalid pointerId=" + mActivePointerId) + " in onTouchEvent");
                    break;
                }
                final int x = ((int) (ev.getX(activePointerIndex)));
                int deltaX = mLastMotionX - x;
                if ((!mIsBeingDragged) && (java.lang.Math.abs(deltaX) > mTouchSlop)) {
                    final android.view.ViewParent parent = getParent();
                    if (parent != null) {
                        parent.requestDisallowInterceptTouchEvent(true);
                    }
                ...
                if (mIsBeingDragged) {
                    ...
                    if (canOverscroll) {
                        ...
                        if (pulledToX < 0) {
                            mEdgeGlowLeft.onPull(((float) (deltaX)) / getWidth(), 1.0F - (ev.getY(activePointerIndex) / getHeight()));
                    ...
                    if (!mIsBeingDragged) {
                        ...
                        if (event.isFromSource(android.view.InputDevice.SOURCE_CLASS_POINTER)) {
                            if ((event.getMetaState() & android.view.KeyEvent.META_SHIFT_ON) != 0) {
                                ...
                                axisValue = event.getAxisValue(android.view.MotionEvent.AXIS_HSCROLL);
                            }
                        } else
                        ...
                        final int delta = java.lang.Math.round(axisValue * mHorizontalScrollFactor);
                        if (delta != 0) {
                            final int range = getScrollRange();
            ...
            case R.id.accessibilityActionScrollRight :
                {
                    ...
                    final int targetScrollX = java.lang.Math.min(mScrollX + viewportWidth, getScrollRange());
            ...
            case R.id.accessibilityActionScrollLeft :
                {
                    ...
                    final int viewportWidth = (getWidth() - mPaddingLeft) - mPaddingRight;
                    final int targetScrollX = java.lang.Math.max(0, mScrollX - viewportWidth);
                    if (targetScrollX != mScrollX) {
                        smoothScrollTo(targetScrollX, 0);
                        return true;
                    }
    ...
    public void onInitializeAccessibilityNodeInfoInternal(android.view.accessibility.AccessibilityNodeInfo info) {
        ...
        if (scrollRange > 0) {
            ...
            if (isEnabled() && (mScrollX > 0)) {
                ...
                info.addAction(android.view.accessibility.AccessibilityNodeInfo.AccessibilityAction.ACTION_SCROLL_LEFT);
            }
            if (isEnabled() && (mScrollX < scrollRange)) {
                ...
                info.addAction(android.view.accessibility.AccessibilityNodeInfo.AccessibilityAction.ACTION_SCROLL_RIGHT);
            }
        }
    }
    ...
    private int getScrollRange() {
        ...
        if (getChildCount() > 0) {
            android.view.View child = getChildAt(0);
            scrollRange = java.lang.Math.max(0, child.getWidth() - ((getWidth() - mPaddingLeft) - mPaddingRight));
        }
        return scrollRange;
    }
     ...
     */
    private android.view.View findFocusableViewInMyBounds(final boolean leftFocus, final int left, android.view.View preferredFocusable) {
     ...
     *
     * @param direction
     * 		the scroll direction: {@link android.view.View#FOCUS_LEFT}
     * 		to go one page left or {@link android.view.View#FOCUS_RIGHT}
     * 		to go one page right
     * @return true if the key event is consumed by this method, false otherwise
     */
    public boolean pageScroll(int direction) {
        boolean right = direction == android.view.View.FOCUS_RIGHT;
        int width = getWidth();
        if (right) {
    ...
    private boolean scrollAndFocus(int direction, int left, int right) {
        ...
        boolean goLeft = direction == android.view.View.FOCUS_LEFT;
    ...
    public final void smoothScrollBy(int dx, int dy) {
        ...
        if (duration > android.widget.HorizontalScrollView.ANIMATED_SCROLL_GAP) {
            final int width = (getWidth() - mPaddingRight) - mPaddingLeft;
            final int right = getChildAt(0).getWidth();
            final int maxX = java.lang.Math.max(0, right - width);
            final int scrollX = mScrollX;
            dx = java.lang.Math.max(0, java.lang.Math.min(scrollX + dx, maxX)) - scrollX;
    ...
    protected int computeHorizontalScrollRange() {
        ...
        final int scrollX = mScrollX;
        final int overscrollRight = java.lang.Math.max(0, scrollRange - contentWidth);
        if (scrollX < 0) {
            scrollRange -= scrollX;
        } else
    ...
    protected void measureChild(android.view.View child, int parentWidthMeasureSpec, int parentHeightMeasureSpec) {
        ...
        final int childWidthMeasureSpec = android.view.View.MeasureSpec.makeSafeMeasureSpec(java.lang.Math.max(0, android.view.View.MeasureSpec.getSize(parentWidthMeasureSpec) - horizontalPadding), android.view.View.MeasureSpec.UNSPECIFIED);
    ...
    protected void measureChildWithMargins(android.view.View child, int parentWidthMeasureSpec, int widthUsed, int parentHeightMeasureSpec, int heightUsed) {
        ...
        final int childWidthMeasureSpec = android.view.View.MeasureSpec.makeSafeMeasureSpec(java.lang.Math.max(0, android.view.View.MeasureSpec.getSize(parentWidthMeasureSpec) - usedTotal), android.view.View.MeasureSpec.UNSPECIFIED);
        child.measure(childWidthMeasureSpec, childHeightMeasureSpec);
    }
    ...
    protected int computeScrollDeltaToGetChildRectOnScreen(android.graphics.Rect rect) {
        ...
        if ((rect.right > screenRight) && (rect.left > screenLeft)) {
            ...
            scrollXDelta = java.lang.Math.min(scrollXDelta, distanceToRight);
        } else
            if ((rect.left < screenLeft) && (rect.right < screenRight)) {
                ...
                scrollXDelta = java.lang.Math.max(scrollXDelta, -getScrollX());
            }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (getChildCount() > 0) {
            ...
            android.widget.FrameLayout.LayoutParams childParams = ((android.widget.FrameLayout.LayoutParams) (getChildAt(0).getLayoutParams()));
        ...
        if ((mChildToScrollTo != null) && android.widget.HorizontalScrollView.isViewDescendantOf(mChildToScrollTo, this)) {
            scrollToChild(mChildToScrollTo);
        }
        mChildToScrollTo = null;
        if (!isLaidOut()) {
            final int scrollRange = java.lang.Math.max(0, childWidth - (((r - l) - mPaddingLeft) - mPaddingRight));
            if (mSavedState != null) {
                mScrollX = (isLayoutRtl()) ? scrollRange - mSavedState.scrollOffsetFromStart : mSavedState.scrollOffsetFromStart;
                mSavedState = null;
            } else {
                if (isLayoutRtl()) {
    ...
    private static boolean isViewDescendantOf(android.view.View child, android.view.View parent) {
        ...
        return (theParent instanceof android.view.ViewGroup) && android.widget.HorizontalScrollView.isViewDescendantOf(((android.view.View) (theParent)), parent);
    }
    ...
    public void fling(int velocityX) {
        if (getChildCount() > 0) {
            ...
            mScroller.fling(mScrollX, mScrollY, velocityX, 0, 0, java.lang.Math.max(0, right - width), 0, 0, width / 2, 0);
            final boolean movingRight = velocityX > 0;
            android.view.View currentFocused = findFocus();
            android.view.View newFocused = findFocusableViewInMyBounds(movingRight, mScroller.getFinalX(), currentFocused);
            if (newFocused == null) {
                newFocused = this;
            }
            if (newFocused != currentFocused) {
                newFocused.requestFocus(movingRight ? android.view.View.FOCUS_RIGHT : android.view.View.FOCUS_LEFT);
            }
            postInvalidateOnAnimation();
        }
    }
    ...
    public void scrollTo(int x, int y) {
        ...
        if (getChildCount() > 0) {
            android.view.View child = getChildAt(0);
            x = android.widget.HorizontalScrollView.clamp(x, (getWidth() - mPaddingRight) - mPaddingLeft, child.getWidth());
            y = android.widget.HorizontalScrollView.clamp(y, (getHeight() - mPaddingBottom) - mPaddingTop, child.getHeight());
    ...
    public void draw(android.graphics.Canvas canvas) {
        ...
        if (shouldDisplayEdgeEffects()) {
            ...
            if (!mEdgeGlowLeft.isFinished()) {
                ...
                canvas.rotate(270);
                canvas.translate((-height) + mPaddingTop, java.lang.Math.min(0, scrollX));
            ...
            if (!mEdgeGlowRight.isFinished()) {
                ...
                final int height = (getHeight() - mPaddingTop) - mPaddingBottom;
                canvas.rotate(90);
                canvas.translate(-mPaddingTop, -(java.lang.Math.max(getScrollRange(), scrollX) + width));
    ...
    protected void onRestoreInstanceState(android.os.Parcelable state) {
        ...
        android.widget.HorizontalScrollView.SavedState ss = ((android.widget.HorizontalScrollView.SavedState) (state));
    ...
    protected android.os.Parcelable onSaveInstanceState() {
        ...
        android.widget.HorizontalScrollView.SavedState ss = new android.widget.HorizontalScrollView.SavedState(superState);
    ...
    static class SavedState extends android.view.View.BaseSavedState {
        ...
        public java.lang.String toString() {
            return ((("HorizontalScrollView.SavedState{" + java.lang.Integer.toHexString(java.lang.System.identityHashCode(this))) + " scrollPosition=") + scrollOffsetFromStart) + "}";
        }

        @android.annotation.NonNull
        public static final android.os.Parcelable.Creator<android.widget.HorizontalScrollView.SavedState> CREATOR = new android.os.Parcelable.Creator<android.widget.HorizontalScrollView.SavedState>() {
