...
public class ScrollViewButtonsAndLabelsTest extends android.test.ActivityInstrumentationTestCase<android.widget.scroll.ScrollViewButtonsAndLabels> {
    ...
    public ScrollViewButtonsAndLabelsTest() {
        super("com.android.frameworks.coretests", android.widget.scroll.ScrollViewButtonsAndLabels.class);
    }
    ...
    public void testPreconditions() {
        assertTrue("vertical fading edge width needs to be non-zero for this " + "test to be worth anything", mScrollView.getVerticalFadingEdgeLength() > 0);
    }
    ...
    public void testArrowScrollDownOffScreenVerticalFadingEdge() {
        ...
        android.widget.Button firstButtonOffScreen = getActivity().getButton(offScreenIndex);
    ...
    public void testArrowScrollDownToBottomElementOnScreen() {
        ...
        android.widget.Button lastButton = getActivity().getButton(numGroups - 1);
    ...
    public void testArrowScrollUpOffScreenVerticalFadingEdge() {
        ...
        android.widget.Button offScreenButton = getActivity().getButton(offScreenIndex);
    ...
    public void testArrowScrollUpToTopElementOnScreen() {
        ...
        getInstrumentation().waitForIdleSync();
        android.widget.Button topButton = getActivity().getButton(0);
        assertTrue("should be back at top button", topButton.hasFocus());
        int[] buttonLoc = new int[]{ 0, 0 };
        topButton.getLocationOnScreen(buttonLoc);
        assertEquals("top of top button should be at top of screen; no need to take" + " into account vertical fading edge.", mScreenTop, buttonLoc[1]);
    }

    private int goToBottomButton() {
        int numButtons = getActivity().getNumButtons();
        android.widget.Button lastButton = getActivity().getButton(numButtons - 1);
        for (int i = 0; i < numButtons; i++) {
            sendKeys(android.view.KeyEvent.KEYCODE_DPAD_DOWN);
        }
        getInstrumentation().waitForIdleSync();
    ...
    private int findFirstButtonOffScreenTop2Bottom() {
        ...
        for (int i = 0; i < getActivity().getNumButtons(); i++) {
            int[] buttonLoc = new int[]{ 0, 0 };
            android.widget.Button button = getActivity().getButton(i);
