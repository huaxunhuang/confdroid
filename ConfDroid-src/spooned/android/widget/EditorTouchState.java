...
public class EditorTouchState {
    ...
    @android.annotation.IntDef({ android.widget.EditorTouchState.MultiTapStatus.NONE, android.widget.EditorTouchState.MultiTapStatus.FIRST_TAP, android.widget.EditorTouchState.MultiTapStatus.DOUBLE_TAP, android.widget.EditorTouchState.MultiTapStatus.TRIPLE_CLICK })
    ...
    private int mMultiTapStatus = android.widget.EditorTouchState.MultiTapStatus.NONE;
    ...
    public boolean isDoubleTap() {
        return mMultiTapStatus == android.widget.EditorTouchState.MultiTapStatus.DOUBLE_TAP;
    }

    public boolean isTripleClick() {
        return mMultiTapStatus == android.widget.EditorTouchState.MultiTapStatus.TRIPLE_CLICK;
    }

    public boolean isMultiTap() {
        return (mMultiTapStatus == android.widget.EditorTouchState.MultiTapStatus.DOUBLE_TAP) || (mMultiTapStatus == android.widget.EditorTouchState.MultiTapStatus.TRIPLE_CLICK);
    }
     ...
     */
    public void update(android.view.MotionEvent event, android.view.ViewConfiguration config) {
        ...
        if (action == android.view.MotionEvent.ACTION_DOWN) {
            ...
            if (((millisSinceLastUp <= android.view.ViewConfiguration.getDoubleTapTimeout()) && (millisBetweenLastDownAndLastUp <= android.view.ViewConfiguration.getDoubleTapTimeout())) && ((mMultiTapStatus == android.widget.EditorTouchState.MultiTapStatus.FIRST_TAP) || ((mMultiTapStatus == android.widget.EditorTouchState.MultiTapStatus.DOUBLE_TAP) && isMouse))) {
                if (mMultiTapStatus == android.widget.EditorTouchState.MultiTapStatus.FIRST_TAP) {
                    mMultiTapStatus = android.widget.EditorTouchState.MultiTapStatus.DOUBLE_TAP;
                } else {
                    mMultiTapStatus = android.widget.EditorTouchState.MultiTapStatus.TRIPLE_CLICK;
                }
                mMultiTapInSameArea = android.widget.EditorTouchState.isDistanceWithin(mLastDownX, mLastDownY, event.getX(), event.getY(), config.getScaledDoubleTapSlop());
                if (android.widget.TextView.DEBUG_CURSOR) {
                    java.lang.String status = (isDoubleTap()) ? "double" : "triple";
                    java.lang.String inSameArea = (mMultiTapInSameArea) ? "in same area" : "not in same area";
                    android.widget.Editor.logCursor("EditorTouchState", "ACTION_DOWN: %s tap detected, %s", status, inSameArea);
                }
            } else {
                mMultiTapStatus = android.widget.EditorTouchState.MultiTapStatus.FIRST_TAP;
                mMultiTapInSameArea = false;
                if (android.widget.TextView.DEBUG_CURSOR) {
                    android.widget.Editor.logCursor("EditorTouchState", "ACTION_DOWN: first tap detected");
                }
            }
            ...
            if (action == android.view.MotionEvent.ACTION_UP) {
                if (android.widget.TextView.DEBUG_CURSOR) {
                    android.widget.Editor.logCursor("EditorTouchState", "ACTION_UP");
                }
                mLastUpX = event.getX();
                mLastUpY = event.getY();
                ...
                if (action == android.view.MotionEvent.ACTION_MOVE) {
                    if (!mMovedEnoughForDrag) {
                        ...
                        if (mMovedEnoughForDrag) {
                            ...
                            mIsDragCloseToVertical = java.lang.Math.abs(deltaX) <= java.lang.Math.abs(deltaY);
                        }
                    }
                } else
                    if (action == android.view.MotionEvent.ACTION_CANCEL) {
                        mLastDownMillis = 0;
                        mLastUpMillis = 0;
                        mMultiTapStatus = android.widget.EditorTouchState.MultiTapStatus.NONE;
                        mMultiTapInSameArea = false;
                        mMovedEnoughForDrag = false;
                        mIsDragCloseToVertical = false;
                    }
