 ...
 */
@android.annotation.SystemService(android.content.Context.RESTRICTIONS_SERVICE)
public class RestrictionsManager {
     ...
     * @unknown 
     */
    public RestrictionsManager(android.content.Context context, android.content.IRestrictionsManager service) {
        mContext = context;
        mService = service;
    }
     ...
     */
    public void requestPermission(java.lang.String requestType, java.lang.String requestId, android.os.PersistableBundle request) {
        if (requestType == null) {
     ...
     * 		if any of the required parameters are missing.
     */
    public void notifyPermissionResponse(java.lang.String packageName, android.os.PersistableBundle response) {
        if (packageName == null) {
        ...
        if (!response.containsKey(android.content.RestrictionsManager.REQUEST_KEY_ID)) {
            throw new java.lang.IllegalArgumentException("REQUEST_KEY_ID must be specified");
        }
        if (!response.containsKey(android.content.RestrictionsManager.RESPONSE_KEY_RESULT)) {
     ...
     */
    public java.util.List<android.content.RestrictionEntry> getManifestRestrictions(java.lang.String packageName) {
        android.content.pm.ApplicationInfo appInfo = null;
        try {
            appInfo = mContext.getPackageManager().getApplicationInfo(packageName, android.content.pm.PackageManager.GET_META_DATA);
        } catch (android.content.pm.PackageManager.NameNotFoundException pnfe) {
            throw new java.lang.IllegalArgumentException("No such package " + packageName);
        }
        if ((appInfo == null) || (!appInfo.metaData.containsKey(android.content.RestrictionsManager.META_DATA_APP_RESTRICTIONS))) {
            return null;
        }
        android.content.res.XmlResourceParser xml = appInfo.loadXmlMetaData(mContext.getPackageManager(), android.content.RestrictionsManager.META_DATA_APP_RESTRICTIONS);
        return loadManifestRestrictions(packageName, xml);
    }

    private java.util.List<android.content.RestrictionEntry> loadManifestRestrictions(java.lang.String packageName, android.content.res.XmlResourceParser xml) {
        ...
        java.util.ArrayList<android.content.RestrictionEntry> restrictions = new java.util.ArrayList<>();
        ...
        try {
            ...
            android.util.Log.w(android.content.RestrictionsManager.TAG, "Reading restriction metadata for " + packageName, e);
            return null;
    ...
    private android.content.RestrictionEntry loadRestrictionElement(android.content.Context appContext, android.content.res.XmlResourceParser xml) throws java.io.IOException, org.xmlpull.v1.XmlPullParserException {
        if (xml.getName().equals(android.content.RestrictionsManager.TAG_RESTRICTION)) {
            android.util.AttributeSet attrSet = android.util.Xml.asAttributeSet(xml);
            if (attrSet != null) {
                android.content.res.TypedArray a = appContext.obtainStyledAttributes(attrSet, com.android.internal.R.styleable.RestrictionEntry);
    ...
    private android.content.RestrictionEntry loadRestriction(android.content.Context appContext, android.content.res.TypedArray a, android.content.res.XmlResourceParser xml) throws java.io.IOException, org.xmlpull.v1.XmlPullParserException {
        java.lang.String key = a.getString(R.styleable.RestrictionEntry_key);
        int restrictionType = a.getInt(R.styleable.RestrictionEntry_restrictionType, -1);
        java.lang.String title = a.getString(R.styleable.RestrictionEntry_title);
        java.lang.String description = a.getString(R.styleable.RestrictionEntry_description);
        ...
        if (restrictionType == (-1)) {
            android.util.Log.w(android.content.RestrictionsManager.TAG, "restrictionType cannot be omitted");
            return null;
        }
        if (key == null) {
            android.util.Log.w(android.content.RestrictionsManager.TAG, "key cannot be omitted");
            return null;
        }
        android.content.RestrictionEntry restriction = new android.content.RestrictionEntry(restrictionType, key);
        ...
        switch (restrictionType) {
            case android.content.RestrictionEntry.TYPE_NULL :
                // hidden
            case android.content.RestrictionEntry.TYPE_STRING :
            case android.content.RestrictionEntry.TYPE_CHOICE :
                restriction.setSelectedString(a.getString(R.styleable.RestrictionEntry_defaultValue));
                break;
            case android.content.RestrictionEntry.TYPE_INTEGER :
                restriction.setIntValue(a.getInt(R.styleable.RestrictionEntry_defaultValue, 0));
                break;
            case android.content.RestrictionEntry.TYPE_MULTI_SELECT :
                ...
                break;
            case android.content.RestrictionEntry.TYPE_BOOLEAN :
                restriction.setSelectedState(a.getBoolean(R.styleable.RestrictionEntry_defaultValue, false));
                break;
            case android.content.RestrictionEntry.TYPE_BUNDLE :
            case android.content.RestrictionEntry.TYPE_BUNDLE_ARRAY :
                final int outerDepth = xml.getDepth();
                java.util.List<android.content.RestrictionEntry> restrictionEntries = new java.util.ArrayList<>();
                while (com.android.internal.util.XmlUtils.nextElementWithin(xml, outerDepth)) {
                    android.content.RestrictionEntry childEntry = loadRestrictionElement(appContext, xml);
                    if (childEntry == null) {
                        android.util.Log.w(android.content.RestrictionsManager.TAG, "Child entry cannot be loaded for bundle restriction " + key);
                    } else {
                        restrictionEntries.add(childEntry);
                        if ((restrictionType == android.content.RestrictionEntry.TYPE_BUNDLE_ARRAY) && (childEntry.getType() != android.content.RestrictionEntry.TYPE_BUNDLE)) {
                            android.util.Log.w(android.content.RestrictionsManager.TAG, ("bundle_array " + key) + " can only contain entries of type bundle");
                        }
                    }
                } 
                restriction.setRestrictions(restrictionEntries.toArray(new android.content.RestrictionEntry[restrictionEntries.size()]));
                break;
            default :
                android.util.Log.w(android.content.RestrictionsManager.TAG, "Unknown restriction type " + restrictionType);
        }
        return restriction;
    }
     ...
     */
    public static android.os.Bundle convertRestrictionsToBundle(java.util.List<android.content.RestrictionEntry> entries) {
        final android.os.Bundle bundle = new android.os.Bundle();
        for (android.content.RestrictionEntry entry : entries) {
            android.content.RestrictionsManager.addRestrictionToBundle(bundle, entry);
        }
        return bundle;
    }

    private static android.os.Bundle addRestrictionToBundle(android.os.Bundle bundle, android.content.RestrictionEntry entry) {
        switch (entry.getType()) {
            case android.content.RestrictionEntry.TYPE_BOOLEAN :
                bundle.putBoolean(entry.getKey(), entry.getSelectedState());
                break;
            case android.content.RestrictionEntry.TYPE_CHOICE :
            case android.content.RestrictionEntry.TYPE_CHOICE_LEVEL :
            case android.content.RestrictionEntry.TYPE_MULTI_SELECT :
            ...
            case android.content.RestrictionEntry.TYPE_NULL :
                ...
                break;
            case android.content.RestrictionEntry.TYPE_BUNDLE :
                android.content.RestrictionEntry[] restrictions = entry.getRestrictions();
                android.os.Bundle childBundle = android.content.RestrictionsManager.convertRestrictionsToBundle(java.util.Arrays.asList(restrictions));
                bundle.putBundle(entry.getKey(), childBundle);
                break;
            case android.content.RestrictionEntry.TYPE_BUNDLE_ARRAY :
                android.content.RestrictionEntry[] bundleRestrictionArray = entry.getRestrictions();
                android.os.Bundle[] bundleArray = new android.os.Bundle[bundleRestrictionArray.length];
                for (int i = 0; i < bundleRestrictionArray.length; i++) {
                    android.content.RestrictionEntry[] bundleRestrictions = bundleRestrictionArray[i].getRestrictions();
                    if (bundleRestrictions == null) {
                        // Non-bundle entry found in bundle array.
                        android.util.Log.w(android.content.RestrictionsManager.TAG, "addRestrictionToBundle: " + "Non-bundle entry found in bundle array");
                        bundleArray[i] = new android.os.Bundle();
                    } else {
                        bundleArray[i] = android.content.RestrictionsManager.convertRestrictionsToBundle(java.util.Arrays.asList(bundleRestrictions));
                    }
                }
                bundle.putParcelableArray(entry.getKey(), bundleArray);
                break;
            default :
                throw new java.lang.IllegalArgumentException("Unsupported restrictionEntry type: " + entry.getType());
        }
        return bundle;
    }
}
