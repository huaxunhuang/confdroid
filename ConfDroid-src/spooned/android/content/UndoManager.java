...
public class UndoManager {
    ...
    private final android.util.ArrayMap<java.lang.String, android.content.UndoOwner> mOwners = /* capacity */
    new android.util.ArrayMap<java.lang.String, android.content.UndoOwner>(1);

    private final java.util.ArrayList<android.content.UndoManager.UndoState> mUndos = new java.util.ArrayList<android.content.UndoManager.UndoState>();

    private final java.util.ArrayList<android.content.UndoManager.UndoState> mRedos = new java.util.ArrayList<android.content.UndoManager.UndoState>();
    ...
    public android.content.UndoOwner getOwner(java.lang.String tag, java.lang.Object data) {
        if (tag == null) {
        ...
        android.content.UndoOwner owner = mOwners.get(tag);
        if (owner != null) {
            if (owner.mData != data) {
                if (owner.mData != null) {
                    throw new java.lang.IllegalStateException((((("Owner " + owner) + " already exists with data ") + owner.mData) + " but giving different data ") + data);
                }
                owner.mData = data;
            }
            return owner;
        }
        owner = new android.content.UndoOwner(tag, this);
        owner.mData = data;
        mOwners.put(tag, owner);
        return owner;
    }

    void removeOwner(android.content.UndoOwner owner) {
     ...
     */
    @android.annotation.UnsupportedAppUsage
    public void saveInstanceState(android.os.Parcel p) {
        if (mUpdateCount > 0) {
    ...
    void saveOwner(android.content.UndoOwner owner, android.os.Parcel out) {
     ...
     */
    @android.annotation.UnsupportedAppUsage
    public void restoreInstanceState(android.os.Parcel p, java.lang.ClassLoader loader) {
        if (mUpdateCount > 0) {
            throw new java.lang.IllegalStateException("Can't save state while updating");
        }
        forgetUndos(null, -1);
        ...
        mStateOwners = new android.content.UndoOwner[p.readInt()];
        int stype;
        while ((stype = p.readInt()) != 0) {
            android.content.UndoManager.UndoState ustate = new android.content.UndoManager.UndoState(this, p, loader);
            if (stype == 1) {
                mUndos.add(0, ustate);
            } else {
                mRedos.add(0, ustate);
    ...
    android.content.UndoOwner restoreOwner(android.os.Parcel in) {
        int idx = in.readInt();
        android.content.UndoOwner owner = mStateOwners[idx];
        if (owner == null) {
     ...
     * @param count
     * 		Number of undo states to pop.
     * @return Returns the number of undo states that were actually popped.
     */
    @android.annotation.UnsupportedAppUsage
    public int undo(android.content.UndoOwner[] owners, int count) {
        if (mWorking != null) {
        ...
        int i = -1;
        mInUndo = true;
        android.content.UndoManager.UndoState us = getTopUndo(null);
        if (us != null) {
            us.makeExecuted();
        }
        while ((count > 0) && ((i = findPrevState(mUndos, owners, i)) >= 0)) {
            android.content.UndoManager.UndoState state = mUndos.remove(i);
            state.undo();
            mRedos.add(state);
            count--;
            num++;
     ...
     */
    @android.annotation.UnsupportedAppUsage
    public int redo(android.content.UndoOwner[] owners, int count) {
        if (mWorking != null) {
            throw new java.lang.IllegalStateException("Can't be called during an update");
        }
        int num = 0;
        ...
        while ((count > 0) && ((i = findPrevState(mRedos, owners, i)) >= 0)) {
            android.content.UndoManager.UndoState state = mRedos.remove(i);
            state.redo();
            mUndos.add(state);
            count--;
            num++;
    ...
    @android.annotation.UnsupportedAppUsage
    public int forgetUndos(android.content.UndoOwner[] owners, int count) {
        if (count < 0) {
            count = mUndos.size();
        }
        int removed = 0;
        ...
        while ((i < mUndos.size()) && (removed < count)) {
            android.content.UndoManager.UndoState state = mUndos.get(i);
    ...
    @android.annotation.UnsupportedAppUsage
    public int forgetRedos(android.content.UndoOwner[] owners, int count) {
        if (count < 0) {
            count = mRedos.size();
        }
        int removed = 0;
        ...
        while ((i < mRedos.size()) && (removed < count)) {
            android.content.UndoManager.UndoState state = mRedos.get(i);
            if ((count > 0) && matchOwners(state, owners)) {
                state.destroy();
                mRedos.remove(i);
                removed++;
    ...
    public java.lang.CharSequence getUndoLabel(android.content.UndoOwner[] owners) {
        android.content.UndoManager.UndoState state = getTopUndo(owners);
    ...
    public java.lang.CharSequence getRedoLabel(android.content.UndoOwner[] owners) {
        android.content.UndoManager.UndoState state = getTopRedo(owners);
        return state != null ? state.getLabel() : null;
    }
    ...
    private void createWorkingState() {
        mWorking = new android.content.UndoManager.UndoState(this, mCommitId++);
        if (mCommitId < 0) {
            mCommitId = 1;
        }
    }
     ...
     *
     * @param mergeMode
     * 		May be either {@link #MERGE_MODE_NONE} or {@link #MERGE_MODE_ANY}.
     */
     ...
     * @param mergeMode
     * 		May be either {@link #MERGE_MODE_NONE}, {@link #MERGE_MODE_UNIQUE},
     * 		or {@link #MERGE_MODE_ANY}.
     */
    public android.content.UndoOperation<?> getLastOperation(android.content.UndoOwner owner, int mergeMode) {
     ...
     */
    @android.annotation.UnsupportedAppUsage
    public <T extends android.content.UndoOperation> T getLastOperation(java.lang.Class<T> clazz, android.content.UndoOwner owner, int mergeMode) {
        if (mWorking == null) {
            throw new java.lang.IllegalStateException("Must be called during an update");
        }
        if (((mergeMode != android.content.UndoManager.MERGE_MODE_NONE) && (!mMerged)) && (!mWorking.hasData())) {
            android.content.UndoManager.UndoState state = getTopUndo(null);
            android.content.UndoOperation<?> last;
            if ((((state != null) && ((mergeMode == android.content.UndoManager.MERGE_MODE_ANY) || (!state.hasMultipleOwners()))) && state.canMerge()) && ((last = state.getLastOperation(clazz, owner)) != null)) {
                if (last.allowMerge()) {
                    mWorking.destroy();
                    mWorking = state;
                    mUndos.remove(state);
     ...
     */
    @android.annotation.UnsupportedAppUsage
    public void addOperation(android.content.UndoOperation<?> op, int mergeMode) {
        if (mWorking == null) {
            throw new java.lang.IllegalStateException("Must be called during an update");
        }
        android.content.UndoOwner owner = op.getOwner();
        if (owner.mManager != this) {
            throw new java.lang.IllegalArgumentException("Given operation's owner is not in this undo manager.");
        }
        if (((mergeMode != android.content.UndoManager.MERGE_MODE_NONE) && (!mMerged)) && (!mWorking.hasData())) {
            android.content.UndoManager.UndoState state = getTopUndo(null);
            if ((((state != null) && ((mergeMode == android.content.UndoManager.MERGE_MODE_ANY) || (!state.hasMultipleOwners()))) && state.canMerge()) && state.hasOperation(op.getOwner())) {
                mWorking.destroy();
                mWorking = state;
                mUndos.remove(state);
                mMerged = true;
     ...
     */
    public boolean uncommitState(int commitId, android.content.UndoOwner owner) {
    ...
    android.content.UndoManager.UndoState getTopUndo(android.content.UndoOwner[] owners) {
        if (mUndos.size() <= 0) {
            return null;
        }
    ...
    android.content.UndoManager.UndoState getTopRedo(android.content.UndoOwner[] owners) {
        if (mRedos.size() <= 0) {
            return null;
        }
    ...
    boolean matchOwners(android.content.UndoManager.UndoState state, android.content.UndoOwner[] owners) {
        if (owners == null) {
            return true;
        }
    ...
    int findPrevState(java.util.ArrayList<android.content.UndoManager.UndoState> states, android.content.UndoOwner[] owners, int from) {
        ...
        while (from >= 0) {
            android.content.UndoManager.UndoState state = states.get(from);
    ...
    int findNextState(java.util.ArrayList<android.content.UndoManager.UndoState> states, android.content.UndoOwner[] owners, int from) {
        final int N = states.size();
        if (from < 0) {
            from = 0;
        }
    ...
    static final class UndoState {
        ...
        private final java.util.ArrayList<android.content.UndoOperation<?>> mOperations = new java.util.ArrayList<android.content.UndoOperation<?>>();
        ...
        UndoState(android.content.UndoManager manager, int commitId) {
            mManager = manager;
            mCommitId = commitId;
        }

        UndoState(android.content.UndoManager manager, android.os.Parcel p, java.lang.ClassLoader loader) {
            mManager = manager;
            mCommitId = p.readInt();
            ...
            for (int i = 0; i < N; i++) {
                android.content.UndoOwner owner = mManager.restoreOwner(p);
                android.content.UndoOperation op = ((android.content.UndoOperation) (p.readParcelable(loader)));
                op.mOwner = owner;
                mOperations.add(op);
            }
        }

        void writeToParcel(android.os.Parcel p) {
            if (mRecent != null) {
            ...
            android.text.TextUtils.writeToParcel(mLabel, p, 0);
            final int N = mOperations.size();
            p.writeInt(N);
            for (int i = 0; i < N; i++) {
                android.content.UndoOperation op = mOperations.get(i);
        ...
        void setLabel(java.lang.CharSequence label) {
            mLabel = label;
        }

        void updateLabel(java.lang.CharSequence label) {
            if (mLabel != null) {
                mLabel = label;
            }
        }
        ...
        boolean hasOperation(android.content.UndoOwner owner) {
            final int N = mOperations.size();
            if (owner == null) {
                return N != 0;
            }
        ...
        boolean hasMultipleOwners() {
            ...
            android.content.UndoOwner owner = mOperations.get(0).getOwner();
            for (int i = 1; i < N; i++) {
                if (mOperations.get(i).getOwner() != owner) {
                    return true;
                }
        ...
        void addOperation(android.content.UndoOperation<?> op) {
            if (mOperations.contains(op)) {
                throw new java.lang.IllegalStateException("Already holds " + op);
            }
            mOperations.add(op);
            if (mRecent == null) {
                mRecent = new java.util.ArrayList<android.content.UndoOperation<?>>();
                mRecent.add(op);
            }
            op.mOwner.mOpCount++;
        }

        <T extends android.content.UndoOperation> T getLastOperation(java.lang.Class<T> clazz, android.content.UndoOwner owner) {
            final int N = mOperations.size();
        ...
        boolean matchOwner(android.content.UndoOwner owner) {
        ...
        void destroy() {
            for (int i = mOperations.size() - 1; i >= 0; i--) {
                android.content.UndoOwner owner = mOperations.get(i).mOwner;
                owner.mOpCount--;
                if (owner.mOpCount <= 0) {
                    if (owner.mOpCount < 0) {
                        throw new java.lang.IllegalStateException((("Underflow of op count on owner " + owner) + " in op ") + mOperations.get(i));
                    }
