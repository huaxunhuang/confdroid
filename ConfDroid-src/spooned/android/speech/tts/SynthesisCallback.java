...
public interface SynthesisCallback {
    ...
    @android.annotation.IntDef({ android.media.AudioFormat.ENCODING_PCM_8BIT, android.media.AudioFormat.ENCODING_PCM_16BIT, android.media.AudioFormat.ENCODING_PCM_FLOAT })
     ...
     * 		above.
     * @param channelCount
     * 		The number of channels. Must be {@code 1} or {@code 2}.
     * @return {@link TextToSpeech#SUCCESS}, {@link TextToSpeech#ERROR} or
    {@link TextToSpeech#STOPPED}.
     */
    public int start(int sampleRateInHz, @android.speech.tts.SynthesisCallback.SupportedAudioFormat
    int audioFormat, @android.annotation.IntRange(from = 1, to = 2)
    int channelCount);
     ...
     *
     * @param buffer
     * 		The generated audio data. This method will not hold on to {@code buffer},
     * 		so the caller is free to modify it after this method returns.
     * @param offset
     * 		The offset into {@code buffer} where the audio data starts.
     * @param length
     * 		The number of bytes of audio data in {@code buffer}. This must be
     ...
     * 		{@link TextToSpeech}
     */
    public void error(@android.speech.tts.TextToSpeech.Error
    int errorCode);
