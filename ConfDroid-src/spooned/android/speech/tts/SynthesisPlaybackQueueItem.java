 ...
 */
final class SynthesisPlaybackQueueItem extends android.speech.tts.PlaybackQueueItem {
     ...
     */
    private final java.util.concurrent.locks.Lock mListLock = new java.util.concurrent.locks.ReentrantLock();

    private final java.util.concurrent.locks.Condition mReadReady = mListLock.newCondition();

    private final java.util.concurrent.locks.Condition mNotFull = mListLock.newCondition();

    // Guarded by mListLock.
    private final java.util.LinkedList<android.speech.tts.SynthesisPlaybackQueueItem.ListEntry> mDataBufferList = new java.util.LinkedList<android.speech.tts.SynthesisPlaybackQueueItem.ListEntry>();
    ...
    SynthesisPlaybackQueueItem(android.speech.tts.TextToSpeechService.AudioOutputParams audioParams, int sampleRate, int audioFormat, int channelCount, android.speech.tts.TextToSpeechService.UtteranceProgressDispatcher dispatcher, java.lang.Object callerIdentity, android.speech.tts.AbstractEventLogger logger) {
        super(dispatcher, callerIdentity);
        mUnconsumedBytes = 0;
        mStopped = false;
        mDone = false;
        mStatusCode = android.speech.tts.TextToSpeech.SUCCESS;
        mAudioTrack = new android.speech.tts.BlockingAudioTrack(audioParams, sampleRate, audioFormat, channelCount);
        mLogger = logger;
    }
    ...
    public void run() {
        ...
        try {
            ...
            if (android.speech.tts.SynthesisPlaybackQueueItem.DBG)
                android.util.Log.d(android.speech.tts.SynthesisPlaybackQueueItem.TAG, "Interrupted waiting for buffers, cleaning up.");
    ...
    void put(byte[] buffer) throws java.lang.InterruptedException {
        try {
            mListLock.lock();
            long unconsumedAudioMs = 0;
            while (((unconsumedAudioMs = mAudioTrack.getAudioLengthMs(mUnconsumedBytes)) > android.speech.tts.SynthesisPlaybackQueueItem.MAX_UNCONSUMED_AUDIO_MS) && (!mStopped)) {
                mNotFull.await();
            } 
            ...
            mDataBufferList.add(new android.speech.tts.SynthesisPlaybackQueueItem.ListEntry(buffer));
    ...
    private byte[] take() throws java.lang.InterruptedException {
        try {
            ...
            android.speech.tts.SynthesisPlaybackQueueItem.ListEntry entry = mDataBufferList.poll();
