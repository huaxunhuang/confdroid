...
public class RecognizerIntent {
     ...
     */
    public static final int RESULT_NO_MATCH = android.app.Activity.RESULT_FIRST_USER;

    /**
     * Result code returned when there is a generic client error
     */
    public static final int RESULT_CLIENT_ERROR = android.app.Activity.RESULT_FIRST_USER + 1;

    /**
     * Result code returned when the recognition server returns an error
     */
    public static final int RESULT_SERVER_ERROR = android.app.Activity.RESULT_FIRST_USER + 2;

    /**
     * Result code returned when a network error was encountered
     */
    public static final int RESULT_NETWORK_ERROR = android.app.Activity.RESULT_FIRST_USER + 3;

    /**
     * Result code returned when an audio error was encountered
     */
    public static final int RESULT_AUDIO_ERROR = android.app.Activity.RESULT_FIRST_USER + 4;
    ...
    public static final android.content.Intent getVoiceDetailsIntent(android.content.Context context) {
        android.content.Intent voiceSearchIntent = new android.content.Intent(android.speech.RecognizerIntent.ACTION_WEB_SEARCH);
        android.content.pm.ResolveInfo ri = context.getPackageManager().resolveActivity(voiceSearchIntent, android.content.pm.PackageManager.GET_META_DATA);
        if (((ri == null) || (ri.activityInfo == null)) || (ri.activityInfo.metaData == null))
            return null;

        java.lang.String className = ri.activityInfo.metaData.getString(android.speech.RecognizerIntent.DETAILS_META_DATA);
        if (className == null)
            return null;

        android.content.Intent detailsIntent = new android.content.Intent(android.speech.RecognizerIntent.ACTION_GET_LANGUAGE_DETAILS);
        detailsIntent.setComponent(new android.content.ComponentName(ri.activityInfo.packageName, className));
