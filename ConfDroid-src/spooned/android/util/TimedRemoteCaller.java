...
public abstract class TimedRemoteCaller<T> {
    ...
    private int mReceivedSequence = android.util.TimedRemoteCaller.UNDEFINED_SEQUENCE;

    private int mAwaitedSequence = android.util.TimedRemoteCaller.UNDEFINED_SEQUENCE;
    ...
    public final T getResultTimed(int sequence) throws java.util.concurrent.TimeoutException {
        synchronized(mLock) {
            final boolean success = waitForResultTimedLocked(sequence);
            if (!success) {
                throw new java.util.concurrent.TimeoutException("No reponse for sequence: " + sequence);
            }
            T result = mResult;
            mResult = null;
            return result;
    ...
    private boolean waitForResultTimedLocked(int sequence) {
        final long startMillis = android.os.SystemClock.uptimeMillis();
        while (true) {
            try {
                if (mReceivedSequence == sequence) {
                    return true;
                }
                final long elapsedMillis = android.os.SystemClock.uptimeMillis() - startMillis;
