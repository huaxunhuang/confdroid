...
public final class DisplayManager {
    ...
    private final android.util.SparseArray<android.view.Display> mDisplays = new android.util.SparseArray<android.view.Display>();

    private final java.util.ArrayList<android.view.Display> mTempDisplays = new java.util.ArrayList<android.view.Display>();
    ...
    public DisplayManager(android.content.Context context) {
        mContext = context;
        mGlobal = android.hardware.display.DisplayManagerGlobal.getInstance();
    }
    ...
    public android.view.Display getDisplay(int displayId) {
        synchronized(mLock) {
            return /* assumeValid */
            getOrCreateDisplayLocked(displayId, false);
        }
    }
        ...
        synchronized(mLock) {
            try {
                if (category == null) {
                    ...
                    if (category.equals(android.hardware.display.DisplayManager.DISPLAY_CATEGORY_PRESENTATION)) {
                        addPresentationDisplaysLocked(mTempDisplays, displayIds, android.view.Display.TYPE_WIFI);
                        addPresentationDisplaysLocked(mTempDisplays, displayIds, android.view.Display.TYPE_HDMI);
                        addPresentationDisplaysLocked(mTempDisplays, displayIds, android.view.Display.TYPE_OVERLAY);
                        addPresentationDisplaysLocked(mTempDisplays, displayIds, android.view.Display.TYPE_VIRTUAL);
                    }

                return mTempDisplays.toArray(new android.view.Display[mTempDisplays.size()]);
            } finally {
                mTempDisplays.clear();
            }
        }
    ...
    private void addAllDisplaysLocked(java.util.ArrayList<android.view.Display> displays, int[] displayIds) {
        for (int i = 0; i < displayIds.length; i++) {
            android.view.Display display = /* assumeValid */
            getOrCreateDisplayLocked(displayIds[i], true);
            if (display != null) {
    ...
    private void addPresentationDisplaysLocked(java.util.ArrayList<android.view.Display> displays, int[] displayIds, int matchType) {
        for (int i = 0; i < displayIds.length; i++) {
            android.view.Display display = /* assumeValid */
            getOrCreateDisplayLocked(displayIds[i], true);
            if (((display != null) && ((display.getFlags() & android.view.Display.FLAG_PRESENTATION) != 0)) && (display.getType() == matchType)) {
                displays.add(display);
            }
        }
    }

    private android.view.Display getOrCreateDisplayLocked(int displayId, boolean assumeValid) {
        android.view.Display display = mDisplays.get(displayId);
        if (display == null) {
     ...
     *
     * @param deviceAddress
     * 		The MAC address of the device to rename.
     * @param alias
     ...
     *
     * @param deviceAddress
     * 		The MAC address of the device to forget.
     * @unknown 
     */
    ...
    java.lang.String name, int width, int height, int densityDpi, @android.annotation.Nullable
     ...
     * @param callback
     * 		Callback to call when the state of the {@link VirtualDisplay} changes
     * @param handler
     * 		The handler on which the listener should be invoked, or null
     * 		if the listener should be invoked on the calling thread's looper.
    ...
    public android.hardware.display.VirtualDisplay createVirtualDisplay(@android.annotation.NonNull
    java.lang.String name, int width, int height, int densityDpi, @android.annotation.Nullable
    android.view.Surface surface, int flags, @android.annotation.Nullable
     ...
     * @unknown 
     */
    public android.hardware.display.VirtualDisplay createVirtualDisplay(@android.annotation.Nullable
    android.media.projection.MediaProjection projection, @android.annotation.NonNull
    java.lang.String name, int width, int height, int densityDpi, @android.annotation.Nullable
    android.view.Surface surface, int flags, @android.annotation.Nullable
    android.hardware.display.VirtualDisplay.Callback callback, @android.annotation.Nullable
    android.os.Handler handler) {
        return mGlobal.createVirtualDisplay(mContext, projection, name, width, height, densityDpi, surface, flags, callback, handler);
    }
