...
public class SerialManager {
     ...
     * {@hide }
     */
    public SerialManager(android.content.Context context, android.hardware.ISerialManager service) {
        mContext = context;
        mService = service;
    }
     ...
     * 		of the serial port
     * @param speed
     * 		at which to open the serial port
     * @return the serial port
     */
    public android.hardware.SerialPort openSerialPort(java.lang.String name, int speed) throws java.io.IOException {
        try {
            android.os.ParcelFileDescriptor pfd = mService.openSerialPort(name);
            if (pfd != null) {
                android.hardware.SerialPort port = new android.hardware.SerialPort(name);
                port.open(pfd, speed);
                return port;
            } else {
                throw new java.io.IOException("Could not open serial port " + name);
            }
        } catch (android.os.RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
