...
public class UsbRequest {
     ...
     */
    public boolean initialize(android.hardware.usb.UsbDeviceConnection connection, android.hardware.usb.UsbEndpoint endpoint) {
        mEndpoint = endpoint;
    ...
    protected void finalize() throws java.lang.Throwable {
        try {
            if (mEndpoint != null) {
                android.util.Log.v(android.hardware.usb.UsbRequest.TAG, "endpoint still open in finalize(): " + this);
                close();
            }
        } finally {
            super.finalize();
     ...
     */
    public boolean queue(java.nio.ByteBuffer buffer, int length) {
        boolean out = mEndpoint.getDirection() == android.hardware.usb.UsbConstants.USB_DIR_OUT;
        boolean result;
        if (buffer.isDirect()) {
    ...
    void dequeue() {
        ...
        if (bytesRead >= 0) {
            mBuffer.position(java.lang.Math.min(bytesRead, mLength));
        }
        mBuffer = null;
        mLength = 0;
    }
    ...
    private native boolean native_init(android.hardware.usb.UsbDeviceConnection connection, int ep_address, int ep_attributes, int ep_max_packet_size, int ep_interval);
