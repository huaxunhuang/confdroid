...
public class Camera {
    ...
    private static final int CAMERA_MSG_ERROR = 0x1;

    private static final int CAMERA_MSG_SHUTTER = 0x2;

    private static final int CAMERA_MSG_FOCUS = 0x4;

    private static final int CAMERA_MSG_ZOOM = 0x8;

    private static final int CAMERA_MSG_PREVIEW_FRAME = 0x10;

    private static final int CAMERA_MSG_VIDEO_FRAME = 0x20;

    private static final int CAMERA_MSG_POSTVIEW_FRAME = 0x40;

    private static final int CAMERA_MSG_RAW_IMAGE = 0x80;
    ...
    public static void getCameraInfo(int cameraId, android.hardware.Camera.CameraInfo cameraInfo) {
        android.hardware.Camera._getCameraInfo(cameraId, cameraInfo);
        android.os.IBinder b = android.os.ServiceManager.getService(android.content.Context.AUDIO_SERVICE);
        android.media.IAudioService audioService = IAudioService.Stub.asInterface(b);
        try {
            ...
            android.util.Log.e(android.hardware.Camera.TAG, "Audio service is unavailable for queries");
        }
    }
     ...
     * 		if the {@code halVersion} is invalid
     * @throws RuntimeException
     * 		if opening the camera fails (for example, if the
     ...
     * @unknown 
     */
    public static android.hardware.Camera openLegacy(int cameraId, int halVersion) {
        if (halVersion < android.hardware.Camera.CAMERA_HAL_API_VERSION_1_0) {
            throw new java.lang.IllegalArgumentException("Invalid HAL version " + halVersion);
        }
        return new android.hardware.Camera(cameraId, halVersion);
    }
    ...
    private Camera(int cameraId, int halVersion) {
        ...
        if (android.hardware.Camera.checkInitErrors(err)) {
            if (err == (-android.system.OsConstants.EACCES)) {
                throw new java.lang.RuntimeException("Fail to connect to camera service");
            } else
                if (err == (-android.system.OsConstants.ENODEV)) {
                    throw new java.lang.RuntimeException("Camera initialization failed");
                } else
                    if (err == (-android.system.OsConstants.ENOSYS)) {
                        throw new java.lang.RuntimeException("Camera initialization failed because some methods" + " are not implemented");
                    } else
                        if (err == (-android.system.OsConstants.EOPNOTSUPP)) {
                            throw new java.lang.RuntimeException("Camera initialization failed because the hal" + " version is not supported by this device");
                        } else
                            if (err == (-android.system.OsConstants.EINVAL)) {
                                throw new java.lang.RuntimeException("Camera initialization failed because the input" + " arugments are invalid");
                            } else
                                if (err == (-android.system.OsConstants.EBUSY)) {
                                    throw new java.lang.RuntimeException("Camera initialization failed because the camera" + " device was already opened");
                                } else
                                    if (err == (-android.system.OsConstants.EUSERS)) {
                                        throw new java.lang.RuntimeException("Camera initialization failed because the max" + " number of camera devices were already opened");
                                    }
    ...
    private int cameraInitVersion(int cameraId, int halVersion) {
        ...
        if ((looper = android.os.Looper.myLooper()) != null) {
            mEventHandler = new android.hardware.Camera.EventHandler(this, looper);
        } else
        ...
        return native_setup(new java.lang.ref.WeakReference<android.hardware.Camera>(this), cameraId, halVersion, android.app.ActivityThread.currentOpPackageName());
    }

    private int cameraInitNormal(int cameraId) {
        return cameraInitVersion(cameraId, android.hardware.Camera.CAMERA_HAL_API_VERSION_NORMAL_CONNECT);
    }
    ...
    public int cameraInitUnspecified(int cameraId) {
        return cameraInitVersion(cameraId, android.hardware.Camera.CAMERA_HAL_API_VERSION_UNSPECIFIED);
    }
    ...
    public static boolean checkInitErrors(int err) {
        return err != android.hardware.Camera.NO_ERROR;
    }
    ...
    public final void setPreviewDisplay(android.view.SurfaceHolder holder) throws java.io.IOException {
        if (holder != null) {
            ...
            setPreviewSurface(((android.view.Surface) (null)));
        }
    }
     ...
     * 		the {@link SurfaceTexture} to which the preview
    ...
    public final void addCallbackBuffer(byte[] callbackBuffer) {
        _addCallbackBuffer(callbackBuffer, android.hardware.Camera.CAMERA_MSG_PREVIEW_FRAME);
    }
    ...
    public final void addRawImageCallbackBuffer(byte[] callbackBuffer) {
        addCallbackBuffer(callbackBuffer, android.hardware.Camera.CAMERA_MSG_RAW_IMAGE);
    }

    private final void addCallbackBuffer(byte[] callbackBuffer, int msgType) {
        // CAMERA_MSG_VIDEO_FRAME may be allowed in the future.
        if ((msgType != android.hardware.Camera.CAMERA_MSG_PREVIEW_FRAME) && (msgType != android.hardware.Camera.CAMERA_MSG_RAW_IMAGE)) {
            throw new java.lang.IllegalArgumentException("Unsupported message type: " + msgType);
        }
     ...
     *
     * @param rs
     * 		the RenderScript context for this Allocation.
     * @param usage
     ...
     * @unknown 
     */
    public final android.renderscript.Allocation createPreviewAllocation(android.renderscript.RenderScript rs, int usage) throws android.renderscript.RSIllegalArgumentException {
        android.hardware.Camera.Parameters p = getParameters();
        android.hardware.Camera.Size previewSize = p.getPreviewSize();
        android.renderscript.Type.Builder yuvBuilder = new android.renderscript.Type.Builder(rs, android.renderscript.Element.createPixel(rs, android.renderscript.Element.DataType.UNSIGNED_8, android.renderscript.Element.DataKind.PIXEL_YUV));
        // Use YV12 for wide compatibility. Changing this requires also
        // adjusting camera service's format selection.
        yuvBuilder.setYuvFormat(android.graphics.ImageFormat.YV12);
        yuvBuilder.setX(previewSize.width);
        yuvBuilder.setY(previewSize.height);
        android.renderscript.Allocation a = android.renderscript.Allocation.createTyped(rs, yuvBuilder.create(), usage | android.renderscript.Allocation.USAGE_IO_INPUT);
        return a;
    }
     ...
     * 		the allocation to use as destination for preview
     * @throws IOException
     ...
     * @unknown 
     */
    public final void setPreviewCallbackAllocation(android.renderscript.Allocation previewAllocation) throws java.io.IOException {
        android.view.Surface previewSurface = null;
        if (previewAllocation != null) {
            ...
            if ((previewSize.width != previewAllocation.getType().getX()) || (previewSize.height != previewAllocation.getType().getY())) {
                throw new java.lang.IllegalArgumentException(((((((("Allocation dimensions don't match preview dimensions: " + "Allocation is ") + previewAllocation.getType().getX()) + ", ") + previewAllocation.getType().getY()) + ". Preview is ") + previewSize.width) + ", ") + previewSize.height);
            }
            if ((previewAllocation.getUsage() & android.renderscript.Allocation.USAGE_IO_INPUT) == 0) {
                throw new java.lang.IllegalArgumentException("Allocation usage does not include USAGE_IO_INPUT");
            }
            if (previewAllocation.getType().getElement().getDataKind() != android.renderscript.Element.DataKind.PIXEL_YUV) {
                throw new java.lang.IllegalArgumentException("Allocation is not of a YUV type");
            }
    ...
    private class EventHandler extends android.os.Handler {
        ...
        public EventHandler(android.hardware.Camera c, android.os.Looper looper) {
            super(looper);
            mCamera = c;
        }
        ...
        public void handleMessage(android.os.Message msg) {
            switch (msg.what) {
                case android.hardware.Camera.CAMERA_MSG_SHUTTER :
                    if (mShutterCallback != null) {
                        mShutterCallback.onShutter();
                    }
                    return;
                case android.hardware.Camera.CAMERA_MSG_RAW_IMAGE :
                    if (mRawImageCallback != null) {
                        mRawImageCallback.onPictureTaken(((byte[]) (msg.obj)), mCamera);
                    }
                    return;
                case android.hardware.Camera.CAMERA_MSG_COMPRESSED_IMAGE :
                    if (mJpegCallback != null) {
                        mJpegCallback.onPictureTaken(((byte[]) (msg.obj)), mCamera);
                    }
                    return;
                case android.hardware.Camera.CAMERA_MSG_PREVIEW_FRAME :
                    android.hardware.Camera.PreviewCallback pCb = mPreviewCallback;
                    if (pCb != null) {
                        if (mOneShot) {
                    ...
                    return;
                case android.hardware.Camera.CAMERA_MSG_POSTVIEW_FRAME :
                    if (mPostviewCallback != null) {
                        mPostviewCallback.onPictureTaken(((byte[]) (msg.obj)), mCamera);
                    }
                    return;
                case android.hardware.Camera.CAMERA_MSG_FOCUS :
                    android.hardware.Camera.AutoFocusCallback cb = null;
                    synchronized(mAutoFocusCallbackLock) {
                    ...
                    return;
                case android.hardware.Camera.CAMERA_MSG_ZOOM :
                    if (mZoomListener != null) {
                        mZoomListener.onZoomChange(msg.arg1, msg.arg2 != 0, mCamera);
                    }
                    return;
                case android.hardware.Camera.CAMERA_MSG_PREVIEW_METADATA :
                    if (mFaceListener != null) {
                        mFaceListener.onFaceDetection(((android.hardware.Camera.Face[]) (msg.obj)), mCamera);
                    }
                    return;
                case android.hardware.Camera.CAMERA_MSG_ERROR :
                    android.util.Log.e(android.hardware.Camera.TAG, "Error " + msg.arg1);
                    if (mErrorCallback != null) {
                        mErrorCallback.onError(msg.arg1, mCamera);
                    }
                    return;
                case android.hardware.Camera.CAMERA_MSG_FOCUS_MOVE :
                    if (mAutoFocusMoveCallback != null) {
                        mAutoFocusMoveCallback.onAutoFocusMoving(msg.arg1 == 0 ? false : true, mCamera);
                    }
                    return;
                default :
                    android.util.Log.e(android.hardware.Camera.TAG, "Unknown message type " + msg.what);
                    return;
            }
        }
    }
    ...
    private static void postEventFromNative(java.lang.Object camera_ref, int what, int arg1, int arg2, java.lang.Object obj) {
        android.hardware.Camera c = ((android.hardware.Camera) (((java.lang.ref.WeakReference) (camera_ref)).get()));
        if (c == null)
            return;

        if (c.mEventHandler != null) {
            android.os.Message m = c.mEventHandler.obtainMessage(what, arg1, arg2, obj);
    ...
    public interface AutoFocusCallback {
         ...
         */
        void onAutoFocus(boolean success, android.hardware.Camera camera);
    }
    ...
    public final void cancelAutoFocus() {
        ...
        mEventHandler.removeMessages(android.hardware.Camera.CAMERA_MSG_FOCUS);
    }
     ...
     *
     * <p>This is only supported in continuous autofocus modes -- {@link Parameters#FOCUS_MODE_CONTINUOUS_VIDEO} and {@link Parameters#FOCUS_MODE_CONTINUOUS_PICTURE}. Applications can show
    ...
    public interface AutoFocusMoveCallback {
         ...
         * 		the Camera service object
         */
        void onAutoFocusMoving(boolean start, android.hardware.Camera camera);
    }
    ...
    public interface PictureCallback {
         ...
         * 		the Camera service object
         */
        void onPictureTaken(byte[] data, android.hardware.Camera camera);
    }
     ...
     */
    public final void takePicture(android.hardware.Camera.ShutterCallback shutter, android.hardware.Camera.PictureCallback raw, android.hardware.Camera.PictureCallback postview, android.hardware.Camera.PictureCallback jpeg) {
        ...
        int msgType = 0;
        if (mShutterCallback != null) {
            msgType |= android.hardware.Camera.CAMERA_MSG_SHUTTER;
        }
        if (mRawImageCallback != null) {
            msgType |= android.hardware.Camera.CAMERA_MSG_RAW_IMAGE;
        }
        if (mPostviewCallback != null) {
            msgType |= android.hardware.Camera.CAMERA_MSG_POSTVIEW_FRAME;
        }
        if (mJpegCallback != null) {
            msgType |= android.hardware.Camera.CAMERA_MSG_COMPRESSED_IMAGE;
        }
    ...
    /**
     * Zooms to the requested value smoothly. The driver will notify {@link OnZoomChangeListener} of the zoom value and whether zoom is stopped at
     ...
     *
     * @param value
     * 		zoom value. The valid range is 0 to {@link android.hardware.Camera.Parameters#getMaxZoom}.
     * @throws IllegalArgumentException
    ...
    /**
     * Stops the smooth zoom. Applications should wait for the {@link OnZoomChangeListener} to know when the zoom is actually stopped. This
     ...
     *
     * <p>This does not affect the order of byte array passed in {@link PreviewCallback#onPreviewFrame}, JPEG pictures, or recorded videos. This
    ...
    public interface OnZoomChangeListener {
         ...
         * 		the Camera service object
         */
        void onZoomChange(int zoomValue, boolean stopped, android.hardware.Camera camera);
    }
    ...
    public interface FaceDetectionListener {
         ...
         * @param camera
         * 		The {@link Camera} service object
         */
        void onFaceDetection(android.hardware.Camera.Face[] faces, android.hardware.Camera camera);
    }
     ...
     * stop the face detection. This method is supported if {@link Parameters#getMaxNumDetectedFaces()} returns a number larger than 0.
    ...
    public final void startFaceDetection() {
        ...
        _startFaceDetection(android.hardware.Camera.CAMERA_FACE_DETECTION_HW);
    ...
    public interface ErrorCallback {
         ...
         * 		the Camera service object
         */
        void onError(int error, android.hardware.Camera camera);
    }
    ...
    public void setParameters(android.hardware.Camera.Parameters params) {
        ...
        if (mUsingPreviewAllocation) {
            android.hardware.Camera.Size newPreviewSize = params.getPreviewSize();
            android.hardware.Camera.Size currentPreviewSize = getParameters().getPreviewSize();
    ...
    public android.hardware.Camera.Parameters getParameters() {
        ...
        java.lang.String s = native_getParameters();
        p.unflatten(s);
        return p;
    }
    ...
    public static android.hardware.Camera.Parameters getParametersCopy(android.hardware.Camera.Parameters parameters) {
        ...
        android.hardware.Camera camera = parameters.getOuter();
        android.hardware.Camera.Parameters p = camera.new Parameters();
    ...
    public class Size {
        ...
        public boolean equals(java.lang.Object obj) {
            ...
            android.hardware.Camera.Size s = ((android.hardware.Camera.Size) (obj));
    ...
    public static class Area {
        ...
        public boolean equals(java.lang.Object obj) {
            ...
            android.hardware.Camera.Area a = ((android.hardware.Camera.Area) (obj));
            if (rect == null) {
                if (a.rect != null)
                    return false;
    ...
    public class Parameters {
         ...
         * continuously. Applications should not call {@link #autoFocus(AutoFocusCallback)} in this mode.
         */
         ...
         * set.
         *
         * <p>Since API level 14, applications can call {@link #autoFocus(AutoFocusCallback)} in this mode. The focus callback will
        ...
        private Parameters() {
            ...
            new java.util.LinkedHashMap<java.lang.String, java.lang.String>(64);
        }
        ...
        @java.lang.Deprecated
        public void dump() {
            android.util.Log.e(android.hardware.Camera.TAG, "dump: size=" + mMap.size());
            for (java.lang.String k : mMap.keySet()) {
                android.util.Log.e(android.hardware.Camera.TAG, (("dump: " + k) + "=") + mMap.get(k));
            }
        }
        ...
        public java.lang.String flatten() {
            java.lang.StringBuilder flattened = new java.lang.StringBuilder(128);
            for (java.lang.String k : mMap.keySet()) {
                flattened.append(k);
                flattened.append("=");
        ...
        public void unflatten(java.lang.String flattened) {
            mMap.clear();
            android.text.TextUtils.StringSplitter splitter = new android.text.TextUtils.SimpleStringSplitter(';');
            splitter.setString(flattened);
            for (java.lang.String kv : splitter) {
                int pos = kv.indexOf('=');
                if (pos == (-1)) {
                    continue;
                }
                java.lang.String k = kv.substring(0, pos);
                java.lang.String v = kv.substring(pos + 1);
                mMap.put(k, v);
            }
        }
        ...
        public void set(java.lang.String key, java.lang.String value) {
            if (((key.indexOf('=') != (-1)) || (key.indexOf(';') != (-1))) || (key.indexOf(0) != (-1))) {
                android.util.Log.e(android.hardware.Camera.TAG, ("Key \"" + key) + "\" contains invalid character (= or ; or \\0)");
                return;
            }
            if (((value.indexOf('=') != (-1)) || (value.indexOf(';') != (-1))) || (value.indexOf(0) != (-1))) {
                android.util.Log.e(android.hardware.Camera.TAG, ("Value \"" + value) + "\" contains invalid character (= or ; or \\0)");
                return;
            }
            put(key, value);
        }
        ...
        public void set(java.lang.String key, int value) {
            put(key, java.lang.Integer.toString(value));
        }

        private void put(java.lang.String key, java.lang.String value) {
        ...
        private void set(java.lang.String key, java.util.List<android.hardware.Camera.Area> areas) {
            if (areas == null) {
                set(key, "(0,0,0,0,0)");
            } else {
                java.lang.StringBuilder buffer = new java.lang.StringBuilder();
                for (int i = 0; i < areas.size(); i++) {
                    android.hardware.Camera.Area area = areas.get(i);
        ...
        public int getInt(java.lang.String key) {
            return java.lang.Integer.parseInt(mMap.get(key));
        }
        ...
        public void setPreviewSize(int width, int height) {
            java.lang.String v = (java.lang.Integer.toString(width) + "x") + java.lang.Integer.toString(height);
            set(android.hardware.Camera.Parameters.KEY_PREVIEW_SIZE, v);
        }
        ...
        public android.hardware.Camera.Size getPreviewSize() {
            java.lang.String pair = get(android.hardware.Camera.Parameters.KEY_PREVIEW_SIZE);
        ...
        public java.util.List<android.hardware.Camera.Size> getSupportedPreviewSizes() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_PREVIEW_SIZE + android.hardware.Camera.Parameters.SUPPORTED_VALUES_SUFFIX);
        ...
        public java.util.List<android.hardware.Camera.Size> getSupportedVideoSizes() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_VIDEO_SIZE + android.hardware.Camera.Parameters.SUPPORTED_VALUES_SUFFIX);
            return splitSize(str);
        }
        ...
        public android.hardware.Camera.Size getPreferredPreviewSizeForVideo() {
            java.lang.String pair = get(android.hardware.Camera.Parameters.KEY_PREFERRED_PREVIEW_SIZE_FOR_VIDEO);
            return strToSize(pair);
        }
         ...
         * thumbnail.</p>
         *
         * <p>Applications need to consider the display orientation. See {@link #setPreviewSize(int,int)} for reference.</p>
         *
         * @param width
        ...
        public void setJpegThumbnailSize(int width, int height) {
            set(android.hardware.Camera.Parameters.KEY_JPEG_THUMBNAIL_WIDTH, width);
            set(android.hardware.Camera.Parameters.KEY_JPEG_THUMBNAIL_HEIGHT, height);
        }
        ...
        public android.hardware.Camera.Size getJpegThumbnailSize() {
            return new android.hardware.Camera.Size(getInt(android.hardware.Camera.Parameters.KEY_JPEG_THUMBNAIL_WIDTH), getInt(android.hardware.Camera.Parameters.KEY_JPEG_THUMBNAIL_HEIGHT));
        }
        ...
        public java.util.List<android.hardware.Camera.Size> getSupportedJpegThumbnailSizes() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_JPEG_THUMBNAIL_SIZE + android.hardware.Camera.Parameters.SUPPORTED_VALUES_SUFFIX);
            return splitSize(str);
        }
        ...
        public void setJpegThumbnailQuality(int quality) {
            set(android.hardware.Camera.Parameters.KEY_JPEG_THUMBNAIL_QUALITY, quality);
        }
        ...
        public int getJpegThumbnailQuality() {
            return getInt(android.hardware.Camera.Parameters.KEY_JPEG_THUMBNAIL_QUALITY);
        }
        ...
        public void setJpegQuality(int quality) {
            set(android.hardware.Camera.Parameters.KEY_JPEG_QUALITY, quality);
        }
        ...
        public int getJpegQuality() {
            return getInt(android.hardware.Camera.Parameters.KEY_JPEG_QUALITY);
        }
        ...
        public void setPreviewFrameRate(int fps) {
            set(android.hardware.Camera.Parameters.KEY_PREVIEW_FRAME_RATE, fps);
        }
        ...
        public int getPreviewFrameRate() {
            return getInt(android.hardware.Camera.Parameters.KEY_PREVIEW_FRAME_RATE);
        }
        ...
        public java.util.List<java.lang.Integer> getSupportedPreviewFrameRates() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_PREVIEW_FRAME_RATE + android.hardware.Camera.Parameters.SUPPORTED_VALUES_SUFFIX);
            return splitInt(str);
        }
        ...
        public void setPreviewFpsRange(int min, int max) {
            set(android.hardware.Camera.Parameters.KEY_PREVIEW_FPS_RANGE, (("" + min) + ",") + max);
        }
        ...
        public void getPreviewFpsRange(int[] range) {
            ...
            splitInt(get(android.hardware.Camera.Parameters.KEY_PREVIEW_FPS_RANGE), range);
        }
        ...
        public java.util.List<int[]> getSupportedPreviewFpsRange() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_PREVIEW_FPS_RANGE + android.hardware.Camera.Parameters.SUPPORTED_VALUES_SUFFIX);
         ...
         * planes:
         *
         * <pre>{@code yStride   = (int) ceil(width / 16.0) * 16;
        ...
        public void setPreviewFormat(int pixel_format) {
            java.lang.String s = cameraFormatForPixelFormat(pixel_format);
            if (s == null) {
                throw new java.lang.IllegalArgumentException("Invalid pixel_format=" + pixel_format);
            }
            set(android.hardware.Camera.Parameters.KEY_PREVIEW_FORMAT, s);
        }
        ...
        public int getPreviewFormat() {
            return pixelFormatForCameraFormat(get(android.hardware.Camera.Parameters.KEY_PREVIEW_FORMAT));
        }
        ...
        public java.util.List<java.lang.Integer> getSupportedPreviewFormats() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_PREVIEW_FORMAT + android.hardware.Camera.Parameters.SUPPORTED_VALUES_SUFFIX);
            java.util.ArrayList<java.lang.Integer> formats = new java.util.ArrayList<java.lang.Integer>();
            for (java.lang.String s : split(str)) {
                int f = pixelFormatForCameraFormat(s);
                if (f == android.graphics.ImageFormat.UNKNOWN)
                    continue;
        ...
        public void setPictureSize(int width, int height) {
            java.lang.String v = (java.lang.Integer.toString(width) + "x") + java.lang.Integer.toString(height);
            set(android.hardware.Camera.Parameters.KEY_PICTURE_SIZE, v);
        }
        ...
        public android.hardware.Camera.Size getPictureSize() {
            java.lang.String pair = get(android.hardware.Camera.Parameters.KEY_PICTURE_SIZE);
            return strToSize(pair);
        }
        ...
        public java.util.List<android.hardware.Camera.Size> getSupportedPictureSizes() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_PICTURE_SIZE + android.hardware.Camera.Parameters.SUPPORTED_VALUES_SUFFIX);
            return splitSize(str);
        }
        ...
        public void setPictureFormat(int pixel_format) {
            ...
            set(android.hardware.Camera.Parameters.KEY_PICTURE_FORMAT, s);
        }
        ...
        public int getPictureFormat() {
            return pixelFormatForCameraFormat(get(android.hardware.Camera.Parameters.KEY_PICTURE_FORMAT));
        }
        ...
        public java.util.List<java.lang.Integer> getSupportedPictureFormats() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_PICTURE_FORMAT + android.hardware.Camera.Parameters.SUPPORTED_VALUES_SUFFIX);
            java.util.ArrayList<java.lang.Integer> formats = new java.util.ArrayList<java.lang.Integer>();
            for (java.lang.String s : split(str)) {
                int f = pixelFormatForCameraFormat(s);
                if (f == android.graphics.ImageFormat.UNKNOWN)
        ...
        private java.lang.String cameraFormatForPixelFormat(int pixel_format) {
            switch (pixel_format) {
                case android.graphics.ImageFormat.NV16 :
                    return android.hardware.Camera.Parameters.PIXEL_FORMAT_YUV422SP;
                case android.graphics.ImageFormat.NV21 :
                    return android.hardware.Camera.Parameters.PIXEL_FORMAT_YUV420SP;
                case android.graphics.ImageFormat.YUY2 :
                    return android.hardware.Camera.Parameters.PIXEL_FORMAT_YUV422I;
                case android.graphics.ImageFormat.YV12 :
                    return android.hardware.Camera.Parameters.PIXEL_FORMAT_YUV420P;
                case android.graphics.ImageFormat.RGB_565 :
                    return android.hardware.Camera.Parameters.PIXEL_FORMAT_RGB565;
                case android.graphics.ImageFormat.JPEG :
                    return android.hardware.Camera.Parameters.PIXEL_FORMAT_JPEG;
                default :
                    return null;
            }
        }

        private int pixelFormatForCameraFormat(java.lang.String format) {
            if (format == null)
                return android.graphics.ImageFormat.UNKNOWN;

            if (format.equals(android.hardware.Camera.Parameters.PIXEL_FORMAT_YUV422SP))
                return android.graphics.ImageFormat.NV16;

            if (format.equals(android.hardware.Camera.Parameters.PIXEL_FORMAT_YUV420SP))
                return android.graphics.ImageFormat.NV21;

            if (format.equals(android.hardware.Camera.Parameters.PIXEL_FORMAT_YUV422I))
                return android.graphics.ImageFormat.YUY2;

            if (format.equals(android.hardware.Camera.Parameters.PIXEL_FORMAT_YUV420P))
                return android.graphics.ImageFormat.YV12;

            if (format.equals(android.hardware.Camera.Parameters.PIXEL_FORMAT_RGB565))
                return android.graphics.ImageFormat.RGB_565;

            if (format.equals(android.hardware.Camera.Parameters.PIXEL_FORMAT_JPEG))
                return android.graphics.ImageFormat.JPEG;
        ...
        public void setRotation(int rotation) {
            if ((((rotation == 0) || (rotation == 90)) || (rotation == 180)) || (rotation == 270)) {
                set(android.hardware.Camera.Parameters.KEY_ROTATION, java.lang.Integer.toString(rotation));
            } else {
        ...
        public void setGpsLatitude(double latitude) {
            set(android.hardware.Camera.Parameters.KEY_GPS_LATITUDE, java.lang.Double.toString(latitude));
        }
        ...
        public void setGpsLongitude(double longitude) {
            set(android.hardware.Camera.Parameters.KEY_GPS_LONGITUDE, java.lang.Double.toString(longitude));
        }
        ...
        public void setGpsAltitude(double altitude) {
            set(android.hardware.Camera.Parameters.KEY_GPS_ALTITUDE, java.lang.Double.toString(altitude));
        }
        ...
        public void setGpsTimestamp(long timestamp) {
            set(android.hardware.Camera.Parameters.KEY_GPS_TIMESTAMP, java.lang.Long.toString(timestamp));
        }
        ...
        public void setGpsProcessingMethod(java.lang.String processing_method) {
            set(android.hardware.Camera.Parameters.KEY_GPS_PROCESSING_METHOD, processing_method);
        }
        ...
        public void removeGpsData() {
            remove(android.hardware.Camera.Parameters.KEY_GPS_LATITUDE);
            remove(android.hardware.Camera.Parameters.KEY_GPS_LONGITUDE);
            remove(android.hardware.Camera.Parameters.KEY_GPS_ALTITUDE);
            remove(android.hardware.Camera.Parameters.KEY_GPS_TIMESTAMP);
            remove(android.hardware.Camera.Parameters.KEY_GPS_PROCESSING_METHOD);
        }
        ...
        public java.lang.String getWhiteBalance() {
            return get(android.hardware.Camera.Parameters.KEY_WHITE_BALANCE);
        }
        ...
        public void setWhiteBalance(java.lang.String value) {
            java.lang.String oldValue = get(android.hardware.Camera.Parameters.KEY_WHITE_BALANCE);
            if (same(value, oldValue))
                return;

            set(android.hardware.Camera.Parameters.KEY_WHITE_BALANCE, value);
            set(android.hardware.Camera.Parameters.KEY_AUTO_WHITEBALANCE_LOCK, android.hardware.Camera.Parameters.FALSE);
        }
        ...
        public java.util.List<java.lang.String> getSupportedWhiteBalance() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_WHITE_BALANCE + android.hardware.Camera.Parameters.SUPPORTED_VALUES_SUFFIX);
            return split(str);
        }
        ...
        public java.lang.String getColorEffect() {
            return get(android.hardware.Camera.Parameters.KEY_EFFECT);
        }
        ...
        public void setColorEffect(java.lang.String value) {
            set(android.hardware.Camera.Parameters.KEY_EFFECT, value);
        }
        ...
        public java.util.List<java.lang.String> getSupportedColorEffects() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_EFFECT + android.hardware.Camera.Parameters.SUPPORTED_VALUES_SUFFIX);
            return split(str);
        }
        ...
        public java.lang.String getAntibanding() {
            return get(android.hardware.Camera.Parameters.KEY_ANTIBANDING);
        }
        ...
        public void setAntibanding(java.lang.String antibanding) {
            set(android.hardware.Camera.Parameters.KEY_ANTIBANDING, antibanding);
        }
        ...
        public java.util.List<java.lang.String> getSupportedAntibanding() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_ANTIBANDING + android.hardware.Camera.Parameters.SUPPORTED_VALUES_SUFFIX);
            return split(str);
        }
        ...
        public java.lang.String getSceneMode() {
            return get(android.hardware.Camera.Parameters.KEY_SCENE_MODE);
        }
        ...
        public void setSceneMode(java.lang.String value) {
            set(android.hardware.Camera.Parameters.KEY_SCENE_MODE, value);
        }
        ...
        public java.util.List<java.lang.String> getSupportedSceneModes() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_SCENE_MODE + android.hardware.Camera.Parameters.SUPPORTED_VALUES_SUFFIX);
            return split(str);
        }
        ...
        public java.lang.String getFlashMode() {
            return get(android.hardware.Camera.Parameters.KEY_FLASH_MODE);
        }
        ...
        public void setFlashMode(java.lang.String value) {
            set(android.hardware.Camera.Parameters.KEY_FLASH_MODE, value);
        }
        ...
        public java.util.List<java.lang.String> getSupportedFlashModes() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_FLASH_MODE + android.hardware.Camera.Parameters.SUPPORTED_VALUES_SUFFIX);
            return split(str);
        }
        ...
        value. Applications should call {@link #autoFocus(AutoFocusCallback)} to start the focus if focus
        ...
        public java.lang.String getFocusMode() {
            return get(android.hardware.Camera.Parameters.KEY_FOCUS_MODE);
        }
        ...
        public void setFocusMode(java.lang.String value) {
            set(android.hardware.Camera.Parameters.KEY_FOCUS_MODE, value);
        }
        ...
        public java.util.List<java.lang.String> getSupportedFocusModes() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_FOCUS_MODE + android.hardware.Camera.Parameters.SUPPORTED_VALUES_SUFFIX);
            return split(str);
        }
        ...
        public float getFocalLength() {
            return java.lang.Float.parseFloat(get(android.hardware.Camera.Parameters.KEY_FOCAL_LENGTH));
        }
        ...
        public float getHorizontalViewAngle() {
            return java.lang.Float.parseFloat(get(android.hardware.Camera.Parameters.KEY_HORIZONTAL_VIEW_ANGLE));
        }
        ...
        public float getVerticalViewAngle() {
            return java.lang.Float.parseFloat(get(android.hardware.Camera.Parameters.KEY_VERTICAL_VIEW_ANGLE));
        }

        /**
         * Gets the current exposure compensation index.
         *
         * @return current exposure compensation index. The range is {@link #getMinExposureCompensation} to {@link #getMaxExposureCompensation}. 0 means exposure is not
        adjusted.
         */
        public int getExposureCompensation() {
            return getInt(android.hardware.Camera.Parameters.KEY_EXPOSURE_COMPENSATION, 0);
        }
         ...
         * 		from {@link #getMinExposureCompensation} (inclusive) to {@link #getMaxExposureCompensation} (inclusive). 0 means exposure is
        ...
        public void setExposureCompensation(int value) {
            set(android.hardware.Camera.Parameters.KEY_EXPOSURE_COMPENSATION, value);
        }
        ...
        public int getMaxExposureCompensation() {
            return getInt(android.hardware.Camera.Parameters.KEY_MAX_EXPOSURE_COMPENSATION, 0);
        }
        ...
        public int getMinExposureCompensation() {
            return getInt(android.hardware.Camera.Parameters.KEY_MIN_EXPOSURE_COMPENSATION, 0);
        }
        ...
        public float getExposureCompensationStep() {
            return getFloat(android.hardware.Camera.Parameters.KEY_EXPOSURE_COMPENSATION_STEP, 0);
        }
        ...
        public void setAutoExposureLock(boolean toggle) {
            set(android.hardware.Camera.Parameters.KEY_AUTO_EXPOSURE_LOCK, toggle ? android.hardware.Camera.Parameters.TRUE : android.hardware.Camera.Parameters.FALSE);
        }
        ...
        public boolean getAutoExposureLock() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_AUTO_EXPOSURE_LOCK);
            return android.hardware.Camera.Parameters.TRUE.equals(str);
        }
        ...
        public boolean isAutoExposureLockSupported() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_AUTO_EXPOSURE_LOCK_SUPPORTED);
            return android.hardware.Camera.Parameters.TRUE.equals(str);
        }
        ...
        public void setAutoWhiteBalanceLock(boolean toggle) {
            set(android.hardware.Camera.Parameters.KEY_AUTO_WHITEBALANCE_LOCK, toggle ? android.hardware.Camera.Parameters.TRUE : android.hardware.Camera.Parameters.FALSE);
        }
        ...
        public boolean getAutoWhiteBalanceLock() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_AUTO_WHITEBALANCE_LOCK);
            return android.hardware.Camera.Parameters.TRUE.equals(str);
        }
        ...
        public boolean isAutoWhiteBalanceLockSupported() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_AUTO_WHITEBALANCE_LOCK_SUPPORTED);
            return android.hardware.Camera.Parameters.TRUE.equals(str);
        }
         ...
         * using this method.
         *
         * @return the current zoom value. The range is 0 to {@link #getMaxZoom}. 0 means the camera is not zoomed.
         */
        public int getZoom() {
            return getInt(android.hardware.Camera.Parameters.KEY_ZOOM, 0);
        }
        ...
        public void setZoom(int value) {
            set(android.hardware.Camera.Parameters.KEY_ZOOM, value);
        }
        ...
        public boolean isZoomSupported() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_ZOOM_SUPPORTED);
            return android.hardware.Camera.Parameters.TRUE.equals(str);
        }
        ...
        public int getMaxZoom() {
            return getInt(android.hardware.Camera.Parameters.KEY_MAX_ZOOM, 0);
        }
        ...
        returned as 320. The number of elements is {@link #getMaxZoom} + 1. The list is sorted from small to large. The
        first element is always 100. The last element is the zoom
        ratio of the maximum zoom value.
         */
        public java.util.List<java.lang.Integer> getZoomRatios() {
            return splitInt(get(android.hardware.Camera.Parameters.KEY_ZOOM_RATIOS));
        }
        ...
        public boolean isSmoothZoomSupported() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_SMOOTH_ZOOM_SUPPORTED);
            return android.hardware.Camera.Parameters.TRUE.equals(str);
        }
         ...
         *
         * <p>Focus distances may change after calling {@link #autoFocus(AutoFocusCallback)}, {@link #cancelAutoFocus}, or {@link #startPreview()}. Applications can call {@link #getParameters()}
        ...
        public void getFocusDistances(float[] output) {
            ...
            splitFloat(get(android.hardware.Camera.Parameters.KEY_FOCUS_DISTANCES), output);
        }
        ...
        public int getMaxNumFocusAreas() {
            return getInt(android.hardware.Camera.Parameters.KEY_MAX_NUM_FOCUS_AREAS, 0);
        }
        ...
        public java.util.List<android.hardware.Camera.Area> getFocusAreas() {
            return splitArea(get(android.hardware.Camera.Parameters.KEY_FOCUS_AREAS));
        }
        ...
        public void setFocusAreas(java.util.List<android.hardware.Camera.Area> focusAreas) {
            set(android.hardware.Camera.Parameters.KEY_FOCUS_AREAS, focusAreas);
        }
        ...
        public int getMaxNumMeteringAreas() {
            return getInt(android.hardware.Camera.Parameters.KEY_MAX_NUM_METERING_AREAS, 0);
        }
        ...
        public java.util.List<android.hardware.Camera.Area> getMeteringAreas() {
            return splitArea(get(android.hardware.Camera.Parameters.KEY_METERING_AREAS));
        }
        ...
        public void setMeteringAreas(java.util.List<android.hardware.Camera.Area> meteringAreas) {
            set(android.hardware.Camera.Parameters.KEY_METERING_AREAS, meteringAreas);
        }
        ...
        public int getMaxNumDetectedFaces() {
            return getInt(android.hardware.Camera.Parameters.KEY_MAX_NUM_DETECTED_FACES_HW, 0);
        }

        /**
         * Sets recording mode hint. This tells the camera that the intent of
         * the application is to record videos {@link android.media.MediaRecorder#start()}, not to take still pictures
        ...
        public void setRecordingHint(boolean hint) {
            set(android.hardware.Camera.Parameters.KEY_RECORDING_HINT, hint ? android.hardware.Camera.Parameters.TRUE : android.hardware.Camera.Parameters.FALSE);
        }
        ...
        public boolean isVideoSnapshotSupported() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_VIDEO_SNAPSHOT_SUPPORTED);
            return android.hardware.Camera.Parameters.TRUE.equals(str);
        }
        ...
        public void setVideoStabilization(boolean toggle) {
            set(android.hardware.Camera.Parameters.KEY_VIDEO_STABILIZATION, toggle ? android.hardware.Camera.Parameters.TRUE : android.hardware.Camera.Parameters.FALSE);
        }
        ...
        public boolean getVideoStabilization() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_VIDEO_STABILIZATION);
            return android.hardware.Camera.Parameters.TRUE.equals(str);
        }
        ...
        public boolean isVideoStabilizationSupported() {
            java.lang.String str = get(android.hardware.Camera.Parameters.KEY_VIDEO_STABILIZATION_SUPPORTED);
            return android.hardware.Camera.Parameters.TRUE.equals(str);
        }
        ...
        private java.util.ArrayList<java.lang.String> split(java.lang.String str) {
            ...
            for (java.lang.String s : splitter) {
                substrings.add(s);
            }
            return substrings;
        }
        ...
        private java.util.ArrayList<java.lang.Integer> splitInt(java.lang.String str) {
            ...
            android.text.TextUtils.StringSplitter splitter = new android.text.TextUtils.SimpleStringSplitter(',');
            splitter.setString(str);
            java.util.ArrayList<java.lang.Integer> substrings = new java.util.ArrayList<java.lang.Integer>();
            for (java.lang.String s : splitter) {
                substrings.add(java.lang.Integer.parseInt(s));
            }
        ...
        private void splitInt(java.lang.String str, int[] output) {
            if (str == null)
                return;
            ...
            int index = 0;
            for (java.lang.String s : splitter) {
                output[index++] = java.lang.Integer.parseInt(s);
            }
        }

        // Splits a comma delimited string to an ArrayList of Float.
        private void splitFloat(java.lang.String str, float[] output) {
            if (str == null)
                return;
            ...
            int index = 0;
            for (java.lang.String s : splitter) {
                output[index++] = java.lang.Float.parseFloat(s);
            }
        }

        // Returns the value of a float parameter.
        private float getFloat(java.lang.String key, float defaultValue) {
            try {
                return java.lang.Float.parseFloat(mMap.get(key));
        ...
        private java.util.ArrayList<android.hardware.Camera.Size> splitSize(java.lang.String str) {
            ...
            android.text.TextUtils.StringSplitter splitter = new android.text.TextUtils.SimpleStringSplitter(',');
            splitter.setString(str);
            java.util.ArrayList<android.hardware.Camera.Size> sizeList = new java.util.ArrayList<android.hardware.Camera.Size>();
            for (java.lang.String s : splitter) {
                android.hardware.Camera.Size size = strToSize(s);
                if (size != null)
                    sizeList.add(size);
        ...
        private android.hardware.Camera.Size strToSize(java.lang.String str) {
            ...
            if (pos != (-1)) {
                java.lang.String width = str.substring(0, pos);
                java.lang.String height = str.substring(pos + 1);
                return new android.hardware.Camera.Size(java.lang.Integer.parseInt(width), java.lang.Integer.parseInt(height));
            }
        ...
        private java.util.ArrayList<int[]> splitRange(java.lang.String str) {
            ...
            java.util.ArrayList<int[]> rangeList = new java.util.ArrayList<int[]>();
            int endIndex;
            int fromIndex = 1;
            do {
                int[] range = new int[2];
        ...
        private java.util.ArrayList<android.hardware.Camera.Area> splitArea(java.lang.String str) {
            ...
            java.util.ArrayList<android.hardware.Camera.Area> result = new java.util.ArrayList<android.hardware.Camera.Area>();
            int endIndex;
            int fromIndex = 1;
            int[] array = new int[5];
            do {
                ...
                android.graphics.Rect rect = new android.graphics.Rect(array[0], array[1], array[2], array[3]);
                result.add(new android.hardware.Camera.Area(rect, array[4]));
                fromIndex = endIndex + 3;
            } while (endIndex != (str.length() - 1) );
            if (result.size() == 0)
                return null;
            ...
            if (result.size() == 1) {
                android.hardware.Camera.Area area = result.get(0);
                android.graphics.Rect rect = area.rect;
