...
public final class ConsumerIrManager {
     ...
     *
     *
     * @unknown to prevent subclassing from outside of the framework
     */
    public ConsumerIrManager(android.content.Context context) {
        mPackageName = context.getPackageName();
        mService = IConsumerIrService.Stub.asInterface(android.os.ServiceManager.getService(android.content.Context.CONSUMER_IR_SERVICE));
    }
    ...
    public boolean hasIrEmitter() {
        if (mService == null) {
            android.util.Log.w(android.hardware.ConsumerIrManager.TAG, "no consumer ir service.");
            return false;
        }
        try {
    ...
    public void transmit(int carrierFrequency, int[] pattern) {
        if (mService == null) {
            android.util.Log.w(android.hardware.ConsumerIrManager.TAG, "failed to transmit; no consumer ir service.");
            return;
        }
        try {
    ...
    public final class CarrierFrequencyRange {
         ...
         *
         * @param min
         * 		The minimum transmittable frequency in this range segment.
         * @param max
    ...
    public android.hardware.ConsumerIrManager.CarrierFrequencyRange[] getCarrierFrequencies() {
        ...
        try {
            ...
            if ((freqs.length % 2) != 0) {
                android.util.Log.w(android.hardware.ConsumerIrManager.TAG, "consumer ir service returned an uneven number of frequencies.");
                return null;
            }
            android.hardware.ConsumerIrManager.CarrierFrequencyRange[] range = new android.hardware.ConsumerIrManager.CarrierFrequencyRange[freqs.length / 2];
            for (int i = 0; i < freqs.length; i += 2) {
                range[i / 2] = new android.hardware.ConsumerIrManager.CarrierFrequencyRange(freqs[i], freqs[i + 1]);
            }
            return range;
        } catch (android.os.RemoteException e) {
            throw e.rethrowFromSystemServer();
