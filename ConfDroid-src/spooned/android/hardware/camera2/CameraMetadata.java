...
public abstract class CameraMetadata<TKey> {
    ...
    public java.util.List<TKey> getKeys() {
        java.lang.Class<android.hardware.camera2.CameraMetadata<TKey>> thisClass = ((java.lang.Class<android.hardware.camera2.CameraMetadata<TKey>>) (getClass()));
        return java.util.Collections.unmodifiableList(/* filterTags */
        android.hardware.camera2.CameraMetadata.getKeysStatic(thisClass, getKeyClass(), this, null));
    }
    ...
    /* package */
    @java.lang.SuppressWarnings("unchecked")
    static <TKey> java.util.ArrayList<TKey> getKeysStatic(java.lang.Class<?> type, java.lang.Class<TKey> keyClass, android.hardware.camera2.CameraMetadata<TKey> instance, int[] filterTags) {
        if (android.hardware.camera2.CameraMetadata.DEBUG)
            android.util.Log.v(android.hardware.camera2.CameraMetadata.TAG, "getKeysStatic for " + type);

        // TotalCaptureResult does not have any of the keys on it, use CaptureResult instead
        if (type.equals(android.hardware.camera2.TotalCaptureResult.class)) {
            type = android.hardware.camera2.CaptureResult.class;
        }
        if (filterTags != null) {
            java.util.Arrays.sort(filterTags);
        }
        java.util.ArrayList<TKey> keyList = new java.util.ArrayList<TKey>();
        java.lang.reflect.Field[] fields = type.getDeclaredFields();
        for (java.lang.reflect.Field field : fields) {
            // Filter for Keys that are public
            if (field.getType().isAssignableFrom(keyClass) && ((field.getModifiers() & java.lang.reflect.Modifier.PUBLIC) != 0)) {
                TKey key;
                try {
                    ...
                    throw new java.lang.AssertionError("Can't get IllegalArgumentException", e);
                }
                if ((instance == null) || (instance.getProtected(key) != null)) {
                    if (android.hardware.camera2.CameraMetadata.shouldKeyBeAdded(key, field, filterTags)) {
                        keyList.add(key);
                        if (android.hardware.camera2.CameraMetadata.DEBUG) {
                            android.util.Log.v(android.hardware.camera2.CameraMetadata.TAG, "getKeysStatic - key was added - " + key);
                        }
                    } else
                        if (android.hardware.camera2.CameraMetadata.DEBUG) {
                            android.util.Log.v(android.hardware.camera2.CameraMetadata.TAG, "getKeysStatic - key was filtered - " + key);
                        }

                }
            }
        }
        java.util.ArrayList<TKey> vendorKeys = android.hardware.camera2.impl.CameraMetadataNative.getAllVendorKeys(keyClass);
        if (vendorKeys != null) {
            for (TKey k : vendorKeys) {
                java.lang.String keyName;
                if (k instanceof android.hardware.camera2.CaptureRequest.Key<?>) {
                    keyName = ((android.hardware.camera2.CaptureRequest.Key<?>) (k)).getName();
                } else
                    if (k instanceof android.hardware.camera2.CaptureResult.Key<?>) {
                        keyName = ((android.hardware.camera2.CaptureResult.Key<?>) (k)).getName();
                    } else
                        if (k instanceof android.hardware.camera2.CameraCharacteristics.Key<?>) {
                            keyName = ((android.hardware.camera2.CameraCharacteristics.Key<?>) (k)).getName();
                        } else {
                            continue;
                        }


                if ((filterTags == null) || (java.util.Arrays.binarySearch(filterTags, android.hardware.camera2.impl.CameraMetadataNative.getTag(keyName)) >= 0)) {
                    keyList.add(k);
                }
            }
        }
    ...
    private static <TKey> boolean shouldKeyBeAdded(TKey key, java.lang.reflect.Field field, int[] filterTags) {
        if (key == null) {
        ...
        if (key instanceof android.hardware.camera2.CameraCharacteristics.Key) {
            nativeKey = ((android.hardware.camera2.CameraCharacteristics.Key) (key)).getNativeKey();
        } else
            if (key instanceof android.hardware.camera2.CaptureResult.Key) {
                nativeKey = ((android.hardware.camera2.CaptureResult.Key) (key)).getNativeKey();
            } else
                if (key instanceof android.hardware.camera2.CaptureRequest.Key) {
                    nativeKey = ((android.hardware.camera2.CaptureRequest.Key) (key)).getNativeKey();
                } else {
        ...
        if (field.getAnnotation(android.hardware.camera2.impl.PublicKey.class) == null) {
        ...
        return java.util.Arrays.binarySearch(filterTags, keyTag) >= 0;
    }
