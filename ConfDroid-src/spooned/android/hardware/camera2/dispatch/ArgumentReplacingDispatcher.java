 ...
 */
public class ArgumentReplacingDispatcher<T, TArg> implements android.hardware.camera2.dispatch.Dispatchable<T> {
     ...
     * @param argumentIndex
     * 		the numeric index of the argument {@code >= 0}
     * @param replaceWith
     * 		arguments matching {@code argumentIndex} will be replaced with this object
     */
    public ArgumentReplacingDispatcher(android.hardware.camera2.dispatch.Dispatchable<T> target, int argumentIndex, TArg replaceWith) {
        mTarget = android.hardware.camera2.dispatch.ArgumentReplacingDispatcher.checkNotNull(target, "target must not be null");
        mArgumentIndex = android.hardware.camera2.dispatch.ArgumentReplacingDispatcher.checkArgumentNonnegative(argumentIndex, "argumentIndex must not be negative");
        mReplaceWith = android.hardware.camera2.dispatch.ArgumentReplacingDispatcher.checkNotNull(replaceWith, "replaceWith must not be null");
    }
    ...
    public java.lang.Object dispatch(java.lang.reflect.Method method, java.lang.Object[] args) throws java.lang.Throwable {
        if (args.length > mArgumentIndex) {
            args = android.hardware.camera2.dispatch.ArgumentReplacingDispatcher.arrayCopy(args);// don't change in-place since it can affect upstream dispatches
