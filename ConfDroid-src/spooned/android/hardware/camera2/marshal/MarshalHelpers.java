/**
 * Copyright (C) 2014 The Android Open Source Project
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package android.hardware.camera2.marshal;


/**
 * Static functions in order to help implementing various marshaler functionality.
 *
 * <p>The intention is to statically import everything from this file into another file when
 * implementing a new marshaler (or marshal queryable).</p>
 *
 * <p>The helpers are centered around providing primitive knowledge of the native types,
 * such as the native size, the managed class wrappers, and various precondition checks.</p>
 */
public final class MarshalHelpers {
    public static final int SIZEOF_BYTE = 1;

    public static final int SIZEOF_INT32 = java.lang.Integer.SIZE / java.lang.Byte.SIZE;

    public static final int SIZEOF_INT64 = java.lang.Long.SIZE / java.lang.Byte.SIZE;

    public static final int SIZEOF_FLOAT = java.lang.Float.SIZE / java.lang.Byte.SIZE;

    public static final int SIZEOF_DOUBLE = java.lang.Double.SIZE / java.lang.Byte.SIZE;

    public static final int SIZEOF_RATIONAL = android.hardware.camera2.marshal.MarshalHelpers.SIZEOF_INT32 * 2;

    /**
     * Get the size in bytes for the native camera metadata type.
     *
     * <p>This used to determine how many bytes it would take to encode/decode a single value
     * of that {@link nativeType}.</p>
     *
     * @param nativeType
     * 		the native type, e.g.
     * 		{@link android.hardware.camera2.impl.CameraMetadataNative#TYPE_BYTE TYPE_BYTE}.
     * @return size in bytes >= 1
     * @throws UnsupportedOperationException
     * 		if nativeType was not one of the built-in types
     */
    public static int getPrimitiveTypeSize(int nativeType) {
        switch (nativeType) {
            case android.hardware.camera2.impl.CameraMetadataNative.TYPE_BYTE :
                return android.hardware.camera2.marshal.MarshalHelpers.SIZEOF_BYTE;
            case android.hardware.camera2.impl.CameraMetadataNative.TYPE_INT32 :
                return android.hardware.camera2.marshal.MarshalHelpers.SIZEOF_INT32;
            case android.hardware.camera2.impl.CameraMetadataNative.TYPE_FLOAT :
                return android.hardware.camera2.marshal.MarshalHelpers.SIZEOF_FLOAT;
            case android.hardware.camera2.impl.CameraMetadataNative.TYPE_INT64 :
                return android.hardware.camera2.marshal.MarshalHelpers.SIZEOF_INT64;
            case android.hardware.camera2.impl.CameraMetadataNative.TYPE_DOUBLE :
                return android.hardware.camera2.marshal.MarshalHelpers.SIZEOF_DOUBLE;
            case android.hardware.camera2.impl.CameraMetadataNative.TYPE_RATIONAL :
                return android.hardware.camera2.marshal.MarshalHelpers.SIZEOF_RATIONAL;
        }
        throw new java.lang.UnsupportedOperationException("Unknown type, can't get size for " + nativeType);
    }

    /**
     * Ensure that the {@code klass} is one of the metadata-primitive classes.
     *
     * @param klass
     * 		a non-{@code null} reference
     * @return {@code klass} instance
     * @throws UnsupportedOperationException
     * 		if klass was not one of the built-in classes
     * @throws NullPointerException
     * 		if klass was null
     * @see #isPrimitiveClass
     */
    public static <T> java.lang.Class<T> checkPrimitiveClass(java.lang.Class<T> klass) {
        checkNotNull(klass, "klass must not be null");
        if (android.hardware.camera2.marshal.MarshalHelpers.isPrimitiveClass(klass)) {
            return klass;
        }
        throw new java.lang.UnsupportedOperationException(("Unsupported class '" + klass) + "'; expected a metadata primitive class");
    }

    /**
     * Checks whether or not {@code klass} is one of the metadata-primitive classes.
     *
     * <p>The following types (whether boxed or unboxed) are considered primitive:
     * <ul>
     * <li>byte
     * <li>int
     * <li>float
     * <li>double
     * <li>Rational
     * </ul>
     * </p>
     *
     * <p>This doesn't strictly follow the java understanding of primitive since
     * boxed objects are included, Rational is included, and other types such as char and
     * short are not included.</p>
     *
     * @param klass
     * 		a {@link Class} instance; using {@code null} will return {@code false}
     * @return {@code true} if primitive, {@code false} otherwise
     */
    public static <T> boolean isPrimitiveClass(java.lang.Class<T> klass) {
        if (klass == null) {
            return false;
        }
        if ((klass == byte.class) || (klass == java.lang.Byte.class)) {
            return true;
        } else
            if ((klass == int.class) || (klass == java.lang.Integer.class)) {
                return true;
            } else
                if ((klass == float.class) || (klass == java.lang.Float.class)) {
                    return true;
                } else
                    if ((klass == long.class) || (klass == java.lang.Long.class)) {
                        return true;
                    } else
                        if ((klass == double.class) || (klass == java.lang.Double.class)) {
                            return true;
                        } else
                            if (klass == android.util.Rational.class) {
                                return true;
                            }





        return false;
    }

    /**
     * Wrap {@code klass} with its wrapper variant if it was a {@code Class} corresponding
     * to a Java primitive.
     *
     * <p>Non-primitive classes are passed through as-is.</p>
     *
     * <p>For example, for a primitive {@code int.class => Integer.class},
     * but for a non-primitive {@code Rational.class => Rational.class}.</p>
     *
     * @param klass
     * 		a {@code Class} reference
     * @return wrapped class object, or same class object if non-primitive
     */
    @java.lang.SuppressWarnings("unchecked")
    public static <T> java.lang.Class<T> wrapClassIfPrimitive(java.lang.Class<T> klass) {
        if (klass == byte.class) {
            return ((java.lang.Class<T>) (java.lang.Byte.class));
        } else
            if (klass == int.class) {
                return ((java.lang.Class<T>) (java.lang.Integer.class));
            } else
                if (klass == float.class) {
                    return ((java.lang.Class<T>) (java.lang.Float.class));
                } else
                    if (klass == long.class) {
                        return ((java.lang.Class<T>) (java.lang.Long.class));
                    } else
                        if (klass == double.class) {
                            return ((java.lang.Class<T>) (java.lang.Double.class));
                        }




        return klass;
    }

    /**
     * Return a human-readable representation of the {@code nativeType}, e.g. "TYPE_INT32"
     *
     * <p>Out-of-range values return a string with "UNKNOWN" as the prefix.</p>
     *
     * @param nativeType
     * 		the native type
     * @return human readable type name
     */
    public static java.lang.String toStringNativeType(int nativeType) {
        switch (nativeType) {
            case android.hardware.camera2.impl.CameraMetadataNative.TYPE_BYTE :
                return "TYPE_BYTE";
            case android.hardware.camera2.impl.CameraMetadataNative.TYPE_INT32 :
                return "TYPE_INT32";
            case android.hardware.camera2.impl.CameraMetadataNative.TYPE_FLOAT :
                return "TYPE_FLOAT";
            case android.hardware.camera2.impl.CameraMetadataNative.TYPE_INT64 :
                return "TYPE_INT64";
            case android.hardware.camera2.impl.CameraMetadataNative.TYPE_DOUBLE :
                return "TYPE_DOUBLE";
            case android.hardware.camera2.impl.CameraMetadataNative.TYPE_RATIONAL :
                return "TYPE_RATIONAL";
        }
        return ("UNKNOWN(" + nativeType) + ")";
    }

    /**
     * Ensure that the {@code nativeType} is one of the native types supported
     * by {@link CameraMetadataNative}.
     *
     * @param nativeType
     * 		the native type
     * @return the native type
     * @throws UnsupportedOperationException
     * 		if the native type was invalid
     */
    public static int checkNativeType(int nativeType) {
        switch (nativeType) {
            case android.hardware.camera2.impl.CameraMetadataNative.TYPE_BYTE :
            case android.hardware.camera2.impl.CameraMetadataNative.TYPE_INT32 :
            case android.hardware.camera2.impl.CameraMetadataNative.TYPE_FLOAT :
            case android.hardware.camera2.impl.CameraMetadataNative.TYPE_INT64 :
            case android.hardware.camera2.impl.CameraMetadataNative.TYPE_DOUBLE :
            case android.hardware.camera2.impl.CameraMetadataNative.TYPE_RATIONAL :
                return nativeType;
        }
        throw new java.lang.UnsupportedOperationException("Unknown nativeType " + nativeType);
    }

    /**
     * Ensure that the expected and actual native types are equal.
     *
     * @param expectedNativeType
     * 		the expected native type
     * @param actualNativeType
     * 		the actual native type
     * @return the actual native type
     * @throws UnsupportedOperationException
     * 		if the types are not equal
     */
    public static int checkNativeTypeEquals(int expectedNativeType, int actualNativeType) {
        if (expectedNativeType != actualNativeType) {
            throw new java.lang.UnsupportedOperationException(java.lang.String.format("Expected native type %d, but got %d", expectedNativeType, actualNativeType));
        }
        return actualNativeType;
    }

    private MarshalHelpers() {
        throw new java.lang.AssertionError();
    }
}

