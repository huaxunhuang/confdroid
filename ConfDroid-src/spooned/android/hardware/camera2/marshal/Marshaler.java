...
public abstract class Marshaler<T> {
     ...
     *
     * @param query
     * 		an instance of {@link MarshalQueryable}
     * @param typeReference
     ...
     * 		if any args were {@code null}
     * @throws UnsupportedOperationException
     * 		if the type mapping was not supported
     */
    protected Marshaler(android.hardware.camera2.marshal.MarshalQueryable<T> query, android.hardware.camera2.utils.TypeReference<T> typeReference, int nativeType) {
        mTypeReference = android.hardware.camera2.marshal.Marshaler.checkNotNull(typeReference, "typeReference must not be null");
        mNativeType = android.hardware.camera2.marshal.MarshalHelpers.checkNativeType(nativeType);
        if (!query.isTypeMappingSupported(typeReference, nativeType)) {
            throw new java.lang.UnsupportedOperationException((("Unsupported type marshaling for managed type " + typeReference) + " and native type ") + android.hardware.camera2.marshal.MarshalHelpers.toStringNativeType(nativeType));
        }
    }
     ...
     *
     * @param value
     * 		the value of type T that we wish to write into the byte buffer
     * @param buffer
     * 		the byte buffer into which the marshaled object will be written
     */
    public abstract void marshal(T value, java.nio.ByteBuffer buffer);
     ...
     */
    public abstract T unmarshal(java.nio.ByteBuffer buffer);
