 ...
 *
 * <pre><code>{@code CameraCharacteristics characteristics = cameraManager.getCameraCharacteristics(cameraId);
 * StreamConfigurationMap configs = characteristics.get(
 *         CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP);}</code></pre>
 *
...
public final class StreamConfigurationMap {
     ...
     * 		a non-{@code null} array of {@link StreamConfiguration}
     * @param minFrameDurations
     * 		a non-{@code null} array of {@link StreamConfigurationDuration}
     * @param stallDurations
     * 		a non-{@code null} array of {@link StreamConfigurationDuration}
     * @param highSpeedVideoConfigurations
     * 		an array of {@link HighSpeedVideoConfiguration}, null if
     * 		camera device does not support high speed video recording
     * @param listHighResolution
     ...
     * @unknown 
     */
    public StreamConfigurationMap(android.hardware.camera2.params.StreamConfiguration[] configurations, android.hardware.camera2.params.StreamConfigurationDuration[] minFrameDurations, android.hardware.camera2.params.StreamConfigurationDuration[] stallDurations, android.hardware.camera2.params.StreamConfiguration[] depthConfigurations, android.hardware.camera2.params.StreamConfigurationDuration[] depthMinFrameDurations, android.hardware.camera2.params.StreamConfigurationDuration[] depthStallDurations, android.hardware.camera2.params.HighSpeedVideoConfiguration[] highSpeedVideoConfigurations, android.hardware.camera2.params.ReprocessFormatsMap inputOutputFormatsMap, boolean listHighResolution) {
        if (configurations == null) {
            ...
            mConfigurations = new android.hardware.camera2.params.StreamConfiguration[0];
            mMinFrameDurations = new android.hardware.camera2.params.StreamConfigurationDuration[0];
            mStallDurations = new android.hardware.camera2.params.StreamConfigurationDuration[0];
        } else {
        ...
        if (depthConfigurations == null) {
            mDepthConfigurations = new android.hardware.camera2.params.StreamConfiguration[0];
            mDepthMinFrameDurations = new android.hardware.camera2.params.StreamConfigurationDuration[0];
            mDepthStallDurations = new android.hardware.camera2.params.StreamConfigurationDuration[0];
        } else {
        ...
        if (highSpeedVideoConfigurations == null) {
            mHighSpeedVideoConfigurations = new android.hardware.camera2.params.HighSpeedVideoConfiguration[0];
        } else {
        ...
        for (android.hardware.camera2.params.StreamConfiguration config : mConfigurations) {
            int fmt = config.getFormat();
            android.util.SparseIntArray map = null;
            if (config.isOutput()) {
                mAllOutputFormats.put(fmt, mAllOutputFormats.get(fmt) + 1);
                long duration = 0;
                if (mListHighResolution) {
                    for (android.hardware.camera2.params.StreamConfigurationDuration configurationDuration : mMinFrameDurations) {
                ...
                map = (duration <= android.hardware.camera2.params.StreamConfigurationMap.DURATION_20FPS_NS) ? mOutputFormats : mHighResOutputFormats;
            } else {
                map = mInputFormats;
            }
        ...
        for (android.hardware.camera2.params.StreamConfiguration config : mDepthConfigurations) {
            if (!config.isOutput()) {
        ...
        if ((configurations != null) && (mOutputFormats.indexOfKey(android.hardware.camera2.params.StreamConfigurationMap.HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED) < 0)) {
        ...
        for (android.hardware.camera2.params.HighSpeedVideoConfiguration config : mHighSpeedVideoConfigurations) {
            android.util.Size size = config.getSize();
            android.util.Range<java.lang.Integer> fpsRange = config.getFpsRange();
            java.lang.Integer fpsRangeCount = mHighSpeedVideoSizeMap.get(size);
            if (fpsRangeCount == null) {
                fpsRangeCount = 0;
            }
            mHighSpeedVideoSizeMap.put(size, fpsRangeCount + 1);
            java.lang.Integer sizeCount = mHighSpeedVideoFpsRangeMap.get(fpsRange);
            if (sizeCount == null) {
                sizeCount = 0;
            }
     ...
     *
     * @param format
     * 		a format from {@link #getInputFormats}
     * @return a non-empty array of sizes, or {@code null} if the format was not available.
     */
    public android.util.Size[] getInputSizes(final int format) {
        return /* output */
        /* highRes */
        getPublicFormatSizes(format, false, false);
    }
     ...
     *
     * @param format
     * 		an image format from either {@link ImageFormat} or {@link PixelFormat}
    ...
    public boolean isOutputSupportedFor(int format) {
        android.hardware.camera2.params.StreamConfigurationMap.checkArgumentFormat(format);
        int internalFormat = android.hardware.camera2.params.StreamConfigurationMap.imageFormatToInternal(format);
        int dataspace = android.hardware.camera2.params.StreamConfigurationMap.imageFormatToDataspace(format);
        if (dataspace == android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_DEPTH) {
     ...
     * @throws NullPointerException
     * 		if {@code klass} was {@code null}
     * @see CameraDevice#createCaptureSession
     * @see #isOutputSupportedFor(Surface)
     */
    public static <T> boolean isOutputSupportedFor(java.lang.Class<T> klass) {
        checkNotNull(klass, "klass must not be null");
        if (klass == android.media.ImageReader.class) {
            return true;
        } else
            if (klass == android.media.MediaRecorder.class) {
                return true;
            } else
                if (klass == android.media.MediaCodec.class) {
                    return true;
                } else
                    if (klass == android.renderscript.Allocation.class) {
                        return true;
                    } else
     ...
     *
     * @param surface
     * 		a non-{@code null} {@link Surface} object reference
     * @return {@code true} if this is supported, {@code false} otherwise
     * @throws NullPointerException
     * 		if {@code surface} was {@code null}
     * @throws IllegalArgumentException
    ...
    public boolean isOutputSupportedFor(android.view.Surface surface) {
        ...
        boolean isFlexible = android.hardware.camera2.utils.SurfaceUtils.isFlexibleConsumer(surface);
        // Override RGB formats to IMPLEMENTATION_DEFINED, b/9487482
        if ((surfaceFormat >= android.hardware.camera2.legacy.LegacyMetadataMapper.HAL_PIXEL_FORMAT_RGBA_8888) && (surfaceFormat <= android.hardware.camera2.legacy.LegacyMetadataMapper.HAL_PIXEL_FORMAT_BGRA_8888)) {
            surfaceFormat = android.hardware.camera2.params.StreamConfigurationMap.HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED;
        }
        android.hardware.camera2.params.StreamConfiguration[] configs = (surfaceDataspace != android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_DEPTH) ? mConfigurations : mDepthConfigurations;
        for (android.hardware.camera2.params.StreamConfiguration config : configs) {
            if ((config.getFormat() == surfaceFormat) && config.isOutput()) {
                ...
                if (config.getSize().equals(surfaceSize)) {
                    ...
                    if (isFlexible && (config.getSize().getWidth() <= android.hardware.camera2.legacy.LegacyCameraDevice.MAX_DIMEN_FOR_ROUNDING)) {
                        return true;
                    }
    ...
    public <T> android.util.Size[] getOutputSizes(java.lang.Class<T> klass) {
        if (android.hardware.camera2.params.StreamConfigurationMap.isOutputSupportedFor(klass) == false) {
            return null;
        }
        return /* output */
        /* highRes */
        getInternalFormatSizes(android.hardware.camera2.params.StreamConfigurationMap.HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED, android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_UNKNOWN, true, false);
    }
     ...
     *
     * @param format
     * 		an image format from {@link ImageFormat} or {@link PixelFormat}
    ...
    public android.util.Size[] getOutputSizes(int format) {
        return /* output */
        /* highRes */
        getPublicFormatSizes(format, true, false);
    }
    ...
    public android.util.Size[] getHighSpeedVideoSizes() {
        java.util.Set<android.util.Size> keySet = mHighSpeedVideoSizeMap.keySet();
        return keySet.toArray(new android.util.Size[keySet.size()]);
    }
     ...
     *
     * @param size
     * 		one of the sizes returned by {@link #getHighSpeedVideoSizes()}
    ...
    public android.util.Range<java.lang.Integer>[] getHighSpeedVideoFpsRangesFor(android.util.Size size) {
        ...
        android.util.Range<java.lang.Integer>[] fpsRanges = new android.util.Range[fpsRangeCount];
        int i = 0;
        for (android.hardware.camera2.params.HighSpeedVideoConfiguration config : mHighSpeedVideoConfigurations) {
    ...
    public android.util.Range<java.lang.Integer>[] getHighSpeedVideoFpsRanges() {
        java.util.Set<android.util.Range<java.lang.Integer>> keySet = mHighSpeedVideoFpsRangeMap.keySet();
        return keySet.toArray(new android.util.Range[keySet.size()]);
    }
     ...
     *
     * @param fpsRange
     * 		one of the FPS range returned by {@link #getHighSpeedVideoFpsRanges()}
    ...
    public android.util.Size[] getHighSpeedVideoSizesFor(android.util.Range<java.lang.Integer> fpsRange) {
        ...
        if ((sizeCount == null) || (sizeCount == 0)) {
            throw new java.lang.IllegalArgumentException(java.lang.String.format("FpsRange %s does not support high speed video recording", fpsRange));
        }
        android.util.Size[] sizes = new android.util.Size[sizeCount];
        int i = 0;
        for (android.hardware.camera2.params.HighSpeedVideoConfiguration config : mHighSpeedVideoConfigurations) {
    ...
    public android.util.Size[] getHighResolutionOutputSizes(int format) {
        ...
        return /* output */
        /* highRes */
        getPublicFormatSizes(format, true, true);
    }
     ...
     * @throws IllegalArgumentException
     * 		if {@code format} or {@code size} was not supported
     * @throws NullPointerException
    ...
    public long getOutputMinFrameDuration(int format, android.util.Size size) {
        ...
        return getInternalFormatDuration(android.hardware.camera2.params.StreamConfigurationMap.imageFormatToInternal(format), android.hardware.camera2.params.StreamConfigurationMap.imageFormatToDataspace(format), size, android.hardware.camera2.params.StreamConfigurationMap.DURATION_MIN_FRAME);
    }
    ...
    0 if the minimum frame duration is not available.
     * @throws IllegalArgumentException
     * 		if {@code klass} or {@code size} was not supported
     * @throws NullPointerException
     * 		if {@code size} or {@code klass} was {@code null}
     * @see CaptureRequest#SENSOR_FRAME_DURATION
     * @see ImageFormat
     * @see PixelFormat
     */
    public <T> long getOutputMinFrameDuration(final java.lang.Class<T> klass, final android.util.Size size) {
        if (!android.hardware.camera2.params.StreamConfigurationMap.isOutputSupportedFor(klass)) {
            throw new java.lang.IllegalArgumentException("klass was not supported");
        }
        return getInternalFormatDuration(android.hardware.camera2.params.StreamConfigurationMap.HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED, android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_UNKNOWN, size, android.hardware.camera2.params.StreamConfigurationMap.DURATION_MIN_FRAME);
    }
    ...
    public long getOutputStallDuration(int format, android.util.Size size) {
        /* output */
        checkArgumentFormatSupported(format, true);
        return getInternalFormatDuration(android.hardware.camera2.params.StreamConfigurationMap.imageFormatToInternal(format), android.hardware.camera2.params.StreamConfigurationMap.imageFormatToDataspace(format), size, android.hardware.camera2.params.StreamConfigurationMap.DURATION_STALL);
    }
     ...
     * @see PixelFormat
     */
    public <T> long getOutputStallDuration(final java.lang.Class<T> klass, final android.util.Size size) {
        if (!android.hardware.camera2.params.StreamConfigurationMap.isOutputSupportedFor(klass)) {
            throw new java.lang.IllegalArgumentException("klass was not supported");
        }
        return getInternalFormatDuration(android.hardware.camera2.params.StreamConfigurationMap.HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED, android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_UNKNOWN, size, android.hardware.camera2.params.StreamConfigurationMap.DURATION_STALL);
    }
    ...
    public boolean equals(final java.lang.Object obj) {
        ...
        if (obj instanceof android.hardware.camera2.params.StreamConfigurationMap) {
            final android.hardware.camera2.params.StreamConfigurationMap other = ((android.hardware.camera2.params.StreamConfigurationMap) (obj));
            // XX: do we care about order?
            return (((java.util.Arrays.equals(mConfigurations, other.mConfigurations) && java.util.Arrays.equals(mMinFrameDurations, other.mMinFrameDurations)) && java.util.Arrays.equals(mStallDurations, other.mStallDurations)) && java.util.Arrays.equals(mDepthConfigurations, other.mDepthConfigurations)) && java.util.Arrays.equals(mHighSpeedVideoConfigurations, other.mHighSpeedVideoConfigurations);
        }
        return false;
    }
    ...
    public int hashCode() {
        // XX: do we care about order?
        return android.hardware.camera2.utils.HashCodeHelpers.hashCodeGeneric(mConfigurations, mMinFrameDurations, mStallDurations, mDepthConfigurations, mHighSpeedVideoConfigurations);
    }
    ...
    private int checkArgumentFormatSupported(int format, boolean output) {
        android.hardware.camera2.params.StreamConfigurationMap.checkArgumentFormat(format);
        int internalFormat = android.hardware.camera2.params.StreamConfigurationMap.imageFormatToInternal(format);
        int internalDataspace = android.hardware.camera2.params.StreamConfigurationMap.imageFormatToDataspace(format);
        if (output) {
            if (internalDataspace == android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_DEPTH) {
        ...
        throw new java.lang.IllegalArgumentException(java.lang.String.format("format %x is not supported by this stream configuration map", format));
    }
    ...
    static int checkArgumentFormatInternal(int format) {
        switch (format) {
            case android.hardware.camera2.params.StreamConfigurationMap.HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED :
            case android.hardware.camera2.params.StreamConfigurationMap.HAL_PIXEL_FORMAT_BLOB :
            case android.hardware.camera2.params.StreamConfigurationMap.HAL_PIXEL_FORMAT_RAW_OPAQUE :
            case android.hardware.camera2.params.StreamConfigurationMap.HAL_PIXEL_FORMAT_Y16 :
                return format;
            ...
            default :
                return android.hardware.camera2.params.StreamConfigurationMap.checkArgumentFormat(format);
        }
    }
     ...
     * @return the format
     * @throws IllegalArgumentException
     * 		if the format was not user-defined
     */
    static int checkArgumentFormat(int format) {
        if ((!android.graphics.ImageFormat.isPublicFormat(format)) && (!android.graphics.PixelFormat.isPublicFormat(format))) {
            throw new java.lang.IllegalArgumentException(java.lang.String.format("format 0x%x was not defined in either ImageFormat or PixelFormat", format));
        }
        return format;
    }
     ...
     *
     * @param format
     * 		image format from {@link ImageFormat} or {@link PixelFormat}
    ...
    static int depthFormatToPublic(int format) {
        switch (format) {
            case android.hardware.camera2.params.StreamConfigurationMap.HAL_PIXEL_FORMAT_BLOB :
                return android.graphics.ImageFormat.DEPTH_POINT_CLOUD;
            case android.hardware.camera2.params.StreamConfigurationMap.HAL_PIXEL_FORMAT_Y16 :
            ...
            default :
                throw new java.lang.IllegalArgumentException("Unknown DATASPACE_DEPTH format " + format);
        }
    }
    ...
    static int[] imageFormatToPublic(int[] formats) {
        ...
        for (int i = 0; i < formats.length; ++i) {
            formats[i] = android.hardware.camera2.params.StreamConfigurationMap.imageFormatToPublic(formats[i]);
        }
        return formats;
    }
     ...
     *
     * @param format
     * 		public image format from {@link ImageFormat} or {@link PixelFormat}
     * @return the converted image formats
     * @see ImageFormat
     * @see PixelFormat
     * @throws IllegalArgumentException
    ...
    static int imageFormatToInternal(int format) {
        switch (format) {
            ...
            case android.graphics.ImageFormat.DEPTH_POINT_CLOUD :
                return android.hardware.camera2.params.StreamConfigurationMap.HAL_PIXEL_FORMAT_BLOB;
            case android.graphics.ImageFormat.DEPTH16 :
                return android.hardware.camera2.params.StreamConfigurationMap.HAL_PIXEL_FORMAT_Y16;
            default :
                return format;
        }
    }
    ...
    static int imageFormatToDataspace(int format) {
        switch (format) {
            case android.graphics.ImageFormat.JPEG :
                return android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_V0_JFIF;
            case android.graphics.ImageFormat.DEPTH_POINT_CLOUD :
            case android.graphics.ImageFormat.DEPTH16 :
                return android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_DEPTH;
            default :
                return android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_UNKNOWN;
        }
    }
    ...
    public static int[] imageFormatToInternal(int[] formats) {
        ...
        for (int i = 0; i < formats.length; ++i) {
            formats[i] = android.hardware.camera2.params.StreamConfigurationMap.imageFormatToInternal(formats[i]);
        }
        return formats;
    }

    private android.util.Size[] getPublicFormatSizes(int format, boolean output, boolean highRes) {
        try {
    ...
    private android.util.Size[] getInternalFormatSizes(int format, int dataspace, boolean output, boolean highRes) {
        // All depth formats are non-high-res.
        if ((dataspace == android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_DEPTH) && highRes) {
            return new android.util.Size[0];
        }
        android.util.SparseIntArray formatsMap = (!output) ? mInputFormats : dataspace == android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_DEPTH ? mDepthOutputFormats : highRes ? mHighResOutputFormats : mOutputFormats;
        int sizesCount = formatsMap.get(format);
        if ((((!output) || (dataspace == android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_DEPTH)) && (sizesCount == 0)) || ((output && (dataspace != android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_DEPTH)) && (mAllOutputFormats.get(format) == 0))) {
            // Only throw if this is really not supported at all
            throw new java.lang.IllegalArgumentException("format not available");
        }
        android.util.Size[] sizes = new android.util.Size[sizesCount];
        int sizeIndex = 0;
        android.hardware.camera2.params.StreamConfiguration[] configurations = (dataspace == android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_DEPTH) ? mDepthConfigurations : mConfigurations;
        android.hardware.camera2.params.StreamConfigurationDuration[] minFrameDurations = (dataspace == android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_DEPTH) ? mDepthMinFrameDurations : mMinFrameDurations;
        for (android.hardware.camera2.params.StreamConfiguration config : configurations) {
            int fmt = config.getFormat();
            if ((fmt == format) && (config.isOutput() == output)) {
                if (output && mListHighResolution) {
                    ...
                    for (int i = 0; i < minFrameDurations.length; i++) {
                        android.hardware.camera2.params.StreamConfigurationDuration d = minFrameDurations[i];
                        if (((d.getFormat() == fmt) && (d.getWidth() == config.getSize().getWidth())) && (d.getHeight() == config.getSize().getHeight())) {
                            duration = d.getDuration();
                            break;
                        }
                    }
                    if ((dataspace != android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_DEPTH) && (highRes != (duration > android.hardware.camera2.params.StreamConfigurationMap.DURATION_20FPS_NS))) {
                        continue;
                    }
                }
        ...
        if (sizeIndex != sizesCount) {
            throw new java.lang.AssertionError(((("Too few sizes (expected " + sizesCount) + ", actual ") + sizeIndex) + ")");
        }
        return sizes;
    }
    ...
    private int[] getPublicFormats(boolean output) {
        ...
        int i = 0;
        android.util.SparseIntArray map = getFormatsMap(output);
        for (int j = 0; j < map.size(); j++) {
            int format = map.keyAt(j);
            formats[i++] = android.hardware.camera2.params.StreamConfigurationMap.imageFormatToPublic(format);
        }
        if (output) {
            for (int j = 0; j < mDepthOutputFormats.size(); j++) {
                formats[i++] = android.hardware.camera2.params.StreamConfigurationMap.depthFormatToPublic(mDepthOutputFormats.keyAt(j));
            }
        }
        if (formats.length != i) {
            throw new java.lang.AssertionError((("Too few formats " + i) + ", expected ") + formats.length);
        }
        return formats;
    }
     ...
     */
    private android.util.SparseIntArray getFormatsMap(boolean output) {
        return output ? mAllOutputFormats : mInputFormats;
    }

    private long getInternalFormatDuration(int format, int dataspace, android.util.Size size, int duration) {
        ...
        android.hardware.camera2.params.StreamConfigurationDuration[] durations = getDurations(duration, dataspace);
        for (android.hardware.camera2.params.StreamConfigurationDuration configurationDuration : durations) {
     ...
     * @see #DURATION_STALL
     */
    private android.hardware.camera2.params.StreamConfigurationDuration[] getDurations(int duration, int dataspace) {
        switch (duration) {
            case android.hardware.camera2.params.StreamConfigurationMap.DURATION_MIN_FRAME :
                return dataspace == android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_DEPTH ? mDepthMinFrameDurations : mMinFrameDurations;
            case android.hardware.camera2.params.StreamConfigurationMap.DURATION_STALL :
                return dataspace == android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_DEPTH ? mDepthStallDurations : mStallDurations;
            default :
    ...
    private int getPublicFormatCount(boolean output) {
        android.util.SparseIntArray formatsMap = getFormatsMap(output);
    ...
    private static <T> boolean arrayContains(T[] array, T element) {
        ...
        for (T el : array) {
            if (java.util.Objects.equals(el, element)) {
                return true;
            }
        }
        return false;
    ...
    private boolean isSupportedInternalConfiguration(int format, int dataspace, android.util.Size size) {
        android.hardware.camera2.params.StreamConfiguration[] configurations = (dataspace == android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_DEPTH) ? mDepthConfigurations : mConfigurations;
    ...
    private void appendOutputsString(java.lang.StringBuilder sb) {
        ...
        for (int format : formats) {
            android.util.Size[] sizes = getOutputSizes(format);
            for (android.util.Size size : sizes) {
                long minFrameDuration = getOutputMinFrameDuration(format, size);
                long stallDuration = getOutputStallDuration(format, size);
                sb.append(java.lang.String.format("[w:%d, h:%d, format:%s(%d), min_duration:%d, " + "stall:%d], ", size.getWidth(), size.getHeight(), formatToString(format), format, minFrameDuration, stallDuration));
            }
        }
        // Remove the pending ", "
    ...
    private void appendHighResOutputsString(java.lang.StringBuilder sb) {
        ...
        int[] formats = getOutputFormats();
        for (int format : formats) {
            android.util.Size[] sizes = getHighResolutionOutputSizes(format);
            if (sizes == null)
                continue;
    ...
    private void appendInputsString(java.lang.StringBuilder sb) {
        ...
        for (int format : formats) {
            android.util.Size[] sizes = getInputSizes(format);
            for (android.util.Size size : sizes) {
                sb.append(java.lang.String.format("[w:%d, h:%d, format:%s(%d)], ", size.getWidth(), size.getHeight(), formatToString(format), format));
            }
        }
        // Remove the pending ", "
        if (sb.charAt(sb.length() - 1) == ' ') {
    ...
    private void appendValidOutputFormatsForInputString(java.lang.StringBuilder sb) {
        ...
        for (int inputFormat : inputFormats) {
            sb.append(java.lang.String.format("[in:%s(%d), out:", formatToString(inputFormat), inputFormat));
            int[] outputFormats = getValidOutputFormatsForInput(inputFormat);
            for (int i = 0; i < outputFormats.length; i++) {
                sb.append(java.lang.String.format("%s(%d)", formatToString(outputFormats[i]), outputFormats[i]));
    ...
    private void appendHighSpeedVideoConfigurationsString(java.lang.StringBuilder sb) {
        ...
        for (android.util.Size size : sizes) {
            android.util.Range<java.lang.Integer>[] ranges = getHighSpeedVideoFpsRangesFor(size);
            for (android.util.Range<java.lang.Integer> range : ranges) {
                sb.append(java.lang.String.format("[w:%d, h:%d, min_fps:%d, max_fps:%d], ", size.getWidth(), size.getHeight(), range.getLower(), range.getUpper()));
            }
        }
        // Remove the pending ", "
        if (sb.charAt(sb.length() - 1) == ' ') {
    ...
    private java.lang.String formatToString(int format) {
        switch (format) {
    ...
    private static final int HAL_DATASPACE_V0_JFIF = ((2 << android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_STANDARD_SHIFT) | (3 << android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_TRANSFER_SHIFT)) | (1 << android.hardware.camera2.params.StreamConfigurationMap.HAL_DATASPACE_RANGE_SHIFT);
     ...
     */
    private final android.util.SparseIntArray mOutputFormats = new android.util.SparseIntArray();

    /**
     * internal format -> num output sizes mapping for slow high-res sizes, for non-depth
     * dataspaces
     */
    private final android.util.SparseIntArray mHighResOutputFormats = new android.util.SparseIntArray();

    /**
     * internal format -> num output sizes mapping for all non-depth dataspaces
     */
    private final android.util.SparseIntArray mAllOutputFormats = new android.util.SparseIntArray();

    /**
     * internal format -> num input sizes mapping, for input reprocessing formats
     */
    private final android.util.SparseIntArray mInputFormats = new android.util.SparseIntArray();

    /**
     * internal format -> num depth output sizes mapping, for HAL_DATASPACE_DEPTH
     */
    private final android.util.SparseIntArray mDepthOutputFormats = new android.util.SparseIntArray();

    /**
     * High speed video Size -> FPS range count mapping
     */
    private final java.util.HashMap<android.util.Size, java.lang.Integer> mHighSpeedVideoSizeMap = new java.util.HashMap<android.util.Size, java.lang.Integer>();

    /**
     * High speed video FPS range -> Size count mapping
     */
    private final java.util.HashMap<android.util.Range<java.lang.Integer>, java.lang.Integer> mHighSpeedVideoFpsRangeMap = new java.util.HashMap<android.util.Range<java.lang.Integer>, java.lang.Integer>();
}
