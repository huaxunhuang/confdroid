...
class AndroidBidi {
    public static int bidi(int dir, char[] chs, byte[] chInfo, int n, boolean haveInfo) {
        ...
        switch (dir) {
            case android.text.Layout.DIR_REQUEST_LTR :
                dir = 0;
                break;
            case android.text.Layout.DIR_REQUEST_RTL :
                dir = 1;
                break;
            case android.text.Layout.DIR_REQUEST_DEFAULT_LTR :
                dir = -2;
                break;
            case android.text.Layout.DIR_REQUEST_DEFAULT_RTL :
                dir = -1;
                break;
            default :
                dir = 0;
        ...
        int result = android.text.AndroidBidi.runBidi(dir, chs, chInfo, n, haveInfo);
        result = ((result & 0x1) == 0) ? android.text.Layout.DIR_LEFT_TO_RIGHT : android.text.Layout.DIR_RIGHT_TO_LEFT;
        return result;
    }
     ...
     *
     * @param dir
     * 		base line direction, either Layout.DIR_LEFT_TO_RIGHT or
     * 		Layout.DIR_RIGHT_TO_LEFT
     * @param levels
     * 		levels as returned from {@link #bidi}
     * @param lstart
     * 		start of the line in the levels array
     * @param chars
     * 		the character array (used to determine whitespace)
     * @param cstart
     * 		the start of the line in the chars array
     * @param len
     * 		the length of the line
     * @return the directions
     */
    public static android.text.Layout.Directions directions(int dir, byte[] levels, int lstart, char[] chars, int cstart, int len) {
        if (len == 0) {
            return android.text.Layout.DIRS_ALL_LEFT_TO_RIGHT;
        }
        int baseLevel = (dir == android.text.Layout.DIR_LEFT_TO_RIGHT) ? 0 : 1;
        ...
        int levelBits = minLevel << android.text.Layout.RUN_LEVEL_SHIFT;
        {
            ...
            for (int i = lstart, e = lstart + visLen; i < e; ++i) {
                ...
                if (level != curLevel) {
                    ...
                    ld[n++] = i - lstart;
                    levelBits = curLevel << android.text.Layout.RUN_LEVEL_SHIFT;
                    prev = i;
                }
            }
            ...
            if (visLen < len) {
                ld[++n] = visLen;
                ld[++n] = (len - visLen) | (baseLevel << android.text.Layout.RUN_LEVEL_SHIFT);
            }
        }
        ...
        return new android.text.Layout.Directions(ld);
    }
