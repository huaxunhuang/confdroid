...
class TextLine {
    ...
    private final android.text.SpanSet<android.text.style.MetricAffectingSpan> mMetricAffectingSpanSpanSet = new android.text.SpanSet<android.text.style.MetricAffectingSpan>(android.text.style.MetricAffectingSpan.class);

    private final android.text.SpanSet<android.text.style.CharacterStyle> mCharacterStyleSpanSet = new android.text.SpanSet<android.text.style.CharacterStyle>(android.text.style.CharacterStyle.class);

    private final android.text.SpanSet<android.text.style.ReplacementSpan> mReplacementSpanSpanSet = new android.text.SpanSet<android.text.style.ReplacementSpan>(android.text.style.ReplacementSpan.class);
    ...
    static android.text.TextLine obtain() {
        ...
        synchronized(android.text.TextLine.sCached) {
            for (int i = android.text.TextLine.sCached.length; (--i) >= 0;) {
                if (android.text.TextLine.sCached[i] != null) {
                    tl = android.text.TextLine.sCached[i];
                    android.text.TextLine.sCached[i] = null;
                    return tl;
                }
            }
        }
        tl = new android.text.TextLine();
        if (android.text.TextLine.DEBUG) {
    ...
    TextLine
     */
    static android.text.TextLine recycle(android.text.TextLine tl) {
        tl.mText = null;
        tl.mPaint = null;
        tl.mDirections = null;
        tl.mSpanned = null;
        ...
        synchronized(android.text.TextLine.sCached) {
            for (int i = 0; i < android.text.TextLine.sCached.length; ++i) {
                if (android.text.TextLine.sCached[i] == null) {
                    android.text.TextLine.sCached[i] = tl;
                    break;
                }
            }
        }
     ...
     *
     * @param paint
     * 		the base paint for the line
     * @param text
     * 		the text, can be Styled
     * @param start
     * 		the start of the line relative to the text
     * @param limit
     * 		the limit of the line relative to the text
     * @param dir
     * 		the paragraph direction of this line
     * @param directions
     * 		the directions information of this line
     * @param hasTabs
     * 		true if the line might contain tabs
     * @param tabStops
     * 		the tabStops. Can be null.
     */
    void set(android.text.TextPaint paint, java.lang.CharSequence text, int start, int limit, int dir, android.text.Layout.Directions directions, boolean hasTabs, android.text.Layout.TabStops tabStops) {
        mPaint = paint;
        mText = text;
        mStart = start;
        mLen = limit - start;
        ...
        if (mCharsValid) {
            if ((mChars == null) || (mChars.length < mLen)) {
                mChars = com.android.internal.util.ArrayUtils.newUnpaddedCharArray(mLen);
            }
     ...
     */
    void draw(android.graphics.Canvas c, float x, int top, int y, int bottom) {
        if (!mHasTabs) {
        ...
        for (int i = 0; i < runs.length; i += 2) {
            ...
            boolean runIsRtl = (runs[i + 1] & android.text.Layout.RUN_RTL_FLAG) != 0;
            ...
            for (int j = (mHasTabs) ? runStart : runLimit; j <= runLimit; j++) {
                ...
                if (mHasTabs && (j < runLimit)) {
                    ...
                    if (((codept >= 0xd800) && (codept < 0xdc00)) && ((j + 1) < runLimit)) {
                        codept = java.lang.Character.codePointAt(mChars, j);
                        if (codept > 0xffff) {
                            ++j;
                            continue;
                        }
     ...
     *
     * @param fmi
     * 		receives font metrics information, can be null
     * @return the signed width of the line
     */
    float metrics(android.graphics.Paint.FontMetricsInt fmi) {
    ...
    character edge.
     */
    float measure(int offset, boolean trailing, android.graphics.Paint.FontMetricsInt fmi) {
        ...
        for (int i = 0; i < runs.length; i += 2) {
            ...
            for (int j = (mHasTabs) ? runStart : runLimit; j <= runLimit; j++) {
                ...
                if (mHasTabs && (j < runLimit)) {
                    codept = chars[j];
                    if (((codept >= 0xd800) && (codept < 0xdc00)) && ((j + 1) < runLimit)) {
                        codept = java.lang.Character.codePointAt(chars, j);
                        if (codept > 0xffff) {
                            ++j;
                            continue;
                        }
                ...
                if ((j == runLimit) || (codept == '\t')) {
                    ...
                    boolean advance = (mDir == android.text.Layout.DIR_RIGHT_TO_LEFT) == runIsRtl;
     ...
     * 		the bottom of the line
     * @param needWidth
     * 		true if the width value is required.
     * @return the signed width of the run, based on the paragraph direction.
    Only valid if needWidth is true.
     */
    private float drawRun(android.graphics.Canvas c, int start, int limit, boolean runIsRtl, float x, int top, int y, int bottom, boolean needWidth) {
     ...
     *
     * @param start
     * 		the line-relative start of the run
     * @param offset
     * 		the offset to measure to, between start and limit inclusive
     * @param limit
     * 		the line-relative limit of the run
     * @param runIsRtl
     * 		true if the run is right-to-left
     * @param fmi
     * 		receives metrics information about the requested
     ...
     */
    private float measureRun(int start, int offset, int limit, boolean runIsRtl, android.graphics.Paint.FontMetricsInt fmi) {
     ...
     * @param toLeft
     * 		true if the caret is moving to the left.
    ...
    int getOffsetToLeftRightOf(int cursor, boolean toLeft) {
        ...
        while (true) {
            ...
            if ((otherRunIndex >= 0) && (otherRunIndex < runs.length)) {
                ...
                int otherRunLimit = otherRunStart + (runs[otherRunIndex + 1] & android.text.Layout.RUN_LENGTH_MASK);
                if (otherRunLimit > lineEnd) {
                    otherRunLimit = lineEnd;
                }
                int otherRunLevel = (runs[otherRunIndex + 1] >>> android.text.Layout.RUN_LEVEL_SHIFT) & android.text.Layout.RUN_LEVEL_MASK;
    ...
    private int getOffsetBeforeAfter(int runIndex, int runStart, int runLimit, boolean runIsRtl, int offset, boolean after) {
        ...
        if (mSpanned == null) {
            ...
            while (true) {
                spanLimit = mSpanned.nextSpanTransition(mStart + spanStart, limit, android.text.style.MetricAffectingSpan.class) - mStart;
                if (spanLimit >= target) {
                    break;
                }
                spanStart = spanLimit;
            } 
            android.text.style.MetricAffectingSpan[] spans = mSpanned.getSpans(mStart + spanStart, mStart + spanLimit, android.text.style.MetricAffectingSpan.class);
            spans = android.text.TextUtils.removeEmptySpans(spans, mSpanned, android.text.style.MetricAffectingSpan.class);
            if (spans.length > 0) {
        ...
        int dir = (runIsRtl) ? android.graphics.Paint.DIRECTION_RTL : android.graphics.Paint.DIRECTION_LTR;
        int cursorOpt = (after) ? android.graphics.Paint.CURSOR_AFTER : android.graphics.Paint.CURSOR_BEFORE;
        if (mCharsValid) {
     ...
     * 		
     */
    private static void expandMetricsFromPaint(android.graphics.Paint.FontMetricsInt fmi, android.text.TextPaint wp) {
        ...
        android.text.TextLine.updateMetrics(fmi, previousTop, previousAscent, previousDescent, previousBottom, previousLeading);
    }
    ...
    static void updateMetrics(android.graphics.Paint.FontMetricsInt fmi, int previousTop, int previousAscent, int previousDescent, int previousBottom, int previousLeading) {
        fmi.top = java.lang.Math.min(fmi.top, previousTop);
        fmi.ascent = java.lang.Math.min(fmi.ascent, previousAscent);
        fmi.descent = java.lang.Math.max(fmi.descent, previousDescent);
        fmi.bottom = java.lang.Math.max(fmi.bottom, previousBottom);
        fmi.leading = java.lang.Math.max(fmi.leading, previousLeading);
    }
     ...
     * 		true if the run is right-to-left
     * @param c
     * 		the canvas, can be null if rendering is not needed
     * @param x
     * 		the edge of the run closest to the leading margin
     * @param top
     * 		the top of the line
     * @param y
     * 		the baseline
     ...
     * 		the bottom of the line
     * @param fmi
     * 		receives metrics information, can be null
     * @param needWidth
     * 		true if the width of the run is needed
     * @param offset
     * 		the offset for the purpose of measuring
     * @return the signed width of the run based on the run direction; only
    valid if needWidth is true
     */
    private float handleText(android.text.TextPaint wp, int start, int end, int contextStart, int contextEnd, boolean runIsRtl, android.graphics.Canvas c, float x, int top, int y, int bottom, android.graphics.Paint.FontMetricsInt fmi, boolean needWidth, int offset) {
        // Get metrics first (even for empty strings or "0" width runs)
        if (fmi != null) {
            android.text.TextLine.expandMetricsFromPaint(fmi, wp);
        }
     ...
     * 		true if the run is right-to-left
     * @param c
     * 		the canvas, can be null if not rendering
     * @param x
     * 		the edge of the replacement closest to the leading margin
     * @param top
     * 		the top of the line
     * @param y
     * 		the baseline
     ...
     * 		receives metrics information, can be null
     * @param needWidth
     * 		true if the width of the replacement is needed
     * @return the signed width of the run based on the run direction; only
    valid if needWidth is true
     */
    private float handleReplacement(android.text.style.ReplacementSpan replacement, android.text.TextPaint wp, int start, int limit, boolean runIsRtl, android.graphics.Canvas c, float x, int top, int y, int bottom, android.graphics.Paint.FontMetricsInt fmi, boolean needWidth) {
        float ret = 0;
     ...
     * @param x
     * 		the end of the run closest to the leading margin
     * @param top
     * 		the top of the line
     * @param y
     * 		the baseline
     ...
     * 		receives metrics information, can be null
     * @param needWidth
     * 		true if the width is required
     * @return the signed width of the run based on the run direction; only
    valid if needWidth is true
     */
    private float handleRun(int start, int measureLimit, int limit, boolean runIsRtl, android.graphics.Canvas c, float x, int top, int y, int bottom, android.graphics.Paint.FontMetricsInt fmi, boolean needWidth) {
        ...
        for (int i = start, inext; i < measureLimit; i = inext) {
            ...
            int mlimit = java.lang.Math.min(inext, measureLimit);
            android.text.style.ReplacementSpan replacement = null;
            for (int j = 0; j < mMetricAffectingSpanSpanSet.numberOfSpans; j++) {
                ...
                android.text.style.MetricAffectingSpan span = mMetricAffectingSpanSpanSet.spans[j];
                if (span instanceof android.text.style.ReplacementSpan) {
                    replacement = ((android.text.style.ReplacementSpan) (span));
                } else {
            ...
            for (int j = i, jnext; j < mlimit; j = jnext) {
                ...
                int offset = java.lang.Math.min(jnext, mlimit);
                wp.set(mPaint);
                for (int k = 0; k < mCharacterStyleSpanSet.numberOfSpans; k++) {
                    ...
                    android.text.style.CharacterStyle span = mCharacterStyleSpanSet.spans[k];
                    span.updateDrawState(wp);
                }
     ...
     * 		the canvas
     * @param wp
     * 		the paint used to render the text
     * @param start
     * 		the start of the run
     * @param end
     * 		the end of the run
     * @param contextStart
     * 		the start of context for the run
     * @param contextEnd
     * 		the end of the context for the run
     * @param runIsRtl
     * 		true if the run is right-to-left
     * @param x
     * 		the x position of the left edge of the run
     * @param y
     * 		the baseline of the run
     */
    private void drawTextRun(android.graphics.Canvas c, android.text.TextPaint wp, int start, int end, int contextStart, int contextEnd, boolean runIsRtl, float x, int y) {
        if (mCharsValid) {
            int count = end - start;
     ...
     *
     * @param h
     * 		the (unsigned) offset from the leading margin
    ...
    float nextTab(float h) {
        ...
        return android.text.Layout.TabStops.nextDefaultStop(h, android.text.TextLine.TAB_INCREMENT);
    }
