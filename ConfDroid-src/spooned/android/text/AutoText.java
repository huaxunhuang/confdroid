...
public class AutoText {
    ...
    private static android.text.AutoText getInstance(android.view.View view) {
        ...
        java.util.Locale locale = res.getConfiguration().locale;
        android.text.AutoText instance;
        synchronized(android.text.AutoText.sLock) {
            instance = android.text.AutoText.sInstance;
            if (!locale.equals(instance.mLocale)) {
                instance = new android.text.AutoText(res);
                android.text.AutoText.sInstance = instance;
            }
        }
        return instance;
    }
     ...
     */
    public static java.lang.String get(java.lang.CharSequence src, final int start, final int end, android.view.View view) {
        return android.text.AutoText.getInstance(view).lookup(src, start, end);
    }
    ...
    public static int getSize(android.view.View view) {
        return android.text.AutoText.getInstance(view).getSize();
    }
    ...
    private java.lang.String lookup(java.lang.CharSequence src, final int start, final int end) {
        int here = mTrie[android.text.AutoText.TRIE_ROOT];
        for (int i = start; i < end; i++) {
            char c = src.charAt(i);
            for (; here != android.text.AutoText.TRIE_NULL; here = mTrie[here + android.text.AutoText.TRIE_NEXT]) {
                if (c == mTrie[here + android.text.AutoText.TRIE_C]) {
                    if ((i == (end - 1)) && (mTrie[here + android.text.AutoText.TRIE_OFF] != android.text.AutoText.TRIE_NULL)) {
                        int off = mTrie[here + android.text.AutoText.TRIE_OFF];
                        int len = mText.charAt(off);
                        return mText.substring(off + 1, (off + 1) + len);
                    }
                    here = mTrie[here + android.text.AutoText.TRIE_CHILD];
                    break;
                }
            }
            if (here == android.text.AutoText.TRIE_NULL) {
                return null;
            }
        }
        return null;
    ...
    private void init(android.content.res.Resources r) {
        android.content.res.XmlResourceParser parser = r.getXml(com.android.internal.R.xml.autotext);
        java.lang.StringBuilder right = new java.lang.StringBuilder(android.text.AutoText.RIGHT);
        mTrie = new char[android.text.AutoText.DEFAULT];
        mTrie[android.text.AutoText.TRIE_ROOT] = android.text.AutoText.TRIE_NULL;
        mTrieUsed = android.text.AutoText.TRIE_ROOT + 1;
        try {
            com.android.internal.util.XmlUtils.beginDocument(parser, "words");
            java.lang.String odest = "";
            char ooff = 0;
            while (true) {
                ...
                java.lang.String src = parser.getAttributeValue(null, "src");
                if (parser.next() == org.xmlpull.v1.XmlPullParser.TEXT) {
                    java.lang.String dest = parser.getText();
                    char off;
    ...
    private void add(java.lang.String src, char off) {
        int slen = src.length();
        int herep = android.text.AutoText.TRIE_ROOT;
        // Keep track of the size of the dictionary
        mSize++;
        for (int i = 0; i < slen; i++) {
            char c = src.charAt(i);
            boolean found = false;
            for (; mTrie[herep] != android.text.AutoText.TRIE_NULL; herep = mTrie[herep] + android.text.AutoText.TRIE_NEXT) {
                if (c == mTrie[mTrie[herep] + android.text.AutoText.TRIE_C]) {
                    // There is a node for this letter, and this is the
                    // end, so fill in the right hand side fields.
                    if (i == (slen - 1)) {
                        mTrie[mTrie[herep] + android.text.AutoText.TRIE_OFF] = off;
                        return;
                    }
                    // There is a node for this letter, and we need
                    // to go deeper into it to fill in the rest.
                    herep = mTrie[herep] + android.text.AutoText.TRIE_CHILD;
                    found = true;
                    break;
                }
            }
            if (!found) {
                ...
                mTrie[herep] = node;
                mTrie[mTrie[herep] + android.text.AutoText.TRIE_C] = c;
                mTrie[mTrie[herep] + android.text.AutoText.TRIE_OFF] = android.text.AutoText.TRIE_NULL;
                mTrie[mTrie[herep] + android.text.AutoText.TRIE_NEXT] = android.text.AutoText.TRIE_NULL;
                mTrie[mTrie[herep] + android.text.AutoText.TRIE_CHILD] = android.text.AutoText.TRIE_NULL;
    ...
    private char newTrieNode() {
        if ((mTrieUsed + android.text.AutoText.TRIE_SIZEOF) > mTrie.length) {
            char[] copy = new char[mTrie.length + android.text.AutoText.INCREMENT];
            java.lang.System.arraycopy(mTrie, 0, copy, 0, mTrie.length);
            mTrie = copy;
        }
        char ret = mTrieUsed;
        mTrieUsed += android.text.AutoText.TRIE_SIZEOF;
        return ret;
    }
}
