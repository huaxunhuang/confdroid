...
public class Time {
     ...
     */
    public static final int MONDAY_BEFORE_JULIAN_EPOCH = android.text.format.Time.EPOCH_JULIAN_DAY - 3;
     ...
     * @see TimeZone
     */
    public Time(java.lang.String timezoneId) {
        if (timezoneId == null) {
    ...
    private void initialize(java.lang.String timezoneId) {
        ...
        calculator = new android.text.format.Time.TimeCalculator(timezoneId);
    }
    ...
    public int getActualMaximum(int field) {
        switch (field) {
            case android.text.format.Time.SECOND :
                return 59;// leap seconds, bah humbug

            case android.text.format.Time.MINUTE :
                return 59;
            case android.text.format.Time.HOUR :
                return 23;
            case android.text.format.Time.MONTH_DAY :
                {
                    int n = android.text.format.Time.DAYS_PER_MONTH[this.month];
                    if (n != 28) {
                        return n;
                    } else {
                        int y = this.year;
            ...
            case android.text.format.Time.MONTH :
                return 11;
            case android.text.format.Time.YEAR :
                return 2037;
            case android.text.format.Time.WEEK_DAY :
                return 6;
            case android.text.format.Time.YEAR_DAY :
                {
                    int y = this.year;
                    // Year days are numbered from 0, so the last one is usually 364.
                    return ((y % 4) == 0) && (((y % 100) != 0) || ((y % 400) == 0)) ? 365 : 364;
                }
            case android.text.format.Time.WEEK_NUM :
                throw new java.lang.RuntimeException("WEEK_NUM not implemented");
            default :
                throw new java.lang.RuntimeException("bad field=" + field);
        }
    }
    ...
    /**
     * Compare two {@code Time} objects and return a negative number if {@code a} is less than {@code b}, a positive number if {@code a} is greater than
     ...
     *
     * @param a
     * 		first {@code Time} instance to compare
     * @param b
     * 		second {@code Time} instance to compare
     * @throws NullPointerException
     * 		if either argument is {@code null}
     * @throws IllegalArgumentException
     * 		if {@link #allDay} is true but {@code hour}, {@code minute}, and {@code second} are not 0.
    ...
    public static int compare(android.text.format.Time a, android.text.format.Time b) {
        ...
        return android.text.format.Time.TimeCalculator.compare(a.calculator, b.calculator);
    }
    ...
    public java.lang.String toString() {
        ...
        android.text.format.Time.TimeCalculator calculator = new android.text.format.Time.TimeCalculator(this.timezone);
        calculator.copyFieldsFromTime(this);
    ...
    public boolean parse(java.lang.String s) {
        ...
        if (parseInternal(s)) {
            timezone = android.text.format.Time.TIMEZONE_UTC;
            return true;
        }
        return false;
    }
    ...
    private boolean parseInternal(java.lang.String s) {
        int len = s.length();
        if (len < 8) {
            throw new android.util.TimeFormatException(("String is too short: \"" + s) + "\" Expected at least 8 characters.");
        }
        boolean inUtc = false;
        // year
        int n = android.text.format.Time.getChar(s, 0, 1000);
        n += android.text.format.Time.getChar(s, 1, 100);
        n += android.text.format.Time.getChar(s, 2, 10);
        n += android.text.format.Time.getChar(s, 3, 1);
        year = n;
        // month
        n = android.text.format.Time.getChar(s, 4, 10);
        n += android.text.format.Time.getChar(s, 5, 1);
        n--;
        month = n;
        // day of month
        n = android.text.format.Time.getChar(s, 6, 10);
        n += android.text.format.Time.getChar(s, 7, 1);
        monthDay = n;
        if (len > 8) {
            if (len < 15) {
                throw new android.util.TimeFormatException((("String is too short: \"" + s) + "\" If there are more than 8 characters there must be at least") + " 15.");
            }
            checkChar(s, 8, 'T');
            allDay = false;
            // hour
            n = android.text.format.Time.getChar(s, 9, 10);
            n += android.text.format.Time.getChar(s, 10, 1);
            hour = n;
            // min
            n = android.text.format.Time.getChar(s, 11, 10);
            n += android.text.format.Time.getChar(s, 12, 1);
            minute = n;
            // sec
            n = android.text.format.Time.getChar(s, 13, 10);
            n += android.text.format.Time.getChar(s, 14, 1);
            second = n;
            if (len > 15) {
                // Z
                checkChar(s, 15, 'Z');
    ...
    private void checkChar(java.lang.String s, int spos, char expected) {
        ...
        if (c != expected) {
            throw new android.util.TimeFormatException(java.lang.String.format("Unexpected character 0x%02d at pos=%d.  Expected 0x%02d (\'%c\').", ((int) (c)), spos, ((int) (expected)), expected));
        }
    }
    ...
    private static int getChar(java.lang.String s, int spos, int mul) {
        ...
        if (java.lang.Character.isDigit(c)) {
            return java.lang.Character.getNumericValue(c) * mul;
        } else {
            throw new android.util.TimeFormatException("Parse error at pos=" + spos);
        }
    }
    ...
    private boolean parse3339Internal(java.lang.String s) {
        ...
        checkChar(s, 4, '-');
        // month
        n = android.text.format.Time.getChar(s, 5, 10);
        n += android.text.format.Time.getChar(s, 6, 1);
        --n;
        month = n;
        checkChar(s, 7, '-');
        // day
        n = android.text.format.Time.getChar(s, 8, 10);
        n += android.text.format.Time.getChar(s, 9, 1);
        monthDay = n;
        if (len >= 19) {
            // T
            checkChar(s, 10, 'T');
            ...
            checkChar(s, 13, ':');
            // minute
            n = android.text.format.Time.getChar(s, 14, 10);
            n += android.text.format.Time.getChar(s, 15, 1);
            // Note that this.minute is not set here. It is set later.
            int minute = n;
            checkChar(s, 16, ':');
            // second
            n = android.text.format.Time.getChar(s, 17, 10);
            n += android.text.format.Time.getChar(s, 18, 1);
            second = n;
            ...
            if ((tzIndex < len) && (s.charAt(tzIndex) == '.')) {
                do {
                    tzIndex++;
                } while ((tzIndex < len) && java.lang.Character.isDigit(s.charAt(tzIndex)) );
            }
            int offset = 0;
            if (len > tzIndex) {
                ...
                switch (c) {
                    ...
                    case '+' :
                        ...
                        break;
                    default :
                        throw new android.util.TimeFormatException(java.lang.String.format("Unexpected character 0x%02d at position %d.  Expected + or -", ((int) (c)), tzIndex));
                }
                inUtc = true;
                if (offset != 0) {
                    if (len < (tzIndex + 6)) {
                        throw new android.util.TimeFormatException(java.lang.String.format("Unexpected length; should be %d characters", tzIndex + 6));
                    }
                    // hour
                    n = android.text.format.Time.getChar(s, tzIndex + 1, 10);
                    n += android.text.format.Time.getChar(s, tzIndex + 2, 1);
                    n *= offset;
                    hour += n;
                    // minute
                    n = android.text.format.Time.getChar(s, tzIndex + 4, 10);
                    n += android.text.format.Time.getChar(s, tzIndex + 5, 1);
                    n *= offset;
                    minute += n;
                }
            }
    ...
    public int getWeekNumber() {
        ...
        int closestThursday = yearDay + android.text.format.Time.sThursdayOffset[weekDay];
        // Year days start at 0
        if ((closestThursday >= 0) && (closestThursday <= 364)) {
            return (closestThursday / 7) + 1;
        }
        // The week crosses a year boundary.
        android.text.format.Time temp = new android.text.format.Time(this);
        temp.monthDay += android.text.format.Time.sThursdayOffset[weekDay];
        /* ignore isDst */
        temp.normalize(true);
    ...
    public java.lang.String format3339(boolean allDay) {
        if (allDay) {
            return format(android.text.format.Time.Y_M_D);
        } else
            if (android.text.format.Time.TIMEZONE_UTC.equals(timezone)) {
                return format(android.text.format.Time.Y_M_D_T_H_M_S_000_Z);
            } else {
                java.lang.String base = format(android.text.format.Time.Y_M_D_T_H_M_S_000);
                java.lang.String sign = (gmtoff < 0) ? "-" : "+";
                int offset = ((int) (java.lang.Math.abs(gmtoff)));
                int minutes = (offset % 3600) / 60;
                int hours = offset / 3600;
                return java.lang.String.format(java.util.Locale.US, "%s%s%02d:%02d", base, sign, hours, minutes);
            }
    ...
    public static boolean isEpoch(android.text.format.Time time) {
        ...
        return android.text.format.Time.getJulianDay(millis, 0) == android.text.format.Time.EPOCH_JULIAN_DAY;
    }
    ...
    public static int getJulianDay(long millis, long gmtoff) {
        ...
        long julianDay = (millis + offsetMillis) / android.text.format.DateUtils.DAY_IN_MILLIS;
        return ((int) (julianDay)) + android.text.format.Time.EPOCH_JULIAN_DAY;
    }
    ...
    public long setJulianDay(int julianDay) {
        ...
        // the day.
        long millis = (julianDay - android.text.format.Time.EPOCH_JULIAN_DAY) * android.text.format.DateUtils.DAY_IN_MILLIS;
        set(millis);
        // Figure out how close we are to the requested Julian day.
        // We can't be off by more than a day.
        int approximateDay = android.text.format.Time.getJulianDay(millis, gmtoff);
    ...
    public static int getWeeksSinceEpochFromJulianDay(int julianDay, int firstDayOfWeek) {
        int diff = android.text.format.Time.THURSDAY - firstDayOfWeek;
        if (diff < 0) {
            diff += 7;
        }
        int refDay = android.text.format.Time.EPOCH_JULIAN_DAY - diff;
    ...
    public static int getJulianMondayFromWeeksSinceEpoch(int week) {
        return android.text.format.Time.MONDAY_BEFORE_JULIAN_EPOCH + (week * 7);
    }
    ...
    private static class TimeCalculator {
        ...
        public TimeCalculator(java.lang.String timezoneId) {
            this.zoneInfo = android.text.format.Time.TimeCalculator.lookupZoneInfo(timezoneId);
            this.wallTime = new libcore.util.ZoneInfo.WallTime();
        }
        ...
        public java.lang.String format(java.lang.String format) {
            ...
            android.text.format.TimeFormatter formatter = new android.text.format.TimeFormatter();
            return formatter.format(format, wallTime, zoneInfo);
        }

        private void updateZoneInfoFromTimeZone() {
            if (!zoneInfo.getID().equals(timezone)) {
                this.zoneInfo = android.text.format.Time.TimeCalculator.lookupZoneInfo(timezone);
            }
        }

        private static libcore.util.ZoneInfo lookupZoneInfo(java.lang.String timezoneId) {
            try {
                libcore.util.ZoneInfo zoneInfo = libcore.util.ZoneInfoDB.getInstance().makeTimeZone(timezoneId);
                if (zoneInfo == null) {
                    zoneInfo = libcore.util.ZoneInfoDB.getInstance().makeTimeZone("GMT");
                }
                if (zoneInfo == null) {
                    throw new java.lang.AssertionError(("GMT not found: \"" + timezoneId) + "\"");
                }
                return zoneInfo;
            } catch (java.io.IOException e) {
                // This should not ever be thrown.
                throw new java.lang.AssertionError(("Error loading timezone: \"" + timezoneId) + "\"", e);
            }
        }
        ...
        public java.lang.String format2445(boolean hasTime) {
            ...
            buf[7] = toChar(n % 10);
            if (!hasTime) {
                return new java.lang.String(buf, 0, 8);
            }
            buf[8] = 'T';
            n = wallTime.getHour();
            buf[9] = toChar(n / 10);
            ...
            if (android.text.format.Time.TIMEZONE_UTC.equals(timezone)) {
                ...
                buf[15] = 'Z';
                return new java.lang.String(buf, 0, 16);
            } else {
                return new java.lang.String(buf, 0, 15);
            }
        }
        ...
        public java.lang.String toStringInternal() {
            ...
            return java.lang.String.format("%04d%02d%02dT%02d%02d%02d%s(%d,%d,%d,%d,%d)", wallTime.getYear(), wallTime.getMonth() + 1, wallTime.getMonthDay(), wallTime.getHour(), wallTime.getMinute(), wallTime.getSecond(), timezone, wallTime.getWeekDay(), wallTime.getYearDay(), wallTime.getGmtOffset(), wallTime.getIsDst(), /* use isDst */
            toMillis(false) / 1000);
        }

        public static int compare(android.text.format.Time.TimeCalculator aObject, android.text.format.Time.TimeCalculator bObject) {
            if (aObject.timezone.equals(bObject.timezone)) {
                ...
                long am = /* use isDst */
                aObject.toMillis(false);
                long bm = /* use isDst */
                bObject.toMillis(false);
                long diff = am - bm;
