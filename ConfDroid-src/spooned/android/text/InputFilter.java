    ...
    public static class AllCaps implements android.text.InputFilter {
        public java.lang.CharSequence filter(java.lang.CharSequence source, int start, int end, android.text.Spanned dest, int dstart, int dend) {
            for (int i = start; i < end; i++) {
                if (java.lang.Character.isLowerCase(source.charAt(i))) {
                    char[] v = new char[end - start];
                    android.text.TextUtils.getChars(source, start, end, v, 0);
                    java.lang.String s = new java.lang.String(v).toUpperCase();
                    if (source instanceof android.text.Spanned) {
                        android.text.SpannableString sp = new android.text.SpannableString(s);
                        android.text.TextUtils.copySpansFrom(((android.text.Spanned) (source)), start, end, null, sp, 0);
                        return sp;
                    } else {
                        return s;
                    }
        ...
        public java.lang.CharSequence filter(java.lang.CharSequence source, int start, int end, android.text.Spanned dest, int dstart, int dend) {
            ...
            if (keep <= 0) {
                ...
                if (keep >= (end - start)) {
                    ...
                    keep += start;
                    if (java.lang.Character.isHighSurrogate(source.charAt(keep - 1))) {
                        --keep;
                        if (keep == start) {
                            return "";
                        }
