...
public final class MtpDevice {
     ...
     *
     * @param device
     * 		the {@link android.hardware.usb.UsbDevice} for the MTP or PTP device
     */
     ...
     *
     * @param connection
     * 		an open {@link android.hardware.usb.UsbDeviceConnection} for the device
     * @return true if the device was successfully opened.
     */
    public boolean open(android.hardware.usb.UsbDeviceConnection connection) {
        boolean result = false;
        android.content.Context context = connection.getContext();
        if (context != null) {
            android.os.UserManager userManager = ((android.os.UserManager) (context.getSystemService(android.content.Context.USER_SERVICE)));
            if (!userManager.hasUserRestriction(android.os.UserManager.DISALLOW_USB_FILE_TRANSFER)) {
    ...
    public byte[] getObject(int objectHandle, int objectSize) {
        com.android.internal.util.Preconditions.checkArgumentNonnegative(objectSize, "objectSize should not be negative");
     ...
     */
    public long getPartialObject(int objectHandle, long offset, long size, byte[] buffer) throws java.io.IOException {
     ...
     */
    public long getPartialObject64(int objectHandle, long offset, long size, byte[] buffer) throws java.io.IOException {
     ...
     */
    public boolean importFile(int objectHandle, java.lang.String destPath) {
     ...
     * @return true if the file transfer succeeds
     */
    public boolean importFile(int objectHandle, android.os.ParcelFileDescriptor descriptor) {
     ...
     * @return true if the file transfer succeeds
     */
    public boolean sendObject(int objectHandle, long size, android.os.ParcelFileDescriptor descriptor) {
     ...
     */
    public android.mtp.MtpObjectInfo sendObjectInfo(android.mtp.MtpObjectInfo info) {
    ...
    public android.mtp.MtpEvent readEvent(@android.annotation.Nullable
    android.os.CancellationSignal signal) throws java.io.IOException {
        final int handle = native_submit_event_request();
        com.android.internal.util.Preconditions.checkState(handle >= 0, "Other thread is reading an event.");
        if (signal != null) {
     ...
     * @unknown 
     */
    public long getObjectSizeLong(int handle, int format) throws java.io.IOException {
    ...
    private native android.mtp.MtpStorageInfo native_get_storage_info(int storageId);

    private native int[] native_get_object_handles(int storageId, int format, int objectHandle);

    private native android.mtp.MtpObjectInfo native_get_object_info(int objectHandle);

    private native byte[] native_get_object(int objectHandle, long objectSize);

    private native long native_get_partial_object(int objectHandle, long offset, long objectSize, byte[] buffer) throws java.io.IOException;

    private native int native_get_partial_object_64(int objectHandle, long offset, long objectSize, byte[] buffer) throws java.io.IOException;
    ...
    private native android.mtp.MtpObjectInfo native_send_object_info(android.mtp.MtpObjectInfo info);

    private native int native_submit_event_request() throws java.io.IOException;

    private native android.mtp.MtpEvent native_reap_event_request(int handle) throws java.io.IOException;

    private native void native_discard_event_request(int handle);

    private native long native_get_object_size_long(int handle, int format) throws java.io.IOException;
}
