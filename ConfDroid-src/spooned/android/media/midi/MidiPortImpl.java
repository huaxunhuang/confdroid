...
class MidiPortImpl {
     ...
     */
    private static final int DATA_PACKET_OVERHEAD = android.media.midi.MidiPortImpl.TIMESTAMP_SIZE + 1;

    /**
     * Maximum amount of MIDI data that can be included in a packet
     */
    public static final int MAX_PACKET_DATA_SIZE = android.media.midi.MidiPortImpl.MAX_PACKET_SIZE - android.media.midi.MidiPortImpl.DATA_PACKET_OVERHEAD;
    ...
    public static int packData(byte[] message, int offset, int size, long timestamp, byte[] dest) {
        if (size > android.media.midi.MidiPortImpl.MAX_PACKET_DATA_SIZE) {
            size = android.media.midi.MidiPortImpl.MAX_PACKET_DATA_SIZE;
        }
        int length = 0;
        // packet type goes first
        dest[length++] = android.media.midi.MidiPortImpl.PACKET_TYPE_DATA;
        // data goes next
        java.lang.System.arraycopy(message, offset, dest, length, size);
        length += size;
        // followed by timestamp
        for (int i = 0; i < android.media.midi.MidiPortImpl.TIMESTAMP_SIZE; i++) {
    ...
    public static int packFlush(byte[] dest) {
        dest[0] = android.media.midi.MidiPortImpl.PACKET_TYPE_FLUSH;
        return 1;
    }
    ...
    public static int getDataSize(byte[] buffer, int bufferLength) {
        ...
        return bufferLength - android.media.midi.MidiPortImpl.DATA_PACKET_OVERHEAD;
    }
