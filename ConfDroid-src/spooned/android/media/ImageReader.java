...
public class ImageReader implements java.lang.AutoCloseable {
     ...
     * @see Image
     */
    public static android.media.ImageReader newInstance(int width, int height, int format, int maxImages) {
        return new android.media.ImageReader(width, height, format, maxImages);
    }
    ...
    protected ImageReader(int width, int height, int format, int maxImages) {
        ...
        mNumPlanes = android.media.ImageUtils.getNumPlanesForFormat(mFormat);
        nativeInit(new java.lang.ref.WeakReference<android.media.ImageReader>(this), width, height, format, maxImages);
        ...
        android.media.ImageUtils.getEstimatedNativeAllocBytes(width, height, format, 1);
    ...
    public android.media.Image acquireLatestImage() {
        android.media.Image image = acquireNextImage();
        if (image == null) {
            return null;
        }
        try {
            for (; ;) {
                android.media.Image next = acquireNextImageNoThrowISE();
                if (next == null) {
                    android.media.Image result = image;
                    image = null;
                    return result;
                }
                image.close();
    ...
    public android.media.Image acquireNextImageNoThrowISE() {
        android.media.ImageReader.SurfaceImage si = new android.media.ImageReader.SurfaceImage(mFormat);
        return acquireNextSurfaceImage(si) == android.media.ImageReader.ACQUIRE_SUCCESS ? si : null;
    }
     ...
     * @see #ACQUIRE_SUCCESS
     */
    private int acquireNextSurfaceImage(android.media.ImageReader.SurfaceImage si) {
        synchronized(mCloseLock) {
            // A null image will eventually be returned if ImageReader is already closed.
            int status = android.media.ImageReader.ACQUIRE_NO_BUFS;
            if (mIsReaderValid) {
                status = nativeImageSetup(si);
            }
            switch (status) {
                case android.media.ImageReader.ACQUIRE_SUCCESS :
                    si.mIsImageValid = true;
                case android.media.ImageReader.ACQUIRE_NO_BUFS :
                case android.media.ImageReader.ACQUIRE_MAX_IMAGES :
                    break;
                default :
                    throw new java.lang.AssertionError("Unknown nativeImageSetup return code " + status);
            }
            // Only keep track the successfully acquired image, as the native buffer is only mapped
            // for such case.
            if (status == android.media.ImageReader.ACQUIRE_SUCCESS) {
     ...
     * @throws IllegalStateException
     * 		if {@code maxImages} images are currently acquired
    ...
    public android.media.Image acquireNextImage() {
        ...
        switch (status) {
            ...
            case android.media.ImageReader.ACQUIRE_NO_BUFS :
                return null;
            case android.media.ImageReader.ACQUIRE_MAX_IMAGES :
                throw new java.lang.IllegalStateException(java.lang.String.format("maxImages (%d) has already been acquired, " + "call #close before acquiring more.", mMaxImages));
            default :
                throw new java.lang.AssertionError("Unknown nativeImageSetup return code " + status);
        }
    }
     ...
     */
    private void releaseImage(android.media.Image i) {
        if (!(i instanceof android.media.ImageReader.SurfaceImage)) {
            throw new java.lang.IllegalArgumentException("This image was not produced by an ImageReader");
        }
        android.media.ImageReader.SurfaceImage si = ((android.media.ImageReader.SurfaceImage) (i));
     ...
     */
    public void setOnImageAvailableListener(android.media.ImageReader.OnImageAvailableListener listener, android.os.Handler handler) {
        synchronized(mListenerLock) {
            if (listener != null) {
                ...
                if ((mListenerHandler == null) || (mListenerHandler.getLooper() != looper)) {
                    mListenerHandler = new android.media.ImageReader.ListenerHandler(looper);
                }
                mListener = listener;
            } else {
                mListener = null;
    ...
    public interface OnImageAvailableListener {
         ...
         * @see Image
         */
        void onImageAvailable(android.media.ImageReader reader);
    }
    ...
    public void close() {
        ...
        synchronized(mCloseLock) {
            ...
            for (android.media.Image image : mAcquiredImages) {
                image.close();
            }
     ...
     *
     * @param image
     * 		The image to be detached from this ImageReader.
     * @throws IllegalStateException
     * 		If the ImageReader or image have been
     * 		closed, or the has been detached, or has not yet been
     * 		acquired.
     */
    void detachImage(android.media.Image image) {
        if (image == null) {
        ...
        android.media.ImageReader.SurfaceImage si = ((android.media.ImageReader.SurfaceImage) (image));
    ...
    private static void postEventFromNative(java.lang.Object selfRef) {
        ...
        java.lang.ref.WeakReference<android.media.ImageReader> weakSelf = ((java.lang.ref.WeakReference<android.media.ImageReader>) (selfRef));
        final android.media.ImageReader ir = weakSelf.get();
        if (ir == null) {
            return;
        }
    ...
    private java.util.List<android.media.Image> mAcquiredImages = new java.util.concurrent.CopyOnWriteArrayList<android.media.Image>();
    ...
    private final class ListenerHandler extends android.os.Handler {
        ...
        public void handleMessage(android.os.Message msg) {
            ...
            if ((listener != null) && isReaderValid) {
                listener.onImageAvailable(android.media.ImageReader.this);
            }
        }
    }
    ...
    private class SurfaceImage extends android.media.Image {
        ...
        public int getFormat() {
            ...
            int readerFormat = android.media.ImageReader.this.getImageFormat();
            // Assume opaque reader always produce opaque images.
            mFormat = (readerFormat == android.graphics.ImageFormat.PRIVATE) ? readerFormat : nativeGetFormat(readerFormat);
            return mFormat;
        }
        ...
        public int getWidth() {
            ...
            switch (getFormat()) {
                ...
                case android.graphics.ImageFormat.RAW_PRIVATE :
                    width = android.media.ImageReader.this.getWidth();
                    break;
                default :
        ...
        public int getHeight() {
            ...
            switch (getFormat()) {
                ...
                case android.graphics.ImageFormat.DEPTH_POINT_CLOUD :
                case android.graphics.ImageFormat.RAW_PRIVATE :
                    height = android.media.ImageReader.this.getHeight();
                    break;
                default :
        ...
        public android.media.Image.Plane[] getPlanes() {
            throwISEIfImageIsInvalid();
            if (mPlanes == null) {
                mPlanes = nativeCreatePlanes(android.media.ImageReader.this.mNumPlanes, android.media.ImageReader.this.mFormat);
            }
        ...
        private class SurfacePlane extends android.media.Image.Plane {
            ...
            public int getPixelStride() {
                ...
                if (android.media.ImageReader.this.mFormat == android.graphics.ImageFormat.RAW_PRIVATE) {
        ...
        private int mFormat = android.graphics.ImageFormat.UNKNOWN;
        ...
        private native synchronized android.media.ImageReader.SurfaceImage.SurfacePlane[] nativeCreatePlanes(int numPlanes, int readerFormat);

        private native synchronized int nativeGetWidth();

        private native synchronized int nativeGetHeight();

        private native synchronized int nativeGetFormat(int readerFormat);
    }
    ...
    private native synchronized void nativeReleaseImage(android.media.Image i);

    private native synchronized android.view.Surface nativeGetSurface();

    private native synchronized int nativeDetachImage(android.media.Image i);
     ...
     * @see #ACQUIRE_MAX_IMAGES
     */
    private native synchronized int nativeImageSetup(android.media.Image i);
