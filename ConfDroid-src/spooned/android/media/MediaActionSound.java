...
public class MediaActionSound {
    ...
    private static final java.lang.String[] SOUND_FILES = new java.lang.String[]{ "/system/media/audio/ui/camera_click.ogg", "/system/media/audio/ui/camera_focus.ogg", "/system/media/audio/ui/VideoRecord.ogg", "/system/media/audio/ui/VideoStop.ogg" };
    ...
    private class SoundState {
        ...
        public SoundState(int name) {
            ...
            state = android.media.MediaActionSound.STATE_NOT_LOADED;
        }
    }
    ...
    public MediaActionSound() {
        mSoundPool = new android.media.SoundPool.Builder().setMaxStreams(android.media.MediaActionSound.NUM_MEDIA_SOUND_STREAMS).setAudioAttributes(new android.media.AudioAttributes.Builder().setUsage(android.media.AudioAttributes.USAGE_ASSISTANCE_SONIFICATION).setFlags(android.media.AudioAttributes.FLAG_AUDIBILITY_ENFORCED).setContentType(android.media.AudioAttributes.CONTENT_TYPE_SONIFICATION).build()).build();
        mSoundPool.setOnLoadCompleteListener(mLoadCompleteListener);
        mSounds = new android.media.MediaActionSound.SoundState[android.media.MediaActionSound.SOUND_FILES.length];
        for (int i = 0; i < mSounds.length; i++) {
            mSounds[i] = new android.media.MediaActionSound.SoundState(i);
        }
    }

    private int loadSound(android.media.MediaActionSound.SoundState sound) {
        int id = mSoundPool.load(android.media.MediaActionSound.SOUND_FILES[sound.name], 1);
        if (id > 0) {
            sound.state = android.media.MediaActionSound.STATE_LOADING;
            sound.id = id;
        }
        return id;
    }
    ...
    public void load(int soundName) {
        if ((soundName < 0) || (soundName >= android.media.MediaActionSound.SOUND_FILES.length)) {
            throw new java.lang.RuntimeException("Unknown sound requested: " + soundName);
        }
        android.media.MediaActionSound.SoundState sound = mSounds[soundName];
        synchronized(sound) {
            switch (sound.state) {
                case android.media.MediaActionSound.STATE_NOT_LOADED :
                    if (loadSound(sound) <= 0) {
                        android.util.Log.e(android.media.MediaActionSound.TAG, "load() error loading sound: " + soundName);
                    }
                    break;
                default :
                    android.util.Log.e(android.media.MediaActionSound.TAG, (("load() called in wrong state: " + sound) + " for sound: ") + soundName);
                    break;
            }
        }
    }
        ...
        synchronized(sound) {
            switch (sound.state) {
                case android.media.MediaActionSound.STATE_NOT_LOADED :
                    loadSound(sound);
                    if (loadSound(sound) <= 0) {
                        android.util.Log.e(android.media.MediaActionSound.TAG, "play() error loading sound: " + soundName);
                        break;
                    }
                    // FALL THROUGH
                case android.media.MediaActionSound.STATE_LOADING :
                    sound.state = android.media.MediaActionSound.STATE_LOADING_PLAY_REQUESTED;
                    break;
                case android.media.MediaActionSound.STATE_LOADED :
                    mSoundPool.play(sound.id, 1.0F, 1.0F, 0, 0, 1.0F);
                    break;
                default :
                    android.util.Log.e(android.media.MediaActionSound.TAG, (("play() called in wrong state: " + sound.state) + " for sound: ") + soundName);
                    break;
            }
        }
    }

    private android.media.SoundPool.OnLoadCompleteListener mLoadCompleteListener = new android.media.SoundPool.OnLoadCompleteListener() {
        public void onLoadComplete(android.media.SoundPool soundPool, int sampleId, int status) {
            for (android.media.MediaActionSound.SoundState sound : mSounds) {
                ...
                synchronized(sound) {
                    if (status != 0) {
                        sound.state = android.media.MediaActionSound.STATE_NOT_LOADED;
                        sound.id = 0;
                        android.util.Log.e(android.media.MediaActionSound.TAG, (("OnLoadCompleteListener() error: " + status) + " loading sound: ") + sound.name);
                        return;
                    }
                    switch (sound.state) {
                        case android.media.MediaActionSound.STATE_LOADING :
                            sound.state = android.media.MediaActionSound.STATE_LOADED;
                            break;
                        case android.media.MediaActionSound.STATE_LOADING_PLAY_REQUESTED :
                            playSoundId = sound.id;
                            sound.state = android.media.MediaActionSound.STATE_LOADED;
                            break;
                        default :
                            android.util.Log.e(android.media.MediaActionSound.TAG, (("OnLoadCompleteListener() called in wrong state: " + sound.state) + " for sound: ") + sound.name);
                            break;
                    }
                }
                if (playSoundId != 0) {
