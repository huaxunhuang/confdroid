...
public class FaceDetector {
    ...
    public class Face {
         ...
         * 		face's mid-point
         */
        public void getMidPoint(android.graphics.PointF point) {
        ...
        public float pose(int euler) {
            ...
            if (euler == android.media.FaceDetector.Face.EULER_X)
                return mPoseEulerX;
            else
                if (euler == android.media.FaceDetector.Face.EULER_Y)
                    return mPoseEulerY;
                else
                    if (euler == android.media.FaceDetector.Face.EULER_Z)
                        return mPoseEulerZ;
    ...
    public FaceDetector(int width, int height, int maxFaces) {
        if (!android.media.FaceDetector.sInitialized) {
            return;
        }
     ...
     *
     * @param bitmap
     * 		the {@link android.graphics.Bitmap} graphic to be analyzed
     * @param faces
     ...
     * 		at initialization
     */
    public int findFaces(android.graphics.Bitmap bitmap, android.media.FaceDetector.Face[] faces) {
        if (!android.media.FaceDetector.sInitialized) {
            return 0;
        }
        ...
        for (int i = 0; i < numFaces; i++) {
            if (faces[i] == null)
                faces[i] = new android.media.FaceDetector.Face();
    ...
    static {
        android.media.FaceDetector.sInitialized = false;
        try {
            java.lang.System.loadLibrary("FFTEm");
            android.media.FaceDetector.nativeClassInit();
            android.media.FaceDetector.sInitialized = true;
