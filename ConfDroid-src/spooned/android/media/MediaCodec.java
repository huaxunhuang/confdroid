 ...
 * <ul>
 * <li><strong>native raw video format:</strong> This is marked by {@link CodecCapabilities#COLOR_FormatSurface} and it can be used with an input or output Surface.</li>
 * <li><strong>flexible YUV buffers</strong> (such as {@link CodecCapabilities#COLOR_FormatYUV420Flexible}): These can be used with an input/output Surface,
 ...
 * For color formats that are equivalent to a flexible format, you can still use {@link #getInputImage getInput}/{@link #getOutputImage OutputImage(int)}.</li>
 * </ul>
 * <p>
 ...
 * decoding a file or a stream, you can get the desired format from {@link MediaExtractor#getTrackFormat MediaExtractor.getTrackFormat}. Inject any specific features that
 ...
 * <p>
 * You can also create the preferred codec for a specific MIME type using {@link #createDecoderByType createDecoder}/{@link #createEncoderByType EncoderByType(String)}.
 ...
 * regular codec (the name of all secure codecs must end in {@code ".secure"}.) {@link #createByCodecName} will throw an {@code IOException} if the codec is not present on the system.
 * <p>
 ...
 * processing such compressed formats, this data must be submitted to the codec after {@link #start} and before any frame data. Such data must be marked using the flag {@link #BUFFER_FLAG_CODEC_CONFIG} in a call to {@link #queueInputBuffer queueInputBuffer}.
 * <p>
 ...
 * buffers. In synchronous mode, call {@link #dequeueInputBuffer dequeueInput}/{@link #dequeueOutputBuffer OutputBuffer(&hellip;)} to obtain (get ownership of) an input or output
 * buffer from the codec. In asynchronous mode, you will automatically receive available buffers via
 * the {@link Callback#onInputBufferAvailable MediaCodec.Callback.onInput}/{@link Callback#onOutputBufferAvailable OutputBufferAvailable(&hellip;)} callbacks.
 * <p>
 * Upon obtaining an input buffer, fill it with data and submit it to the codec using {@link #queueInputBuffer queueInputBuffer} &ndash; or {@link #queueSecureInputBuffer
 ...
 * <p>
 * The codec in turn will return a read-only output buffer via the {@link Callback#onOutputBufferAvailable onOutputBufferAvailable} callback in asynchronous mode, or in
 ...
 * mode changes the state transitions slightly, because you must call {@link #start} after {@link #flush} to transition the codec to the Running sub-state and start receiving input buffers.
 ...
 * time. Specifically, only call {@code getInput}/{@code OutputBuffers} directly after {@link #start} or after having dequeued an output buffer ID with the value of {@link #INFO_OUTPUT_FORMAT_CHANGED}.
 * <p>
 ...
 * {@link #start}, retrieve the buffer arrays using {@link #getInputBuffers getInput}/{@link #getOutputBuffers OutputBuffers()}. Use the buffer ID-s as indices into these arrays (when
 ...
 * output stream by specifying the same end-of-stream flag in the {@link BufferInfo} set in {@link #dequeueOutputBuffer dequeueOutputBuffer} or returned via {@link Callback#onOutputBufferAvailable
 ...
 * <p>
 * The data processing is nearly identical to the ByteBuffer mode when using an output {@link Surface}; however, the output buffers will not be accessible, and are represented as {@code null}
 * values. E.g. {@link #getOutputBuffer getOutputBuffer}/{@link #getOutputImage Image(int)} will
 * return {@code null} and {@link #getOutputBuffers} will return an array containing only {@code null}-s.
 * <p>
 ...
 * <li><strong>Render the buffer with the default timestamp:</strong> Call {@link #releaseOutputBuffer(int, boolean) releaseOutputBuffer(bufferId, true)}.</li>
 * <li><strong>Render the buffer with a specific timestamp:</strong> Call {@link #releaseOutputBuffer(int, long) releaseOutputBuffer(bufferId, timestamp)}.</li>
 * </ul>
 * <p>
 * Since {@link android.os.Build.VERSION_CODES#M}, the default timestamp is the {@linkplain BufferInfo#presentationTimeUs presentation timestamp} of the buffer (converted to nanoseconds).
 ...
 * <p class=note>
 * Note also that as of {@link android.os.Build.VERSION_CODES#N} release, {@link #VIDEO_SCALING_MODE_SCALE_TO_FIT_WITH_CROPPING} mode may not work correctly for videos rotated
 ...
 * playback. You can check if a decoder supports {@linkplain CodecCapabilities#FEATURE_AdaptivePlayback adaptive playback} via {@link CodecCapabilities#isFeatureSupported CodecCapabilities.isFeatureSupported(String)}. Adaptive
 ...
 * <p class=note>
 * <strong>Note:</strong> the format of the data submitted after a flush must not change; {@link #flush} does not support format discontinuities; for that, a full {@link #stop} - {@link #configure configure(&hellip;)} - {@link #start} cycle is necessary.
 *
 * <p class=note>
 ...
 * <p>
 * You will receive an {@link #INFO_OUTPUT_FORMAT_CHANGED} return value from {@link #dequeueOutputBuffer dequeueOutputBuffer} or a {@link Callback#onOutputBufferAvailable
 ...
 * which you must catch or declare to pass up. MediaCodec methods throw {@code IllegalStateException} when the method is called from a codec state that does not allow it; this
 * is typically due to incorrect application API usage. Methods involving secure buffers may throw
 * {@link CryptoException}, which has further error information obtainable from {@link CryptoException#getErrorCode}.
 * <p>
 ...
 * can be determined by calling {@link CodecException#isRecoverable} and {@link CodecException#isTransient}:
 * <ul>
...
public final class MediaCodec {
    ...
    public static final class BufferInfo {
         ...
         */
        public void set(int newOffset, int newSize, long newTimeUs, @android.media.MediaCodec.BufferFlag
        int newFlags) {
            offset = newOffset;
            size = newSize;
        ...
        public android.media.MediaCodec.BufferInfo dup() {
            android.media.MediaCodec.BufferInfo copy = new android.media.MediaCodec.BufferInfo();
     ...
     * @unknown 
     */
    @android.annotation.IntDef(flag = true, value = { android.media.MediaCodec.BUFFER_FLAG_SYNC_FRAME, android.media.MediaCodec.BUFFER_FLAG_KEY_FRAME, android.media.MediaCodec.BUFFER_FLAG_CODEC_CONFIG, android.media.MediaCodec.BUFFER_FLAG_END_OF_STREAM })
    ...
    private java.lang.Object mListenerLock = new java.lang.Object();
    ...
    private class EventHandler extends android.os.Handler {
        ...
        android.media.MediaCodec codec, @android.annotation.NonNull
        ...
        android.os.Message msg) {
            switch (msg.what) {
                case android.media.MediaCodec.EVENT_CALLBACK :
                    {
                        handleCallback(msg);
                        break;
                    }
                case android.media.MediaCodec.EVENT_SET_CALLBACK :
                    {
                        android.media.MediaCodec.this.mCallback = ((android.media.MediaCodec.Callback) (msg.obj));
                        break;
                    }
                case android.media.MediaCodec.EVENT_FRAME_RENDERED :
                    synchronized(mListenerLock) {
                        java.util.Map<java.lang.String, java.lang.Object> map = ((java.util.Map<java.lang.String, java.lang.Object>) (msg.obj));
                        for (int i = 0; ; ++i) {
                            java.lang.Object mediaTimeUs = map.get(i + "-media-time-us");
                            java.lang.Object systemNano = map.get(i + "-system-nano");
        ...
        private void handleCallback(@android.annotation.NonNull
        android.os.Message msg) {
            if (android.media.MediaCodec.this.mCallback == null) {
                return;
            }
            switch (msg.arg1) {
                case android.media.MediaCodec.CB_INPUT_AVAILABLE :
                    {
                        int index = msg.arg2;
                        synchronized(mBufferLock) {
                            validateInputByteBuffer(mCachedInputBuffers, index);
                        }
                        android.media.MediaCodec.this.mCallback.onInputBufferAvailable(mCodec, index);
                        break;
                    }
                case android.media.MediaCodec.CB_OUTPUT_AVAILABLE :
                    {
                        int index = msg.arg2;
                        android.media.MediaCodec.BufferInfo info = ((android.media.MediaCodec.BufferInfo) (msg.obj));
                        synchronized(mBufferLock) {
                            validateOutputByteBuffer(mCachedOutputBuffers, index, info);
                        }
                        android.media.MediaCodec.this.mCallback.onOutputBufferAvailable(mCodec, index, info);
                        break;
                    }
                case android.media.MediaCodec.CB_ERROR :
                    {
                        android.media.MediaCodec.this.mCallback.onError(mCodec, ((android.media.MediaCodec.CodecException) (msg.obj)));
                        break;
                    }
                case android.media.MediaCodec.CB_OUTPUT_FORMAT_CHANGE :
                    {
                        android.media.MediaCodec.this.mCallback.onOutputFormatChanged(mCodec, new android.media.MediaFormat(((java.util.Map<java.lang.String, java.lang.Object>) (msg.obj))));
                        break;
                    }
                default :
                    {
     ...
     */
    @android.annotation.NonNull
    public static android.media.MediaCodec createDecoderByType(@android.annotation.NonNull
    java.lang.String type) throws java.io.IOException {
        return /* nameIsType */
        /* encoder */
        new android.media.MediaCodec(type, true, false);
    }
     ...
     */
    @android.annotation.NonNull
    public static android.media.MediaCodec createEncoderByType(@android.annotation.NonNull
    java.lang.String type) throws java.io.IOException {
        return /* nameIsType */
        /* encoder */
        new android.media.MediaCodec(type, true, true);
    }
     ...
     */
    @android.annotation.NonNull
    public static android.media.MediaCodec createByCodecName(@android.annotation.NonNull
    java.lang.String name) throws java.io.IOException {
        return /* nameIsType */
        /* unused */
        new android.media.MediaCodec(name, false, false);
    }

    private MediaCodec(@android.annotation.NonNull
    java.lang.String name, boolean nameIsType, boolean encoder) {
        android.os.Looper looper;
        if ((looper = android.os.Looper.myLooper()) != null) {
            mEventHandler = new android.media.MediaCodec.EventHandler(this, looper);
        } else
        ...
        mBufferLock = new java.lang.Object();
     ...
     * @unknown 
     */
    @android.annotation.IntDef(flag = true, value = { android.media.MediaCodec.CONFIGURE_FLAG_ENCODE })
    @java.lang.annotation.Retention(java.lang.annotation.RetentionPolicy.SOURCE)
     ...
     * 		non-secure codecs.
     * @param flags
     * 		Specify {@link #CONFIGURE_FLAG_ENCODE} to configure the
     ...
     * 		upon codec error.
     */
    public void configure(@android.annotation.Nullable
    android.media.MediaFormat format, @android.annotation.Nullable
    android.view.Surface surface, @android.annotation.Nullable
    android.media.MediaCrypto crypto, @android.media.MediaCodec.ConfigureFlag
    int flags) {
        ...
        if (format != null) {
            ...
            keys = new java.lang.String[formatMap.size()];
            values = new java.lang.Object[formatMap.size()];
            int i = 0;
            for (java.util.Map.Entry<java.lang.String, java.lang.Object> entry : formatMap.entrySet()) {
                if (entry.getKey().equals(android.media.MediaFormat.KEY_AUDIO_SESSION_ID)) {
                    int sessionId = 0;
                    try {
                        sessionId = ((java.lang.Integer) (entry.getValue()));
                    } catch (java.lang.Exception e) {
                        throw new java.lang.IllegalArgumentException("Wrong Session ID Parameter!");
                    }
                    keys[i] = "audio-hw-sync";
                    values[i] = android.media.AudioSystem.getAudioHwSyncForSession(sessionId);
                } else {
                    keys[i] = entry.getKey();
     ...
     *
     * @param surface
     * 		the output surface to use. It must not be {@code null}.
     * @throws IllegalStateException
     ...
     */
    public void setOutputSurface(@android.annotation.NonNull
    ...
    private native void native_setSurface(@android.annotation.NonNull
    android.view.Surface surface);
    ...
    public static android.view.Surface createPersistentInputSurface() {
        return android.media.MediaCodec.native_createPersistentInputSurface();
    }
     ...
     *
     * @param surface
     * 		a persistent input surface created by {@link #createPersistentInputSurface}
     * @throws IllegalStateException
     ...
     */
    public void setInputSurface(@android.annotation.NonNull
    android.view.Surface surface) {
    ...
    private static final native void native_releasePersistentInputSurface(@android.annotation.NonNull
    android.view.Surface surface);

    private final native void native_setInputSurface(@android.annotation.NonNull
    android.view.Surface surface);

    private final native void native_setCallback(@android.annotation.Nullable
    android.media.MediaCodec.Callback cb);

    private final native void native_configure(@android.annotation.Nullable
    java.lang.String[] keys, @android.annotation.Nullable
    java.lang.Object[] values, @android.annotation.Nullable
    android.view.Surface surface, @android.annotation.Nullable
    android.media.MediaCrypto crypto, @android.media.MediaCodec.ConfigureFlag
    int flags);
    ...
    public final void stop() {
        ...
        synchronized(mListenerLock) {
            if (mCallbackHandler != null) {
                mCallbackHandler.removeMessages(android.media.MediaCodec.EVENT_SET_CALLBACK);
                mCallbackHandler.removeMessages(android.media.MediaCodec.EVENT_CALLBACK);
            }
            if (mOnFrameRenderedHandler != null) {
                mOnFrameRenderedHandler.removeMessages(android.media.MediaCodec.EVENT_FRAME_RENDERED);
            }
        }
    }
    ...
    public static final class CodecException extends java.lang.IllegalStateException {
        CodecException(int errorCode, int actionCode, @android.annotation.Nullable
        java.lang.String detailMessage) {
            ...
            mDiagnosticInfo = ("android.media.MediaCodec.error_" + sign) + java.lang.Math.abs(errorCode);
        }
        ...
        public boolean isTransient() {
            return mActionCode == android.media.MediaCodec.CodecException.ACTION_TRANSIENT;
        }
        ...
        public boolean isRecoverable() {
            return mActionCode == android.media.MediaCodec.CodecException.ACTION_RECOVERABLE;
        }
         ...
         * @unknown 
         */
        @android.annotation.IntDef({ android.media.MediaCodec.CodecException.ERROR_INSUFFICIENT_RESOURCE, android.media.MediaCodec.CodecException.ERROR_RECLAIMED })
        @java.lang.annotation.Retention(java.lang.annotation.RetentionPolicy.SOURCE)
    ...
    public static final class CryptoException extends java.lang.RuntimeException {
        public CryptoException(int errorCode, @android.annotation.Nullable
        java.lang.String detailMessage) {
            super(detailMessage);
            mErrorCode = errorCode;
        }
         ...
         * @unknown 
         */
        @android.annotation.IntDef({ android.media.MediaCodec.CryptoException.ERROR_NO_KEY, android.media.MediaCodec.CryptoException.ERROR_KEY_EXPIRED, android.media.MediaCodec.CryptoException.ERROR_RESOURCE_BUSY, android.media.MediaCodec.CryptoException.ERROR_INSUFFICIENT_OUTPUT_PROTECTION, android.media.MediaCodec.CryptoException.ERROR_SESSION_NOT_OPENED, android.media.MediaCodec.CryptoException.ERROR_UNSUPPORTED_OPERATION })
        @java.lang.annotation.Retention(java.lang.annotation.RetentionPolicy.SOURCE)
     ...
     * {@link #flush} by specifying the flag {@link #BUFFER_FLAG_CODEC_CONFIG}.  However, if you configure the
     * codec with a {@link MediaFormat} containing these keys, they
     * will be automatically submitted by MediaCodec directly after
     * start.  Therefore, the use of {@link #BUFFER_FLAG_CODEC_CONFIG} flag is discouraged and is
     ...
     * 		surface, this will be propagated as the {@link SurfaceTexture#getTimestamp timestamp} for the frame (after
     ...
     * 		{@link #configure}
     */
    public final void queueInputBuffer(int index, int offset, int size, long presentationTimeUs, int flags) throws android.media.MediaCodec.CryptoException {
        synchronized(mBufferLock) {
    ...
    private final native void native_queueInputBuffer(int index, int offset, int size, long presentationTimeUs, int flags) throws android.media.MediaCodec.CryptoException;
    ...
    public static final class CryptoInfo {
         ...
         */
        public void set(int newNumSubSamples, @android.annotation.NonNull
        int[] newNumBytesOfClearData, @android.annotation.NonNull
        int[] newNumBytesOfEncryptedData, @android.annotation.NonNull
        byte[] newKey, @android.annotation.NonNull
        byte[] newIV, int newMode) {
         ...
         */
        public void setPattern(android.media.MediaCodec.CryptoInfo.Pattern newPattern) {
            pattern = newPattern;
        }
        ...
        public java.lang.String toString() {
            ...
            java.lang.String hexdigits = "0123456789abcdef";
            for (int i = 0; i < key.length; i++) {
                builder.append(hexdigits.charAt((key[i] & 0xf0) >> 4));
                builder.append(hexdigits.charAt(key[i] & 0xf));
            }
            builder.append("], iv [");
            for (int i = 0; i < key.length; i++) {
                builder.append(hexdigits.charAt((iv[i] & 0xf0) >> 4));
                builder.append(hexdigits.charAt(iv[i] & 0xf));
            }
            builder.append("], clear ");
            builder.append(java.util.Arrays.toString(numBytesOfClearData));
            builder.append(", encrypted ");
            builder.append(java.util.Arrays.toString(numBytesOfEncryptedData));
     ...
     */
    public final void queueSecureInputBuffer(int index, int offset, @android.annotation.NonNull
    android.media.MediaCodec.CryptoInfo info, long presentationTimeUs, int flags) throws android.media.MediaCodec.CryptoException {
        synchronized(mBufferLock) {
            invalidateByteBuffer(mCachedInputBuffers, index);
            mDequeuedInputBuffers.remove(index);
        }
    ...
    private final native void native_queueSecureInputBuffer(int index, int offset, @android.annotation.NonNull
    android.media.MediaCodec.CryptoInfo info, long presentationTimeUs, int flags) throws android.media.MediaCodec.CryptoException;
     ...
     *
     * @deprecated This return value can be ignored as {@link #getOutputBuffers} has been deprecated.  Client should
     ...
     * @unknown 
     */
    @android.annotation.IntDef({ android.media.MediaCodec.INFO_TRY_AGAIN_LATER, android.media.MediaCodec.INFO_OUTPUT_FORMAT_CHANGED, android.media.MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED })
    @java.lang.annotation.Retention(java.lang.annotation.RetentionPolicy.SOURCE)
    ...
    public final int dequeueOutputBuffer(@android.annotation.NonNull
    android.media.MediaCodec.BufferInfo info, long timeoutUs) {
        int res = native_dequeueOutputBuffer(info, timeoutUs);
        synchronized(mBufferLock) {
            if (res == android.media.MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED) {
                /* input */
                cacheBuffers(false);
            } else
                if (res >= 0) {
    ...
    private final native int native_dequeueOutputBuffer(@android.annotation.NonNull
    android.media.MediaCodec.BufferInfo info, long timeoutUs);
    ...
    public final void releaseOutputBuffer(int index, boolean render) {
        android.media.MediaCodec.BufferInfo info = null;
        synchronized(mBufferLock) {
    ...
    public final android.media.MediaFormat getOutputFormat() {
        return new android.media.MediaFormat(/* input */
        getFormatNative(false));
    }
    ...
    public final android.media.MediaFormat getOutputFormat(int index) {
        return new android.media.MediaFormat(getOutputFormatNative(index));
    }

    @android.annotation.NonNull
    private final native java.util.Map<java.lang.String, java.lang.Object> getFormatNative(boolean input);

    @android.annotation.NonNull
    private final native java.util.Map<java.lang.String, java.lang.Object> getOutputFormatNative(int index);
    ...
    private static class BufferMap {
        ...
        private static class CodecBuffer {
            ...
            public void free() {
                if (mByteBuffer != null) {
                    ...
                    android.media.java.nio.NioUtils.freeDirectBuffer(mByteBuffer);
                    mByteBuffer = null;
                }
                if (mImage != null) {
                    mImage.close();
            ...
            public void setImage(@android.annotation.Nullable
            android.media.Image image) {
                free();
                mImage = image;
            }

            public void setByteBuffer(@android.annotation.Nullable
        ...
        private final java.util.Map<java.lang.Integer, android.media.MediaCodec.BufferMap.CodecBuffer> mMap = new java.util.HashMap<java.lang.Integer, android.media.MediaCodec.BufferMap.CodecBuffer>();

        public void remove(int index) {
            android.media.MediaCodec.BufferMap.CodecBuffer buffer = mMap.get(index);
            if (buffer != null) {
                buffer.free();
                mMap.remove(index);
            }
        ...
        public void put(int index, @android.annotation.Nullable
        java.nio.ByteBuffer newBuffer) {
            android.media.MediaCodec.BufferMap.CodecBuffer buffer = mMap.get(index);
            if (buffer == null) {
                // likely
                buffer = new android.media.MediaCodec.BufferMap.CodecBuffer();
        ...
        public void clear() {
            for (android.media.MediaCodec.BufferMap.CodecBuffer buffer : mMap.values()) {
                buffer.free();
            }
            mMap.clear();
        }
    ...
    private final android.media.MediaCodec.BufferMap mDequeuedInputBuffers = new android.media.MediaCodec.BufferMap();

    private final android.media.MediaCodec.BufferMap mDequeuedOutputBuffers = new android.media.MediaCodec.BufferMap();

    private final java.util.Map<java.lang.Integer, android.media.MediaCodec.BufferInfo> mDequeuedOutputInfos = new java.util.HashMap<java.lang.Integer, android.media.MediaCodec.BufferInfo>();

    private final java.lang.Object mBufferLock;

    private final void invalidateByteBuffer(@android.annotation.Nullable
    java.nio.ByteBuffer[] buffers, int index) {
        if (((buffers != null) && (index >= 0)) && (index < buffers.length)) {
            java.nio.ByteBuffer buffer = buffers[index];
            if (buffer != null) {
    ...
    private final void validateInputByteBuffer(@android.annotation.Nullable
    java.nio.ByteBuffer[] buffers, int index) {
        if (((buffers != null) && (index >= 0)) && (index < buffers.length)) {
            java.nio.ByteBuffer buffer = buffers[index];
            if (buffer != null) {
    ...
    private final void revalidateByteBuffer(@android.annotation.Nullable
    java.nio.ByteBuffer[] buffers, int index) {
        synchronized(mBufferLock) {
            if (((buffers != null) && (index >= 0)) && (index < buffers.length)) {
                java.nio.ByteBuffer buffer = buffers[index];
    ...
    private final void validateOutputByteBuffer(@android.annotation.Nullable
    java.nio.ByteBuffer[] buffers, int index, @android.annotation.NonNull
    ...
    private final void invalidateByteBuffers(@android.annotation.Nullable
    ...
    private final void freeByteBuffer(@android.annotation.Nullable
    java.nio.ByteBuffer buffer) {
        /* && buffer.isDirect() */
        if (buffer != null) {
            // all of our ByteBuffers are direct
            android.media.java.nio.NioUtils.freeDirectBuffer(buffer);
        }
    }

    private final void freeByteBuffers(@android.annotation.Nullable
    java.nio.ByteBuffer[] buffers) {
        if (buffers != null) {
            for (java.nio.ByteBuffer buffer : buffers) {
    ...
    private final void cacheBuffers(boolean input) {
        java.nio.ByteBuffer[] buffers = null;
        try {
     ...
     * buffer change by returning {@link #INFO_OUTPUT_BUFFERS_CHANGED}. After calling this method, any
    ...
    public java.nio.ByteBuffer getInputBuffer(int index) {
        java.nio.ByteBuffer newBuffer = /* input */
        getBuffer(true, index);
        synchronized(mBufferLock) {
            invalidateByteBuffer(mCachedInputBuffers, index);
    ...
    public android.media.Image getInputImage(int index) {
        android.media.Image newImage = /* input */
        getImage(true, index);
        synchronized(mBufferLock) {
            invalidateByteBuffer(mCachedInputBuffers, index);
     ...
     * @unknown 
     */
    @android.annotation.IntDef({ android.media.MediaCodec.VIDEO_SCALING_MODE_SCALE_TO_FIT, android.media.MediaCodec.VIDEO_SCALING_MODE_SCALE_TO_FIT_WITH_CROPPING })
    @java.lang.annotation.Retention(java.lang.annotation.RetentionPolicy.SOURCE)
     ...
     * 		if in the Released state.
     */
    public final native void setVideoScalingMode(@android.media.MediaCodec.VideoScalingMode
    int mode);
     ...
     * 		if in the Released state.
     */
    public final void setParameters(@android.annotation.Nullable
    android.os.Bundle params) {
        if (params == null) {
            return;
        }
        java.lang.String[] keys = new java.lang.String[params.size()];
        java.lang.Object[] values = new java.lang.Object[params.size()];
        int i = 0;
        for (final java.lang.String key : params.keySet()) {
            keys[i] = key;
     ...
     *
     * @param cb
     * 		The callback that will run.  Use {@code null} to clear a previously
     * 		set callback (before {@link #configure configure} is called and run
     * 		in synchronous mode).
     * @param handler
     * 		Callbacks will happen on the handler's thread. If {@code null},
     * 		callbacks are done on the default thread (the caller's thread or the
     * 		main thread.)
     */
    public void setCallback(/* MediaCodec. */
    @android.annotation.Nullable
    android.media.MediaCodec.Callback cb, @android.annotation.Nullable
    android.os.Handler handler) {
        if (cb != null) {
            synchronized(mListenerLock) {
                android.media.MediaCodec.EventHandler newHandler = getEventHandlerOn(handler, mCallbackHandler);
        ...
        if (mCallbackHandler != null) {
            ...
            android.os.Message msg = mCallbackHandler.obtainMessage(android.media.MediaCodec.EVENT_SET_CALLBACK, 0, 0, cb);
    ...
    public interface OnFrameRenderedListener {
         ...
         * @see System#nanoTime
         */
        public void onFrameRendered(@android.annotation.NonNull
        android.media.MediaCodec codec, long presentationTimeUs, long nanoTime);
    }
     ...
     * @param handler
     * 		the callback will be run on the handler's thread. If {@code null},
     ...
     */
    public void setOnFrameRenderedListener(@android.annotation.Nullable
    android.media.MediaCodec.OnFrameRenderedListener listener, @android.annotation.Nullable
    android.os.Handler handler) {
        synchronized(mListenerLock) {
            mOnFrameRenderedListener = listener;
            if (listener != null) {
                android.media.MediaCodec.EventHandler newHandler = getEventHandlerOn(handler, mOnFrameRenderedHandler);
    ...
    private android.media.MediaCodec.EventHandler getEventHandlerOn(@android.annotation.Nullable
    ...
    public static abstract class Callback {
         ...
         */
        public abstract void onInputBufferAvailable(@android.annotation.NonNull
         ...
         * @param info
         * 		Info regarding the available output buffer {@link MediaCodec.BufferInfo}.
         */
        public abstract void onOutputBufferAvailable(@android.annotation.NonNull
        android.media.MediaCodec codec, int index, @android.annotation.NonNull
        android.media.MediaCodec.BufferInfo info);
         ...
         * 		The MediaCodec object.
         * @param e
         * 		The {@link MediaCodec.CodecException} object describing the error.
         */
        public abstract void onError(@android.annotation.NonNull
        android.media.MediaCodec codec, @android.annotation.NonNull
         ...
         */
        public abstract void onOutputFormatChanged(@android.annotation.NonNull
        android.media.MediaCodec codec, @android.annotation.NonNull
        android.media.MediaFormat format);
    }

    private void postEventFromNative(int what, int arg1, int arg2, @android.annotation.Nullable
    java.lang.Object obj) {
        synchronized(mListenerLock) {
            android.media.MediaCodec.EventHandler handler = mEventHandler;
            if (what == android.media.MediaCodec.EVENT_CALLBACK) {
                handler = mCallbackHandler;
            } else
                if (what == android.media.MediaCodec.EVENT_FRAME_RENDERED) {
                    handler = mOnFrameRenderedHandler;
                }

            if (handler != null) {
                android.os.Message msg = handler.obtainMessage(what, arg1, arg2, obj);
                handler.sendMessage(msg);
            }
        }
    }

    private final native void setParameters(@android.annotation.NonNull
    ...
    public android.media.MediaCodecInfo getCodecInfo() {
        return android.media.MediaCodecList.getInfoFor(getName());
    }
    ...
    public static class MediaImage extends android.media.Image {
        ...
        public android.media.Image.Plane[] getPlanes() {
            throwISEIfImageIsInvalid();
            return java.util.Arrays.copyOf(mPlanes, mPlanes.length);
        }

        @java.lang.Override
        public void close() {
            if (mIsImageValid) {
                android.media.java.nio.NioUtils.freeDirectBuffer(mBuffer);
                mIsImageValid = false;
            }
        }
         ...
         */
        @java.lang.Override
        public void setCropRect(@android.annotation.Nullable
        ...
        public MediaImage(@android.annotation.NonNull
        java.nio.ByteBuffer buffer, @android.annotation.NonNull
        java.nio.ByteBuffer info, boolean readOnly, long timestamp, int xOffset, int yOffset, @android.annotation.Nullable
        android.graphics.Rect cropRect) {
            mFormat = android.graphics.ImageFormat.YUV_420_888;
            ...
            if (info.remaining() == 104) {
                ...
                if (type != android.media.MediaCodec.MediaImage.TYPE_YUV) {
                    throw new java.lang.UnsupportedOperationException("unsupported type: " + type);
                }
                int numPlanes = info.getInt();
                if (numPlanes != 3) {
                    throw new java.lang.RuntimeException("unexpected number of planes: " + numPlanes);
                }
                mWidth = info.getInt();
                mHeight = info.getInt();
                if ((mWidth < 1) || (mHeight < 1)) {
                    throw new java.lang.UnsupportedOperationException((("unsupported size: " + mWidth) + "x") + mHeight);
                }
                int bitDepth = info.getInt();
                if (bitDepth != 8) {
                    throw new java.lang.UnsupportedOperationException("unsupported bit depth: " + bitDepth);
                }
                int bitDepthAllocated = info.getInt();
                if (bitDepthAllocated != 8) {
                    throw new java.lang.UnsupportedOperationException("unsupported allocated bit depth: " + bitDepthAllocated);
                }
                mPlanes = new android.media.MediaCodec.MediaImage.MediaPlane[numPlanes];
                for (int ix = 0; ix < numPlanes; ix++) {
                    ...
                    if ((horiz != vert) || (horiz != (ix == 0 ? 1 : 2))) {
                        throw new java.lang.UnsupportedOperationException((((("unexpected subsampling: " + horiz) + "x") + vert) + " on plane ") + ix);
                    }
                    if ((colInc < 1) || (rowInc < 1)) {
                        throw new java.lang.UnsupportedOperationException((((("unexpected strides: " + colInc) + " pixel, ") + rowInc) + " row on plane ") + ix);
                    }
                    buffer.clear();
                    buffer.position(((mBuffer.position() + planeOffset) + ((xOffset / horiz) * colInc)) + ((yOffset / vert) * rowInc));
                    buffer.limit(((buffer.position() + android.media.Utils.divUp(bitDepth, 8)) + (((mHeight / vert) - 1) * rowInc)) + (((mWidth / horiz) - 1) * colInc));
                    mPlanes[ix] = new android.media.MediaCodec.MediaImage.MediaPlane(buffer.slice(), rowInc, colInc);
                }
            } else {
                throw new java.lang.UnsupportedOperationException("unsupported info length: " + info.remaining());
            }
            if (cropRect == null) {
                cropRect = new android.graphics.Rect(0, 0, mWidth, mHeight);
            }
        ...
        private class MediaPlane extends android.media.Image.Plane {
            public MediaPlane(@android.annotation.NonNull
            java.nio.ByteBuffer buffer, int rowInc, int colInc) {
                mData = buffer;
                mRowInc = rowInc;
                mColInc = colInc;
            }
