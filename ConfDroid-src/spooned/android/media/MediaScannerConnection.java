...
public class MediaScannerConnection implements android.content.ServiceConnection {
    ...
    private final IMediaScannerListener.Stub mListener = new android.media.IMediaScannerListener.Stub() {
        public void scanCompleted(java.lang.String path, android.net.Uri uri) {
            android.media.MediaScannerConnection.MediaScannerConnectionClient client = mClient;
            if (client != null) {
     ...
     */
    public MediaScannerConnection(android.content.Context context, android.media.MediaScannerConnection.MediaScannerConnectionClient client) {
        mContext = context;
        mClient = client;
    }
    ...
    public void connect() {
        synchronized(this) {
            if (!mConnected) {
                android.content.Intent intent = new android.content.Intent(android.media.IMediaScannerService.class.getName());
                intent.setComponent(new android.content.ComponentName("com.android.providers.media", "com.android.providers.media.MediaScannerService"));
                mContext.bindService(intent, this, android.content.Context.BIND_AUTO_CREATE);
                mConnected = true;
            }
        }
    }
    ...
    public void disconnect() {
        synchronized(this) {
            if (mConnected) {
                if (false) {
                    android.util.Log.v(android.media.MediaScannerConnection.TAG, "Disconnecting from Media Scanner");
                }
                try {
                    mContext.unbindService(this);
                    if (mClient instanceof android.media.MediaScannerConnection.ClientProxy) {
                        mClient = null;
                    }
                    mService = null;
                } catch (java.lang.IllegalArgumentException ex) {
                    if (false) {
                        android.util.Log.v(android.media.MediaScannerConnection.TAG, "disconnect failed: " + ex);
                    }
                }
                mConnected = false;
            }
    ...
    public void scanFile(java.lang.String path, java.lang.String mimeType) {
        synchronized(this) {
            ...
            try {
                if (false) {
                    android.util.Log.v(android.media.MediaScannerConnection.TAG, "Scanning file " + path);
                }
                mService.requestScanFile(path, mimeType, mListener);
            } catch (android.os.RemoteException e) {
                if (false) {
                    android.util.Log.d(android.media.MediaScannerConnection.TAG, "Failed to scan file " + path);
                }
            }
        }
    }
    ...
    static class ClientProxy implements android.media.MediaScannerConnection.MediaScannerConnectionClient {
        ...
        ClientProxy(java.lang.String[] paths, java.lang.String[] mimeTypes, android.media.MediaScannerConnection.OnScanCompletedListener client) {
            mPaths = paths;
    ...
    public static void scanFile(android.content.Context context, java.lang.String[] paths, java.lang.String[] mimeTypes, android.media.MediaScannerConnection.OnScanCompletedListener callback) {
        android.media.MediaScannerConnection.ClientProxy client = new android.media.MediaScannerConnection.ClientProxy(paths, mimeTypes, callback);
        android.media.MediaScannerConnection connection = new android.media.MediaScannerConnection(context, client);
    ...
    public void onServiceConnected(android.content.ComponentName className, android.os.IBinder service) {
        if (false) {
            android.util.Log.v(android.media.MediaScannerConnection.TAG, "Connected to Media Scanner");
        }
        synchronized(this) {
    ...
    public void onServiceDisconnected(android.content.ComponentName className) {
        if (false) {
            android.util.Log.v(android.media.MediaScannerConnection.TAG, "Disconnected from Media Scanner");
        }
        synchronized(this) {
            mService = null;
        }
