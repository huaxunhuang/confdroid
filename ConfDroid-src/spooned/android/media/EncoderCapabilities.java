...
public class EncoderCapabilities {
    ...
    public static class VideoEncoderCap {
        ...
        public final int mMaxBitRate;// min and max bit rate (bps)


        public final int mMinFrameRate;

        public final int mMaxFrameRate;// min and max frame rate (fps)


        public final int mMinFrameWidth;

        public final int mMaxFrameWidth;// min and max frame width (pixel)


        public final int mMinFrameHeight;

        public final int mMaxFrameHeight;// minn and max frame height (pixel)
    ...
    public static class AudioEncoderCap {
        ...
        public final int mMaxChannels;// min and max number of channels


        public final int mMinSampleRate;

        public final int mMaxSampleRate;// min and max sample rate (hz)
    ...
    public static int[] getOutputFileFormats() {
        int nFormats = android.media.EncoderCapabilities.native_get_num_file_formats();
        if (nFormats == 0)
            return null;

        int[] formats = new int[nFormats];
        for (int i = 0; i < nFormats; ++i) {
            formats[i] = android.media.EncoderCapabilities.native_get_file_format(i);
        }
        return formats;
    }
    ...
    public static java.util.List<android.media.EncoderCapabilities.VideoEncoderCap> getVideoEncoders() {
        int nEncoders = android.media.EncoderCapabilities.native_get_num_video_encoders();
        if (nEncoders == 0)
            return null;

        java.util.List<android.media.EncoderCapabilities.VideoEncoderCap> encoderList = new java.util.ArrayList<android.media.EncoderCapabilities.VideoEncoderCap>();
        for (int i = 0; i < nEncoders; ++i) {
            encoderList.add(android.media.EncoderCapabilities.native_get_video_encoder_cap(i));
        }
        return encoderList;
    }
    ...
    public static java.util.List<android.media.EncoderCapabilities.AudioEncoderCap> getAudioEncoders() {
        int nEncoders = android.media.EncoderCapabilities.native_get_num_audio_encoders();
        if (nEncoders == 0)
            return null;

        java.util.List<android.media.EncoderCapabilities.AudioEncoderCap> encoderList = new java.util.ArrayList<android.media.EncoderCapabilities.AudioEncoderCap>();
        for (int i = 0; i < nEncoders; ++i) {
            encoderList.add(android.media.EncoderCapabilities.native_get_audio_encoder_cap(i));
        }
        return encoderList;
    }
