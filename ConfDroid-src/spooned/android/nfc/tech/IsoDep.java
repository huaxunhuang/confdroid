...
public final class IsoDep extends android.nfc.tech.BasicTagTechnology {
     ...
     *
     * @param tag
     * 		an ISO-DEP compatible tag
     * @return ISO-DEP object
     */
    public static android.nfc.tech.IsoDep get(android.nfc.Tag tag) {
        if (!tag.hasTech(android.nfc.tech.TagTechnology.ISO_DEP))
            return null;

        try {
            return new android.nfc.tech.IsoDep(tag);
     ...
     * @unknown 
     */
    public IsoDep(android.nfc.Tag tag) throws android.os.RemoteException {
        super(tag, android.nfc.tech.TagTechnology.ISO_DEP);
        android.os.Bundle extras = tag.getTechExtras(android.nfc.tech.TagTechnology.ISO_DEP);
        if (extras != null) {
            mHiLayerResponse = extras.getByteArray(android.nfc.tech.IsoDep.EXTRA_HI_LAYER_RESP);
            mHistBytes = extras.getByteArray(android.nfc.tech.IsoDep.EXTRA_HIST_BYTES);
        }
    }
    ...
    public void setTimeout(int timeout) {
        try {
            int err = mTag.getTagService().setTimeout(android.nfc.tech.TagTechnology.ISO_DEP, timeout);
            if (err != android.nfc.ErrorCodes.SUCCESS) {
                throw new java.lang.IllegalArgumentException("The supplied timeout is not valid");
            }
        } catch (android.os.RemoteException e) {
            android.util.Log.e(android.nfc.tech.IsoDep.TAG, "NFC service dead", e);
        }
    }
    ...
    public int getTimeout() {
        try {
            return mTag.getTagService().getTimeout(android.nfc.tech.TagTechnology.ISO_DEP);
        } catch (android.os.RemoteException e) {
            android.util.Log.e(android.nfc.tech.IsoDep.TAG, "NFC service dead", e);
            return 0;
        }
