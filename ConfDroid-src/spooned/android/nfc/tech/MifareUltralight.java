...
public final class MifareUltralight extends android.nfc.tech.BasicTagTechnology {
    ...
    private static final int NXP_MANUFACTURER_ID = 0x4;
    ...
    public static android.nfc.tech.MifareUltralight get(android.nfc.Tag tag) {
        if (!tag.hasTech(android.nfc.tech.TagTechnology.MIFARE_ULTRALIGHT))
            return null;

        try {
            return new android.nfc.tech.MifareUltralight(tag);
     ...
     * @unknown 
     */
    public MifareUltralight(android.nfc.Tag tag) throws android.os.RemoteException {
        super(tag, android.nfc.tech.TagTechnology.MIFARE_ULTRALIGHT);
        // Check if this could actually be a MIFARE
        android.nfc.tech.NfcA a = android.nfc.tech.NfcA.get(tag);
        mType = android.nfc.tech.MifareUltralight.TYPE_UNKNOWN;
        if ((a.getSak() == 0x0) && (tag.getId()[0] == android.nfc.tech.MifareUltralight.NXP_MANUFACTURER_ID)) {
            android.os.Bundle extras = tag.getTechExtras(android.nfc.tech.TagTechnology.MIFARE_ULTRALIGHT);
            if (extras.getBoolean(android.nfc.tech.MifareUltralight.EXTRA_IS_UL_C)) {
                mType = android.nfc.tech.MifareUltralight.TYPE_ULTRALIGHT_C;
            } else {
                mType = android.nfc.tech.MifareUltralight.TYPE_ULTRALIGHT;
            }
        }
    }
     ...
     */
    public byte[] readPages(int pageOffset) throws java.io.IOException {
        android.nfc.tech.MifareUltralight.validatePageIndex(pageOffset);
        checkConnected();
        byte[] cmd = new byte[]{ 0x30, ((byte) (pageOffset)) };
     ...
     * 		if there is an I/O failure, or the operation is canceled
     */
    public void writePage(int pageOffset, byte[] data) throws java.io.IOException {
        android.nfc.tech.MifareUltralight.validatePageIndex(pageOffset);
        checkConnected();
    ...
    public void setTimeout(int timeout) {
        try {
            int err = mTag.getTagService().setTimeout(android.nfc.tech.TagTechnology.MIFARE_ULTRALIGHT, timeout);
            if (err != android.nfc.ErrorCodes.SUCCESS) {
                throw new java.lang.IllegalArgumentException("The supplied timeout is not valid");
            }
        } catch (android.os.RemoteException e) {
            android.util.Log.e(android.nfc.tech.MifareUltralight.TAG, "NFC service dead", e);
        }
    }
    ...
    public int getTimeout() {
        try {
            return mTag.getTagService().getTimeout(android.nfc.tech.TagTechnology.MIFARE_ULTRALIGHT);
        } catch (android.os.RemoteException e) {
            android.util.Log.e(android.nfc.tech.MifareUltralight.TAG, "NFC service dead", e);
            return 0;
        }
    ...
    private static void validatePageIndex(int pageIndex) {
        ...
        if ((pageIndex < 0) || (pageIndex >= android.nfc.tech.MifareUltralight.MAX_PAGE_COUNT)) {
            throw new java.lang.IndexOutOfBoundsException("page out of bounds: " + pageIndex);
        }
    }
}
