...
public final class GestureUtils {
    ...
    private static final float NONUNIFORM_SCALE = ((float) (java.lang.Math.sqrt(2)));
    ...
    public static float[] spatialSampling(android.gesture.Gesture gesture, int bitmapSize) {
        return android.gesture.GestureUtils.spatialSampling(gesture, bitmapSize, false);
    }
    ...
    value at pixel [i%bitmapSize, i/bitmapSize]
     */
    public static float[] spatialSampling(android.gesture.Gesture gesture, int bitmapSize, boolean keepAspectRatio) {
        final float targetPatchSize = bitmapSize - 1;
        float[] sample = new float[bitmapSize * bitmapSize];
        java.util.Arrays.fill(sample, 0);
        android.graphics.RectF rect = gesture.getBoundingBox();
        ...
        if (keepAspectRatio) {
            ...
            if (aspectRatio < android.gesture.GestureUtils.SCALING_THRESHOLD) {
                float scale = (sx < sy) ? sx : sy;
                sx = scale;
                sy = scale;
            } else {
                if (sx > sy) {
                    float scale = sy * android.gesture.GestureUtils.NONUNIFORM_SCALE;
                    if (scale < sx) {
                        sx = scale;
                    }
                } else {
                    float scale = sx * android.gesture.GestureUtils.NONUNIFORM_SCALE;
                    if (scale < sy) {
                        sy = scale;
                    }
                }
        ...
        final java.util.ArrayList<android.gesture.GestureStroke> strokes = gesture.getStrokes();
        ...
        for (int index = 0; index < count; index++) {
            ...
            for (int i = 0; i < size; i += 2) {
                ...
                android.gesture.GestureUtils.plot(segmentStartX, segmentStartY, sample, bitmapSize);
                if (segmentEndX != (-1)) {
                    // Evaluate horizontally
                    if (segmentEndX > segmentStartX) {
                        xpos = ((float) (java.lang.Math.ceil(segmentStartX)));
                        ...
                        while (xpos < segmentEndX) {
                            ...
                            android.gesture.GestureUtils.plot(xpos, ypos, sample, bitmapSize);
                            xpos++;
                        } 
                    } else
                        if (segmentEndX < segmentStartX) {
                            xpos = ((float) (java.lang.Math.ceil(segmentEndX)));
                            float slope = (segmentEndY - segmentStartY) / (segmentEndX - segmentStartX);
                    ...
                    if (segmentEndY > segmentStartY) {
                        ypos = ((float) (java.lang.Math.ceil(segmentStartY)));
                        ...
                        if (segmentEndY < segmentStartY) {
                            ypos = ((float) (java.lang.Math.ceil(segmentEndY)));
                            float invertSlope = (segmentEndX - segmentStartX) / (segmentEndY - segmentStartY);
    ...
    private static void plot(float x, float y, float[] sample, int sampleSize) {
        x = (x < 0) ? 0 : x;
        y = (y < 0) ? 0 : y;
        int xFloor = ((int) (java.lang.Math.floor(x)));
        int xCeiling = ((int) (java.lang.Math.ceil(x)));
        int yFloor = ((int) (java.lang.Math.floor(y)));
        int yCeiling = ((int) (java.lang.Math.ceil(y)));
        // if it's an integer
        if ((x == xFloor) && (y == yFloor)) {
            ...
            final double xFloorSq = java.lang.Math.pow(xFloor - x, 2);
            final double yFloorSq = java.lang.Math.pow(yFloor - y, 2);
            final double xCeilingSq = java.lang.Math.pow(xCeiling - x, 2);
            final double yCeilingSq = java.lang.Math.pow(yCeiling - y, 2);
            float topLeft = ((float) (java.lang.Math.sqrt(xFloorSq + yFloorSq)));
            float topRight = ((float) (java.lang.Math.sqrt(xCeilingSq + yFloorSq)));
            float btmLeft = ((float) (java.lang.Math.sqrt(xFloorSq + yCeilingSq)));
            float btmRight = ((float) (java.lang.Math.sqrt(xCeilingSq + yCeilingSq)));
     ...
     */
    public static float[] temporalSampling(android.gesture.GestureStroke stroke, int numPoints) {
        ...
        int index = 0;
        float currentPointX = java.lang.Float.MIN_VALUE;
        float currentPointY = java.lang.Float.MIN_VALUE;
        ...
        while (i < count) {
            if (currentPointX == java.lang.Float.MIN_VALUE) {
                i++;
                if (i >= count) {
                    break;
                }
            ...
            float distance = ((float) (java.lang.Math.hypot(deltaX, deltaY)));
            if ((distanceSoFar + distance) >= increment) {
                ...
                currentPointX = java.lang.Float.MIN_VALUE;
                currentPointY = java.lang.Float.MIN_VALUE;
    ...
    static float computeTotalLength(float[] points) {
        ...
        for (int i = 0; i < count; i += 2) {
            ...
            sum += java.lang.Math.hypot(dx, dy);
        }
        return sum;
    }

    static float computeStraightness(float[] points) {
        float totalLen = android.gesture.GestureUtils.computeTotalLength(points);
        float dx = points[2] - points[0];
        float dy = points[3] - points[1];
        return ((float) (java.lang.Math.hypot(dx, dy))) / totalLen;
    }
    ...
    static float cosineDistance(float[] vector1, float[] vector2) {
        ...
        return ((float) (java.lang.Math.acos(sum)));
    }
    ...
    static float minimumCosineDistance(float[] vector1, float[] vector2, int numOrientations) {
        ...
        if (a != 0) {
            final float tan = b / a;
            final double angle = java.lang.Math.atan(tan);
            if ((numOrientations > 2) && (java.lang.Math.abs(angle) >= (java.lang.Math.PI / numOrientations))) {
                return ((float) (java.lang.Math.acos(a)));
            } else {
                final double cosine = java.lang.Math.cos(angle);
                final double sine = cosine * tan;
                return ((float) (java.lang.Math.acos((a * cosine) + (b * sine))));
            }
        } else {
            return ((float) (java.lang.Math.PI)) / 2;
        }
    }
    ...
    public static android.gesture.OrientedBoundingBox computeOrientedBoundingBox(java.util.ArrayList<android.gesture.GesturePoint> originalPoints) {
        ...
        for (int i = 0; i < count; i++) {
            android.gesture.GesturePoint point = originalPoints.get(i);
            int index = i * 2;
            points[index] = point.x;
            points[index + 1] = point.y;
        }
        float[] meanVector = android.gesture.GestureUtils.computeCentroid(points);
        return android.gesture.GestureUtils.computeOrientedBoundingBox(points, meanVector);
    }
    ...
    private static android.gesture.OrientedBoundingBox computeOrientedBoundingBox(float[] points, float[] centroid) {
        android.gesture.GestureUtils.translate(points, -centroid[0], -centroid[1]);
        float[][] array = android.gesture.GestureUtils.computeCoVariance(points);
        float[] targetVector = android.gesture.GestureUtils.computeOrientation(array);
        float angle;
        if ((targetVector[0] == 0) && (targetVector[1] == 0)) {
            angle = ((float) (-java.lang.Math.PI)) / 2;
        } else {
            // -PI<alpha<PI
            angle = ((float) (java.lang.Math.atan2(targetVector[1], targetVector[0])));
            android.gesture.GestureUtils.rotate(points, -angle);
        }
        ...
        return new android.gesture.OrientedBoundingBox(((float) ((angle * 180) / java.lang.Math.PI)), centroid[0], centroid[1], maxx - minx, maxy - miny);
    }
    ...
    private static float[] computeOrientation(float[][] covarianceMatrix) {
        ...
        float value = a / 2;
        float rightside = ((float) (java.lang.Math.sqrt(java.lang.Math.pow(value, 2) - b)));
