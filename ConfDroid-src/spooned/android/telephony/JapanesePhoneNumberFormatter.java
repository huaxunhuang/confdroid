...
class JapanesePhoneNumberFormatter {
    ...
    public static void format(android.text.Editable text) {
        ...
        while (i < length) {
            ...
            short value = android.telephony.JapanesePhoneNumberFormatter.FORMAT_MAP[(base + ch) - '0'];
            if (value < 0) {
                if (value <= (-100)) {
                    text.replace(0, length, saved);
                    return;
                }
                int dashPos2 = rootIndex + (java.lang.Math.abs(value) % 10);
                if (length > dashPos2) {
                    text.insert(dashPos2, "-");
                }
                int dashPos1 = rootIndex + (java.lang.Math.abs(value) / 10);
                if (length > dashPos1) {
