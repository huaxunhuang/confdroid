...
public class ReadXmlFilesForFolderApiLevel {
    static int num = 0;
    ...
    public static String mode = "cdep";
    ...
    static List<Triplet<String, String, Integer>> confDroidRuleMap = new ArrayList<>();
    ...
    public static void buildORPLocatorRuleMap(){
        ...
        orpLocatorRuleMap.put("roundIcon",24);
...
//                System.out.println(tempFile.toString());
    ...
    public static void getNodes(Element element, String path) {
        ...
        while(!q.isEmpty()){
            Element top = q.poll();
            nodes.add(top);
            for(Element child : top.elements()){
                q.add(child);
            }
        }
...
//                System.out.println("apiLevelInPath = " + apiLevelInPath);
            ...
            if(mode.equals("orplocator") || mode.equals("lint")) {
                for (Attribute attr : list1) {
                    String name = attr.getName();
                    if (mode.equals("orplocator")) {
                        if (orpLocatorRuleMap.containsKey(name)) {
                            int apiLevelInAttr = orpLocatorRuleMap.get(name);
                ...
                for(Triplet<String, String, Integer> triplet : cDepRuleMap){
                    ...
                    for(Attribute attr : list1){
                        ...
                        if(flag1 && flag2){
                            int apiLevelInAttr = triplet.getValue2();
                            if (apiLevelInPath >= apiLevelInAttr) {
                                System.out.println("path = " + path);
                                System.out.println("element.asXML() = " + node.asXML());
                                break;
...
//                            break;
    ...
    public static void detection(String path) throws Exception {
        if (path.contains("abc_") || path.contains("notification_template") || path.endsWith("design_password_eye.xml") || path
                .endsWith("/preference_list_divider_material.xml") || path.contains("/mtrl_") || path.contains("/styles.xml"))
            return;
